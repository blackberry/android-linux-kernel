...
static int locker_check_caller_access_to_name_by_uid(struct inode *parent_node, const struct qstr *name)
{
	...
	if (!(SDCARDFS_SB(parent_node->i_sb)->options.multiuser)) { // not primary storage
		return 1;
	}
...
#ifdef CONFIG_SDCARD_FS_LOCKER
static ssize_t packages_locker_wl_show(struct packages *packages, char *page)
{
	int i;
...
static ssize_t packages_locker_wl_store(struct packages *packages,
				       const char *page, size_t count)
{
	int i;
	char *tmp;
...
#ifdef CONFIG_SDCARD_FS_LOCKER
PACKAGES_ATTR(locker_wl, S_IRUGO | S_IWUGO, packages_locker_wl_show, packages_locker_wl_store);
#endif /* CONFIG_SDCARD_FS_LOCKER */
