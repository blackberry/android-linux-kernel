				 ...
				 int hide_pid_min)
{
	...
	/* JI:808489 Limit Proc Access */
	static const int UID_WHITELIST[] = {0, 1000, 1012, 1013, 1017};
#endif
	...
	if (in_group_p(pid->pid_gid))
		return true;
#else
	while (i < pid->num_hidepid_gids) {
		if (in_group_p(pid->pid_gid[i]))
			return true;
		i++;
	}
	rcu_read_lock();
	...
	rcu_read_unlock();
	for (i = 0; i < ARRAY_SIZE(UID_WHITELIST); i++) {
		if (tcred->uid == UID_WHITELIST[i])
			return true;
	}
#endif
