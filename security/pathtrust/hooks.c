...
{
#ifdef CONFIG_BBSECURE_PATHTRUST_FIRMWARE
	struct common_audit_data cad;
	struct pathtrust_audit_data pt_data = { .allowed = false, .dev = 0 };
...
/*
 * Only CAP_SYS_ADMIN can change user.pax.flags xattr at runtime on files
 */
static int pathtrust_inode_setxattr(struct dentry *dentry, const char *name,
				  const void *value, size_t size, int flags)
{
	if (!strcmp(name, XATTR_NAME_PAX_FLAGS) && !capable(CAP_SYS_ADMIN))
		return -EPERM;
...
/*
 * Validate that the inode on which the user.pax.flags xattr is read comes from
 * a trusted filesystem
 */
static int pathtrust_inode_getxattr(struct dentry *dentry, const char *name)
{
	struct inode *inode = dentry->d_inode;

	if (!strcmp(name, XATTR_NAME_PAX_FLAGS) && inode->i_op->getxattr) {

		/*
		 * first check if the attribute exists or not on the file
		 * returns ENODATA if it doesn't exist
		 */
		int size = inode->i_op->getxattr(dentry, XATTR_NAME_PAX_FLAGS, NULL, 0);

		/* xattr exists on inode */
		if (size >= 0) {
			/* check if the inode is coming from a trusted mount
			 * by checking the inode superblock dentry mount
			 * point flags for the trusted flag */
			if (is_mount_trusted(inode->i_sb->s_root)) {
				return 0;
			} else {
				if (!pathtrust_enforce)
					return 0;
 ...
 * Only CAP_SYS_ADMIN can change user.pax.flags xattr at runtime on files
 */
static int pathtrust_inode_removexattr(struct dentry *dentry, const char *name)
{
	if (!strcmp(name, XATTR_NAME_PAX_FLAGS) && !capable(CAP_SYS_ADMIN))
		return -EPERM;
...
#ifdef CONFIG_PAX_XATTR_PAX_FLAGS
	LSM_HOOK_INIT(inode_setxattr, pathtrust_inode_setxattr),
	LSM_HOOK_INIT(inode_getxattr, pathtrust_inode_getxattr),
	LSM_HOOK_INIT(inode_removexattr, pathtrust_inode_removexattr),
#endif
};
...
#ifdef CONFIG_BBSECURE_PATHTRUST_WHITELIST_IMPLICIT
	return true;
#endif
...
static int __init pathtrust_init(void){
	...
	char *item;
	char default_trusted_pathnames[] = CONFIG_BBSECURE_PATHTRUST_PATHNAMES;
