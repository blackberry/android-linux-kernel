...
static int pathtrust_sb_mount(const char *dev_name,
			 ...
			 void *data)
#else
static int pathtrust_sb_mount(char *dev_name,
			 struct path *path,
			 char *type,
			 unsigned long flags,
			 void *data)
...
static int pathtrust_bprm_set_creds(struct linux_binprm *bprm)
{
	...
	if (is_root(bprm->cred) || has_banned_caps()) {
		...
		if (!(bprm->file->f_path.mnt->mnt_flags & MNT_TRUSTED)) {
			...
			pt_data.btype = is_root(bprm->cred) ? PATHTRUST_AUDIT_BTYPE_ROOT : PATHTRUST_AUDIT_BTYPE_CAPS;
...
#else
static int pathtrust_file_mmap(struct file *file, unsigned long reqprot,
			     unsigned long prot, unsigned long flags,
...
#endif
{
	...
	if (file && ((prot & PROT_EXEC) ||
		...
		if (is_root(cred) || has_banned_caps()) {
			...
			if (!(file->f_path.mnt->mnt_flags & MNT_TRUSTED)) {
				...
				cad.pathtrust_audit_data = &pt_data;
				pt_data.type = PATHTRUST_AUDIT_TYPE_MMAP;
				pt_data.btype = is_root(cred) ? PATHTRUST_AUDIT_BTYPE_ROOT : PATHTRUST_AUDIT_BTYPE_CAPS;
				cad.u.path = file->f_path;
				pathtrust_lsm_audit(&cad);
 ...
 * Disallow anonymous firmware loading /dev/
...
#else
static struct security_operations pathtrust_ops = {
	.name =						"pathtrust",
	.sb_mount =					pathtrust_sb_mount,
	.sb_kern_mount =			pathtrust_sb_kern_mount,
	.bprm_set_creds =			pathtrust_bprm_set_creds,
	.file_mmap =				pathtrust_file_mmap,
	.kernel_module_from_file =	pathtrust_kernel_module_from_file,
};
#endif
...
#else
	if (register_security(&pathtrust_ops))
