...
#if defined(CONFIG_BBSECURE_SECURITY_SELINUX_DEVELOP_ENFORCE)
/* default into enforcing mode. Need command line enforcing=0 to disable */
int selinux_enforcing = 1;
#else
#ifdef CONFIG_BBSECURE_PROTECTED_VARS
int __ro_protected selinux_enforcing;
#else
...
#ifdef CONFIG_BBSECURE_PROTECTED_VARS
		update_protected(selinux_enforcing, enforcing ? 1 : 0);
#else
...
#ifdef CONFIG_BBSECURE_PROTECTED_VARS
int __ro_protected selinux_enabled = CONFIG_SECURITY_SELINUX_BOOTPARAM_VALUE;
#else
...
#ifdef CONFIG_BBSECURE_PROTECTED_VARS
		update_protected(selinux_enabled, enabled ? 1 : 0);
#else
...
#else
#ifdef CONFIG_BBSECURE_PROTECTED_VARS
int __ro_protected selinux_enabled = 1;
#else
...
static struct security_hook_list selinux_hooks[] = {
	...
	LSM_HOOK_INIT(xfrm_state_alloc_acquire, selinux_xfrm_state_alloc_acquire),
...
#ifdef CONFIG_BBSECURE_PROTECTED_VARS
		update_protected(selinux_enabled, 0);
#else
		selinux_enabled = 0;
#endif
		return 0;
