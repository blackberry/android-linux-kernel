...
#if defined(CONFIG_SECURITY_SELINUX_DEVELOP) || defined(CONFIG_SECURITY_SELINUX_DEVELOP_ENFORCE)
#if defined(CONFIG_SECURITY_SELINUX_DEVELOP_ENFORCE)
...
static int selinux_allow_merge_bio(struct bio *bio1, struct bio *bio2)
{
	return (pft_allow_merge_bio(bio1, bio2) ? 1 : 0);
}
...
static struct security_hook_list selinux_hooks[] __read_only = {
	...
	LSM_HOOK_INIT(inode_create, selinux_inode_post_create),
