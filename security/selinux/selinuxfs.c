...
#ifdef CONFIG_SECURITY_SELINUX_DEVELOP
/* Print or not the avc audit messages to kmsg
 * Mostly used for unit testing to avoid the flood of messages
 * hiding other messages from the klogs due to rate limits
 */
int selinux_avc_auditing = 1;

static ssize_t sel_read_avc_audit(struct file *filp, char __user *buf,
				size_t count, loff_t *ppos)
{
	char tmpbuf[TMPBUFLEN];
	ssize_t length;

	length = scnprintf(tmpbuf, TMPBUFLEN, "%d", selinux_avc_auditing);
	return simple_read_from_buffer(buf, count, ppos, tmpbuf, length);
}

static ssize_t sel_write_avc_audit(struct file *file, const char __user *buf,
				 size_t count, loff_t *ppos)

{
	...
	selinux_avc_auditing = new_value;
...
static const struct file_operations sel_avc_audit_ops = {
	.read		= sel_read_avc_audit,
	.write		= sel_write_avc_audit,
	.llseek		= generic_file_llseek,
};
#endif
...
#endif
#ifdef CONFIG_SECURITY_SELINUX_DEVELOP
		{ "audit", &sel_avc_audit_ops, S_IRUGO|S_IWUSR },
#endif
	};
