			...
			unsigned int dev)
{
	if (unlikely(IS_PRIVATE(dir->dentry->d_inode)))
		return 0;
	return call_int_hook(path_mkdir, 0, dir, dentry, mode, dev);
}
...
{
	return return call_int_hook(path_chroot, 0, path);
}
#endif
...
{
	return call_bool_hook(allow_merge_bio, true, bio1, bio2);
}
