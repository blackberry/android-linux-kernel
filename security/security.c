...
int security_allow_merge_bio(struct bio *bio1, struct bio *bio2)
{
...
#ifdef CONFIG_BBRY
int security_kernel_module_init(struct module *mod)
{
	return call_int_hook(kernel_module_init, 0, mod);
}

void security_kernel_module_free(struct module *mod)
{
	call_void_hook(kernel_module_free, mod);
}
#endif
...
#ifdef CONFIG_BBRY
	.kernel_module_init =
		LIST_HEAD_INIT(security_hook_heads.kernel_module_init),
	.kernel_module_free =
		LIST_HEAD_INIT(security_hook_heads.kernel_module_free),
#endif
	.task_fix_setuid =
...
#if defined(CONFIG_SECURITY_SELINUX_DISABLE) || defined(CONFIG_BBRY_DEBUG)
...
#ifdef CONFIG_BBRY_DEBUG
/* Used for LSM stack verification */
EXPORT_SYMBOL(security_delete_hooks);
#endif
