...
#ifdef CONFIG_BBRY
	if(vadc->adc->adc_channels[dt_index].adc_sec_scale_fn < SCALE_NONE) {
		phy_tmp = result->physical;
		scale_type = vadc->adc->adc_channels[dt_index].adc_sec_scale_fn;
		vadc_scale_fn[scale_type].chan(vadc, result->adc_code,
			vadc->adc->adc_prop, vadc->adc->amux_prop->chan_prop, result);
		result->secondary = result->physical;
		result->physical = phy_tmp;
	} else
		result->secondary = -1;
#endif
			...
			struct device_attribute *devattr, char *buf)
{
	...
	if(result.secondary >= 0)
		return snprintf(buf, QPNP_ADC_HWMON_NAME_LENGTH,
			"Result:%lld Raw:%d Sec:%lld\n", result.physical, result.adc_code, result.secondary);
#endif
	return snprintf(buf, QPNP_ADC_HWMON_NAME_LENGTH,
