...
#ifdef CONFIG_BBRY
		if (dotg->otg_oc_irq >= 0)
			enable_irq(dotg->otg_oc_irq);
#endif
...
skip_psy_type:
	...
	if (slimport_dongle_is_connected() && mA) {
...
#ifdef CONFIG_BBRY
/**
 * dwc3_otg_overcurrent_uevent - notifies user space of otg overcurrent
 * @dotg: Pointer to the dwc3_otg structure
 *
 */
static void dwc3_otg_overcurrent_uevent(struct dwc3_otg *dotg)
{
	char *overcurrent[2] = { "USB_OTG_STATE=OVERCURRENT", NULL };
	kobject_uevent_env(&dotg->dev->kobj, KOBJ_CHANGE,
				   overcurrent);
}
#endif
...
#ifdef CONFIG_BBRY
		if (test_bit(OTG_OVERCURRENT, &dotg->inputs)) {
			/* just turn off regulator, so we can pop a dialog */
			if (regulator_disable(dotg->vbus_otg))
				dev_err(phy->dev, "unable to disable vbus_otg\n");
			phy->state = OTG_STATE_A_OVERCURRENT;
			dev_dbg(phy->dev, "in overcurrent state\n");
			clear_bit(OTG_OVERCURRENT, &dotg->inputs);
			dwc3_otg_overcurrent_uevent(dotg);
		} else if (test_bit(OTG_DISABLED, &dotg->inputs)) {
			/* turn off host mode completely, so we go into LPM */
			dwc3_otg_start_host(&dotg->otg, 0);
			phy->state = OTG_STATE_A_DISABLED;
			dev_dbg(phy->dev, "in disabled state\n");
			clear_bit(OTG_DISABLED, &dotg->inputs);
		} else
#endif
		if (test_bit(ID, &dotg->inputs)) {
			dev_dbg(phy->dev, "id\n");
...
#ifdef CONFIG_BBRY
	case OTG_STATE_A_OVERCURRENT:
		if (test_bit(OTG_DISABLED, &dotg->inputs)) {
			/* turn off host mode completely, so we go into LPM */
			dwc3_otg_start_host(&dotg->otg, 0);
			phy->state = OTG_STATE_A_DISABLED;
			dev_dbg(phy->dev, "in disabled state\n");
		...
		/* fallthrough */
	case OTG_STATE_A_DISABLED:
		/*
		 * only way to leave overcurrent or disabled state is to
		 * remove OTG dongle, bringing ID pin high
		 */
		if (test_bit(ID, &dotg->inputs)) {
			dev_dbg(phy->dev, "id. leaving overcurrent/disabled state\n");
			if (phy->state == OTG_STATE_A_OVERCURRENT)
				dwc3_otg_start_host(&dotg->otg, 0);
			phy->state = OTG_STATE_B_IDLE;
			dotg->vbus_retry_count = 0;
			work = 1;
...
#ifdef CONFIG_BBRY
static irqreturn_t dwc3_otg_oc_irq(int irq, void *data)
{
	struct dwc3_otg *dotg = data;
	dev_err(dotg->dwc->dev, "%s: got overcurrent interrupt\n", __func__);

	set_bit(OTG_OVERCURRENT, &dotg->inputs);
	queue_delayed_work(system_nrt_wq, &dotg->sm_work, 0);

	return IRQ_HANDLED;
}

static ssize_t dwc3_otg_state_show(struct device *pdev,
			struct device_attribute *attr, char *buf)
{
	struct dwc3_otg *dotg = dev_get_drvdata(pdev);
	char *state;

	switch (dotg->otg.phy->state) {
	case OTG_STATE_A_OVERCURRENT:
		state = "OVERCURRENT";
		break;
	case OTG_STATE_A_DISABLED:
...
static ssize_t dwc3_otg_state_store(struct device *pdev,
			struct device_attribute *attr, const char *buf,
			size_t size)
{
	struct dwc3_otg *dotg = dev_get_drvdata(pdev);

	if (strncmp(buf, "DISABLED", size) == 0) {
		if ((dotg->otg.phy->state == OTG_STATE_A_HOST) ||
				(dotg->otg.phy->state == OTG_STATE_A_OVERCURRENT)) {
			set_bit(OTG_DISABLED, &dotg->inputs);
			queue_delayed_work(system_nrt_wq, &dotg->sm_work, 0);
			return size;
		} else {
			return -EINVAL;
...
static DEVICE_ATTR(state, S_IRUGO | S_IWUSR, dwc3_otg_state_show,
			dwc3_otg_state_store);

static struct device_attribute *dwc3_otg_attributes[] = {
	&dev_attr_state,
	NULL
};

static int dwc3_otg_create_device(struct dwc3_otg *dotg)
{
	struct device_attribute **attrs = dwc3_otg_attributes;
	struct device_attribute *attr;
	int err;

	dotg->class = class_create(THIS_MODULE, "usb_otg");
	if (IS_ERR(dotg->class))
		return PTR_ERR(dotg->class);

	dotg->dev = device_create(dotg->class, NULL, MKDEV(0, 0),
		NULL, "usb_otg");
	if (IS_ERR(dotg->dev)) {
		class_destroy(dotg->class);
		return PTR_ERR(dotg->dev);
	}

	dev_set_drvdata(dotg->dev, dotg);

	while ((attr = *attrs++)) {
		err = device_create_file(dotg->dev, attr);
		if (err) {
			device_destroy(dotg->class, dotg->dev->devt);
			class_destroy(dotg->class);
			return err;
		}
	}
...
static int dwc3_otg_reboot_cb(struct notifier_block *nfb,
				unsigned long val,
				void *data) {
	struct dwc3_otg *dotg =
			container_of(nfb, struct dwc3_otg, reboot_notifier);

	if (dotg->otg.phy->state == OTG_STATE_A_HOST) {
		dev_info(dotg->dwc->dev, "rebooting, turn off host mode\n");
		dwc3_otg_start_host(&dotg->otg, 0);
...
int dwc3_otg_init(struct dwc3 *dwc)
{
	...
	dotg->otg_oc_irq = -1;
#ifndef CONFIG_BBRY_MFG
	dotg->otg_oc_gpio = of_get_named_gpio(dwc->dev->parent->of_node,
			"usb_otg_oc-gpio", 0);
	if (!gpio_is_valid(dotg->otg_oc_gpio)) {
		dev_err(dwc->dev, "otg_oc_gpio isn't valid\n");
		goto skip_oc_setup;
	}
	ret = gpio_request(dotg->otg_oc_gpio, "usb_otg_oc");
	if (ret) {
		dev_err(dwc->dev, "Failed to request otg_oc_gpio: %d\n", ret);
		goto skip_oc_setup;
	}
	dotg->otg_oc_irq = gpio_to_irq(dotg->otg_oc_gpio);
	if (dotg->otg_oc_irq < 0) {
		dev_err(dwc->dev, "couldn't get otg_oc_irq\n");
		goto skip_oc_setup;
	}
	irq_set_status_flags(dotg->otg_oc_irq, IRQ_NOAUTOEN);
	ret = devm_request_irq(dwc->dev->parent, dotg->otg_oc_irq,
			dwc3_otg_oc_irq, IRQF_TRIGGER_FALLING,
		       "otg_oc_irq", dotg);
	if (ret) {
		dev_err(dwc->dev, "irqreq otg_oc_irq failed\n");
		dotg->otg_oc_irq = -1;
		goto skip_oc_setup;
	}
skip_oc_setup:
	...
	if (dwc3_otg_create_device(dotg))
		dev_err(dwc->dev, "couldn't create dwc3_otg device\n");

	dotg->reboot_notifier.notifier_call = dwc3_otg_reboot_cb;
	register_reboot_notifier(&dotg->reboot_notifier);
#endif /* CONFIG_BBRY */
...
void dwc3_otg_exit(struct dwc3 *dwc)
{
	...
	if (dotg) {
#ifdef CONFIG_BBRY
		unregister_reboot_notifier(&dotg->reboot_notifier);
#endif
		if (dotg->charger)
