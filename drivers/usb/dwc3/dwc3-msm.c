				  ...
				  union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_IRQ_CTRL:
		val->intval = 0;
		break;
#endif
	default:
...
#if defined(CONFIG_BBRY)
	dev_info(&pdev->dev, "DWC3 init done\n");
#endif
...
#endif
		dev_dbg(mdwc->dev,
			"SKIP setting power supply type again,chg_type = %d, mA=%d \n",
			mdwc->chg_type, mA);
		goto skip_psy_type;
	}
...
#if defined(CONFIG_BBRY)
		/* Disable charging */
		if((mdwc->chg_type == DWC3_INVALID_CHARGER) || (mA == 2)) {
...
#if defined(CONFIG_BBRY)
			dwc3_msm_gadget_vbus_draw(mdwc, 2);
#else
			dwc3_msm_gadget_vbus_draw(mdwc, 0);
#endif
