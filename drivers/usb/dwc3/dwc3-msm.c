...
extern int port_is_connected;	//ADD by TCTNB.YQJ, 2016-08-04,defect-2197097
 ...
 */
 /*MODIFIED-BEGIN by TCTNB.YQJ, 2016-08-04,defect-2197097*/
struct dwc3_msm *mdwc_globle;

int dwc3_msm_config_vbus(int on)
{
	int ret;

	if (IS_ERR_OR_NULL(mdwc_globle->vbus_reg))
		return -EPERM;

	if (on) {
		ret = regulator_enable(mdwc_globle->vbus_reg);
		if (ret) {
			dev_err(mdwc_globle->dev, "unable to enable usb3 vbus_reg\n");
			return ret;
		}
	} else {
		ret = regulator_disable(mdwc_globle->vbus_reg);
		if (ret) {
			dev_err(mdwc_globle->dev, "unable to disable usb3 vbus_reg\n");
			return ret;
		}
	}
...
EXPORT_SYMBOL_GPL(dwc3_msm_config_gdsc);
/*MODIFIED-END by TCTNB.YQJ, defect-2197097*/
static int dwc3_otg_start_host(struct dwc3_msm *mdwc, int on)
{
	...
	if (on) {
		...
		dbg_event(0xFF, "StopHost psync",
			atomic_read(&mdwc->dev->power.usage_count));
		port_is_connected = 0;	//ADD by TCTNB.YQJ, 2016-08-04,defect-2197097
	}
...
static void dwc3_otg_sm_work(struct work_struct *w)
{
	...
	const char *state;
	mdwc_globle = container_of(w, struct dwc3_msm, sm_work.work);	//ADD by TCTNB.YQJ, 2016-08-04,defect-2197097
	if (mdwc->dwc3)
		dwc = platform_get_drvdata(mdwc->dwc3);
