...
//[BUGFIX]-Add-BEGIN by TCTNB.93391,01/20/2016,ALM1430271,USB Driver Auto Install
...
extern u8 remove_cd_rom_flag;
extern u8 is_cd_rom_inited;
extern u8 is_power_off_charging;


extern char function_buff[256];
extern char idProduct_buff[16];
#endif
//[BUGFIX]-Add-END by TCTNB.93391,01/20/2016,ALM1430271,USB Driver Auto Install
...
static int functionfs_ready_callback(struct ffs_data *ffs)
{
	...
	/* [BUGFIX]-Add-BEGIN by TCTNB.Alvin,04/12/2016,ALM#1920321, Case#02417343, temp solution for crash */
	if (!dev)
		return -ENODEV;
	...
	mutex_unlock(&dev->mutex);
	/* [BUGFIX]-Add-END by TCTNB.Alvin,04/12/2016,ALM#1920321 */
	return 0;
}
...
/* BlackBerry */
static int
bb_function_init(struct android_usb_function *f,
		struct usb_composite_dev *cdev)
{
	return bb_setup();
}

static void bb_function_cleanup(struct android_usb_function *f)
{
	bb_cleanup();
}

static int
bb_function_bind_config(struct android_usb_function *f,
		struct usb_configuration *c)
{
	return bb_bind_config(c);
}
static struct android_usb_function bb_function = {
	.name		= "blackberry",
	.init		= bb_function_init,
	.cleanup	= bb_function_cleanup,
	.bind_config	= bb_function_bind_config,
};
#endif
					...
					struct usb_composite_dev *cdev)
{
	...
	fsg_mod_data.removable[0] = true;
        //[BUGFIX]-Add-BEGIN by TCTNB.93391,01/20/2016,ALM1430271,Add CD-ROM
        ...
        //[BUGFIX]-Add-END by TCTNB.93391,01/20/2016,ALM1430271,Add CD-ROM
			    ...
			    const char *buff, size_t size)
{
	...
	if (enabled && !dev->enabled) {
		...
		if (err < 0) {
			...
			dev->connected = 0;
			dev->enabled = true; // [PLATFORM]-MODIFIED by TCTNB.YQJ, 04/21/2016,defect-1944938, qcom patch for ubs adb config switch failed
			mutex_unlock(&dev->mutex);
			return size;
		}
		dev->enabled = true;
