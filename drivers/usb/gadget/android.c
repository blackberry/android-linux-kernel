...
/* [PLATFORM]-Mod-BEGIN by TCTNB.YQJ, 2015/12/08, PR-8980392 ptp device display as mtp device because it is mtp ctrrequest,so delete it as before */
#if 0
...
#endif
/* [PLATFORM]-Mod-END by TCTNB.YQJ, 2015/12/08 */
					...
					struct usb_composite_dev *cdev)
{
	...
	if (dev->pdata && dev->pdata->cdrom) {
		...
		config->fsg.nluns++;
		//[BUGFIX]-Add-BEGIN by TCTNB.93391,11/30/2015,ALM927723,USB Driver Auto Install
		#if defined(CONFIG_JRD_CD_ROM_EMUM_EJECT) && !defined (FEATURE_TCTNB_MMITEST)
		is_cd_rom_inited = TRUE;
		printk("lun0\n");
		#endif
		//[BUGFIX]-Add-END by TCTNB.93391,11/30/2015,ALM927723,USB Driver Auto Install
	}
