...
/* BBAuth */
static int
bbauth_function_init(struct android_usb_function *f,
		struct usb_composite_dev *cdev)
{
	return bbauth_setup();
}

static void bbauth_function_cleanup(struct android_usb_function *f)
{
	bbauth_cleanup();
}

static int
bbauth_function_bind_config(struct android_usb_function *f,
		struct usb_configuration *c)
{
	return bbauth_bind_config(c);
}
static struct android_usb_function bbauth_function = {
	.name		= "bbauth",
	.init		= bbauth_function_init,
	.cleanup	= bbauth_function_cleanup,
	.bind_config	= bbauth_function_bind_config,
};
...
#ifdef CONFIG_BBRY
	/* Read Mass Storage device vendor name */
	ret = of_property_read_string(pdev->dev.of_node,
			"ms_vendor_name",
			&vendor_name);
	if (ret < 0) {
		pr_err("%s(): Failed to read ms_vendor_name\n", __func__);
	}

	/* Read Mass Storage device product name */
	ret = of_property_read_string(pdev->dev.of_node,
			"ms_product_name",
			&product_name);
	if (ret < 0) {
		pr_err("%s(): Failed to read ms_product_name\n", __func__);
	}
#endif
