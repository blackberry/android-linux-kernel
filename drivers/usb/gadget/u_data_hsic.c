		...
		spinlock_t *lock)
{
	...
	pr_debug("%s: ep:%s head:%pK num:%d cb:%pK", __func__, // MODIFIED by xuewei.gu, 2016-11-14,BUG-3462008
			ep->name, head, num, cb);
...
static int ghsic_data_receive(void *p, void *data, size_t len)
{
	...
	pr_debug("%s: p:%pK#%d skb_len:%d\n", __func__, // MODIFIED by xuewei.gu, 2016-11-14,BUG-3462008
...
static void ghsic_data_start_rx(struct gdata_port *port)
{
	...
	pr_debug("%s: port:%pK\n", __func__, port); // MODIFIED by xuewei.gu, 2016-11-14,BUG-3462008
	if (!port)
		return;
...
static void ghsic_data_connect_w(struct work_struct *w)
{
	...
	/* MODIFIED-BEGIN by xuewei.gu, 2016-11-14,BUG-3462008*/
	...
	pr_debug("%s: port:%pK\n", __func__, port);
	/* MODIFIED-END by xuewei.gu,BUG-3462008*/
...
static int ghsic_data_port_alloc(unsigned port_num, enum gadget_type gtype)
{
	...
	pr_debug("%s: port:%pK portno:%d\n", __func__, port, port_num); // MODIFIED by xuewei.gu, 2016-11-14,BUG-3462008
...
static void dbg_timestamp(char *event, struct sk_buff * skb)
{
	...
	scnprintf(dbg_data.buf[dbg_data.idx], DBG_DATA_MSG,
		  "%pK %u[%s] %u %u %u %u %u %u\n", // MODIFIED by xuewei.gu, 2016-11-14,BUG-3462008
