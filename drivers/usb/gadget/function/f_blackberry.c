...
static struct bb_instance *to_bb_instance(struct config_item *item)
{
	return container_of(to_config_group(item), struct bb_instance,
		func_inst.group);
}

static void bb_attr_release(struct config_item *item)
{
	struct bb_instance *fi_bb = to_bb_instance(item);

	usb_put_function_instance(&fi_bb->func_inst);
}

static struct configfs_item_operations bb_item_ops = {
	.release        = bb_attr_release,
};

static struct config_item_type bb_func_type = {
	.ct_item_ops    = &bb_item_ops,
	.ct_owner       = THIS_MODULE,
};

static struct bb_instance *to_fi_bb(struct usb_function_instance *fi)
{
	return container_of(fi, struct bb_instance, func_inst);
}

static int bb_set_inst_name(struct usb_function_instance *fi, const char *name)
{
	struct bb_instance *fi_bb;
	char *ptr;
	int name_len;
...
static void bb_free_inst(struct usb_function_instance *fi)
{
	struct bb_instance *fi_bb;
...
static void bb_free(struct usb_function *f)
{
	pr_debug("bb_free.\n");
	return;
}

static struct usb_function_instance *bb_alloc_inst(void)
{
	struct bb_instance *fi_bb;
	int err;

	fi_bb = kzalloc(sizeof(*fi_bb), GFP_KERNEL);
	if (!fi_bb)
		return ERR_PTR(-ENOMEM);

	fi_bb->func_inst.set_inst_name = bb_set_inst_name;
	fi_bb->func_inst.free_func_inst = bb_free_inst;

	err = bb_setup();
	if (err) {
		kfree(fi_bb);
		pr_err("Error setting blackberry instance\n");
		return ERR_PTR(err);
	}

	config_group_init_type_name(&fi_bb->func_inst.group,
					"", &bb_func_type);

	return  &fi_bb->func_inst;
}

static struct usb_function *bb_alloc(struct usb_function_instance *fi)
{
	struct bb_dev *dev = _bb_dev;

	dev->function.name = "bb";
	...
	dev->function.free_func = bb_free;

	return &dev->function;
}

DECLARE_USB_FUNCTION_INIT(bb, bb_alloc_inst, bb_alloc);
MODULE_LICENSE("GPL");
