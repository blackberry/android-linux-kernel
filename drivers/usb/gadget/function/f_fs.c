			     ...
			     size_t len, loff_t *ptr)
{
	...
	case FFS_READ_STRINGS:
		...
		if (ffs->state == FFS_READ_DESCRIPTORS) {
			...
			//set_bit(FFS_FL_CALL_CLOSED_CALLBACK, &ffs->flags);  //[PLATFORM]-MODIFIED by TCTNB.YQJ, 04/21/2016,defect-1944938, qcom patch for ubs adb config switch failed
			return len;
		}
		break;
...
static void ffs_data_clear(struct ffs_data *ffs)
{
	...
	//if (test_and_clear_bit(FFS_FL_CALL_CLOSED_CALLBACK, &ffs->flags))  //[PLATFORM]-MODIFIED by TCTNB.YQJ, 04/21/2016,defect-1944938, qcom patch for ubs adb config switch failed
		ffs_closed(ffs);
...
/*[PLATFORM]-MODIFIED BEGIN by TCTNB.YQJ, 04/21/2016,defect-1944938, qcom patch for ubs adb config switch failed */
...
/*[PLATFORM]-MODIFIED END by TCTNB.YQJ, 04/21/2016*/
done:
	ffs_dev_unlock();
	return ret;
}
