...
static void send_file_work(struct work_struct *data)
{
	...
	/* [PLATFORM]-Add-BEGIN by TCTNB.Alvin, Defect#5586537, Merge the Google security patch, CVE-2017-13163 */
	if (count < 0) {
...
static long mtp_ioctl(struct file *fp, unsigned code, unsigned long value)
{
	...
	break;
	case MTP_SET_STATE:
		if (value > STATE_ERROR || value == STATE_OFFLINE)
			goto fail;
		spin_lock_irq(&dev->lock);
		if (dev->state != STATE_OFFLINE)
			dev->state = value;
	...
	unsigned long value)
{
	...
	case COMPAT_MTP_SEND_EVENT:
		...
		break;
#ifdef CONFIG_BBRY
	case COMPAT_MTP_GET_STATE:
		spin_lock_irq(&dev->lock);
		ret = dev->state;
		spin_unlock_irq(&dev->lock);
		return ret;
	case COMPAT_MTP_SET_STATE:
		if (value > STATE_ERROR || value == STATE_OFFLINE)
			goto fail;
		spin_lock_irq(&dev->lock);
		if (dev->state != STATE_OFFLINE)
