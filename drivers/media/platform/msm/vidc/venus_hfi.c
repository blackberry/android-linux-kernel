		...
		bool *rx_req_is_set)
{
	...
	u32 empty_space, read_idx, write_idx; // MODIFIED by hongwei.tian, 2019-05-08,BUG-7711520
	u32 *write_ptr;
	...
	/* MODIFIED-BEGIN by hongwei.tian, 2019-05-08,BUG-7711520*/
	...
	empty_space = (write_idx >=  read_idx) ?
		...
		(read_idx - write_idx);
		/* MODIFIED-END by hongwei.tian,BUG-7711520*/
		...
		void *pkt, bool *requires_interrupt)
{
	...
	if (!__core_in_valid_state(device)) {
		dprintk(VIDC_ERR, "%s - fw not in init state\n", __func__); // MODIFIED by hongwei.tian, 2019-02-28,BUG-7446002
		result = -EINVAL;
		goto err_q_null;
	}
...
static int __response_handler(struct venus_hfi_device *device)
{
	...
	while (!__iface_msgq_read(device, raw_packet)) {
		...
		/* MODIFIED-BEGIN by hongwei.tian, 2019-02-28,BUG-7446002*/
		/* do not read packets after sys error packet */
		if (info->response_type == HAL_SYS_ERROR)
			break;
			/* MODIFIED-END by hongwei.tian,BUG-7446002*/
	}
...
err_no_work:
	...
	for (i = 0; !IS_ERR_OR_NULL(device->response_pkt) &&
		...
		/* MODIFIED-BEGIN by Haojun Chen, 2019-02-14,BUG-7446002*/
		if (!__core_in_valid_state(device)) {
			dprintk(VIDC_ERR,
		...
		/* MODIFIED-END by Haojun Chen,BUG-7446002*/
