...
static int32_t msm_flash_status(
	struct msm_flash_ctrl_t *flash_ctrl,
	struct msm_flash_cfg_data_t *flash_data)
{
	int32_t rc = 0;
	...
	for (i = 0; i < flash_ctrl->flash_num_sources; i++) {
		if (flash_ctrl->flash_trigger[i]) {
			rc = led_trigger_get_status(flash_ctrl->flash_trigger[i], &reg_value, &curr_l);
			if (rc < 0) {
				pr_err("%s:%d get flash status failed rc = %d",
					__func__, __LINE__, rc);
				return rc;
			}
			CDBG(
				"PMIC Flash status[%d] reg 0x%x, current %d mA\n",
				i, reg_value, curr_l);
			/* use flash_duration to hold FAULT STATUS register reading */
			flash_data->flash_duration[i] = reg_value;
			flash_data->flash_current[i] = curr_l;
		}
	}

	for (i = 0; i < flash_ctrl->torch_num_sources; i++) {
		if (flash_ctrl->torch_trigger[i]) {
			rc = led_trigger_get_status(flash_ctrl->torch_trigger[i], &reg_value, &curr_l);
			if (rc < 0) {
				pr_err("%s:%d get flash status failed rc = %d",
					__func__, __LINE__, rc);
				return rc;
			...
			CDBG(
				"PMIC Torch status[%d] reg 0x%x, current %d mA\n",
				i, reg_value, curr_l);
			/* use flash_duration to hold FAULT STATUS register reading */
			flash_data->flash_duration[i] = reg_value;
			flash_data->flash_current[i] = curr_l;
	...
	void __user *argp)
{
	...
	case CFG_FLASH_STATUS:
		rc = flash_ctrl->func_tbl->camera_flash_status(
			flash_ctrl, flash_data);
		break;
#endif
	default:
...
#ifdef CONFIG_BBRY_MFG
		.camera_flash_status = msm_flash_status,
#endif /*ifdef CONFIG_BBRY_MFG */
	},
};
