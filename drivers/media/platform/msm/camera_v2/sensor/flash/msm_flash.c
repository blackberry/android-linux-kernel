...
extern int brightness_set_frontcamera_mode(int enable);
extern int TCT_get_sensor_name(void);
/* MODIFIED-BEGIN by yongzhi.li, 2016-12-30,BUG-3877090*/
static int TCT_torch_mode = 0;
static int TCT_release = 0;
static int TCT_flash_init = 0; // MODIFIED by yongzhi.li, 2017-01-11,BUG-3996240
/* MODIFIED-END by yongzhi.li,BUG-3877090*/
	...
	struct msm_flash_cfg_data_t *flash_data)
{
	...
	if (TCT_get_sensor_name()&&(TCT_torch_mode==0)){ // MODIFIED by yongzhi.li, 2016-12-23,BUG-3821899
			if (TCT_release==1){
			     brightness_set_frontcamera_mode(3);
			}
            TCT_release=0;
		return 0;
	}
	...
	struct msm_flash_cfg_data_t *flash_data)
{
	...
	enum msm_flash_driver_type flash_driver_type = FLASH_DRIVER_DEFAULT;
        ...
        TCT_flash_init = TCT_flash_init + 1; // MODIFIED by yongzhi.li, 2017-01-11,BUG-3996240
	...
	struct msm_flash_cfg_data_t *flash_data)
{
	...
	/* MODIFIED-BEGIN by yongzhi.li, 2016-12-23,BUG-3821899*/
	if ((TCT_get_sensor_name() == 1) && (TCT_torch_mode == 0)){
	    brightness_set_frontcamera_mode(0);
            TCT_release=1;
	    /* MODIFIED-END by yongzhi.li,BUG-3821899*/
		brightness_set_frontcamera_mode(1);
		return 0;
	}
#ifdef CONFIG_BBRY
	flash_ctrl->current_flash_cfg = flash_data->cfg_type;
	...
	struct msm_flash_cfg_data_t *flash_data)
{
	...
	/* MODIFIED-BEGIN by yongzhi.li, 2016-12-16,BUG-3767349*/
	if (TCT_get_sensor_name()){
	        brightness_set_frontcamera_mode(0);
                TCT_release=1; // MODIFIED by yongzhi.li, 2016-12-30,BUG-3877090
		brightness_set_frontcamera_mode(2);
		return 0;
	}
...
{
	int32_t rc = 0;
        /* MODIFIED-BEGIN by yongzhi.li, 2017-01-11,BUG-3996240*/
        TCT_flash_init = TCT_flash_init - 1;
	if (flash_ctrl->flash_state == MSM_CAMERA_FLASH_RELEASE || TCT_flash_init > 0) {
		pr_err("%s:%d Invalid flash state or have flash exist = %d",
		/* MODIFIED-END by yongzhi.li,BUG-3996240*/
			__func__, __LINE__, flash_ctrl->flash_state);
		return 0;
	}
