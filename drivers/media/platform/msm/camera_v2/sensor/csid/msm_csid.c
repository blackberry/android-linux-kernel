...
static int32_t msm_csid_read_reg(struct csid_device *csid_dev, void *arg)
{
	csid_reg_t *regInfo;
	...
	if (csid_dev->base == NULL) {
		rc = -ENOMEM;
		goto exit_function;
	}

	regInfo = (csid_reg_t *)arg;
	offset  = regInfo->offset;

	if (regInfo->offset >= resource_size(csid_dev->mem)) {
		rc = -EFAULT;
		goto exit_function;
	}

	value = msm_camera_io_r(csid_dev->base + regInfo->offset);
	rc    = 0;
...
exit_function:
	...
	CDBG("%s:%d reg(0x%08x) = 0x%08x [rc=%d]\n",
		__func__, __LINE__, offset, value, rc);
			...
			unsigned int cmd, void *arg)
{
	...
	case MSM_SD_SHUTDOWN:
		...
		break;
#ifdef CONFIG_BBRY_MFG
	case VIDIOC_MSM_CSID_READ_REGISTER:
		rc = msm_csid_read_reg(csid_dev, arg);
		break;
#endif
	default:
