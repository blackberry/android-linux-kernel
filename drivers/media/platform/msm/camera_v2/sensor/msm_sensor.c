...
#include "ov8856_mercury_OTP.h"
#include "Qtech_s5k4h8_mercury.h" // MODIFIED by yongzhi.li, 2016-12-15,BUG-3403685
#include "Qtech_s5k4h8_krypton.h"
#endif
...
/* MODIFIED-BEGIN by yongzhi.li, 2016-12-16,BUG-3767349*/
static const char *Mercury_sensor_name;

int TCT_get_sensor_name(void)
{
      if ((!strcmp(Mercury_sensor_name, "ov8856_krypton"))||
         (!strcmp(Mercury_sensor_name, "s5k4h8_mercury")) ||
         (!strcmp(Mercury_sensor_name, "ov8856")) ||
	 (!strcmp(Mercury_sensor_name, "s5k4h8_krypton"))) {
        return 1;
    }
    else
        return 0;
}
/* MODIFIED-END by yongzhi.li,BUG-3767349*/
...
static int TCT_update_OTP(struct msm_sensor_ctrl_t *s_ctrl)
{
    ...
    if (!strcmp(s_ctrl->sensordata->sensor_name, "ov8856")) {
        printk("OTP: %s update\n",s_ctrl->sensordata->sensor_name);
        ov8856_mercury_update_otp_wb(s_ctrl->sensor_i2c_client);
        ov8856_mercury_update_otp_lenc(s_ctrl->sensor_i2c_client);
    }
    /* MODIFIED-BEGIN by yongzhi.li, 2016-12-15,BUG-3403685*/
    if ((!strcmp(s_ctrl->sensordata->sensor_name, "s5k4h8_mercury"))) {
        printk("OTP: %s update\n",s_ctrl->sensordata->sensor_name);
         s5k4h8_mercury_check_otp_info(s_ctrl->sensor_i2c_client);
    }
    /* MODIFIED-END by yongzhi.li,BUG-3403685*/
	if ((!strcmp(s_ctrl->sensordata->sensor_name, "s5k4h8_krypton"))) {
        printk("OTP: %s update\n",s_ctrl->sensordata->sensor_name);
         s5k4h8_krypton_check_otp_info(s_ctrl->sensor_i2c_client);
    }
	else {
...
int msm_sensor_power_up(struct msm_sensor_ctrl_t *s_ctrl)
{
    ...
    Mercury_sensor_name = s_ctrl->sensordata->sensor_name; // MODIFIED by yongzhi.li, 2016-12-16,BUG-3767349
...
int msm_sensor_match_id(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	if (msm_sensor_id_by_mask(s_ctrl, chipid) != slave_info->sensor_id) {
       ...
       /* MODIFIED-BEGIN by yongzhi.li, 2016-12-01,BUG-3610766*/
       rc = msm_sensor_match_module(s_ctrl);
        if (rc < 0){
            return -ENODEV;
        }
	return rc;
...
int msm_sensor_match_module(struct msm_sensor_ctrl_t *s_ctrl)
{
    int rc = 0;
    int j = 832;
    uint16_t chipid = 0;
    ...
    sensor_i2c_client = s_ctrl->sensor_i2c_client;
    sensor_name = s_ctrl->sensordata->sensor_name;
    if ((!strcmp(s_ctrl->sensordata->sensor_name, "imx378"))) {
         rc=sensor_i2c_client->i2c_func_tbl->i2c_write(
	    sensor_i2c_client, 0x0136,
	    0x0c, MSM_CAMERA_I2C_BYTE_DATA);
         rc=sensor_i2c_client->i2c_func_tbl->i2c_write(
	    sensor_i2c_client, 0x0137,
	    0x00, MSM_CAMERA_I2C_BYTE_DATA);
         rc=sensor_i2c_client->i2c_func_tbl->i2c_write(
	    sensor_i2c_client, 0x0A02,
	    13, MSM_CAMERA_I2C_BYTE_DATA);
         rc=sensor_i2c_client->i2c_func_tbl->i2c_write(
	    sensor_i2c_client, 0x0A00,
	    0x01, MSM_CAMERA_I2C_BYTE_DATA);
         for(chipid1=0x0A04; chipid1<=0x0A43; chipid1++){
            rc = sensor_i2c_client->i2c_func_tbl->i2c_read(
		 sensor_i2c_client, chipid1,
		 &chipid, MSM_CAMERA_I2C_BYTE_DATA);
             i++;
        }
        rc = sensor_i2c_client->i2c_func_tbl->i2c_read(
		sensor_i2c_client, 0x0A3F,
		&chipid, MSM_CAMERA_I2C_BYTE_DATA);
        if(0x00 == chipid)
             rc = 0;
        else
             rc = -1;
    }else if((!strcmp(s_ctrl->sensordata->sensor_name, "imx378_Qtech"))) {
        rc=sensor_i2c_client->i2c_func_tbl->i2c_write(
	    sensor_i2c_client, 0x0136,
	    0x0c, MSM_CAMERA_I2C_BYTE_DATA);
         rc=sensor_i2c_client->i2c_func_tbl->i2c_write(
        ...
        if(0x06 == chipid)
         ...
         rc = 0;
    pr_err("sensor name %sand rc %d",sensor_name,rc);
    return rc;
}
