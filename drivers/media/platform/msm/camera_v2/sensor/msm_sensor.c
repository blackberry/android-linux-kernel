...
int msm_sensor_config(struct msm_sensor_ctrl_t *s_ctrl, void __user *argp)
{
	...
	case CFG_GET_SENSOR_INFO:
		...
		memcpy(cdata->cfg.sensor_info.actuator_name,
			s_ctrl->sensordata->actuator_name,
			sizeof(cdata->cfg.sensor_info.actuator_name));
		memcpy(cdata->cfg.sensor_info.eeprom_name,
			s_ctrl->sensordata->eeprom_name,
			sizeof(cdata->cfg.sensor_info.eeprom_name));
		pr_err("%s:%d eeprom %s \n", __func__, __LINE__,
			s_ctrl->sensordata->eeprom_name);
