...
#include "Qtech_s5k4h8_idol4.h"
#include "Qtech_s5k4h8_idol4s.h" //MODIFIED by XuNan, 2016-04-08,BUG-1924423
#endif
/*xiaoming.hu@tcl.com ov8858 idol4 otp*/
#undef CDBG
...
#endif
    } else if(!strcmp(s_ctrl->sensordata->sensor_name, "s5k4h8_idol4")){
	    pr_err("s5k4h8_idol4 should I do calbiration by this way?!\n");
    	s5k4h8_idol4_check_otp_info(s_ctrl->sensor_i2c_client);
    /*MODIFIED-BEGIN by XuNan, 2016-04-08,BUG-1924423*/
    } else if(!strcmp(s_ctrl->sensordata->sensor_name, "s5k4h8_idol4s")){
	    pr_err("s5k4h8_idol4s should I do calbiration by this way?!\n");
        s5k4h8_idol4s_check_otp_info(s_ctrl->sensor_i2c_client);
        /*MODIFIED-END by XuNan,BUG-1924423*/
#ifdef CONFIG_BBRY
    } else if (!strcmp(s_ctrl->sensordata->sensor_name, "ov8858")) {
        pr_err("OTP: %s update\n",s_ctrl->sensordata->sensor_name);
        ov8858_idol4_update_otp_wb(s_ctrl->sensor_i2c_client);
        ov8858_idol4_update_otp_lenc(s_ctrl->sensor_i2c_client);
#endif /* ifdef CONFIG_BBRY */
    } else {
        pr_err("%s, %s unregister OTP info here!\n", __func__, s_ctrl->sensordata->sensor_name);
    }
