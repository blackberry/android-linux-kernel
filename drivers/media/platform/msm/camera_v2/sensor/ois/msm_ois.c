		...
		case MSM_OIS_READ: {
			...
			case MSM_CAMERA_I2C_WORD_DATA:
				rc = o_ctrl->i2c_client.i2c_func_tbl
					...
					settings[i].reg_addr,
					(uint16_t*)&settings[i].reg_data,
					settings[i].data_type);
				CDBG("ois settings[%d].reg_addr = %d, settings.reg_data = %d, settings.data_type = %d\n", i, settings[i].reg_addr, settings[i].reg_data, settings[i].data_type);
				break;
	...
	struct msm_ois_set_info_t *set_info)
{
	...
	if (set_info->ois_params.setting_size > 0 &&
		...
		rc = msm_ois_write_settings(o_ctrl,
			...
			settings);
#ifdef CONFIG_BBRY_MFG
		if (copy_to_user((void __user *)set_info->ois_params.settings,
			(void *)settings,
			set_info->ois_params.setting_size *
			sizeof(struct reg_settings_ois_t))) {
