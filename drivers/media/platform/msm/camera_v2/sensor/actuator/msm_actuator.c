	...
	int16_t next_lens_position, uint32_t hw_params, uint16_t delay)
{
	...
	for (i = 0; i < size; i++) {
		...
		if (write_arr[i].reg_write_type == MSM_ACTUATOR_WRITE_DAC) {
            ...
            /*MODIFIED-BEGIN by hui.dang, 2016-04-09,BUG-1644627*/
            // add by danghui for bu64297 start
            if(write_arr[i].reg_addr == 0xC000)
            {
				value |= write_arr[i].hw_mask;
				i2c_byte1 = (value & 0xFF00) >> 8;
				i2c_byte2 = value & 0xFF;
            }
            else // add by danghui for bu64297 end
            {
            /*MODIFIED-END by hui.dang,BUG-1644627*/
            ...
            } //MODIFIED by hui.dang, 2016-04-09,BUG-1644627
			} else {
				i2c_byte1 = (value & 0xFF00) >> 8;
				i2c_byte2 = value & 0xFF;
			}
...
static int32_t msm_actuator_park_lens(struct msm_actuator_ctrl_t *a_ctrl)
{
	...
	//add by danghui for mid_openloop vcm park 20160416 Start
	if(a_ctrl->reg_tbl[0].reg_addr == 0xC000)
	{
		const uint16_t DEST_LENS_POS = 512;

		while (next_lens_pos != DEST_LENS_POS) {
			/* conditions which help to reduce park lens time */
			if(next_lens_pos > DEST_LENS_POS)
			{
				if (next_lens_pos-DEST_LENS_POS > (a_ctrl->park_lens.max_step *
					PARK_LENS_LONG_STEP)) {
					next_lens_pos = next_lens_pos -
						(a_ctrl->park_lens.max_step *
						PARK_LENS_LONG_STEP);
				} else if (next_lens_pos-DEST_LENS_POS > (a_ctrl->park_lens.max_step *
					PARK_LENS_MID_STEP)) {
					next_lens_pos = next_lens_pos -
						(a_ctrl->park_lens.max_step *
						PARK_LENS_MID_STEP);
					...
					next_lens_pos = (next_lens_pos-DEST_LENS_POS >
						a_ctrl->park_lens.max_step) ?
						(next_lens_pos - a_ctrl->park_lens.
						max_step) : DEST_LENS_POS;
				}
			}
			else
			{
				if (DEST_LENS_POS - next_lens_pos > (a_ctrl->park_lens.max_step *
					PARK_LENS_LONG_STEP)) {
					next_lens_pos = next_lens_pos +
						(a_ctrl->park_lens.max_step *
						PARK_LENS_LONG_STEP);
				} else if (DEST_LENS_POS - next_lens_pos > (a_ctrl->park_lens.max_step *
					PARK_LENS_MID_STEP)) {
					next_lens_pos = next_lens_pos +
						(a_ctrl->park_lens.max_step *
						PARK_LENS_MID_STEP);
					...
					next_lens_pos = (DEST_LENS_POS - next_lens_pos >
						a_ctrl->park_lens.max_step) ?
						(next_lens_pos + a_ctrl->park_lens.
						max_step) : DEST_LENS_POS;
				}
	...
	{
	//add by danghui for mid_openloop vcm park 20160416 End
		while (next_lens_pos) {
			/* conditions which help to reduce park lens time */
