	...
	struct msm_camera_cci_ctrl *c_ctrl, enum cci_i2c_queue_t queue)
{
	...
	uint8_t data[12];
#else
	uint8_t data[I2C_SEQ_REG_DATA_MAX+2+1];
...
#ifdef CONFIG_BBRY
static int32_t bbry_cci_pins_control(struct v4l2_subdev *sd)
{
	struct cci_device *cci_dev;
	int32_t rc = 0;
	...
	if (!(cci_dev->cci_pinctrl_status) && !(cci_dev->ref_count)) {
		rc = msm_cci_pinctrl_init(cci_dev);
...
static void bbry_cci_pins_release(struct v4l2_subdev *sd)
{
	struct cci_device *cci_dev;
...
{
	int32_t needToReleasePins = 0;
	int32_t rc = 0;
	struct cci_device *cci_dev;

	cci_dev = v4l2_get_subdevdata(sd);

	needToReleasePins = bbry_cci_pins_control(sd);
	...
	if(cci_dev->cci_pinctrl_status) {
		if (cci_dev->gpio_state_cci0_pull_down) {
			...
			if (rc)
				pr_err("%s:%d cannot set CCI0 pins to pull down state\n",
					__func__, __LINE__);
		}
	}

	if (needToReleasePins)
		bbry_cci_pins_release(sd);
	...
	if (cci_dev->cci_pinctrl_status) {
		if(!(cci_dev->ref_count)) {
			...
			rc = pinctrl_select_state(cci_dev->cci_pinctrl.pinctrl,
				cci_dev->cci_pinctrl.gpio_state_active);
			if (rc)
				pr_err("%s:%d cannot set pins to active state\n",
				__func__, __LINE__);
		}
	}
