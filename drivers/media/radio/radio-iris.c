...
/* [PLATFORM]-Mod-BEGIN by YuBin, 2017-05-03, fm lna gpio */
#ifdef CONFIG_KRYPTON
	gpio_direction_output(radio->fm_lna_gpio, 1);
#endif
/* [PLATFORM]-Mod-END by YuBin */
	if (!radio) {
		FMDERR(":radio is null");
...
/* [PLATFORM]-Mod-BEGIN by YuBin, 2017-05-03, fm lna gpio */
#ifdef CONFIG_KRYPTON
	gpio_direction_output(radio->fm_lna_gpio, 0);
#endif
/* [PLATFORM]-Mod-END by YuBin */
...
/* [PLATFORM]-Mod-BEGIN by YuBin, 2017-05-03, fm lna gpio */
#ifdef CONFIG_KRYPTON
	radio->fm_lna_gpio = of_get_named_gpio(pdev->dev.of_node, "qcom,fm_lna", 0);
		pr_err(" 000 panluqin  fm  gpio141=%d  \n",radio->fm_lna_gpio  );

			if (gpio_is_valid(radio->fm_lna_gpio))
			{
				if(gpio_request(radio->fm_lna_gpio, "fm_lna_gpio"))
					pr_err("%s fm_lna_gpio request failed!\n", __func__);
				else
					gpio_direction_output(radio->fm_lna_gpio, 0);
			}
			else
			{
				pr_err("%s fm_lna_gpio is not valid!\n", __func__);
			}
#endif
/* [PLATFORM]-Mod-END by YuBin */
