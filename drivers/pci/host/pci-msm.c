...
#ifdef CONFIG_BBRY
/* pr_alert is not the right macro to use here, switch to pr_info */
#undef pr_alert
#define pr_alert pr_info
#endif

#ifdef CONFIG_BBRY
#define DDT_CREATOR_ID_RC "pcie_rc"
#define DDT_CREATOR_ID_EP "pcie_ep"
#endif /* CONFIG_BBRY */
...
struct msm_pcie_dev_t {
	...
	spinlock_t			aer_lock;
#ifdef CONFIG_BBRY
	struct work_struct	     handle_aer_work;
	struct mutex		     process_aer_lock;
	bool				aer_happens;
	bool				ddt_to_send;
	uint32_t			ddt_event;
	struct logworthy_event_details_t ddt_details;
#endif /* CONFIG_BBRY */
					...
					u32 testcase)
{
	...
	case 13: /* dump all registers of base_sel */
...
#ifdef CONFIG_BBRY
		/*
		 * JI 1371090, QC case 02230149, test code
		 * dump 1KB PCIe config space when aer happens
		 */
			base_sel_size = 0x400;
#else
		...
		for (i = 0; i < base_sel_size; i += 32) {
			 ...
			 * JI 1371090, QC case 02230149, test code
			 */
			PCIE_DUMP(dev, "0x%04x %08x %08x %08x %08x %08x %08x %08x %08x\n",
#else
...
#ifdef CONFIG_BBRY
	PCIE_INFO(dev, "PCIe: RC%d gpio %d, irq %d\n",
		dev->rc_idx, dev->gpio[MSM_PCIE_GPIO_WAKE].num, dev->wake_n);
#endif /* CONFIG_BBRY */
...
#ifdef CONFIG_BBRY
		/* Use notify->data to pass type information to registered EP clients. */
		struct msm_pcie_notify_data notify_data;
		switch (event) {
		case MSM_PCIE_EVENT_LINKDOWN:
			notify_data.type = MSM_PCIE_NOTIFY_DATA_LINKDOWN;
			break;
		case MSM_PCIE_EVENT_WAKEUP:
			notify_data.type = MSM_PCIE_NOTIFY_DATA_WAKEUP;
			break;
		default:
			notify_data.type = MSM_PCIE_NOTIFY_DATA_INVALID;
			PCIE_ERR(dev, "PCIe: RC%d event %d unexpected.\n",
				dev->rc_idx, event);
			break;
		}
		notify->data = (void *) &notify_data;
#endif /* CONFIG_BBRY */
		notify->event = event;
...
#ifdef CONFIG_BBRY
static int process_aer_notify(struct msm_pcie_dev_t *dev, enum msm_pcie_event event)
{
	int ret = 0;
	/* We don't bother to support multiple EPs. */
	if ( dev->event_reg && dev->event_reg->callback &&
		(dev->event_reg->events & event)) {
		struct msm_pcie_notify_data notify_data;
		struct msm_pcie_notify *notify = &dev->event_reg->notify;

		notify_data.type = MSM_PCIE_NOTIFY_DATA_AER;
		notify->data = (void *) &notify_data;
		notify->event = event;
		notify->user = dev->event_reg->user;
		...
		dev->user_suspend = true;
		PCIE_INFO(dev,
			"PCIe: creator %s d1 0x%x d2 0x%x RC%d %s\n",
			dev->ddt_details.creator_id,
			dev->ddt_details.d1,
			dev->ddt_details.d2,
			dev->rc_idx,
			pci_name(dev->dev));

		/* Callback to dhd driver for processing */
		dev->event_reg->callback(notify);

		notify->data = NULL;

		/* Send out a quip event */
		ret = ddt_send(dev->ddt_event,
			(const struct logworthy_event_details_t *) &dev->ddt_details,
			"['kerneldump:///',"
			"'file:///sys/kernel/debug/ipc_logging/pcie0-short/log',"
			"'file:///sys/kernel/debug/ipc_logging/pcie0-long/log',"
			"'file:///sys/kernel/debug/ipc_logging/pcie0-dump/log',"
			"'dumpsys://wifi',"
			"'logcat://* /-v threadtime']");
		if (ret != 0) {
			PCIE_ERR(dev, "error ddt_send %d\n", ret);
		} else {
			PCIE_INFO(dev, "succ ddt_send %d\n", ret);
		}
	} else {
		ret = -EBADSLT;
		PCIE_ERR(dev,
			"PCIe: creator %s d1 0x%x d2 0x%x RC%d %s, invalid event_reg %p\n",
			dev->ddt_details.creator_id,
			dev->ddt_details.d1,
			dev->ddt_details.d2,
			dev->rc_idx,
	...
	/* Clear the ddt data */
	dev->ddt_details.d1 = 0;
	dev->ddt_details.d2 = 0;
	dev->ddt_event = 0;
	dev->ddt_details.creator_id = NULL;
	/* Now open to new aer report */
	dev->aer_happens = false;
	dev->ddt_to_send = false;
	return ret;
}

static void handle_aer_func(struct work_struct *work)
{
	struct msm_pcie_dev_t *dev = container_of(work, struct msm_pcie_dev_t,
					handle_aer_work);

	PCIE_DBG(dev, "PCIe: AER work for RC%d, dev %p\n", dev->rc_idx, dev);

	mutex_lock(&dev->process_aer_lock);
	process_aer_notify(dev, MSM_PCIE_EVENT_LINKDOWN);
	mutex_unlock(&dev->process_aer_lock);
	return;
}
#endif /* CONFIG_BBRY */
...
static irqreturn_t handle_aer_irq(int irq, void *data)
{
	...
	rc_sel = 1 << dev->rc_idx;
	base_sel = 5;
	msm_pcie_sel_debug_testcase(dev, 13);
#endif /* CONFIG_BBRY */
	...
	/* RC AER */
	if ((rc_dev_ctrlstts >> 18) & 0x1) {
		dev->aer_happens = true;
		dev->ddt_event = LW_EVENT_PCIE_AER_00002541;
		/* RC bdf is 0 */
		dev->ddt_details.d1 = dev->rc_idx << 16;
		dev->ddt_details.d2 = uncorr_val;
		dev->ddt_details.creator_id = DDT_CREATOR_ID_RC;
	} else if ((rc_dev_ctrlstts >> 17) & 0x1) {
		dev->aer_happens = true;
		dev->ddt_event = LW_EVENT_PCIE_AER_00002542;
		/* RC bdf is 0 */
		dev->ddt_details.d1 = dev->rc_idx << 16;
		dev->ddt_details.d2 = corr_val;
		dev->ddt_details.creator_id = DDT_CREATOR_ID_RC;
	} else if ((rc_dev_ctrlstts >> 16) & 0x1) {
...
#ifdef CONFIG_BBRY
		/* EP AER.
		 * Overwriting may happen when multiple incidents come in side by side.
		 * The later one overwrites the previous one.
		 */
		if ((ep_dev_ctrlstts >> 18) & 0x1) {
			dev->aer_happens = true;
			dev->ddt_event = LW_EVENT_PCIE_AER_00002541;
			dev->ddt_details.d1 = dev->rc_idx << 16 | ep_src_bdf >> 16;
			dev->ddt_details.d2 = ep_uncorr_val;
			dev->ddt_details.creator_id = DDT_CREATOR_ID_EP;
		} else if ((ep_dev_ctrlstts >> 17) & 0x1) {
			dev->aer_happens = true;
			dev->ddt_event = LW_EVENT_PCIE_AER_00002542;
			/* RC bdf is 0 */
			dev->ddt_details.d1 = dev->rc_idx << 16;
			dev->ddt_details.d2 = ep_corr_val;
			dev->ddt_details.creator_id = DDT_CREATOR_ID_EP;
		} else if ((ep_dev_ctrlstts >> 16) & 0x1) {
			/* so far we do nothing here */
		}
#endif /* CONFIG_BBRY */
	}
out:
	...
	spin_unlock_irqrestore(&dev->aer_lock, irqsave_flags);
#ifdef CONFIG_BBRY
	/* There may be more aer irq happens before the current scheduled one
	 * being executed. In this situation we don't schedule new work any more,
	 * until the scheduled one has been executed. The quip report data will
	 * keep updating thus it carries the information of the latest one. */
	if (dev->aer_happens == true && dev->ddt_to_send == false) {
		schedule_work(&dev->handle_aer_work);
		dev->ddt_to_send = true;
	}
#endif /* CONFIG_BBRY */
	return IRQ_HANDLED;
}
...
int32_t msm_pcie_irq_init(struct msm_pcie_dev_t *dev)
{
	...
	unsigned int irq_flags = IRQF_TRIGGER_NONE;
...
#ifdef CONFIG_BBRY
	INIT_WORK(&dev->handle_aer_work, handle_aer_func);
#endif /* CONFIG_BBRY */
	...
	if (np) {
		...
		if (intr_src) {
			if (strncmp(intr_src, "gpio0", sizeof("gpio0")) == 0) {
				irq_flags = IRQF_TRIGGER_RISING;
				PCIE_INFO(dev, "PCIe: RC%d: Wlan interrupt source: %s\n",
					dev->rc_idx, intr_src);
			} else {
				PCIE_ERR(dev, "PCIe: RC%d: %s is not a real interrupt source\n",
					dev->rc_idx, intr_src);
			}
		} else {
			PCIE_ERR(dev, "PCIe: RC%d: Unable to find wlan interrupt source: intr_src\n",
				dev->rc_idx);
		}
	} else {
		PCIE_ERR(dev, "PCIe: RC%d: Unable to find wlan device tree node\n",
			dev->rc_idx);
	}

	if (irq_flags != IRQF_TRIGGER_NONE) {
		PCIE_INFO(dev, "PCIe: RC%d: Wlan interrupt is %s edge triggered\n",
			dev->rc_idx,
			irq_flags == IRQF_TRIGGER_FALLING ? "falling" : "rising");

		rc = devm_request_irq(pdev,
			dev->wake_n, handle_wake_irq, irq_flags,
			 "msm_pcie_wake", dev);
		if (rc) {
...
#ifdef CONFIG_BBRY
	msm_pcie_dev[rc_idx].aer_happens = false;
	msm_pcie_dev[rc_idx].ddt_to_send = false;
	msm_pcie_dev[rc_idx].ddt_event = 0;
	msm_pcie_dev[rc_idx].ddt_details.d1 = 0;
	msm_pcie_dev[rc_idx].ddt_details.d2 = 0;
	msm_pcie_dev[rc_idx].ddt_details.d3 = 0;
	msm_pcie_dev[rc_idx].ddt_details.d4 = 0;
	msm_pcie_dev[rc_idx].ddt_details.creator_id = NULL;
#endif /* CONFIG_BBRY */
...
#ifdef CONFIG_BBRY
		mutex_init(&msm_pcie_dev[i].process_aer_lock);
#endif /* CONFIG_BBRY */
