...
static void msm_pcie_config_link_state(struct msm_pcie_dev_t *dev)
{
	...
	if (dev->l1ss_supported) {
		...
		if (dev->l0s_supported) {
			/* EP: Set ASPM(0xbc) (enable L0s & L1) */
			msm_pcie_write_mask(dev->conf + PCIE20_CAP_LINKCTRLSTATUS_BRCM, 0,
				BIT(8)|BIT(6)|BIT(1)|BIT(0));
			/* RC: Set ASPM and ComClkConfig (enable L0s & L1) */
			msm_pcie_write_mask(dev->dm_core + PCIE20_CAP_LINKCTRLSTATUS, 0,
				BIT(6)|BIT(1)|BIT(0));
		} else {
			/* EP: Set ASPM(0xbc) (enable L1 only) */
			msm_pcie_write_mask(dev->conf + PCIE20_CAP_LINKCTRLSTATUS_BRCM, 0,
				BIT(8)|BIT(6)|BIT(1));
			/* RC: Set ASPM and ComClkConfig (enable L1 only) */
			msm_pcie_write_mask(dev->dm_core + PCIE20_CAP_LINKCTRLSTATUS, 0,
				BIT(6)|BIT(1));
		}
...
int32_t msm_pcie_irq_init(struct msm_pcie_dev_t *dev)
{
	...
	if (np) {
		...
		if (intr_src) {
			PCIE_DBG(dev, "PCIe: RC%d: Wlan interrupt source: %s\n", dev->rc_idx,
				intr_src);
...
#if defined(CONFIG_BBRY) && defined(CONFIG_ARCH_MSM8994)
int msm_pcie_is_user_suspend(struct pci_dev *dev)
{
	int rc_idx = 0;
	struct msm_pcie_dev_t *pcie_dev;
	...
	if (!dev->bus->sysdata) {
		PCIE_DBG(pcie_dev,
			"driver name %s, sysdata null ptr\n",
			dev->driver ? dev->driver->driver.name : "NA");
		return -EINVAL;
	} else {
		pcie_dev = PCIE_BUS_PRIV_DATA(dev);
	}

	while (rc_idx < MAX_RC_NUM) {
		if (pcie_dev == &msm_pcie_dev[rc_idx]) {
			PCIE_INFO(pcie_dev,
				"%d, driver %s, rc %d, from %pS\n",
				msm_pcie_dev[rc_idx].user_suspend,
				dev->driver?dev->driver->driver.name:"NA",
				rc_idx,
				__builtin_return_address(0));
			return msm_pcie_dev[rc_idx].user_suspend;
		}
		rc_idx++;
	}

	PCIE_ERR(pcie_dev, "PCIe: rc_idx value %d, invalid\n", rc_idx);
	return -ENODEV;
}
#endif
