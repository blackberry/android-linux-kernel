...
int msm_pcie_is_user_suspend(struct pci_dev *dev)
{
	...
	if (!dev->bus->sysdata) {
		...
		pcie_dev = PCIE_BUS_PRIV_DATA(dev);
		PCIE_DBG(pcie_dev,
			"driver name %s, sysdata %p, pcie_dev %p\n",
			dev->driver?dev->driver->driver.name:"NA", dev->bus->sysdata, pcie_dev);
	}

	while (rc_idx < MAX_RC_NUM) {
		if (pcie_dev == &msm_pcie_dev[rc_idx]) {
			PCIE_INFO(pcie_dev,
				"driver name %s matching rc %d, user_suspend %d\n",
				dev->driver?dev->driver->driver.name:"NA",
				rc_idx, msm_pcie_dev[rc_idx].user_suspend);
