...
static void update_cpu_freq(int cpu)
{
	...
	if (cpu_online(cpu)) {
		...
		trace_thermal_post_frequency_mit(cpu,
			...
			cpus[cpu].limited_min_freq);
		if (ret)
/* [PLATFORM]-Mod-BEGIN by TCTNB.WPL, wangpl, 2016/03/22, defect-1660471, power on performance, temp modify log level*/
			pr_warn("Unable to update policy for cpu:%d. err:%d\n",
				cpu, ret);
	...
	for_each_possible_cpu(cpu) {
		if (cpus_offlined & BIT(cpu)) {
			...
			if (ret && ret == notifier_to_errno(NOTIFY_BAD)) {
				...
				pend_hotplug_req = true;
#ifdef CONFIG_BBRY
				/* If both cpu_dev->offline and cpu_online(cpu) are false
				   it's a faulty condition, try to fix it */
				pr_err_ratelimited(
					"Unable to online CPU%d. err:%d offline:%d\n",
					cpu, ret, cpu_dev->offline);
				if (!cpu_dev->offline)
...
fail:
	if (ret)
/* [PLATFORM]-Mod-BEGIN by TCTNB.WPL, wangpl, 2016/03/22, defect-1660471, power on performance, temp modify log level*/
		pr_warn("Failed reading node=%s, key=%s. err:%d\n",
