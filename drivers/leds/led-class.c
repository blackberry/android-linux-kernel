...
#ifdef CONFIG_BBRY
static ssize_t led_manual_override_en_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct led_classdev *led_cdev = dev_get_drvdata(dev);
...
static ssize_t led_manual_override_en_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	struct led_classdev *led_cdev = dev_get_drvdata(dev);
	unsigned long state;
	...
	led_cdev->manual_override_en = (state != 0);

	return size;
}

static ssize_t led_brightness_override_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return led_brightness_show(dev, attr, buf);
}

static ssize_t led_brightness_override_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	struct led_classdev *led_cdev = dev_get_drvdata(dev);
	unsigned long state;
...
#ifdef CONFIG_BBRY
	__ATTR(manual_override_en, 0644, led_manual_override_en_show, led_manual_override_en_store),
	__ATTR(brightness_override, 0644, led_brightness_override_show, led_brightness_override_store),
#endif
