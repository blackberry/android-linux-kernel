		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (ret)
		return ret;
#ifndef CONFIG_BBRY
		/* In case we want a range >255, don't want to clamp to LED_FULL (255) when max_brightness is modified */
		if (state > LED_FULL)
			state = LED_FULL;
#endif
...
static ssize_t brightness_limit_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct led_classdev *led_cdev = dev_get_drvdata(dev);

	return sprintf(buf, "%u\n", led_cdev->brightness_limit);
}

static ssize_t brightness_limit_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	struct led_classdev *led_cdev = dev_get_drvdata(dev);
	unsigned long state;
	...
	led_cdev->brightness_limit = state;
	__led_set_brightness(led_cdev, led_cdev->usr_brightness_req);

	return size;
}
static DEVICE_ATTR_RW(brightness_limit);
#endif
...
#ifdef CONFIG_BBRY
	&dev_attr_brightness_limit.attr,
