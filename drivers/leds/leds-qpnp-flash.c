...
/* MODIFIED-BEGIN by qiuwei, 2016-04-26,BUG-1663322*/
#define FLASH_TRIM1(base)  					(base + 0xF1)
#define FLASH_TRIM3(base)					(base + 0xF3)
/* MODIFIED-END by qiuwei,BUG-1663322*/
...
struct flash_led_platform_data {
	...
	bool  				max_current_1500_en; // MODIFIED by qiuwei, 2016-04-26,BUG-1663322
};
...
static void qpnp_flash_led_work(struct work_struct *work)
{
	...
	if (flash_node->type == TORCH) {
		...
		if (flash_node->trigger & FLASH_LED1_TRIGGER)
			max_curr_avail_ma += flash_node->max_current;
			max_curr_avail_ma = (max_curr_avail_ma * 1200)/1000; // MODIFIED by qiuwei, 2016-04-26,BUG-1663322
		...
		if (flash_node->id == FLASH_LED_SWITCH) {
			...
			rc = spmi_ext_register_readl(led->spmi_dev->ctrl,led->spmi_dev->sid,FLASH_TRIM1(led->base),&val, 1);
							led->pdata->flash_trim1 = val;
												}
			rc = qpnp_led_masked_write(led->spmi_dev,FLASH_LED_UNLOCK_SECURE(led->base),FLASH_SECURE_MASK,FLASH_UNLOCK_SECURE);
					if(rc){
						dev_err(&led->spmi_dev->dev,"Secure reg write failed\n");
				goto exit_flash_led_work;
				}
rc = qpnp_led_masked_write(led->spmi_dev, FLASH_TRIM1(led->base), 0xFF,(led->pdata->flash_trim1 + 10));
					if (rc){
			dev_err(&led->spmi_dev->dev,"TRIM1 reg write failed\n");
		goto exit_flash_led_work;
		}
			val = (u8)(flash_node->prgm_current *
			...
			if (led->pdata->flash_trim3 == 0){
					rc = spmi_ext_register_readl(led->spmi_dev->ctrl,led->spmi_dev->sid,FLASH_TRIM3(led->base),&val, 1);
				...
				rc = qpnp_led_masked_write(led->spmi_dev,FLASH_TRIM3(led->base),0xFF,(led->pdata->flash_trim3 + 10));
					if (rc){
						dev_err(&led->spmi_dev->dev,"TRIM3 reg write failed\n");
					goto exit_flash_led_work;
					}
			val = (u8)(flash_node->prgm_current2 * FLASH_MAX_LEVEL / flash_node->max_current);
			/* MODIFIED-END by qiuwei,BUG-1663322*/
			rc = qpnp_led_masked_write(led->spmi_dev,
			...
			}else { // MODIFIED by qiuwei, 2016-04-26,BUG-1663322
						...
						enum led_brightness value)
{
	struct flash_node_data *flash_node;
	struct qpnp_flash_led *led;
	int max_flash_node_current; // MODIFIED by qiuwei, 2016-04-26,BUG-1663322
	flash_node = container_of(led_cdev, struct flash_node_data, cdev);
	led = dev_get_drvdata(&flash_node->spmi_dev->dev);
	...
	if (led->flash_node[led->num_leds - 1].id ==
		...
		if (flash_node->id == FLASH_LED_0 ||
			...
			if (!value) {
				...
				u16 total_current;
				total_current = flash_node->prgm_current + flash_node->prgm_current2;
				max_flash_node_current = (led->pdata->max_current_1500_en) ? FLASH_LED_MAX_LIMITING_CURRENT_1500_MA : FLASH_LED_MAX_LIMITING_CURRENT_2000_MA;
						if (total_current > max_flash_node_current){
						flash_node->prgm_current = (flash_node->prgm_current * max_flash_node_current)/total_current;
						flash_node->prgm_current2 = (flash_node->prgm_current2 * max_flash_node_current)/total_current;
						}
					}
			}
		} else{
						...
						struct device_node *node)
{
	...
	led->pdata->max_current_1500_en =
			of_property_read_bool(node,"qcom,max_current_1500");
	if (led->pdata->thermal_derate_en) {
