...
#define FLASH_FAULT_STATUS(base)				(base + 0x08)
#define FLASH_INT_RT_STS(base)					(base + 0x10)
#define FLASH_TIMEOUT_MASK						0x80
...
#else
#define	FLASH_LED_MASK_MODULE_MASK2_ENABLE			0x40
#endif
...
#ifdef CONFIG_BBRY
#define FLASH_HW_STROBE_SEL					0x04
#define FLASH_HW_STROBE_EDGE					0x02
#define FLASH_HW_STROBE_POL_HIGH				0x01
#define FLASH_STROBE_CTRL_MASK (FLASH_LED0_TRIGGER | FLASH_LED1_TRIGGER | FLASH_HW_STROBE_SEL | FLASH_HW_STROBE_EDGE | FLASH_HW_STROBE_POL_HIGH)
#endif /* ifdef CONFIG_BBRY */
...
#ifdef CONFIG_BBRY
#define BBRY_NUM_LINES_FOR_OUTPUT				2
#define	BBRY_FLASH_TORCH_MAX_LEVEL	(FLASH_TORCH_MAX_LEVEL/BBRY_NUM_LINES_FOR_OUTPUT)
#define PMIC_FLASH_MULTIPLIER_OF_10				10
#define PMIC_FLASH_MIN_STEPS_TIMES_10_MA			125
#define PMIC_FLASH_2LINES_MIN_CURRENT_MA			25
#endif /* ifdef CONFIG_BBRY */
...
#ifdef CONFIG_BBRY_MFG
static int qpnp_flash_led_status_get(struct led_classdev *led_cdev,
				u8 *reg_val)
{
	int rc = 0;
	u8 flash_timeout = 0x0;
	struct flash_node_data *flash_node =
			container_of(led_cdev, struct flash_node_data, cdev);
	struct qpnp_flash_led *led =
			dev_get_drvdata(&flash_node->spmi_dev->dev);
	rc = spmi_ext_register_readl(led->spmi_dev->ctrl,
				led->spmi_dev->sid,
				FLASH_FAULT_STATUS(led->base),
				reg_val, sizeof(u8));
	if (rc) {
		dev_err(&led->spmi_dev->dev,
				"Unable to read FLASH_FAULT_STATUS reg\n");
		*reg_val = 0xFF;
		return -EINVAL;
	}
	rc = spmi_ext_register_readl(led->spmi_dev->ctrl,
				led->spmi_dev->sid,
				FLASH_INT_RT_STS(led->base),
				&flash_timeout, sizeof(u8));
	if (rc) {
		dev_err(&led->spmi_dev->dev,
				"Unable to read FLASH_INT_RT_STS reg\n");
		*reg_val = 0xFF;
		return -EINVAL;
	}
	*reg_val |= (flash_timeout & FLASH_TIMEOUT_MASK);
	return rc;
}
#endif /*ifdef CONFIG_BBRY_MFG */
...
#else
__always_inline int32_t pmic_flash_get_current_from_steps(int32_t steps)
{
	if (steps < 0)
		return 0;
	...
	if (steps % 2)
		return ((((steps - 1) / BBRY_NUM_LINES_FOR_OUTPUT) *
		PMIC_FLASH_2LINES_MIN_CURRENT_MA) +
		FLASH_LED_MIN_CURRENT_MA);
	else
		return ((steps / BBRY_NUM_LINES_FOR_OUTPUT) *
			PMIC_FLASH_2LINES_MIN_CURRENT_MA);

	return 0;
}

__always_inline int32_t pmic_flash_get_steps_from_current(int32_t current_mA)
{
	if (current_mA < PMIC_FLASH_2LINES_MIN_CURRENT_MA)
		return 0;

	return ((current_mA * PMIC_FLASH_MULTIPLIER_OF_10) /
		PMIC_FLASH_MIN_STEPS_TIMES_10_MA) - 1;
}

__always_inline int32_t pmic_flash_round_current(int32_t current_mA)
{
	/* Round down the current to the next multiple of
	    the steps used for current in PMIC Flash */
	int32_t steps = 0;

	steps = pmic_flash_get_steps_from_current(current_mA);

	return pmic_flash_get_current_from_steps(steps);
}
...
static void qpnp_flash_led_work(struct work_struct *work)
{
	...
	if (flash_node->prgm_current >= PMIC_FLASH_2LINES_MIN_CURRENT_MA)
		flash_node->trigger |= FLASH_LED1_TRIGGER;
	else
		flash_node->trigger &= ~FLASH_LED1_TRIGGER;

	stepsLed0 = (u8)pmic_flash_get_steps_from_current(
			flash_node->prgm_current /
			BBRY_NUM_LINES_FOR_OUTPUT);

	stepsLed1 = (u8)pmic_flash_get_steps_from_current(
			flash_node->prgm_current -
			pmic_flash_get_current_from_steps(stepsLed0));
	...
	if (flash_node->type == TORCH) {
		...
		rc = qpnp_led_masked_write(led->spmi_dev,
			FLASH_MAX_CURRENT(led->base),
			FLASH_CURRENT_MASK, BBRY_FLASH_TORCH_MAX_LEVEL);
		if (rc) {
			dev_err(&led->spmi_dev->dev,
					"Max current reg write failed\n");
			goto exit_flash_led_work;
...
turn_off:
	rc = qpnp_led_masked_write(led->spmi_dev,
			FLASH_LED_STROBE_CTRL(led->base),
			FLASH_STROBE_CTRL_MASK, FLASH_LED_DISABLE);
	if (rc) {
		dev_err(&led->spmi_dev->dev, "Strobe disable failed\n");
		goto exit_flash_led_work;
	}
...
#else
	if (value < FLASH_LED_MIN_CURRENT_MA)
		flash_node->cdev.brightness = 0;
	else
		flash_node->cdev.brightness = pmic_flash_round_current(value);
...
#ifdef CONFIG_BBRY
			flash_node->trigger |= FLASH_LED1_TRIGGER;
			if (flash_node->use_hw_strobe) {
				flash_node->trigger |= (FLASH_HW_STROBE_SEL | FLASH_HW_STROBE_POL_HIGH);
				flash_node->trigger &= ~FLASH_HW_STROBE_EDGE;
			}
...
#ifdef CONFIG_BBRY_MFG
		led->flash_node[i].cdev.status_get = qpnp_flash_led_status_get;
#endif /* ifdef CONFIG_BBRY_MFG */
