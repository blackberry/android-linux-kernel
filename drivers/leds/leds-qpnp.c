					...
					const char *name)
{
	...
	if (pwm_cfg->pwm_dev) {
		...
		pwm = pwm_request_from_chip(pwm_cfg->pwm_dev->chip, pwm_cfg->pwm_dev->hwpwm, NULL);
		if (IS_ERR(pwm)) {
			dev_err(&spmi_dev->dev, "Failed to get device usage\n");
			return (int)PTR_ERR(pwm);
		}
#endif
...
#else
	find_digit(&buffer); /* Clear out any leading whitespace */
	for (i = 0; i < max_duty_pcts; i++) {
		if (buffer == NULL)
			break;

		ret = sscanf((const char *)buffer, "%u", &value);
		if (ret == 0)
			break;

		pwm_cfg->old_duty_pcts[i] = value;
		num_duty_pcts++;
		find_next_entry(&buffer);
	}
#endif
...
#ifdef CONFIG_BBRY
	/* We can free this here because we aren't using the device right now */
	pwm_free(pwm_cfg->pwm_dev);
#endif
