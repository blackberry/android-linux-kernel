	...
	const char *buf, size_t count)
{
	...
	for (i = 0; i < max_duty_pcts; i++) {
		if (buffer == NULL)
			break;
/*[BUGFIX]-Add-BEGIN by TCTNB.XQJ, FR-1644774 2016/02/18, modify attribute interface with  charge LED and incident LED*/
#ifdef CONFIG_TCT_8996_COMMON
		 if (memcmp (led->cdev.name, "red", 3) == 0 )
		 {
				 ret = sscanf((const char *)buffer, "%x,%s", &uvalue, buffer);
				  if (ret < 1)
					  break;
				  buffer+=3;
				  pwm_cfg->old_duty_pcts[i] =uvalue ;
				  num_duty_pcts++;
		  }
		  else
#endif
