...
struct qpnp_wled *wled_globle;

 int qpnp_wled_read_reg_fcamera(u8 *data, u16 addr) 
{
	int rc;

	rc = spmi_ext_register_readl(wled_globle->spmi->ctrl, wled_globle->spmi->sid,
							addr, data, 1);
	if (rc < 0)
		dev_err(&wled_globle->spmi->dev,
			"Error reading address: %x(%d)\n", addr, rc);

	return rc;
}
EXPORT_SYMBOL(qpnp_wled_read_reg_fcamera);

 int qpnp_wled_write_reg_fcamera( u8 *data, u16 addr) 
{
	int rc;

	rc = spmi_ext_register_writel(wled_globle->spmi->ctrl, wled_globle->spmi->sid,
							addr, data, 1);
	if (rc < 0)
		dev_err(&wled_globle->spmi->dev,
			"Error writing address: %x(%d)\n", addr, rc);

	dev_dbg(&wled_globle->spmi->dev, "write: WLED_0x%x = 0x%x\n", addr, *data);

	return rc;
}
EXPORT_SYMBOL(qpnp_wled_write_reg_fcamera);
#endif
...
static int qpnp_wled_set_level(struct qpnp_wled *wled, int level)
{
	...
	printk(KERN_ERR "%s---->set backlight level: %d\n", __func__, level);
