...
#ifdef CONFIG_BBRY
	u32 max_pwms[QPNP_WLED_MAX_STRINGS];
	s32 pwms[QPNP_WLED_MAX_STRINGS];
#endif
};

#ifdef CONFIG_BBRY
#define LED_CLASSDEV_MAX_LEVEL 255
static int qpnp_wled_set_pwms(struct qpnp_wled *wled);
#endif
...
static int qpnp_wled_set_level(struct qpnp_wled *wled, int level)
{
	...
	for (i = 0; i < wled->num_strings; i++) {
		/* Scale individual string brightnesses based on max_pwms[] */
		wled->pwms[i] = level * wled->max_pwms[i] / wled->cdev.max_brightness;
		pr_debug("pwms[%d] %d\n", i, wled->pwms[i]);
	}
	return qpnp_wled_set_pwms(wled);
}
...
#ifdef CONFIG_BBRY
/* sysfs show function for full scale current in ua*/
static ssize_t qpnp_wled_pwms_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct qpnp_wled *wled = dev_get_drvdata(dev);
	...
	int i, count = 0;
	for (i = 0; i < wled->num_strings; i++)
		count += snprintf(buf+count, PAGE_SIZE-count, "%d ", wled->pwms[i]);
	buf[strlen(buf)-1] = '\n';
...
static ssize_t qpnp_wled_pwms_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	struct qpnp_wled *wled = dev_get_drvdata(dev);
	int i, rc;
	...
	for (i = 0; i < wled->num_strings && p < buf+count; i++) {
		char *end;
		long int value;
		...
		pr_info("brightnesses sysfs string %d: %ld\n", i, value);
		wled->pwms[i] = value;
		if (value)
			enable = true;
...
#ifdef CONFIG_BBRY
	__ATTR(pwms, (S_IRUGO | S_IWUSR | S_IWGRP),
			qpnp_wled_pwms_show,
			qpnp_wled_pwms_store),
#endif
};
...
#ifdef CONFIG_BBRY
	if (of_find_property(spmi->dev.of_node,	"oem,max-pwms", &temp_val)) {
		rc = of_property_read_u32_array(spmi->dev.of_node, "oem,max-pwms", wled->max_pwms, wled->num_strings);
		if (rc) {
			dev_err(&spmi->dev,	"Couldn't read oem,max-pwms rc = %d\n", rc);
			return rc;
		}
		for (i = 0; i < wled->num_strings; i++){
			dev_err(&spmi->dev, "Parsed %d", wled->max_pwms[i]);
		}
	} else {
		dev_err(&spmi->dev, "oem,max-pwms not found");
		for (i = 0; i < wled->num_strings; i++)
			wled->max_pwms[i] = WLED_MAX_LEVEL_4095;
	}
#endif
...
#ifdef CONFIG_BBRY
	wled->cdev.max_brightness = LED_CLASSDEV_MAX_LEVEL;
#else
