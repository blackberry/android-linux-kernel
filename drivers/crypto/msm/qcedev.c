						...
						struct qcedev_control *podev)
{
	...
	for (i = 0; i < req->entries; i++) {
		if (req->vbuf.dst[i].len >= U32_MAX - total) {//[BUGFIX]-ADD by yinzhang,09/14/2016,PR-2902903,
	...
	for (i = 0, total = 0; i < req->entries; i++) {
		if (req->vbuf.src[i].len > U32_MAX - total) {//[BUGFIX]-ADD-BEGIN by yinzhang,09/14/2016,PR-2902903,
						...
						struct qcedev_control *podev)
{
	...
	/* Check for sum of all src length is equal to data_len  */
	for (i = 0, total = 0; i < req->entries; i++) {
		if (req->data[i].len > U32_MAX - total) {//[BUGFIX]-ADD by yinzhang,09/14/2016,PR-2902903,
