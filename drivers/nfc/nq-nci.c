...
#include <linux/clk.h>//[BUGFIX]-Add-BEGIN by TCTNB.LongNa,2016/04/19,defect 1920568
...
//[BUGFIX]-Add-BEGIN by TCTNB.LongNa,2016/04/19,defect 1920568
...
//[BUGFIX]-Add-END by TCTNB.LongNa
	...
	nqx_dev->count_irq++;
	spin_unlock_irqrestore(&nqx_dev->irq_enabled_lock, flags);
	nqx_disable_irq(nqx_dev);//[BUGFIX]-Add-BEGIN by TCTNB.LongNa,2016/04/19,defect 1920568
...
#ifdef CONFIG_NFC_HW_CHECK //add by LongNa for Task1923610 2016-04-08 Power on time improvement
			...
			const struct i2c_device_id *id)
{
	int r = 0;
	int irqn = 0;
	struct clk *nfc_clk = NULL;//[BUGFIX]-Add-BEGIN by TCTNB.LongNa,2016/04/19,defect 1920568
...
//[BUGFIX]-Add-END by TCTNB.LongNa
	if (gpio_is_valid(platform_data->en_gpio)) {
		...
		if (r) {
			...
			goto err_enable_clk;//[BUGFIX]-Mod-BEGIN by TCTNB.LongNa,2016/04/19,defect 1920568
		}
	...
	r = request_irq(client->irq, nqx_dev_irq_handler,
			  IRQF_TRIGGER_HIGH, client->name, nqx_dev);//[BUGFIX]-Mod-BEGIN by TCTNB.LongNa,2016/04/19,defect 1920568
	if (r) {
...
{
	struct i2c_client *client = to_i2c_client(device);
/*[PLATFORM]-MOD BEGIN by TCTNB.YQJ,2016/03/08,for resume time , task-1629668*/
	struct nqx_dev *nqx_dev = i2c_get_clientdata(client);
	if (device_may_wakeup(&client->dev) && nqx_dev->irq_enabled)
