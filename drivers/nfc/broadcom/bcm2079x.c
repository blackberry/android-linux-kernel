...
static int bcm2079x_dev_open(struct inode *inode, struct file *filp)
{
	...
	/* always pull down REG_PU in case it was left HIGH */
	gpio_set_value(bcm2079x_dev->ena_gpio, 0);

	/* per datasheet, sleep for 200ms before pulling WAKE down and enabling VDDIO */
	msleep(200);

	/* making sure the NFC chip is awake by pulling down WAKE*/
	...
	/* finally, pulling up REG_PU to enable the NFCC */
...
static int bcm2079x_dev_release(struct inode *inode, struct file *filp)
{
	...
	/* set NFC_WAKE to high */
	gpio_set_value(bcm2079x_dev->wak_gpio, 1);
	/* disable vddio */
			  ...
			  const struct i2c_device_id *id)
{
	...
	/* init NFC_WAKE to high */
	ret = gpio_request_one(bcm2079x_dev->wak_gpio, GPIOF_OUT_INIT_HIGH,
			       "nfc_wak");
	if (ret) {
