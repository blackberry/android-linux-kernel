...
#else
	np = of_find_compatible_node(NULL, NULL, "oem,mpm2-g-rd-cntr");
#endif
	if (!np) {
...
#else
	u64 now;
	pr_info("KPI: Bootloader start count\t = %9u\n",
		readl_relaxed(&boot_stats->bootloader_start));
	pr_info("KPI: Bootloader display count\t = %9u\n",
		readl_relaxed(&boot_stats->bootloader_display));
	pr_info("KPI: Bootloader end count\t = %9u\n",
		readl_relaxed(&boot_stats->bootloader_end));
	now = readl_relaxed(mpm_counter_base) * 1000000ULL;
	pr_info("KPI: Bootloader counter\t = %9u\n", (u32)div_u64(now, mpm_counter_freq));
#endif /* CONFIG_BBRY */
}
