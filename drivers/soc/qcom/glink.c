...
static struct channel_ctx *ch_name_to_ch_ctx_create(
					struct glink_core_xprt_ctx *xprt_ctx,
					const char *name, bool local); // MODIFIED by hongwei.tian, 2019-05-22,BUG-7794349
 ...
 * @name:	Name of the desired channel.
 // MODIFIED-BEGIN by hongwei.tian, 2019-05-22, BUG-7794349
...
static struct channel_ctx *ch_name_to_ch_ctx_create(
					...
					/* MODIFIED-END by hongwei.tian,BUG-7794349*/
{
	struct channel_ctx *entry;
	struct channel_ctx *ctx;
...
void *glink_open(const struct glink_open_config *cfg)
{
	 ...
	 */
	ctx = ch_name_to_ch_ctx_create(transport_ptr, cfg->name, true); // MODIFIED by hongwei.tian, 2019-05-22,BUG-7794349
	if (ctx == NULL) {
	...
	uint32_t rcid, const char *name, uint16_t req_xprt)
{
	...
	ctx = ch_name_to_ch_ctx_create(if_ptr->glink_core_priv, name, false); // MODIFIED by hongwei.tian, 2019-05-22,BUG-7794349
	if (ctx == NULL) {
		...
		struct glink_transport_if *if_ptr, uint32_t rcid)
{
	...
	struct glink_core_xprt_ctx *xprt_ptr = if_ptr->glink_core_priv;
	unsigned long flags; // MODIFIED by hongwei.tian, 2019-05-22,BUG-7794349
