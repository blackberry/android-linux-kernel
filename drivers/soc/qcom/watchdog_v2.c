...
#ifdef CONFIG_BBRY
	printk(KERN_INFO "cpu_online_mask: %lx\n",
...
static void pet_watchdog_work(struct work_struct *work)
{
	...
	struct msm_watchdog_data *wdog_dd = container_of(delayed_work,
						struct msm_watchdog_data,
							dogwork_struct);
#ifdef CONFIG_BBRY
	wdog_dd->petting_cpu = raw_smp_processor_id();
#endif
...
#ifdef CONFIG_BBRY
	wdog_dd->petting_cpu = PETTING_CPU_NONE;
#endif
...
void msm_trigger_wdog_bite(void)
{
	...
	mdelay(1);
#else
	/* Give TZ a bit more time to do its work */
	mdelay(20);
#endif
...
#ifdef CONFIG_BBRY
	printk(KERN_INFO "Tgid: %d, comm: %.20s\n",
		current->tgid, find_task_by_vpid(current->tgid)->comm);
	printk(KERN_INFO "Current CPU: %d, petting CPU: %x\n",
		smp_processor_id(), wdog_dd->petting_cpu);
	printk(KERN_INFO "Current name:%.20s pid: %d preempt_count:0x%x\n",
