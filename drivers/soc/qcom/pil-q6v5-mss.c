...
#endif
{
	...
	for (e = mpss_crash_reasons; e->reason; e++) {
		if ( e->description && strnstr( reason, e->description, min(size, MAX_SSR_REASON_LEN) ) ) {
			pr_info("is_modem_crash_expected: modem subsystem failure reason found: %s\n", reason);
			*indication |= 0x1;
			break;
		}
	}
	for (ea = mpss_crash_init_ap; ea->reason; ea++) {
		if ( ea->description && strnstr( reason, ea->description, min(size, MAX_SSR_REASON_LEN) ) ) {
			pr_info("is_AP_ramdump_needed: modem subsystem failure reason found: %s\n", reason);
			*indication |= 0x2;
			break;
		}
	}
	...
	ssr_check("modem", reason); //[FEATURE]-ADD by TCTSH.lijuan.wu, mini log, task-1488899
