...
static void subsystem_restart_wq_func(struct work_struct *work)
{
	...
	#endif
		do_ssrminilog();//[FEATURE]-ADD by TCTSH.lijuan.wu, mini log, task-1488899

#ifdef CONFIG_BBRY
	if (dev->supressRamdump)
...
#ifdef CONFIG_BBRY
void subsystem_ramdump_indication(struct subsys_device *dev, int indication)
{
	pr_info("subsystem_ramdump_indication = %d.\n", indication);

	if ( (indication & 0x1) == 0x1)
		dev->supressRamdump = true;
	else
		dev->supressRamdump = false;
