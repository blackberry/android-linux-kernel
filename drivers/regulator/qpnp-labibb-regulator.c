...
/* [PLATFORM]-Mod-BEGIN by TCTNB.WPL, PR-1908923, 2016/04/15, modify for configure lab register for display */
#if defined(CONFIG_TCT_8996_IDOL4S_CN)
/* [BUGFIX]-Mod-BEGIN by TCTNB.WPL, PR-2417388, 2016/06/27, modify for configure */
		val = 0xC3;
		rc = qpnp_labibb_write(labibb, labibb->lab_base + 0x52, &val, 1);
/* [BUGFIX]-Mod-END by TCTNB.WPL, 2016/06/27 */
		val = 0x02;
		rc = qpnp_labibb_write(labibb, labibb->lab_base + 0x4D, &val, 1);
		val = 0x0E;
		rc = qpnp_labibb_write(labibb, labibb->lab_base + 0x4E, &val, 1);
		val = 0x0B;
		rc = qpnp_labibb_write(labibb, labibb->lab_base + 0x4C, &val, 1);
		val = 0x80;
		rc = qpnp_labibb_write(labibb, labibb->lab_base + 0x51, &val, 1);
		val = 0x00;
		rc = qpnp_labibb_write(labibb, labibb->lab_base + 0x50, &val, 1);
		val = 0x04;
		rc = qpnp_labibb_write(labibb, labibb->lab_base + 0x60, &val, 1);
#endif
/* [PLATFORM]-Mod-END by TCTNB.WPL, 2016/04/15*/
...
/* [BUGFIX]-Mod-BEGIN by TCTNB.WPL, 2016/08/29, PR-2776930, change timing 1ms to 8ms */
#if defined(CONFIG_TCT_8996_IDOL4S_CN)
	val = 0xA5;
	rc = qpnp_labibb_write(labibb, labibb->ibb_base + 0xD0, &val, 1);

	val = 0xF4;
	rc = qpnp_labibb_write(labibb, labibb->ibb_base + REG_IBB_PWRUP_PWRDN_CTL_1, &val, 1);
#endif
/* [BUGFIX]-Mod-END by TCTNB.WPL, 2016/08/29 */
