				  ...
				  int error)
{
	...
	if (pm_print_times_enabled) {
#ifdef CONFIG_BBRY
		unsigned long long delta_in_usec;
#endif
		rettime = ktime_get();
...
#else
		delta_in_usec = (unsigned long long)ktime_to_ns(delta) >> 10;
		if (delta_in_usec >= pm_print_time_threshold){
			pr_warn("call %s+ returned %d after %Ld usecs\n", dev_name(dev),
				error, delta_in_usec);
		}
#endif
	}
