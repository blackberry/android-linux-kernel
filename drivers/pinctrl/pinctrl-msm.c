...
#ifdef CONFIG_BBRY
/* This flag is used during init to allow the pin controller to set default
 * values for all pins and then release ownership of those pins without
 * reverting the pin hardware state.
 */
static bool suspend_hw_updates;
#endif
...
#ifdef CONFIG_BBRY
	/* By default the pin control core code will eventually call
	 * msm_pmx_disable() via the msm_pmxops.disable when a pin is released
	 * and that function reverts the pin to GPIO function.
	 * Setting this flag prevents this driver from modifying the hardware
	 * state while we release any owned pins. This allows us to have a
	 * default pin config which we can apply at kernel start but still
	 * allow later drivers to own pins.
	 */
	suspend_hw_updates = true;
	pinctrl_put(dd->pctl_dev->p);
	suspend_hw_updates = false;
#endif
	return 0;
}
