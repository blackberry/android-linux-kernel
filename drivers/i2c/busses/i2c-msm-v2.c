...
#define DEFAULT_I2C_RATELIMIT_INTERVAL (HZ / 5)
#define DEFAULT_I2C_RATELIMIT_BURST 5
#endif
...
#ifdef CONFIG_BBRY_I2C_MSM_EVL
struct evl_device_attribute {
	int lock:1;
	int snapshot:1;
	struct device_attribute dattr;
};

static u32 evl_get_delta(ktime_t later, ktime_t earlier)
{
	s64 delta;

	delta = ktime_to_ns(ktime_sub(later, earlier));
	if (delta > U32_MAX)
		delta = U32_MAX;
	return delta;
}
				...
				u64 data0, u32 data1, u32 data2)
{
	...
	struct i2c_msm_evlog_all *ev;
	struct i2c_msm_evlog *evl;
	int idx;
	ktime_t now;

	ev = &xfer->evlog;
	if (ev) {
		idx = ev->index % EVLOG_NUM_ENTRY;
		evl = &ev->entry[idx];
		now = ktime_get_boottime();
		switch (event_type) {
		case I2C_MSM_XFER_BEG:
			memset(evl, 0, sizeof(*evl));
			ev->nivcsw = current->nivcsw;
#if defined(CONFIG_SCHEDSTATS) || defined(CONFIG_TASK_DELAY_ACCT)
			evl->wait = current->sched_info.run_delay;
#endif
			evl->start = now;
			evl->slave = (u8)data1;
			break;
		case I2C_MSM_SCAN_SUM:
			evl->rx = data0 & 0xFF;
			evl->tx = data1 & 0xFF;
			break;
		case I2C_MSM_IRQ_BGN:
			evl->irq_start_ns = evl_get_delta(now, evl->start);
			break;
		case I2C_MSM_IRQ_END:
			evl->irq_end_ns = evl_get_delta(now, evl->start);
			break;
		case I2C_MSM_XFER_END:
			evl->end_ns = evl_get_delta(now, evl->start);
			ev->sum += evl->end_ns;
			evl->error = data1;
			if (evl->error)
				memcpy(&ev->last_err, evl, sizeof(*evl));
			if (evl->end_ns >= ev->longest.end_ns)
				memcpy(&ev->longest, evl, sizeof(*evl));
			evl->nivcsw = (current->nivcsw - ev->nivcsw) & 0xFF;
#if defined(CONFIG_SCHEDSTATS) || defined(CONFIG_TASK_DELAY_ACCT)
			evl->wait = current->sched_info.run_delay - evl->wait;
#endif
			ev->index++;
			break;
		default:
...
static ssize_t clear_log_store(struct device *dev,
				...
				const char *buf, size_t size)
{
	...
	memset(&xfer->evlog, 0, sizeof(xfer->evlog));
	mutex_unlock(&ctrl->xfer.mtx);

	return size;
}

static int print_evl_entry(struct i2c_msm_evlog *evl, char *buf,
				ssize_t *total)
{
	ssize_t size = *total;
	struct timeval tv;
	u64 ns = ktime_to_ns(evl->start);

	if (!ns)
		return -EINVAL;

	tv = ktime_to_timeval(evl->start);
	size += snprintf(buf + size, PAGE_SIZE - size,
		"%ld.%06ld\t%x\t%u\t%u\t%u\t%lu\t\t%llu\t\t%u\n",
		tv.tv_sec, tv.tv_usec, evl->slave, evl->rx, evl->tx,
		evl->error, evl->end_ns/NSEC_PER_USEC,
		evl->wait/NSEC_PER_USEC, evl->nivcsw
		);
	*total = size;
	return 0;
}

static ssize_t logs_dump(struct device *dev,
				struct device_attribute *attr,
				char *buf)
{
	struct evl_device_attribute *evla;
	struct i2c_msm_evlog_all *ev;
	struct i2c_msm_ctrl *ctrl = dev_get_drvdata(dev);
	struct i2c_msm_xfer *xfer  = &ctrl->xfer;
	ssize_t total = 0;
	...
	evla = container_of(attr, struct evl_device_attribute , dattr);

	ev = (evla->snapshot) ? xfer->snapshot : &xfer->evlog;
	if (!ev)
		return snprintf(buf, PAGE_SIZE, "no snapshot present\n");
	if (!ev->index)
		return 0;
	...
	idx = (ev->index < EVLOG_NUM_ENTRY) ? 0 : ev->index;
	total += snprintf(buf + total, PAGE_SIZE - total,
			"NUM EVENTS : %d AVG DURATION %lluus\n",
			ev->index, (ev->sum/ev->index)/NSEC_PER_USEC);
	total += snprintf(buf + total, PAGE_SIZE - total,
			"TIME\t\tSLAVE\tRX\tTX\tERROR\tDURATION(us)\tWAIT(us)\tNIVCSM\n");
	for (i = 0; i < EVLOG_NUM_ENTRY; i++, idx++)  {
		if (print_evl_entry(&ev->entry[idx % EVLOG_NUM_ENTRY],
					buf, &total))
			break;
	}

	if (ktime_to_ns(ev->last_err.start) > 0) {
		total += snprintf(buf + total,
					PAGE_SIZE - total, "LAST ERROR\n");
		print_evl_entry(&ev->last_err, buf, &total);
	}
	if (ktime_to_ns(ev->longest.start) > 0) {
		total += snprintf(buf + total,
					PAGE_SIZE - total, "LONGEST\n");
		print_evl_entry(&ev->longest, buf, &total);
	}
	if (evla->lock)
		mutex_unlock(&ctrl->xfer.mtx);
	return total;
...
static ssize_t snapshot_trigger(struct device *dev,
				struct device_attribute *attr,
				const char *buf, size_t size)
{
	struct i2c_msm_ctrl *ctrl = dev_get_drvdata(dev);
	...
	if (!xfer->snapshot) {
		xfer->snapshot = kzalloc(sizeof(xfer->evlog), GFP_KERNEL);
		if (!xfer->snapshot) {
			dev_err(ctrl->dev, "Failed allocation\n");
			return -ENOMEM;
		}
	}

	mutex_lock(&ctrl->xfer.mtx);
	memcpy(xfer->snapshot, &xfer->evlog, sizeof(xfer->evlog));
	mutex_unlock(&ctrl->xfer.mtx);
...
#define EVL_ATTRIBUTE(_name, _mode, _show, _store, _lock, _snapshot) \
	struct evl_device_attribute dev_attr_##_name = \
	{.lock = _lock, .snapshot = _snapshot, \
	.dattr = __ATTR(_name, _mode, _show, _store),}
static EVL_ATTRIBUTE(clear_logs, 0200, NULL, clear_log_store, 0, 0);
static EVL_ATTRIBUTE(logs, 0400, logs_dump, NULL, 0, 0);
static EVL_ATTRIBUTE(snapshot, 0600, logs_dump, snapshot_trigger, 0, 1);

static struct attribute *evl_attrs[] = {
	&dev_attr_clear_logs.dattr.attr,
	&dev_attr_logs.dattr.attr,
	&dev_attr_snapshot.dattr.attr,
	NULL,
};

static struct attribute_group evl_group = {
	.attrs = evl_attrs,
};


int i2c_msm_sysfs_evl(struct i2c_msm_ctrl *ctrl)
{
	return sysfs_create_group(&ctrl->dev->kobj, &evl_group);
}
#endif
...
static int i2c_msm_probe(struct platform_device *pdev)
{
	...
	ctrl->pwr_state = I2C_MSM_PM_RT_SUSPENDED;
#ifdef CONFIG_BBRY
	ratelimit_state_init(&ctrl->rs,
				DEFAULT_I2C_RATELIMIT_INTERVAL,
				DEFAULT_I2C_RATELIMIT_BURST);
#endif
