		...
		struct _mem_entry *next)
{
	...
	if (p != NULL) {
		...
		prev->pid = pid_nr(private->pid); // MODIFIED by hongwei.tian, 2020-12-15,BUG-10459375
	...
	if (n != NULL) {
		...
		next->pid = pid_nr(private->pid); // MODIFIED by hongwei.tian, 2020-12-15,BUG-10459375
