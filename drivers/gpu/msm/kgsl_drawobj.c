 ...
 * kick the dispatcher
 // MODIFIED-BEGIN by hongwei.tian, 2020-04-10, BUG-9194848
	...
	struct kgsl_drawobj_sync_event *event)
{
	...
	if (!test_and_clear_bit(event->id, &syncobj->pending))
		return false; // MODIFIED by hongwei.tian, 2020-04-10,BUG-9194848
	...
	if (!kgsl_drawobj_events_pending(event->syncobj)) {
        ...
        return true; // MODIFIED by hongwei.tian, 2020-04-10,BUG-9194848
}
...
static void drawobj_destroy_sync(struct kgsl_drawobj *drawobj)
{
	...
	unsigned long flags; // MODIFIED by hongwei.tian, 2020-04-10,BUG-9194848
	unsigned int i;
	 ...
	 */
	if (!bitmap_empty(&syncobj->pending, KGSL_MAX_SYNCPOINTS) && // MODIFIED by hongwei.tian, 2020-04-10,BUG-9194848
