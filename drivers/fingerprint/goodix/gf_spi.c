...
/* MODIFIED-BEGIN by siguo.cheng, 2018-03-14,BUG-6013485*/
...
/* MODIFIED-BEGIN by Ji.Chen, 2018-09-11,BUG-6969577*/
#define  FP_TOUCH    551
#define  FP_UP    552
#define  FP_DOWN    553
#define  FP_LEFT    554
#define  FP_RIGHT    555
#define  FP_LONG_TOUCH    556
#define  FP_DOUBLE_TOUCH    557
#define  FP_POWER    558

struct gf_key_map key_map[] =
{
      {  "POWER",  KEY_POWER, FP_POWER  }, // MODIFIED by siguo.cheng, 2018-04-13,BUG-6181506
      {  "HOME" ,  KEY_HOME, 0},
      {  "MENU" ,  KEY_MENU, 0},
      {  "BACK" ,  KEY_BACK, 0},
      {  "UP"   ,  KEY_UP, FP_UP},
      {  "DOWN" ,  KEY_DOWN, FP_DOWN},
      {  "LEFT" ,  KEY_LEFT, FP_LEFT},
      {  "RIGHT",  KEY_RIGHT, FP_RIGHT},
      {  "FORCE",  KEY_F9, 0},
      {  "CLICK",  KEY_F19, 0},
      /* MODIFIED-BEGIN by siguo.cheng, 2018-04-13,BUG-6181506*/
      {  "CUSTOMER_KEY_SINGLE",  KEY_F1, FP_TOUCH},
      {  "CUSTOMER_KEY_DOUBLE",  KEY_F2, FP_DOUBLE_TOUCH},
      {  "CUSTOMER_KEY_LONGPRESS",  KEY_F3, FP_LONG_TOUCH},
      /* MODIFIED-END by Ji.Chen,BUG-6969577*/
	  /* MODIFIED-END by siguo.cheng,BUG-6181506*/
};
...
static int gf_major = 0; // MODIFIED by hongwei.tian, 2017-04-19,BUG-4622740 // MODIFIED by siguo.cheng, 2018-05-17,BUG-6139087
...
static long gf_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    if(gf_dev->device_available == 0) // MODIFIED by siguo.cheng, 2018-03-14,BUG-6013485
    {
	...
	case GF_IOC_SENDKEY:
		...
		pr_warn("nav key = %d .\n",gf_key.key); // MODIFIED by siguo.cheng, 2018-05-17,BUG-6139087
                for(i = 0; i< ARRAY_SIZE(key_map); i++) {
                    /* MODIFIED-BEGIN by Ji.Chen, 2018-09-11,BUG-6969577*/
                    if(key_map[i].val == gf_key.key && key_map[i].keycode){
                        input_report_key(gf_dev->input, key_map[i].keycode, gf_key.value);
                        /* MODIFIED-END by hongwei.tian,BUG-4622740*/
                        /* MODIFIED-END by Ji.Chen,BUG-6969577*/
...
static void gf_reg_key_kernel(struct gf_dev *gf_dev)
{
    ...
    if(key_map[i].keycode)
         set_bit(key_map[i].keycode, gf_dev->input->keybit);
         /* MODIFIED-END by Ji.Chen,BUG-6969577*/
    }
...
*/
	...
	if (minor < N_SPI_MINORS) {
		...
		gf_dev->devt = MKDEV(gf_major, minor); // MODIFIED by hongwei.tian, 2017-04-19,BUG-4622740 // MODIFIED by siguo.cheng, 2018-05-17,BUG-6139087
...
{
	int status;
	int major = 0; // MODIFIED by hongwei.tian, 2017-04-19,BUG-4622740 // MODIFIED by siguo.cheng, 2018-05-17,BUG-6139087
	FUNC_ENTRY();
	...
	/* MODIFIED-BEGIN by siguo.cheng, 2018-03-14,BUG-6013485*/
	major = register_chrdev(0, CHRD_DRIVER_NAME, &gf_fops);
	if (major < 0) {
		pr_warn("Failed to register char device! (%d)\n",major);
		FUNC_EXIT();
		return major;
	}
	gf_major = major;
	...
	if (IS_ERR(gf_class)) {
		...
		/* MODIFIED-END by siguo.cheng,BUG-6013485*/
	...
	if (status < 0) {
		...
		unregister_chrdev(gf_major, gf_driver.driver.name); // MODIFIED by hongwei.tian, 2017-04-19,BUG-4622740 // MODIFIED by siguo.cheng, 2018-05-17,BUG-6139087
