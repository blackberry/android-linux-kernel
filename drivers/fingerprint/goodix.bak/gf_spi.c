...
{
   //   {  "POWER",  KEY_POWER  }, // MODIFIED by siguo.cheng, 2018-04-13,BUG-6181506
      {  "HOME" ,  KEY_HOME   },
      {  "MENU" ,  KEY_MENU   },
      {  "BACK" ,  KEY_BACK   },
      {  "UP"   ,  KEY_UP     },
...
static void gf_reg_key_kernel(struct gf_dev *gf_dev)
{
    ...
    for(i = 0; i< ARRAY_SIZE(key_map); i++) {
        set_bit(key_map[i].val, gf_dev->input->keybit); // MODIFIED by siguo.cheng, 2018-03-14,BUG-6013485
    }
...
*/
	...
	if (minor < N_SPI_MINORS) {
		...
		gf_dev->devt = MKDEV(SPIDEV_MAJOR, minor); // MODIFIED by siguo.cheng, 2018-03-14,BUG-6013485
...
static int __init gf_init(void)
{
	...
	if (status < 0) {
		...
		unregister_chrdev(SPIDEV_MAJOR, gf_driver.driver.name); // MODIFIED by siguo.cheng, 2018-03-14,BUG-6013485
