...
extern int g_emergency;
static int g_AP_skips_hdcp;
static int g_hdcp_skipped;
static int g_ds_hdmi_hdcp_retry_cnt;
static int g_hdcp_err_cnt;
#define MAX_HDCP_ERR (5)
#endif
#define SLIMPORT_DRV_DEBUG

#ifndef XTAL_CLK_DEF
#ifdef CONFIG_BBRY
#define XTAL_CLK_DEF XTAL_24M
#else
...
#ifndef CONFIG_BBRY
#define HDCP_REPEATER_MODE (bit)(__i2c_read_reg(RX_P1, 0x2A) & REPEATER_M)
#endif
...
#ifdef CONFIG_BBRY
/* Put the driver back to link training, ensure
   that HDMI driver restarts and the HDMI display
   is recreated as secure/insecure based on hdcp_cap */
static void reset_hdcp_cap(unchar hdcp_cap)
{
	g_hdcp_cap_bak = hdcp_cap;
	sp_tx_set_sys_state(STATE_LINK_TRAINING);
	hdmi_rx_mute_audio(1);
	hdmi_rx_mute_video(1);
	hdmi_rx_set_hpd(0);
	slimport_publish_hdcp_cap(g_hdcp_cap_bak);
	msleep(10);
	hdmi_rx_set_hpd(1);
	DEV_NOTICE(" Restart Link Training, DS hdcp_cap = %d", hdcp_cap);
}

/* Reset 7816, dongle and driver in hopes that HDCP doesn't fail
   on a second try.
   If it does, then settle for no HDCP and an insecure display.
*/
static void ds_hdcp_retry(void)
{
	g_ds_hdmi_hdcp_retry_cnt++;
	if (g_ds_hdmi_hdcp_retry_cnt == 1) {
		DEV_ERR(" retry HDCP, restart everything");
		vbus_power_ctrl(0);
		sp_write_reg_or(TX_P2, SP_TX_RST_CTRL_REG, HW_RST);
		sp_tx_set_sys_state(STATE_SP_INITIALIZED);
		msleep(50);
	} else if (g_ds_hdmi_hdcp_retry_cnt == 2) {
		reset_hdcp_cap(0);
	}
}
...
void slimport_sink_connection(void)
{
	...
	switch(sp_tx_sc_state) {
		...
		default:
#ifdef CONFIG_BBRY
		        /* Report slimport offline, to cover case where we were connected but have
		           lost connection and dongle is still inserted */
...
void slimport_edid_process(void)
{
	...
	if(g_read_edid_flag == 0){
...
#ifdef CONFIG_BBRY
		{
			...
			memset(edid_blocks, 0, sizeof(edid_blocks));
			if (sp_tx_rx_type == DWN_STRM_IS_HDMI &&
...
#ifdef CONFIG_BBRY /* Don't notify AP until we've determined bandwidth */
	slimport_publish_hdcp_cap(g_hdcp_cap_bak);
	slimport_publish_online(1);
	if(!HDCP_REPEATER_MODE){
		hdmi_rx_set_hpd(1);
		pr_info("%s %s : hdmi_rx_set_hpd 1 !\n", LOG_TAG, __func__);
		hdmi_rx_set_termination(1);
		...
		case LT_FINISH:
			...
			if ((temp_value&0x07) == 0x07) {// for one lane case.
				...
				if(!sp_tx_test_lt) {
					...
					if (have_custom_sw_pre) {
						DEV_NOTICE("Overwrite Sw/Pre: 0x%x", custom_sw_pre & 0x1B);
						sp_write_reg(TX_P0, SP_TX_LT_SET_REG, custom_sw_pre & 0x1B);
					}
#endif
...
#else
			unsigned char cap_check = 0;
			if (check_rx_hdcp_status()) {
				/*AP enable HDCP*/
				DEV_NOTICE("%s : Source has HDCP enabled.\n",
						__func__);

				if (sp_tx_rx_type == DWN_STRM_IS_HDMI ||
						sp_tx_rx_type == DWN_STRM_IS_DIGITAL) {
					cap_check = slimport_hdcp_cap_check();

					/* Check for HDCP unsupported conditions */
					DEV_NOTICE(" DS hdcp cap_check = %d, cap_bak = %d",
						   cap_check, g_hdcp_cap_bak);
					if ((g_hdcp_cap_bak == 1 &&  !cap_check)) {
						DEV_ERR("HDCP capable sink is reporting no HDCP capability");
						HDCP_state = HDCP_NOT_SUPPORT;
					} else if ((g_hdcp_cap_bak == 0 &&
						    cap_check && g_hdcp_err_cnt == 0 &&
						    g_ds_hdmi_hdcp_retry_cnt == 0)) {
						DEV_ERR("Sink now supports HDCP, reset hdcp capability");
						reset_hdcp_cap(cap_check);
						HDCP_state = HDCP_CAPABLE_CHECK;
					} else if (g_hdcp_cap_bak == 0 && cap_check) {
						/* In this case the HDCP has been disabled,
						 * likely because of HDCP errors. Even though
						 * the cap_check reports positive for
						 * HDCP, we stick with our previous decision
						 * to disable it: e.g, see next case */
						DEV_NOTICE("DS has had trouble with HDCP, skip it");
						HDCP_state = HDCP_NOT_SUPPORT;
					} else if (g_hdcp_err_cnt > MAX_HDCP_ERR && cap_check) {
						DEV_ERR("HDCP error count too high (%d)", g_hdcp_err_cnt);
						g_hdcp_err_cnt = 0;
						ds_hdcp_retry();
						HDCP_state = HDCP_NOT_SUPPORT;
					} else if (!cap_check) {
						DEV_NOTICE("sink fails to support HDCP");
						HDCP_state = HDCP_NOT_SUPPORT;
					}
				} else {
					/*AP does not do HDCP*/
				...
				g_AP_skips_hdcp = 1;
				HDCP_state = HDCP_NOT_SUPPORT;
				DEV_NOTICE("%s %s : Source disable HDCP.\n",
						LOG_TAG, __func__);
			}
#endif
			#else
...
#ifdef CONFIG_BBRY
		g_hdcp_skipped = 1;
		/* Don't unmute the video if we should have HDCP enabled */
		if (!(sp_tx_rx_type == DWN_STRM_IS_HDMI ||
		      sp_tx_rx_type == DWN_STRM_IS_DIGITAL) ||
		    !g_hdcp_cap_bak)
#endif
...
#ifdef CONFIG_BBRY
	g_hdcp_err_cnt++;
	DEV_NOTICE(" hdcp err cnt: %d", g_hdcp_err_cnt);
#endif
}
...
#ifdef CONFIG_BBRY
			/* Link down = device reset, helps with HDCP CTS */
			DEV_NOTICE("%s %s : link is down, treat as HPD", LOG_TAG, __func__);
			vbus_power_ctrl(0);
			reg_hardware_reset();
			return_value = 1;
#else
...
#ifdef CONFIG_BBRY
				/* HDMI rx video only gets unmuted in VIDEO OUTPUT */
				if (sp_tx_system_state <= STATE_VIDEO_OUTPUT)
#endif
				hdmi_rx_mute_video(1);
				sp_tx_video_mute(1);
...
#ifdef CONFIG_BBRY
	/* This will get set to 1 if any of the aux ops fail */
	g_edid_break = 0;
#endif
	sp_tx_aux_wr(0x00);
...
#ifdef CONFIG_BBRY
		&& (sp_tx_cur_states() > STATE_SINK_CONNECTION)
#endif
...
#ifdef CONFIG_BBRY
	} else if (sp_tx_cur_states() >= STATE_AUDIO_OUTPUT) {
			bak_states++;
			if (bak_states > 4) { /* 2s in AUDIO / PLAYBACK */
				if ((sp_tx_rx_type == DWN_STRM_IS_HDMI ||
				     sp_tx_rx_type == DWN_STRM_IS_DIGITAL) &&
				     g_hdcp_skipped && g_hdcp_cap_bak && hdcp_enable) {
					ds_hdcp_retry();
				}
			}
			if (bak_states > 12 && (g_hdcp_err_cnt || g_ds_hdmi_hdcp_retry_cnt)) {
				/* Consider HDCP to be stable after 6s of playback */
				DEV_ERR(" Output is stable, reset hdcp err counts");
				bak_states = 0;
				g_hdcp_err_cnt = 0;
				g_ds_hdmi_hdcp_retry_cnt = 0;
			}
...
void slimport_hdcp_repeater_reauth(void) /* A patch for HDCP repeater mode*/
{
	...
	if (HDCP_REPEATER_MODE
		...
		if ((c & AUTH_EN) != 0) {
			...
			sp_read_reg(TX_P0,TX_HDCP_CTRL0, &hdcp_ctrl);
			if ((hdcp_ctrl & HARD_AUTH_EN) == 0) {
				pr_info("%s: Repeater Mode: Enable HW HDCP\n", LOG_TAG);
				assisted_HDCP_repeater = HDCP_ERROR;
				...
				if (((hdcp_staus & SP_TX_HDCP_AUTH_PASS) == 0)
					& ((hdcp_staus & SP_TX_HDCP_AUTH_FAIL) == 1)) {
					assisted_HDCP_repeater = HDCP_ERROR;
					pr_info("%s: Repeater Mode: Clean HDCP and re-auth HDCP\n", LOG_TAG);
				} else if((hdcp_staus & SP_TX_HDCP_AUTH_PASS) == 1) {
					pr_info("%s: Repeater Mode: $$$$$HDCP Pass$$$$$\n", LOG_TAG);
				}
			}
		}
		...
		if ((hdcp_ctrl == TX_HDCP_FUNCTION_ENABLE)
			...
			pr_err("%s: main link error: HDCP encryption failure %x\n", LOG_TAG, (int)hdcp_staus);
			assisted_HDCP_repeater = HDCP_ERROR;
			//sp_tx_set_sys_state(STATE_LINK_TRAINING);
		}
