...
#define OTG_OC_LIMIT_SHIFT			(2)
#define OTG_UVLO_TH_SHIFT			(0)
#endif
...
#if defined(CONFIG_BBRY)
	} else if (current_ma > 685 && current_ma < 1000) {
		/* USB 3.0 - 900mA */
		reg = CMD_USB_3_MODE | CMD_USB_500_MODE;
#endif
	} else if (current_ma > USB2_MAX_CURRENT_MA) {
...
#if defined(CONFIG_BBRY) && !defined(CONFIG_KRYPTON)
static int smb1351_parallel_otg_init(struct smb1351_charger *chip)
{
	int rc=0;
	u8 val=0;
	...
	/* set chg en by pin active high via qcom,parallel-en-pin-polarity = <1>;  */
	val = chip->parallel_pin_polarity_setting | USBCS_CTRL_BY_I2C;
	rc = smb1351_masked_write(chip, CHG_PIN_EN_CTRL_REG,
				EN_PIN_CTRL_MASK | USBCS_CTRL_BIT, val);
	if (rc) {
	...
	rc = smb1351_masked_write(chip, OTG_MODE_POWER_OPTIONS_REG, 
				OTG_HICCUP_MODE_BIT, 0);
	if (rc) {
		pr_err("Couldn't set OTG_MODE_POWER_OPTIONS_REG(bit2) rc=%d\n", rc);
		return rc;
	}

	rc = smb1351_masked_write(chip, OTG_USBIN_AICL_CTRL_REG, 
				OTG_ID_PIN_CTRL_MASK, 0x88);
	if (rc) {
		pr_err("Couldn't set OTG_USBIN_AICL_CTRL_REG(bit7-6) rc=%d\n", rc);
		return rc;
	}

	/* defaultly using max current(5v-1.8A) for otg device, uvlo=3.3V  */
	val = chip->otg_oc_limit_idx << OTG_OC_LIMIT_SHIFT
			| chip->otg_uvlo_idx << OTG_UVLO_TH_SHIFT;
	rc = smb1351_masked_write(chip, OTG_TLIM_CTRL_REG, 
				OTG_OC_LIMIT_MASK|OTG_BATT_UVLO_TH_MASK, 
				val);
	if (rc) {
		pr_err("Couldn't set OTG_TLIM_CTRL_REG(bit3-0,bit6-7) rc=%d\n", rc);
		return rc;
	}

	/* disable unwanted irqs */
	rc = smb1351_masked_write(chip, CHG_PIN_EN_CTRL_REG, 
				CHG_ERR_BIT | APSD_DONE_BIT, 0);
	if (rc) {
		pr_err("Couldn't set CHG_PIN_EN_CTRL_REG(bit2-1) rc=%d\n", rc);
		return rc;
	}

	/* enable usbid irq */
	rc = smb1351_masked_write(chip, STATUS_INT_CFG_REG, 
				0xFF, RID_CHANGE_BIT);
	if (rc) {
		pr_err("Couldn't set STATUS_INT_CFG_REG(bit7-0) rc=%d\n", rc);
		return rc;
	}

	/* enable OC and UVLO irq */
	rc = smb1351_masked_write(chip, FAULT_INT_CFG_REG, 
				0xFF, BATT_UVLO_IN_OTG_BIT|OTG_OC_BIT);
	if (rc) {
		pr_err("Couldn't set FAULT_INT_CFG_REG(bit4-5) rc=%d\n", rc);
		return rc;
	}

	rc = smb1351_read_reg(chip, STATUS_4_REG, &val);
	if (rc < 0) {
		pr_err("Couldn't read OTG enable bit rc=%d\n", rc);
		return rc;
	}
	else
		pr_debug("bootup with OTG status: 0x%02x\n", val);

	if(val & STATUS_OTG_BIT) {
		rc = smb1351_masked_write(chip, CMD_CHG_REG, 
					CMD_OTG_EN_BIT, 0);
		if (rc) {
			pr_err("Couldn't disable OTG mode rc=%d\n", rc);
			return rc;
...
#if defined(CONFIG_BBRY) && !defined(CONFIG_KRYPTON)
	POWER_SUPPLY_PROP_VREG_CTRL,
	POWER_SUPPLY_PROP_OEM_OTG,
...
#if defined(CONFIG_BBRY)
	if (present == chip->parallel_charger_present && present) {
		pr_debug("skip for present already 1\n");
		return 0;
	}
#else
				       ...
				       const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_VOLTAGE_MAX:
		...
		break;
#if defined(CONFIG_BBRY) && !defined(CONFIG_KRYPTON)
	case POWER_SUPPLY_PROP_OEM_OTG:
		if(val->intval) {
			rc = smb1351_parallel_otg_init(chip);
			if (rc)
				pr_err("Couldn't initialize smb1351 otg rc=%d\n", rc);
			else
				chip->otg_init_done = true;
		}
		else
			chip->otg_init_done = false;
		break;
	case POWER_SUPPLY_PROP_VREG_CTRL:
		rc = smb1351_masked_write(chip, CMD_CHG_REG, CMD_OTG_EN_BIT,
								!!val->intval);
		if (rc)
			pr_err("Couldn't %s OTG mode rc=%d\n", 
				       ...
				       union power_supply_propval *val)
{
...
#if defined(CONFIG_BBRY) && !defined(CONFIG_KRYPTON)
	case POWER_SUPPLY_PROP_OEM_OTG:
		if(chip->otg_init_done) {
			rc = smb1351_read_reg(chip, IRQ_F_REG, &reg);
			if (rc < 0) {
				pr_err("Couldn't read OTG irq status, rc=%d\n", rc);
				return rc;
			}
			pr_debug("usb_otg,reg=0x%02x\n", reg);
			val->intval = reg;
		}
		else
			val->intval = 0;
	...
	case POWER_SUPPLY_PROP_VREG_CTRL:
		if(chip->otg_init_done) {
			...
			pr_debug("vreg_ctrl,reg=0x%02x\n", reg);
			val->intval = (reg & STATUS_OTG_BIT) ? 1 : 0;
		}
		else
			val->intval = 0;
		break;
	case POWER_SUPPLY_PROP_USB_ID:
		if(chip->otg_init_done) {
			...
			if (rc < 0) {
				pr_err("Couldn't read usbid status, rc=%d\n", rc);
				return rc;
			}
			pr_debug("usbid,reg=0x%02x\n", reg);
			val->intval = (reg & STATUS_RID_FLOAT_STATE_MACHINE_BIT) ? 1 : 0;
		}
		else
			val->intval = 1;
		break;
...
#if defined(CONFIG_BBRY) && !defined(CONFIG_KRYPTON)
#define FIRST_IRQ_STATUS_REG	0x40
#define LAST_IRQ_STATUS_REG		0x47
static int show_irq_status_regs(struct seq_file *m, void *data)
{
	struct smb1351_charger *chip = m->private;
	int rc;
	u8 reg;
	...
	for (addr = FIRST_IRQ_STATUS_REG; addr <= LAST_IRQ_STATUS_REG; addr++) {
		rc = smb1351_read_reg(chip, addr, &reg);
		if (!rc)
			seq_printf(m, "0x%02x = 0x%02x\n", addr, reg);
	}
...
static int irq_status_debugfs_open(struct inode *inode, struct file *file)
{
	struct smb1351_charger *chip = inode->i_private;

	return single_open(file, show_irq_status_regs, chip);
}

static const struct file_operations irq_status_debugfs_ops = {
	.owner		= THIS_MODULE,
	.open		= irq_status_debugfs_open,
	.read		= seq_read,
	.llseek		= seq_lseek,
	.release	= single_release,
};
...
#if defined(CONFIG_BBRY) && !defined(CONFIG_KRYPTON)
		ent = debugfs_create_file("irq_status_reg", S_IFREG | S_IRUGO,
					chip->debug_root, chip,
					&irq_status_debugfs_ops);
		if (!ent)
			pr_err("Couldn't create irq status debug file\n");
#endif
...
#if defined(CONFIG_BBRY) && !defined(CONFIG_KRYPTON)
	/* OTG OC and UVLO configuration */
	of_property_read_u32(node, "oem,otg-oc-limit-idx",
						&chip->otg_oc_limit_idx);
	if(chip->otg_oc_limit_idx<0 || chip->otg_oc_limit_idx>3)
		chip->otg_oc_limit_idx = 0;

	of_property_read_u32(node, "oem,otg-uvlo-limit-idx",
						&chip->otg_uvlo_idx);
	if(chip->otg_uvlo_idx<0 || chip->otg_uvlo_idx>3)
		chip->otg_uvlo_idx = 0;
#endif
