...
struct smbchg_chip {
	...
	/*MODIFIED-BEGIN by jin.wang, 2016-04-14,BUG-1761323*/
	int				vbat_at_uv;    // added qcom SBA, wangjin
	int				disable_parallel_uv; // added qcom SBA, wangjin

// [BUG]-ADD-BEGIN TCTNB.WJ,4/9/2016,PR 1912713
#if defined(CONFIG_TCT_8996_COMMON)
	int				parallel_ctrl_warm;
	int				parallel_ctrl_cool;
#endif
// [BUG]-ADD-END TCTNB.WJ,4/9/2016
/*MODIFIED-END by jin.wang,BUG-1761323*/
...
#if defined(CONFIG_TCT_8996_COMMON) //MODIFIED by jin.wang, 2016-04-14,BUG-1761323
	PR_NBTCLDBG	= BIT(15),
#endif
...
static int smbchg_aicl_value;
//module_param_named(
//	smbchg_aicl_value, smbchg_aicl_value, int, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH
//);
...
static int smbchg_main_chg_fcc_percent = 46;// MODIDIED by TCTNB.YQJ,06/28/2016,defect-2152504
module_param_named(
	main_chg_fcc_percent, smbchg_main_chg_fcc_percent,
	int, S_IRUSR | S_IWUSR
);

static int smbchg_main_chg_icl_percent = 46;// MODIDIED by TCTNB.YQJ,06/28/2016,defect-2152504
module_param_named(
	main_chg_icl_percent, smbchg_main_chg_icl_percent,
	int, S_IRUSR | S_IWUSR
);

// [FEATURE]-MOD-BEGIN TCTNB.WJ,1/27/2015, FR1531254
#if defined(CONFIG_TCT_8996_COMMON)
static int smbchg_default_hvdcp_icl_ma = 1900; // MODIFIED by jin.wang, 2016-04-22,BUG-1988052
#else
static int smbchg_default_hvdcp_icl_ma = 1800;
#endif
// [FEATURE]-MOD-END TCTNB.WJ,1/27/2015, FR1531254
module_param_named(
...
#if defined(CONFIG_TCT_8996_COMMON)
static int smbchg_default_dcp_icl_ma = 2000; // MODIFIED by jin.wang, 2016-04-22,BUG-1988052
#else
...
/* [FEATURE]-MOD-BEGIN TCTNB.WJ,1/19/2016,FR979657 */
/* [PLATFORM]-MOD-by TCTNB.YQJ,05/28/2016,FR- 2219600 */
...
/* [PLATFORM]-MOD-END TCTNB.YQJ,05/28/2016 */
/* [FEATURE]-MOD-END TCTNB.WJ,1/19/2016 */
							...
							int current_ma)
{
	...
	smbchg_aicl_value = current_ma;
	pr_err("===smbchg_aicl_value is set to USB current_ma:%d \n",smbchg_aicl_value);
	}
		...
		int total_current_ma)
{
	...
	//smbchg_aicl_value = fcc_ma;
	//pr_err("===smbchg_aicl_value is set to fcc_ma:%d \n",smbchg_aicl_value);
		...
		int *ret_total_current_ma)
{
	struct power_supply *parallel_psy = get_parallel_psy(chip);
	union power_supply_propval pval = {0, };
	int min_current_thr_ma, rc, type, vbat_uv; //MODIFIED by jin.wang, 2016-04-14,BUG-1761323
...
static void smbchg_external_power_changed(struct power_supply *psy)
{
	...
	if (rc == 0)
		vote(chip->usb_suspend_votable, POWER_SUPPLY_EN_VOTER,
				!prop.intval, 0);
//[BUG]-MOD-BEGIN by Jin.Wang,3/14/2016
#if defined(CONFIG_TCT_8996_COMMON)
	current_limit = 500;
#else
	rc = chip->usb_psy->get_property(chip->usb_psy,
						...
						enum power_supply_type type)
{
	 ...
	 */
// [FEATURE]-MOD-BEGIN TCTNB.WJ,2/19/2016, FR1531254
...
// [FEATURE]-MOD-END TCTNB.WJ
	...
	/* MODIFIED-BEGIN by jin.wang, 2016-04-22,BUG-1988052*/
	/*
	...
	/* MODIFIED-END by jin.wang,BUG-1988052*/
...
static void handle_usb_removal(struct smbchg_chip *chip)
{
	...
	smbchg_aicl_value = 0;
	pr_err("===smbchg_aicl_value is set to 0 \n");
}
...
/*MODIFIED-BEGIN by NBTCT.YQJ, 2016/05/11,BUG-1995421*/
#define USB_CMD_APSD 0x41
#define APSD_RERUN BIT(0)
/*MODIFIED-END by TCTNB.YQJ*/
...
/*MODIFIED-END by jin.wang,BUG-1761323*/
	...
	pr_smb(PR_STATUS, "triggered\n");
/*MODIFIED-BEGIN by NBTCT.YQJ, rerun apsd, 2016/05/11,BUG-1995421*/
	read_usb_type(chip, &usb_type_name, &usb_supply_type);
/* rerun apsd */
	smbchg_sec_masked_write(chip, chip->usb_chgpth_base + USB_AICL_CFG,
		AICL_EN_BIT, 0);
...
/*rerun apsd end*/
/*delay 1000ms to wait the usb state ready to read*/
	msleep(1000);
 /*MODIFIED-END by TCTNB.YQJ*/
	/* usb inserted */
	read_usb_type(chip, &usb_type_name, &usb_supply_type);
	...
	smbchg_detect_parallel_charger(chip); //MODIFIED by jin.wang, 2016-04-14,BUG-1761323
	if (chip->parallel.avail && chip->aicl_done_irq
			&& !chip->enable_aicl_wake) {
...
// [FEATURE]-ADD-BEGIN TCTNB.WJ,12/8/2015,1053888
#if defined(CONFIG_TCT_8996_COMMON) //MODIFIED by jin.wang, 2016-04-14,BUG-1761323
	pr_smb(PR_NBTCLDBG, "TCTNB_CHG_TERM\n");
#endif
// [FEATURE]-ADD-END TCTNB.WJ,12/8/2015,1053888
	/*
...
// [FEATURE]-MOD-BEGIN TCTNB.WJ,1/19/2016,FR979657
	pr_smb_rt(PR_STATUS,
		...
		aicl_level);
// [FEATURE]-MOD-END TCTNB.WJ,1/9/2016
...
struct smbchg_chip *chip_globle = NULL;
static int usbin_aicl_get(char *buf, const struct kernel_param *kp)
{
	//bool enabled;
	int usbin_aicl;
	if((smbchg_aicl_value > 0)&&(chip_globle->usb_supply_type==POWER_SUPPLY_TYPE_USB_HVDCP))
	{
			
		usbin_aicl = 3000;
	}
	else
		usbin_aicl = smbchg_aicl_value;
//	return snprintf(buf,  "%d", usbin_uv);
	return sprintf(buf, "%d", usbin_aicl);
}

static struct kernel_param_ops usbin_aicl_ops = {
	.set = param_set_int,
	.get = usbin_aicl_get,
};

module_param_cb(smbchg_aicl_value, &usbin_aicl_ops, &smbchg_aicl_value, 0644);
...
/*MODIFIED-END by jin.wang,BUG-1761323*/
	...
	smbchg_read(chip, &reg_usb_icl_sts2, 0x1309, 1);
	if ((reg_usb_icl_sts2 & BIT(3)) && !get_effective_result(chip->usb_suspend_votable)) {
