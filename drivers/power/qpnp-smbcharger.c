...
#define ICL_OVERRIDE		BIT(2)
#define ICL_OVERRIDE_EN		BIT(2)
...
/* Set ICL_OVERRIDE in MFI*/
#ifdef CONFIG_BBRY_MFG
	rc = smbchg_masked_write(chip, chip->usb_chgpth_base + CMD_IL,
				ICL_OVERRIDE, ICL_OVERRIDE_EN);
#endif /* CONFIG_BBRY_MFG */
	rc = smbchg_masked_write(chip, chip->usb_chgpth_base + CMD_IL,
	...
	if ((reg & USBIN_UV_BIT) && (reg & USBIN_SRC_DET_BIT)) {
		...
		if ((reg & ICL_MODE_MASK) != ICL_MODE_HIGH_CURRENT) {
			...
			if (rc)
				pr_err("could not enable aicl reruns: %d", rc);
#else /*CONFIG_BBRY*/
			rc = smbchg_set_thermal_limited_usb_current_max(chip, 100);
			if (rc)
				pr_err("could not set ICL to 100mA for weak charger: %d", rc);
#endif /*CONFIG_BBRY*/
		}
		rc = power_supply_set_health_state(chip->usb_psy,
				POWER_SUPPLY_HEALTH_UNSPEC_FAILURE);
