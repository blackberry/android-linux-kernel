...
#ifdef CONFIG_BBRY
	int				float_voltage_comp_now;
        unsigned long			force_apsd_seconds;
	struct delayed_work		force_apsd_work;
#endif /*CONFIG_BBRY*/
	int				resume_delta_mv;
	int				safety_time;
...
// [BUG]-ADD-BEGIN TCTNB.WJ,4/9/2016,PR 1912713
	...
	int				parallel_ctrl_warm;
	int				parallel_ctrl_cool;
#endif
// [BUG]-ADD-END TCTNB.WJ,4/9/2016
	...
	struct qpnp_vadc_chip		*vchg_vadc_dev;
        struct qpnp_vadc_chip		*xo_vadc_dev;//[feature] mod-by sun zhangyang for defect2167030,2016.05.18
#ifdef CONFIG_BBRY
    struct class			*otg_class;
...
/* MODIFIED-BEGIN by jin.wang, 2016-04-23,BUG-1929920*/
#if defined(CONFIG_TCT_8X76_IDOL4S) || defined(CONFIG_TCT_8X76_IDOL4S_VDF)
	THERMAL_FCC_VOTER,
#endif
/* MODIFIED-END by jin.wang,BUG-1929920*/
	NUM_FCC_VOTER,
};
...
#else
static int smbchg_default_hvdcp_icl_ma = 1900; // MODIFIED by jin.wang, 2016-04-22,BUG-1929920
#endif
//sun zhangyang modify charging parameters based on HW, defect1058536,end
module_param_named(
...
#else
static int smbchg_default_dcp_icl_ma = 2000; // MODIFIED by jin.wang, 2016-04-22,BUG-1929920
#endif
...
static int get_prop_batt_health(struct smbchg_chip *chip)
{
	...
	{
		int float_voltage_comp_old = chip->float_voltage_comp_now;

		if ( chip->batt_cold || chip->batt_cool ) {
			/* set float_voltage_comp_now to 0, so we can charger to full at cool condition */
			chip->float_voltage_comp_now = 0;
		} else {
			/* set float_voltage_comp_now to float_voltage_comp */
			chip->float_voltage_comp_now = chip->float_voltage_comp;
		}
		if ( float_voltage_comp_old != chip->float_voltage_comp_now ) {
			int rc;
			rc = smbchg_float_voltage_comp_set(chip,
				chip->float_voltage_comp_now);
			if (rc < 0) {
				dev_err(chip->dev, "Couldn't set float voltage comp to %d rc = %d\n",
					chip->float_voltage_comp_now, rc);
			}
			pr_smb(PR_STATUS, "set float voltage comp now to %d\n",
				chip->float_voltage_comp_now);
		}
	}
#endif /*CONFIG_BBRY*/
...
#ifdef CONFIG_BBRY
static void force_apsd_src_detection(struct smbchg_chip *chip);
static int get_current_time(unsigned long *now_tm_sec);
static void force_apsd_src_detection_work(struct work_struct *work)
{
	struct smbchg_chip *chip = container_of(work,
				struct smbchg_chip,
				force_apsd_work.work);

	get_current_time(&chip->force_apsd_seconds);
	force_apsd_src_detection(chip);
}
#endif /*CONFIG_BBRY*/
							...
							int current_ma)
{
	...
	case POWER_SUPPLY_TYPE_USB:
		if ((current_ma < CURRENT_150_MA) &&
			...
			current_ma = CURRENT_100_MA;//[DEFECT]-MOD by sun zhangyang,19/05/2016, DEFECT:2120077,2119782
...
static void smbchg_parallel_usb_taper(struct smbchg_chip *chip)
{
	...
	if (!parallel_psy || !chip->parallel_charger_detected)
		smbchg_detect_parallel_charger(chip); // MODIFIED by jin.wang, 2016-04-22,BUG-1929920
...
// [BUG]-ADD-BEGIN TCTNB.WJ,4/9/2016,PR 1912713
	...
	if( ( batt_temp > chip->parallel_ctrl_warm )
		|| (batt_temp < chip->parallel_ctrl_cool) ) {
			pr_smb(PR_STATUS, "battery: %d, skipping\n", batt_temp);
			return false;
		}
#endif
// [BUG]-ADD-END TCTNB.WJ,4/9/2016
...
#if defined(CONFIG_TCT_8X76_IDOL4S) || defined(CONFIG_TCT_8X76_IDOL4S_VDF) // MODIFIED by jin.wang, 2016-04-22,BUG-1929920
								...
								int lvl_sel)
{
	...
	/* MODIFIED-BEGIN by jin.wang, 2016-04-22,BUG-1929920*/
	pr_err("system_temp_level: %d, %d\n",
	...
	if (chip->therm_lvl_sel == 0) {
			...
			(int)chip->thermal_mitigation[chip->therm_lvl_sel];
                /* MODIFIED-BEGIN by jin.wang, 2016-04-23,BUG-1929920*/
                rc = vote(chip->fcc_votable, THERMAL_FCC_VOTER, false,
					thermal_icl_ma);
	} else {
		thermal_icl_ma =
			(int)chip->thermal_mitigation[chip->therm_lvl_sel];
	...
	if (chip->therm_lvl_sel == 0) {
		...
		if (rc < 0)
			pr_err("Couldn't disable DC thermal ICL vote rc=%d\n",
				rc);
            /*[FEATURE]-BEGIN, SZY MODIFY FOR DEFECT1924474, 2016.04.08*/
             #if defined(CONFIG_TCT_8X76_IDOL4)
                thermal_icl_ma =
			(int)chip->thermal_mitigation[chip->therm_lvl_sel];
                rc = vote(chip->fcc_votable, BATT_TYPE_FCC_VOTER, true,
					thermal_icl_ma);
             #endif
            /*[FEATURE]-END, SZY MODIFY FOR DEFECT1924474, 2016.04.08*/
	} else {
		thermal_icl_ma =
			(int)chip->thermal_mitigation[chip->therm_lvl_sel];
		...
		rc = vote(chip->usb_icl_votable, THERMAL_ICL_VOTER, true,
					thermal_icl_ma);
             #endif
           /*[FEATURE]-END, SZY MODIFY FOR DEFECT1868908, 2016.03.28*/
		if (rc < 0)
...
#endif // MODIFIED by jin.wang, 2016-04-22,BUG-1929920
...
static void smbchg_external_power_changed(struct power_supply *psy)
{
    ...
    //[DEFECT]-Add-BEGIN by sun zhangyang,19/05/2016, DEFECT:2120077,2119782,begin
	rc = chip->usb_psy->get_property(chip->usb_psy,
...
#ifndef CONFIG_BBRY
   //add the judgement for avoiding led off with usb cable power on and unconfigured current less than 100mA
    if(current_limit<=150)
        current_limit = 100;
    else
        current_limit = 500;
    //[DEFECT]-Add-END by sun zhangyang
#endif
...
#ifdef CONFIG_BBRY
		/* cases show USB type not set but USB gives current limit value, we need do a redection to correct */
		if (current_limit != 0) {
			rc = chip->usb_psy->get_property(chip->usb_psy,
				POWER_SUPPLY_PROP_TYPE, &prop);
			if (rc < 0)
				dev_err(chip->dev,
					"could not read USB property type, rc=%d\n", rc);
			else {
				if (POWER_SUPPLY_TYPE_UNKNOWN == prop.intval) {
					pr_info("force detection USB type %d, limit=%d\n", prop.intval, current_limit);
					schedule_delayed_work(&chip->force_apsd_work,
						msecs_to_jiffies(10));
				}
			}
		    if (current_limit == 100){
			/* current_limit from 0 to !0 meaning real SDP detection
			 * can't use usb_target_current_ma as the value change from handle_usb_insertion */
			   if (cancel_delayed_work(&chip->force_apsd_work)){
				      pr_info("enumeration force_apsd_work canceled\n");
			    }
            }
		}
#endif /*CONFIG_BBRY*/
...
/*[defect]-MOD-BEGIN sun zhangyang ,4/26/2016,PR1996429*/
#if defined(CONFIG_TCT_8X76_IDOL4)
  #define DEFAULT_CDP_MA		500
#else
  #define DEFAULT_CDP_MA		1500
#endif
/*[defect]-MOD-end sun zhangyang ,4/26/2016,PR1996429*/
...
#ifdef CONFIG_BBRY
	/* cancel any pending apsd work */
	if (cancel_delayed_work(&chip->force_apsd_work)){
		pr_info("force_apsd_work canceled\n");
	}
#endif /*CONFIG_BBRY*/

	cancel_delayed_work_sync(&chip->hvdcp_det_work); // MODIFIED by jin.wang, 2016-04-22,BUG-1929920
	smbchg_aicl_deglitch_wa_check(chip);
...
#ifdef CONFIG_BBRY
	/* some time PMIC miss detect wall charger as USB host, we need delay the process and re-check src type */
	if (usb_supply_type == POWER_SUPPLY_TYPE_USB){
		unsigned long now_seconds;
		get_current_time(&now_seconds);
		/* to avoid endless loop only limit the apsd re-run action to do once more than 5 seconds */
		if (now_seconds - chip->force_apsd_seconds > 5) {
			#define APSD_SRC_DELAY_MS 5000
			pr_info("schedule force_apsd_work at %ld\n", now_seconds);
			schedule_delayed_work(&chip->force_apsd_work,
					msecs_to_jiffies(APSD_SRC_DELAY_MS));
		} else {
			pr_info("USB detected at %ld\n", now_seconds);
		}
	}
#endif /*CONFIG_BBRY*/
...
#if defined(CONFIG_TCT_8X76_COMMON)//MODIFY BY SZY, 4/11/2016,PR1929580
...
//[feature] mod-by sun zhangyang for defect2167030,2016.05.18, begin
static int smbchg_get_xo_thermal(struct smbchg_chip *chip)
{
   int rc;
   struct qpnp_vadc_result adc_result;
   if(chip->xo_vadc_dev){
       rc = qpnp_vadc_read(chip->xo_vadc_dev,
				50, &adc_result);
		if (rc) {
			pr_smb(PR_STATUS,"error  read rc = %d\n", rc);
			return 0;
		}
   }
   return adc_result.physical;
}
//[feature] mod-by sun zhangyang,end
...
// [FEATURE]-ADD-BEGIN TCTNB.WJ,12/8/2015,1053888
#if defined(CONFIG_TCT_8X76_IDOL4S) || defined(CONFIG_TCT_8X76_IDOL4S_VDF)
		pr_smb(PR_NBTCLDBG, "TCTNB_FASTCHG:%d\n",
...
// [FEATURE]-ADD-BEGIN TCTNB.WJ,12/8/2015,1053888
#if defined(CONFIG_TCT_8X76_IDOL4S) || defined(CONFIG_TCT_8X76_IDOL4S_VDF)
		pr_smb(PR_NBTCLDBG, "TCTNB_SOC:%d\n", val->intval);
#endif
// [FEATURE]-ADD-END TCTNB.WJ,12/8/2015,1053888
		break;
...
// [FEATURE]-ADD-BEGIN TCTNB.WJ,12/8/2015,1053888
#if defined(CONFIG_TCT_8X76_IDOL4S) || defined(CONFIG_TCT_8X76_IDOL4S_VDF)
		pr_smb(PR_NBTCLDBG, "TCTNB_CURR:%d\n", val->intval);
#endif
// [FEATURE]-ADD-END TCTNB.WJ,12/8/2015,1053888
		break;
...
// [FEATURE]-ADD-BEGIN TCTNB.WJ,12/8/2015,1053888
#if defined(CONFIG_TCT_8X76_IDOL4S) || defined(CONFIG_TCT_8X76_IDOL4S_VDF)
		pr_smb(PR_NBTCLDBG, "TCTNB_VOL:%d\n", val->intval);
#endif
// [FEATURE]-ADD-END TCTNB.WJ,12/8/2015,1053888
		break;
...
//[feature] mod-by sun zhangyang for defect2167030,2016.05.18,begin
     case POWER_SUPPLY_PROP_XO_THERMAL:
          val->intval = smbchg_get_xo_thermal(chip);
	 break;
//[feature] mod-by sun zhangyang,end
	default:
		return -EINVAL;
...
// [FEATURE]-ADD-BEGIN TCTNB.WJ,12/8/2015,1053888
#if defined(CONFIG_TCT_8X76_IDOL4S) || defined(CONFIG_TCT_8X76_IDOL4S_VDF)
	pr_smb(PR_NBTCLDBG, "TCTNB_BATT_HOT:%d\n", chip->batt_hot);
#endif
// [FEATURE]-ADD-END TCTNB.WJ,12/8/2015,1053888
...
// [FEATURE]-ADD-BEGIN TCTNB.WJ,12/8/2015,1053888
#if defined(CONFIG_TCT_8X76_IDOL4S) || defined(CONFIG_TCT_8X76_IDOL4S_VDF)
	pr_smb(PR_NBTCLDBG, "TCTNB_BATT_COLD:%d\n", chip->batt_cold);
#endif
// [FEATURE]-ADD-END TCTNB.WJ,12/8/2015,1053888
...
// [FEATURE]-ADD-BEGIN TCTNB.WJ,12/8/2015,1053888
#if defined(CONFIG_TCT_8X76_IDOL4S) || defined(CONFIG_TCT_8X76_IDOL4S_VDF)
	pr_smb(PR_NBTCLDBG, "TCTNB_BATT_WARM:%d\n", chip->batt_warm);
#endif
// [FEATURE]-ADD-END TCTNB.WJ,12/8/2015,1053888
...
// [FEATURE]-ADD-BEGIN TCTNB.WJ,12/8/2015,1053888
#if defined(CONFIG_TCT_8X76_IDOL4S) || defined(CONFIG_TCT_8X76_IDOL4S_VDF)
	pr_smb(PR_NBTCLDBG, "TCTNB_BATT_COOL:%d\n", chip->batt_cool);
#endif
// [FEATURE]-ADD-END TCTNB.WJ,12/8/2015,1053888
...
// [FEATURE]-ADD-BEGIN TCTNB.WJ,12/8/2015,1053888
#if defined(CONFIG_TCT_8X76_IDOL4S) || defined(CONFIG_TCT_8X76_IDOL4S_VDF)
	pr_smb(PR_NBTCLDBG, "TCTNB_CHG_ERROR:%d\n", reg);
#endif
// [FEATURE]-ADD-END TCTNB.WJ,12/8/2015,1053888
...
// [FEATURE]-ADD-BEGIN TCTNB.WJ,12/8/2015,1053888
#if defined(CONFIG_TCT_8X76_IDOL4S) || defined(CONFIG_TCT_8X76_IDOL4S_VDF)
		pr_smb(PR_NBTCLDBG, "TCTNB_CHG_TERM:%d\n", terminated);
#endif
// [FEATURE]-ADD-END TCTNB.WJ,12/8/2015,1053888
	}
	/*
...
// [FEATURE]-ADD-BEGIN TCTNB.WJ,12/8/2015,1053888
#if defined(CONFIG_TCT_8X76_IDOL4S) || defined(CONFIG_TCT_8X76_IDOL4S_VDF)
	pr_smb(PR_NBTCLDBG, "TCTNB_TAPER:%d\n", reg);
#endif
// [FEATURE]-ADD-END TCTNB.WJ,12/8/2015,1053888
...
// [FEATURE]-ADD-BEGIN TCTNB.WJ,12/8/2015,1053888
#if defined(CONFIG_TCT_8X76_IDOL4S) || defined(CONFIG_TCT_8X76_IDOL4S_VDF)
	pr_smb(PR_NBTCLDBG, "TCTNB_RECHARGE:%d\n", reg);
#endif
// [FEATURE]-ADD-END TCTNB.WJ,12/8/2015,1053888
...
// [FEATURE]-ADD-BEGIN TCTNB.WJ,12/8/2015,1053888
#if defined(CONFIG_TCT_8X76_IDOL4S) || defined(CONFIG_TCT_8X76_IDOL4S_VDF)
	pr_smb(PR_NBTCLDBG, "TCTNB_USBIN_OV:%d\n", reg);
#endif
// [FEATURE]-ADD-END TCTNB.WJ,12/8/2015,1053888
...
// [FEATURE]-ADD-BEGIN TCTNB.WJ,12/8/2015,1053888
#if defined(CONFIG_TCT_8X76_IDOL4S) || defined(CONFIG_TCT_8X76_IDOL4S_VDF)
	pr_smb(PR_NBTCLDBG, "TCTNB_SRC_DETECT:%d\n", src_detect);
#endif
// [FEATURE]-ADD-END TCTNB.WJ,12/8/2015,1053888
...
#ifdef CONFIG_BBRY
/**
 * overcurrent_uevent - notifies user space of otg overcurrent
 * @chip: Pointer to the smbchg_chip structure
 *
 */
static void overcurrent_uevent(struct smbchg_chip *chip)
{
	char *overcurrent[2] = { "USB_OTG_STATE=OVERCURRENT", NULL };
	kobject_uevent_env(&chip->otg_dev->kobj, KOBJ_CHANGE,
								overcurrent);
}


static ssize_t otg_state_show(struct device *pdev,
			struct device_attribute *attr, char *buf)
{
	struct smbchg_chip* chip = dev_get_drvdata(pdev);
	char *state = chip->overcurrent ? "OVERCURRENT" : "";

	return snprintf(buf, PAGE_SIZE, "%s\n", state);
}
static DEVICE_ATTR(state, S_IRUGO, otg_state_show, NULL);

static struct device_attribute *otg_attributes[] = {
	&dev_attr_state,
	NULL
};

static int otg_create_device(struct smbchg_chip *chip)
{
	struct device_attribute **attrs = otg_attributes;
	struct device_attribute *attr;
	int err;

	chip->otg_class = class_create(THIS_MODULE, "usb_otg");
	if (IS_ERR(chip->otg_class))
		return PTR_ERR(chip->otg_class);

	chip->otg_dev = device_create(chip->otg_class, NULL, MKDEV(0, 0),
		NULL, "usb_otg");
	if (IS_ERR(chip->otg_dev)) {
		class_destroy(chip->otg_class);
		return PTR_ERR(chip->otg_dev);
	}

	dev_set_drvdata(chip->otg_dev, chip);

	while ((attr = *attrs++)) {
		err = device_create_file(chip->otg_dev, attr);
		if (err) {
			device_destroy(chip->otg_class, chip->otg_dev->devt);
			class_destroy(chip->otg_class);
			return err;
		}
	}
...
#ifdef CONFIG_BBRY
		chip->overcurrent = true;
		overcurrent_uevent(chip);
#endif
		return IRQ_HANDLED;
	}
...
// [FEATURE]-ADD-BEGIN TCTNB.WJ,09/29/2015,536521
#if defined(CONFIG_TCT_8X76_IDOL4S) || defined(CONFIG_TCT_8X76_IDOL4S_VDF)
			smbchg_safety_timer_enable(chip, false);
#endif
// [FEATURE]-ADD-END TCTNB.WJ,09/29/2015,536521
		}
	}
...
static int smbchg_probe(struct spmi_device *spmi)
{
	...
	struct power_supply *usb_psy;
	//[feature] mod-by sun zhangyang for defect2167030,2016.05.18
	struct qpnp_vadc_chip *vadc_dev, *vchg_vadc_dev, *xo_vadc_dev;
...
//[feature] mod-by sun zhangyang for defect2167030,2016.05.18,begin
		...
		if (IS_ERR(xo_vadc_dev)) {
			rc = PTR_ERR(xo_vadc_dev);
			if (rc != -EPROBE_DEFER)
				dev_err(&spmi->dev, "Couldn't get vadc 'xo' rc=%d\n",
						rc);
			return rc;
		}
	}
//[feature] mod-by sun zhangyang,end
...
#ifdef CONFIG_BBRY
	INIT_DELAYED_WORK(&chip->force_apsd_work, force_apsd_src_detection_work);
	get_current_time(&chip->force_apsd_seconds);
#endif /*CONFIG_BBRY*/
	init_completion(&chip->src_det_lowered);
	init_completion(&chip->src_det_raised);
	...
	chip->vchg_vadc_dev = vchg_vadc_dev;
        chip->xo_vadc_dev = xo_vadc_dev;//[feature] mod-by sun zhangyang for defect2167030,2016.05.18
	chip->spmi = spmi;
	chip->dev = &spmi->dev;
...
// [BUG]-ADD-BEGIN TCTNB.WJ,4/9/2016,PR 1912713
#if defined(CONFIG_TCT_8X76_IDOL4S) || defined(CONFIG_TCT_8X76_IDOL4S_VDF)
	chip->parallel_ctrl_warm = 400;
	chip->parallel_ctrl_cool = 150;
#endif
// [BUG]-ADD-END TCTNB.WJ,4/9/2016
...
#ifdef CONFIG_BBRY
	chip->overcurrent = false;
	if (otg_create_device(chip))
		dev_err(chip->dev, "couldn't create otg device\n");
#endif
	return 0;
