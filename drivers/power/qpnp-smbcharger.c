...
#ifdef CONFIG_BBRY
	int				fastchg_step_target_ma;  /* results from fastchg_step_ma & target_fastchg_current_ma*/
	int				fastchg_step_ma;
	int				fastchg_step_mv;
	unsigned long			usbin_uv_count;
	s64				usbin_uv_time_ms;
	bool				weak_charger_check;
	int				aicl_level_ma;
...
#ifdef CONFIG_BBRY
static int smbchg_dc_max_set_disabled = 0;
module_param_named(
	dc_max_set_disabled, smbchg_dc_max_set_disabled, int, S_IRUSR | S_IWUSR
);
...
#else
	/*
	 * RID_STS gives incorrect ID pin floating result on insertion of some
	 * peripherals. Checking USBID_MSB against USBID_GND_THRESHOLD always
...
#ifdef CONFIG_BBRY
static void recheck_weak_charger(struct smbchg_chip *chip);
#endif /*CONFIG_BBRY*/
static void smbchg_usb_update_online_work(struct work_struct *work)
{
	...
	if ( chip->very_weak_charger )
		recheck_weak_charger(chip);
	if ( !slimport_dongle_is_connected() ) {
							...
							int current_ma)
{
	...
	case POWER_SUPPLY_TYPE_USB:
		...
		if (current_ma == CURRENT_900_MA) {
			...
			/* use override for USB host to test high charging rate */
			rc = smbchg_masked_write(chip,
					chip->usb_chgpth_base + CMD_IL,
					ICL_OVERRIDE_BIT, ICL_OVERRIDE_BIT);
			if (rc < 0)
				pr_err("Couldn't set USB override rc = %d\n", rc);
...
#ifdef CONFIG_BBRY
	/* check current limit */
	if ( current_ma > chip->fastchg_step_target_ma ) {
		pr_smb(PR_STATUS, "current_ma=%d over limit %d\n", current_ma, chip->fastchg_step_target_ma);
		current_ma = chip->fastchg_step_target_ma;
	}
#endif /*CONFIG_BBRY*/
...
#else /*CONFIG_BBRY*/
		/* enable charging when loading battery profile */
		en = (strcmp(prop.strval, UNKNOWN_BATT_TYPE) != 0
				|| chip->charge_unknown_battery);
		pr_smb(PR_MISC, "chg_en=%d Battery-type:%s\n", en, prop.strval);
#endif /*CONFIG_BBRY*/
...
#ifdef CONFIG_BBRY
	pr_smb(PR_INTERRUPT, "aicl count c:%d dgltch:%d first:%ld reg=%x\n",
			chip->aicl_irq_count, chip->aicl_deglitch_short,
			chip->first_aicl_seconds, reg);
#endif /*CONFIG_BBRY*/
...
#ifdef CONFIG_BBRY
{
		/* Only enable aicl rerun when we saw aicl level decrease */
		if ( chip->aicl_irq_count ) {
			int aicl_level_ma = smbchg_get_aicl_level_ma(chip);
			if ( chip->aicl_complete && aicl_level_ma >= chip->aicl_level_ma) {
				pr_smb(PR_INTERRUPT,
					"AICL complete elp:%ld c=%d aicl_level_ma=%d\n",
					elapsed_seconds, chip->aicl_irq_count, aicl_level_ma);
				rc = smbchg_hw_aicl_rerun_en(chip, false);
				if (rc)
					pr_err("could not disable aicl reruns: %d", rc);
				chip->aicl_level_ma = aicl_level_ma;
			} else if ( aicl_level_ma < chip->aicl_level_ma ){
				pr_smb(PR_INTERRUPT,
					"Got low AICL value %d than %d re-enable AICL rerun\n",
					aicl_level_ma, chip->aicl_level_ma);
				rc = smbchg_hw_aicl_rerun_en(chip, true);
				if (rc)
					pr_err("could not enable aicl reruns: %d", rc);
			}
		} else {
			chip->aicl_level_ma = 0;
		}
}
#endif /*CONFIG_BBRY*/
...
#ifdef CONFIG_BBRY
		/* place the code here to align with output from healthd. otherwise should in smbchg_external_power_changed(). */
		#define FASTCHG_STEP_DELTA_MV 100
		if ( chip->fastchg_step_ma != -EINVAL && chip->fastchg_step_mv != -EINVAL){
			/* check battery voltage and set fast charging current */
			int mv = val->intval/1000; //get_prop_batt_voltage_now(chip)/1000;
			int ma = chip->fastchg_step_target_ma;
			if ( mv > chip->fastchg_step_mv ) {
				ma = chip->fastchg_step_ma;
			} else if ( mv < chip->fastchg_step_mv - FASTCHG_STEP_DELTA_MV) {
				ma = chip->target_fastchg_current_ma;
			}
			if ( ma != chip->fastchg_step_target_ma) {
				pr_smb(PR_STATUS, "fastchg_step_target_ma change to %d mv=%d\n", ma, mv);
				chip->fastchg_step_target_ma = ma;
				smbchg_set_fastchg_current_raw(chip, ma);
			}
		}
#endif /*CONFIG_BBRY*/
		break;
				       ...
				       const union power_supply_propval *val)
{
...
#ifdef CONFIG_BBRY
	struct power_supply *wlc_psy = power_supply_get_by_name("wlc");
	...
	case POWER_SUPPLY_PROP_CHARGING_ENABLED:
		...
		rc = wlc_psy->set_property(wlc_psy,
				POWER_SUPPLY_PROP_CHARGING_ENABLED, val);
		if (rc)
			pr_err("Failed to set wlc power status! rc=%d\n", rc);
#endif /*CONFIG_BBRY*/
		break;
...
#ifdef CONFIG_BBRY
static unsigned get_dc_idx(int dc_curr_lim_ma)
{
	int i;

	for (i = 0; i < ARRAY_SIZE(dc_current_table); i++) {
		if (dc_current_table[i] > dc_curr_lim_ma)
			break;
	}
...
#ifdef CONFIG_BBRY
#define DC_BROWNOUT_DISCONN_TIME_MSEC 100
#define DC_MIN_CURRENT_LIMIT 500
#endif /*CONFIG_BBRY*/
...
#ifdef CONFIG_BBRY
		if (dc_present) {
			conn_ts = ktime_to_ms(ktime_get());
			if ((conn_ts - disconn_ts) <=
					DC_BROWNOUT_DISCONN_TIME_MSEC) {
				pr_err("DC_IN brownout detected, period = %lld ms\n", conn_ts - disconn_ts);
				if (chip->dc_max_current_ma >
						DC_MIN_CURRENT_LIMIT)
					dc_curr_lim_ma = dc_current_table[get_dc_idx(chip->dc_max_current_ma)-1];
			} else {
				dc_curr_lim_ma =
						chip->dc_target_current_ma;
			}

			if (chip->dc_max_current_ma != dc_curr_lim_ma) {
				rc = smbchg_set_thermal_limited_dc_current_max(chip, dc_curr_lim_ma);
				if (rc < 0)
					dev_err(chip->dev, "can't set dc current: %d\n", rc);
			}
		} else {
			disconn_ts = ktime_to_ms(ktime_get());
		}
#endif /*CONFIG_BBRY*/
		if (chip->dc_psy_type != -EINVAL && chip->psy_registered)
			power_supply_changed(&chip->dc_psy);
...
static void recheck_weak_charger(struct smbchg_chip *chip)
{
	...
	if ( chip->weak_charger_check ) {
		pr_info("HVDCP status weak workaround running\n");
		return;
	}

	chip->weak_charger_check = true;
	rc = smbchg_read(chip, &reg, chip->usb_chgpth_base + RT_STS, 1);
	if (rc) {
		pr_err("could not read rt sts rc=%d", rc);
		goto out;
	}
	...
	if ((reg & USBIN_UV_BIT) && (reg & USBIN_SRC_DET_BIT))
	{
		...
		if (reg & USBIN_HVDCP_SEL_BIT) {
			/* workaround for QC 2.0, allow 5V to recover from uv condition*/
			pr_info("HVDCP status weak workaround reg=0x%02x\n", reg);
			...
			for (tries = 0; tries < 10; tries++)
			{
				...
				rc = smbchg_read(chip, &reg, chip->usb_chgpth_base + RT_STS, 1);
				if ( !(reg & USBIN_UV_BIT) ) break;
			}
			pr_info("HVDCP 5V tries=%d reg=0x%02x\n", tries, reg);
			rc = smbchg_sec_masked_write(chip,
				chip->usb_chgpth_base + USBIN_CHGR_CFG,
			...
			for (tries = 0; tries < 10; tries++)
			{
				msleep(20); /* sleep for UV condition to disappear */
				rc = smbchg_read(chip, &reg, chip->usb_chgpth_base + RT_STS, 1);
				if ( !(reg & USBIN_SRC_DET_BIT) ) break;
			}
			pr_info("HVDCP 9V tries=%d reg=0x%02x\n", tries, reg);
			/* set back to 5V_9V */
			rc = smbchg_sec_masked_write(chip,
				chip->usb_chgpth_base + USBIN_CHGR_CFG,
...
#else /*CONFIG_BBRY*/
		pr_info("Very weak charger detected rt_sts=0x%02x\n", reg);
#endif /*CONFIG_BBRY*/
		chip->very_weak_charger = true;
		rc = smbchg_read(chip, &reg,
				chip->usb_chgpth_base + ICL_STS_2_REG, 1);
...
#ifdef CONFIG_BBRY
	else if ( reg & USBIN_SRC_DET_BIT && slimport_dongle_is_connected() )
	{	/* for slimport, once AC charger removed, report weak charger but not online,
		 * re-check usb insertion so online can be re-enabled */

		// process for weak USB host case. only check in 1 seconds, to reduce debounce UV condition
		s64 time_now = ktime_to_ms(ktime_get());
		if ( time_now - chip->usbin_uv_time_ms > 1000) {
			chip->usbin_uv_time_ms = time_now;
			chip->usbin_uv_count = 0;
		} else
			chip->usbin_uv_count++;

		if ( chip->usbin_uv_count <= 1) {
			pr_info("already in source detected state, re-do usb insertion rt_sts=0x%02x\n", reg);
			chip->very_weak_charger = false;
			rc = smbchg_primary_usb_en(chip, true,
					REASON_WEAK_CHARGER, &unused);
			if (rc)
				pr_err("could not re-enable charger: %d", rc);
			power_supply_set_present(chip->usb_psy, 0); // force to not present first, so USb can do their work
...
static irqreturn_t usbid_change_handler(int irq, void *_chip)
{
	...
	if (otg_present)
		...
		#define MPP1_EN_CTL		0xA046
		#define MPP_MASTER_EN_BIT	BIT(7)
		// When the accessory is removed, USB_PLUG must go low for ~20ms to let the ANX chip know that this has occurred
		// Enable MPP1 to put USB_PLUG to low
		smbchg_masked_write(chip, MPP1_EN_CTL, MPP_MASTER_EN_BIT, MPP_MASTER_EN_BIT);
		msleep(20);
		smbchg_masked_write(chip, MPP1_EN_CTL, MPP_MASTER_EN_BIT, 0);
	}
#endif /*CONFIG_BBRY*/
...
#ifdef CONFIG_BBRY
	OF_PROP_READ(chip, chip->fastchg_step_ma, "fastchg-step-ma", rc, 1);
	OF_PROP_READ(chip, chip->fastchg_step_mv, "fastchg-step-mv", rc, 1);
	chip->fastchg_step_target_ma = chip->target_fastchg_current_ma;
#endif /*CONFIG_BBRY*/
