...
#if defined(CONFIG_BBRY) && !defined(CONFIG_KRYPTON)
	struct class			*uevent_class;
	struct device			*uevent_dev;

	struct delayed_work		vbus_release_work;
	struct completion		usb_completed;
	unsigned long			otg_start_time;
	bool                    is_otg_present;
	bool					smb135x_otg_vreg;
	int						otg_status;
	int						otg_err_info;
	int						stat_irq;
	struct pinctrl 			*stat_pinctrl;
#endif
...
#if defined(CONFIG_BBRY)
#define AICL_CHECK_TIME_INTVAL_MS	60
...
struct qpnp_smb_irq_info {
	const char		*name;
	unsigned int	error_num;
	int (*smb_irq)(struct smbchg_chip *chip, u8 rt_stat);
};

struct smb_irq_handler_info {
	u8			val;
	u8			prev_val;
	struct qpnp_smb_irq_info	irq_info[4];
};
...
static int rerun_apsd(struct smbchg_chip *chip, bool ignore_irq);
static bool is_src_detect_high(struct smbchg_chip *chip);
static void bbry_rerun_apsd_for_usb(struct smbchg_chip *chip, bool ignore_irq);
...
#if defined(CONFIG_BBRY)
static int dcp_min_icl_ma = 1000;
module_param_named(
	dcp_min_icl_ma, dcp_min_icl_ma,
	int, S_IRUSR | S_IWUSR
);
...
#if defined(CONFIG_BBRY) && !defined(CONFIG_KRYPTON)
static int smbchg_parallel_prop_set(struct smbchg_chip *chip, 
			enum power_supply_property propid, int val)
{
	int rc=-ENODEV;
	struct power_supply *parallel_psy = get_parallel_psy(chip);
	union power_supply_propval pval = {0, };

	if(parallel_psy && parallel_psy->set_property) {
		pval.intval = val;
...
static int smbchg_parallel_prop_get(struct smbchg_chip *chip, 
			enum power_supply_property propid, int *val)
{
	int rc=-ENODEV;
	struct power_supply *parallel_psy = get_parallel_psy(chip);
	...
	if(parallel_psy && parallel_psy->get_property) {
		...
		if (rc < 0) {
			dev_err(chip->dev,
					"Couldn't read parallel prop:%d, rc=%d\n", 
					propid, rc);
			return rc;
		}
		*val = pval.intval;
...
{
	int rc, val=0;
	rc = smbchg_parallel_prop_get(chip, 
					POWER_SUPPLY_PROP_USB_ID, 
					&val);
	if(rc < 0) {
		dev_err(chip->dev, "Parallel not available, otg ignore\n");
		return false;
	}

	pr_smb(PR_OTG, "OTG is '%s'\n", 
			val ? "absent" : "present");
	return !val;
}
#else
...
/*Modify Begin by TCTNB.YQJ, 2016-12-02,task-3615608*/
	if(PD_DETECTED)
		type = 2;
/*Modify End by TCTNB.YQJ, 2016-12-02,task-3615608*/
#endif
		...
		int *ret_total_current_ma)
{
	...
	if (get_usb_supply_type(type) == POWER_SUPPLY_TYPE_USB_CDP) {
		...
		power_supply_set_present(parallel_psy, false);
		chip->parallel_charger_detected = false;
		pr_smb(PR_STATUS, "CDP adapter, suspend parallel\n");
#endif
		return false;
	}
	...
	if (chip->usb_supply_type == POWER_SUPPLY_TYPE_USB_DCP
			&& (chip->vbat_at_uv >= chip->disable_parallel_uv
			|| vbat_uv >= chip->disable_parallel_uv)) {
		pr_smb(PR_STATUS,
			"DCP adapter, vbat_at_uv(%d) vbatt (%d) > disable_parallel_uv(%d)\n",
			chip->vbat_at_uv, vbat_uv, chip->disable_parallel_uv);
		return false;
	}
#endif
	...
	if (get_usb_supply_type(type) == POWER_SUPPLY_TYPE_USB) {
		...
		power_supply_set_present(parallel_psy, false);
		chip->parallel_charger_detected = false;
		pr_smb(PR_STATUS, "SDP adapter, suspend parallel\n");
#endif
		return false;
	}
...
#if defined(CONFIG_BBRY)
static int smbchg_hvdcp_disable_cb(struct device *dev, int disable,
						...
						int last_client)
{
	...
	pr_smb(PR_STATUS, "disable %d last_disable %d\n",
			disable, last_disable);
	rc = smbchg_sec_masked_write(chip,
				chip->usb_chgpth_base + CHGPTH_CFG,
...
#if defined(CONFIG_BBRY)
	pr_smb(PR_STATUS, "set vfloat_mv to %d(mV) \n", vfloat_mv);
#endif
...
#if defined(CONFIG_BBRY)
static int set_vfloat_vote_cb(struct device *dev,
						int vfloat_mv,
						int client,
						int last_vfloat_mv,
						int last_client)
...
#define FCC_LIMIT_VALUE_688    (688)
#define FCC_LIMIT_VALUE_1204	(1100)
#define FCC_LIMIT_VALUE_1720	(1720)
#define FCC_LIMIT_VALUE_1930	(1930)
#define FCC_LIMIT_VALUE_2500	(2500)
...
static void smbchg_check_batt_jetta_settings(struct smbchg_chip *chip)
{
	...
	if(batt_temp<JETTA_POINT_1){ /* battery temp: (0, 12) degree */
		...
		vote(chip->fcc_votable, JIETA_FCC_VOTER, true, FCC_LIMIT_VALUE_1930);
	} else if(batt_temp<JETTA_POINT_3 || !chip->batt_warm) { /* battery temp:  (12, 46) degree */
		vote(chip->vfloat_votable, JIETA_VFLOAT_VOTER, false, 0);
		if(batV>=BATT_UPPER_THRESHOLD
			|| (soc>BATT_SOC_THRESHOLD && batV>BATT_MIDDLE_THRESHOLD)) {
			vote(chip->fcc_votable, JIETA_FCC_VOTER, true, FCC_LIMIT_VALUE_2500);
		}
		else if (batV<BATT_LOWER_THRESHOLD) {
	...
	if(batt_temp<JETTA_POINT_1){ /* battery temp: (0, 12) degree */
		vote(chip->vfloat_votable, JIETA_VFLOAT_VOTER, false, 0);
		vote(chip->fcc_votable, JIETA_FCC_VOTER, true, FCC_LIMIT_VALUE_688);
	} else if(batt_temp<JETTA_POINT_2) { /* battery temp:  (12, 22) degree */
		vote(chip->vfloat_votable, JIETA_VFLOAT_VOTER, false, 0);
		if(batV>=BATT_UPPER_THRESHOLD 
			|| (soc>BATT_SOC_THRESHOLD && batV>BATT_MIDDLE_THRESHOLD)) {
			vote(chip->fcc_votable, JIETA_FCC_VOTER, true, FCC_LIMIT_VALUE_1204);
		}
		else if (batV<BATT_LOWER_THRESHOLD) {
			vote(chip->fcc_votable, JIETA_FCC_VOTER, true, FCC_LIMIT_VALUE_1720);
		}
	} else if(batt_temp<JETTA_POINT_3 || !chip->batt_warm) { /* battery temp:  (22, 46) degree */
		vote(chip->vfloat_votable, JIETA_VFLOAT_VOTER, false, 0);
		if(batV>=BATT_UPPER_THRESHOLD
			|| (soc>BATT_SOC_THRESHOLD && batV>BATT_MIDDLE_THRESHOLD)) {
			vote(chip->fcc_votable, JIETA_FCC_VOTER, true, FCC_LIMIT_VALUE_1720);
...
#if defined(CONFIG_BBRY) && !defined(CONFIG_KRYPTON)
static int smb135x_vbus_ctrl(struct smbchg_chip *chip, bool pmic_or_smb)
{
	int rc = 0;
	static bool last_ctrl = true;

	pr_smb(PR_OTG, "path '%s' Hold the Vbus line: %d->%d \n",
			pmic_or_smb ? "PMIC" : "SMB135x",
			last_ctrl, pmic_or_smb);

	if(last_ctrl==pmic_or_smb)
		return 0;
	...
	if(pmic_or_smb) {
		...
		if (rc < 0)
			dev_err(chip->dev, "Couldn't write back usb allowance rc=%d\n", rc);
...
fail_2:
	vote(chip->hvdcp_disable_votable, HVDCP_OTG_VOTER, false, 0);
fail_1:
	vote(chip->usb_suspend_votable, OTG_EN_VOTER, false, 0);
out:
	return rc;
}

static void smb135x_vbus_release_work(struct work_struct *work)
{
	struct smbchg_chip *chip = container_of(work,
				struct smbchg_chip,
				vbus_release_work.work);
	int rc = 0;

	pr_smb(PR_OTG, "Starting... \n");

	mutex_lock(&chip->usb_status_lock);
	if(is_otg_present_schg_lite(chip)) {
		pr_smb(PR_OTG, 
			"OTG still present, skip release Vbus line\n");
	}
	else {
		pr_smb(PR_OTG, "Releasing VBUS line to PMIC...\n");
		rc = smbchg_parallel_prop_set(chip,POWER_SUPPLY_PROP_VREG_CTRL,0);
		if (rc < 0) {
			pr_emerg("Couldn't 2nd disable OTG boost rc=%d\n", rc);
			BUG_ON(1);
		}

		/* To make sure the otg boost shutdown completed before release vbus line */
		mdelay(100);
		rc = smb135x_vbus_ctrl(chip, true);
		if(rc < 0) {
			pr_emerg("VBUS Line released failed, rc=%d, bug here! \n", rc);
			BUG_ON(1);
		}
	}
	mutex_unlock(&chip->usb_status_lock);
	smbchg_relax(chip, PM_VBUS_CTRL);

	pr_smb(PR_OTG, "Ended, rc=%d \n", rc);
}

#define USBPHY_TIMEOUT_MS	(4000)
#define USBPHY_DELTA_MS		(3000)
#define VBUS_RELEASE_MS		(2000)
static int smb135x_otg_regulator_enable(struct regulator_dev *rdev)
{
	int rc = 0;
	...
	get_current_time(&enable_time);
	delta_time = abs(enable_time - chip->otg_start_time)*1000;
	if(delta_time > USBPHY_DELTA_MS) {
		dev_err(chip->dev, 
				"[%ld]too late(%ld)... ignore enable power\n", 
				enable_time, delta_time);
		return -ETIMEDOUT;
	}

	pr_smb(PR_OTG, "[%ld]Starting... 'enable', '%s', delta:%ld\n",
			enable_time,
			chip->is_otg_present ? "present" : "absent",
			delta_time);

	if(!is_otg_present_schg_lite(chip)) {
		rc = -ENODEV;
		pr_smb(PR_OTG, 
				"OTG absent:%d, skip enable SMB135x boost vreg.\n",
				chip->is_otg_present);
		goto out;
	}

	rc = smbchg_parallel_prop_set(chip,POWER_SUPPLY_PROP_PRESENT,0);
	if (rc < 0) {
		dev_err(chip->dev, "Couldn't suspend parallel chg path, rc=%d\n", rc);
		goto out;
	}

	rc = smbchg_parallel_prop_set(chip,POWER_SUPPLY_PROP_VREG_CTRL,0);
	if (rc < 0) {
		dev_err(chip->dev, "Couldn't 1st disable OTG boost rc=%d\n", rc);
		goto out;
	}

	rc = smb135x_vbus_ctrl(chip, false);
	if (rc < 0) {
		dev_err(chip->dev, "SMB135x couldn't hold Vbus line, rc=%d\n", rc);
		goto out;
	}

	rc = smbchg_parallel_prop_set(chip,POWER_SUPPLY_PROP_VREG_CTRL,1);
	if (rc < 0) {
		dev_err(chip->dev, 
			"Couldn't enable OTG boost rc=%d, release Vbus line\n", 
			rc);

		chip->otg_status = rc;
		complete_all(&chip->usb_completed);

		/* If otg enable failed due to smb135x, must make sure Vbus line released. */
		cancel_delayed_work_sync(&chip->vbus_release_work);
		smbchg_stay_awake(chip, PM_VBUS_CTRL);
		schedule_delayed_work(&chip->vbus_release_work,
								msecs_to_jiffies(VBUS_RELEASE_MS));
		return rc;
	}

	chip->otg_retries = 0;
	chip->otg_enable_time = ktime_get();
	pr_smb(PR_OTG, "Enabled SMB135x OTG Boost Done \n");
...
static int smb135x_otg_regulator_disable(struct regulator_dev *rdev)
{
	int rc = 0;
	...
	if(is_otg_present_schg_lite(chip)) {
		dev_err(chip->dev, 
				"OTG still present, skip disable otg boost, host error?\n");
		return 0;
	}

	get_current_time(&disable_time);
	delta_time = abs(disable_time - chip->otg_start_time)*1000;
	if(delta_time > USBPHY_DELTA_MS) {
		dev_err(chip->dev, 
				"[%ld]too late(%ld)... ignore disable power\n", 
				disable_time, delta_time);
		return 0;
	}

	pr_smb(PR_OTG, "[%ld]Starting... 'disable', '%s', delta:%ld\n",
			disable_time,
			chip->is_otg_present ? "present" : "absent",
			delta_time);
	...
	rc = smbchg_parallel_prop_set(chip,POWER_SUPPLY_PROP_VREG_CTRL,0);
	if (rc < 0) {
		dev_err(chip->dev, "Couldn't 1st disable OTG mode rc=%d\n", rc);
	}
	pr_smb(PR_OTG, "Disabled SMB135x Boost done, rc=%d\n", rc);
	complete_all(&chip->usb_completed);

	pr_smb(PR_OTG, "Delay %d ms to release VBUS line\n", VBUS_RELEASE_MS);
	cancel_delayed_work_sync(&chip->vbus_release_work);
	smbchg_stay_awake(chip, PM_VBUS_CTRL);
	schedule_delayed_work(&chip->vbus_release_work,
							msecs_to_jiffies(VBUS_RELEASE_MS));
...
static int smb135x_otg_regulator_is_enable(struct regulator_dev *rdev)
{
	int rc = 0, val=0;
	struct smbchg_chip *chip = rdev_get_drvdata(rdev);

	if(chip->is_otg_present) {
		rc = smbchg_parallel_prop_get(chip, POWER_SUPPLY_PROP_VREG_CTRL, &val);
		if (rc < 0) {
			dev_err(chip->dev,
					"Couldn't read OTG enable bit rc=%d\n", 
					rc);
			chip->otg_status = -ENODEV;
			complete_all(&chip->usb_completed);
			return -ENODEV;
		}
		rc = val;
	...
	pr_smb(PR_OTG, "Checked OTG(%s) Boost: val=%d \n",
			chip->is_otg_present ? "present" : "absent",
			rc);
	return rc;
}
...
struct regulator_ops smbchg_otg_reg_ops = {
	.enable		= smb135x_otg_regulator_enable,
	.disable	= smb135x_otg_regulator_disable,
	.is_enabled	= smb135x_otg_regulator_is_enable,
};
						...
						enum power_supply_type type)
{
	...
	if (type == POWER_SUPPLY_TYPE_USB)
		...
		current_limit_ma = dcp_min_icl_ma;
...
#if defined(CONFIG_BBRY)
	else if(is_usb_present(chip) && chip->usb_supply_type == POWER_SUPPLY_TYPE_USB_DCP)
	{
		rc = vote(chip->usb_icl_votable, PSY_ICL_VOTER, true,
					smbchg_default_dcp_icl_ma);
		if (rc < 0) {
			pr_err("Couldn't vote for max dcp USB ICL rc=%d\n", rc);
		}
	}
#endif
...
{
	if (!is_src_detect_high(chip)) {
		pr_smb(PR_MISC, "src det low, skip rerun apsd, removed??? \n");
		return;
	}
...
static void handle_usb_insertion(struct smbchg_chip *chip)
{
...
#if defined(CONFIG_BBRY) && !defined(CONFIG_KRYPTON)
	char uevent_env[20];
	char *uevent_envp[2] = { uevent_env, NULL };
#endif
	...
	if(usb_supply_type == POWER_SUPPLY_TYPE_USB)
	{
		bbry_rerun_apsd_for_usb(chip, true);

		read_usb_type(chip, &usb_type_name, &usb_supply_type);
		pr_smb(PR_STATUS,
				"inserted type(new)= %d (%s)", usb_supply_type, usb_type_name);
	}
	cancel_delayed_work_sync(&chip->hvdcp_det_work);
	smbchg_relax(chip, PM_DETECT_HVDCP);
	chip->last_aicl = 0;
...
#if defined(CONFIG_BBRY) && !defined(CONFIG_KRYPTON)
	strlcpy(uevent_env, "PMI_CHG_DET=", sizeof(uevent_env));
	strlcat(uevent_env, usb_type_name, sizeof(uevent_env));
	kobject_uevent_env(&chip->uevent_dev->kobj, KOBJ_CHANGE, uevent_envp);
#endif
}
...
{
#if defined(CONFIG_BBRY) && !defined(CONFIG_KRYPTON)
	char *connected[2]    = { "PMI_USB_STATE=CONNECTED", NULL };
	char *disconnected[2] = { "PMI_USB_STATE=DISCONNECTED", NULL };
	char **uevent_envp = usb_present ? connected : disconnected;
#endif
...
#if defined(CONFIG_BBRY) && !defined(CONFIG_KRYPTON)
static int smb135x_usbirqs_ctrl(struct smbchg_chip *chip, int param)
{
	int rc=0;
	union power_supply_propval prop = {0,};
	...
	if(chip->usb_psy && chip->usb_psy->set_property) {
		prop.intval = param;
		rc = chip->usb_psy->set_property(chip->usb_psy,
					POWER_SUPPLY_PROP_IRQ_CTRL, &prop);
		if (rc) {
			pr_err("Failed to %s usb phy irqs, rc=%d \n", 
					param ? "Enable" : "Disable", rc);
		}
	}
...
#define OTG_RETRY_DELAY_MS (300)
static int smb135x_otg_oc_reset(struct smbchg_chip *chip)
{
	int rc=0;
	pr_smb(PR_OTG, "Disable SMB135x OTG boost\n");
	rc = smbchg_parallel_prop_set(chip,POWER_SUPPLY_PROP_VREG_CTRL,0);
	if (rc) {
		dev_err(chip->dev, "Couldn't disable SMB135x OTG mode rc=%d\n", rc);
		goto out;
	}

	if(smb135x_usbirqs_ctrl(chip, 0))
		dev_err(chip->dev,"disable usbirq failed\n");

	pr_smb(PR_OTG, "Enabling PMIC OTG boost...\n");
	rc = smbchg_masked_write(chip, chip->bat_if_base + CMD_CHG_REG,
						OTG_EN_BIT, OTG_EN_BIT);
	if (rc) {
		dev_err(chip->dev,"Failed to enable PMIC OTG rc=%d\n", rc);
		goto fail_oc;
	}

	msleep(OTG_RETRY_DELAY_MS
			*(NUM_OTG_RETRIES-chip->otg_retries+1));
	...
	if(!is_otg_present_schg_lite(chip)) {
		...
		if (rc) {
			dev_err(chip->dev,
					"OTG absent, Failed to disable PMIC OTG rc=%d\n", 
					rc);
		}
		pr_smb(PR_OTG, "OTG is not present, ignore ocp reset\n");
		goto fail_oc;
	}

	pr_smb(PR_OTG, "Disabling PMIC OTG boost\n");
	rc = smbchg_masked_write(chip, chip->bat_if_base + CMD_CHG_REG,
						OTG_EN_BIT, 0);
	if (rc) {
		dev_err(chip->dev,"Failed to disable PMIC OTG rc=%d\n", rc);
		goto fail_oc;
	}

	mdelay(20);
	if(smb135x_usbirqs_ctrl(chip, 1))
		dev_err(chip->dev,"enable usbirq failed\n");

	pr_smb(PR_OTG, "Enabling SMB135x OTG boost\n");
	rc = smbchg_parallel_prop_set(chip,POWER_SUPPLY_PROP_VREG_CTRL,1);
	if (rc) {
		dev_err(chip->dev, "Couldn't enable SMB135x OTG mode rc=%d\n", rc);
		goto out;
	}
...
fail_oc:
	if(smb135x_usbirqs_ctrl(chip, 1))
		dev_err(chip->dev,"restore usbirq failed\n");
out:
	return rc;
}
#else
...
static int rerun_apsd(struct smbchg_chip *chip, bool ignore_irq)
{
	...
	chip->hvdcp_3_det_ignore_uv = ignore_irq;
				       ...
				       const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_SET_SHIP_MODE:
		rc = smbchg_sec_masked_write(chip, chip->bat_if_base + SHIP_MODE_OFFSET,
			SHIP_MODE_MASK, !val->intval);
		if (rc < 0) {
			dev_err(chip->dev,
				"Couldn't %s ship mode, rc = %d\n",
...
static irqreturn_t fastchg_handler(int irq, void *_chip)
{
	...
	pr_smb(PR_INTERRUPT, "p2f triggered with %d \n", chg_type);
...
#if defined(CONFIG_BBRY)
	set_property_on_fg(chip, POWER_SUPPLY_PROP_UPDATE_NOW, 1);
	chip->vbat_at_uv = get_prop_batt_voltage_now(chip);
	if (chip->vbat_at_uv < 0)
		pr_err("Unable to read vbat_at_uv\n");
#endif
...
#if defined(CONFIG_BBRY) && !defined(CONFIG_KRYPTON)
#define OTG_ERR_FAIL		BIT(1)
#define OTG_ERR_OCP			BIT(2)
#define OTG_ERR_TOUT		BIT(3)
#define OTG_ERR_VREG		BIT(4)

#define IRQ_LATCHED_MASK	(0x02)
#define IRQ_STATUS_MASK		(0x01)
#define BITS_PER_IRQ		(2)

#define OCP_IRQ_MASK		(0xC0)
#define FAIL_IRQ_MASK		(0x30)
#define USBID_IRQ_MASK		(0x0C)

#define OTG_OC_RETRY_DELAY_US	(250000)
static int smb135x_otg_oc_handler(struct smbchg_chip *chip, u8 status)
{
	int rc=0;
	s64 elapsed_us=0;

	pr_smb(PR_OTG, 
			"SMB135x OCP triggered with status: 0x%02x\n",
			status);

	if(!is_otg_present_schg_lite(chip) 		/* if OTG absent, ignore retry */
		|| (status&USBID_IRQ_MASK)  /* if usbid=0 & ocp=1 at the 1st insertion time, ignore retry*/
		|| (status&FAIL_IRQ_MASK)   /* if already failed, ignore retry */
		|| !(status&OCP_IRQ_MASK))  /* if not OCP irq, ignore retry */
		return 0;

	elapsed_us = ktime_us_delta(
					ktime_get(),
					chip->otg_enable_time);

	if (elapsed_us > OTG_OC_RETRY_DELAY_US 
		&& chip->otg_retries<=NUM_OTG_RETRIES)
		chip->otg_retries = 0;
	...
	if (chip->otg_retries < NUM_OTG_RETRIES) {
		...
		rc = smb135x_otg_oc_reset(chip);
		if (rc)
			pr_err("Retry stoped, failed to reset OTG OC state rc=%d\n", rc);
		else
			return 0;
	}

	pr_smb(PR_OTG,
		"Failed to retry OTG with times #%d\n",
		chip->otg_retries);

	chip->otg_err_info |= OTG_ERR_OCP;
	power_supply_changed(&chip->batt_psy);

	return rc;
}

static int smb135x_otg_fail_handler(struct smbchg_chip *chip, u8 status)
{
	pr_smb(PR_OTG, 
			"UVLO triggered for SMB135x path, status=0x%02x \n", 
			status);

	if(!is_otg_present_schg_lite(chip) /* if OTG absent, ignore notify upper layer */
		|| !(status&FAIL_IRQ_MASK))  /* if not real failed irq, ignore notify upper layer */
		return 0;

	chip->otg_err_info |= OTG_ERR_FAIL;
	power_supply_changed(&chip->batt_psy);
	return 0;
}
...
{
	pr_smb(PR_OTG, "PMIC OCP triggered\n");
	return IRQ_HANDLED;
}

static irqreturn_t otg_fail_handler(int irq, void *_chip)
{
	pr_smb(PR_OTG, "PMIC FAIL triggered\n");
	return IRQ_HANDLED;
}
#else
/**
...
static irqreturn_t aicl_done_handler(int irq, void *_chip)
{
	...
	if(usb_present) {
		if(chip->aicl_complete && aicl_level>0) {
			if(aicl_level < 500 || chip->last_aicl != aicl_level) {
				pr_smb(PR_INTERRUPT, "completed, aicl:%d->%d \n", 
						chip->last_aicl, aicl_level);
				chip->last_aicl = aicl_level;
				smbchg_parallel_usb_check_ok(chip);
			}
		}
...
#if defined(CONFIG_BBRY) && !defined(CONFIG_KRYPTON)
static int smb135x_usbid_handler(struct smbchg_chip *chip, u8 status)
{
	long rc = 0;
	bool otg_present = false;

	if((status&USBID_IRQ_MASK) == USBID_IRQ_MASK) {
		pr_smb(PR_OTG, "ignore first usbid irq as status=0x%02x\n", status);
		return 0;
	}

	otg_present = is_otg_present_schg_lite(chip);
	pr_smb(PR_OTG, "otg_present:%d->%d, status:0x%02x\n", 
			chip->is_otg_present, otg_present, status);

	if(otg_present == chip->is_otg_present)
		return 0;

	if(!otg_present) {
		pr_smb(PR_OTG, "Once remove otg, 1st quickly disabling smb135x vreg\n");
		rc = smbchg_parallel_prop_set(chip,POWER_SUPPLY_PROP_VREG_CTRL,0);
		if (rc < 0) {
			dev_err(chip->dev, "Couldn't disable OTG Vreg rc=%ld\n", rc);
		}
	} else if(is_src_detect_high(chip)) {
		pr_err("ignore usbid irq, status:0x%02x, src_det:high!\n", 
				status);
		return 0;
	}

	get_current_time(&chip->otg_start_time);
	reinit_completion(&chip->usb_completed);

	chip->otg_err_info = 0;
	chip->is_otg_present = otg_present;
	if (chip->usb_psy) {
		pr_smb(PR_OTG, "setting usbpsy OTG = %d\n",
				otg_present ? 1 : 0);

		/* dwc3_msm_power_set_property_usb() */
		power_supply_set_usb_otg(chip->usb_psy, otg_present);
	}

	/* If last ops failed, clear status and no wait */
	if(!otg_present && chip->otg_status) {
		pr_err("last ops error(%d),ignore waiting\n",
				chip->otg_status);

		if(chip->otg_status == -ETIMEDOUT) {
			smbchg_stay_awake(chip, PM_VBUS_CTRL);
			rc = schedule_delayed_work(&chip->vbus_release_work,
						msecs_to_jiffies(VBUS_RELEASE_MS));

			pr_err("last ops timeouted, release vbus line, rc=%ld.\n", rc);
		}
		chip->otg_status = 0;
		return 0;
	}

	pr_smb(PR_OTG, "[%ld]: Waiting usbphy to '%s' vreg...\n",
			chip->otg_start_time,
			otg_present? "enabling" : "disabling");

	rc = wait_for_completion_interruptible_timeout(
			&chip->usb_completed,
			msecs_to_jiffies(USBPHY_TIMEOUT_MS));
	if (rc > 0){
		if(otg_present && chip->otg_status) {
			pr_err("Enable ops failed, return %d\n",
					chip->otg_status);
			chip->otg_err_info |= OTG_ERR_VREG;
			power_supply_changed(&chip->batt_psy);
			return chip->otg_status;
		}
		else {
			pr_smb(PR_OTG, "usbphy completed ['%s'][OK], left time=%d(ms) \n",
					otg_present? "enabled" : "disabled", 
					jiffies_to_msecs(rc));
			return 0;
		}
	}
		...
		if(otg_present) {
			chip->otg_status = -ETIMEDOUT;
			chip->otg_err_info |= OTG_ERR_TOUT;
			power_supply_changed(&chip->batt_psy);
		}
		else {
			smbchg_stay_awake(chip, PM_VBUS_CTRL);
		...
		/* usb phy timeouted or interrupted */
		pr_err("usbphy '%s' response timeout, rc=%ld\n",
				otg_present? "enabled" : "disabled", rc);

		return -ETIMEDOUT;
	}
}

static struct smb_irq_handler_info stat_irq_handlers = {
	.val		= 0,
	.prev_val	= 0,
	.irq_info	= {
		{	.name	 = "otg_retry",
		...
		{	.name	 = "usbid",
			.smb_irq = smb135x_usbid_handler,
		},
		{	.name	 = "otg_fail",
			.smb_irq = smb135x_otg_fail_handler,
		},
		{	.name	 = "otg_oc",
			.smb_irq = smb135x_otg_oc_handler,
		},
	},
};

static irqreturn_t smb135x_statirq_handler(int irq, void *_chip)
{
	u8 triggered=0,changed=0,rt_stat=0,prev_rt_stat=0;
	int i, rc=0, val=0;
	struct smbchg_chip *chip = _chip;

	if(!chip->smb135x_otg_vreg) {
		pr_err("SMB135x OTG not availble due to init failed\n");
		return IRQ_HANDLED;
	}

	mutex_lock(&chip->usb_status_lock);
	rc = smbchg_parallel_prop_get(chip, POWER_SUPPLY_PROP_OEM_OTG, 
							&val);
	if (rc) {
		pr_err("Couldn't read OTG irq register, rc = %d, old:%d\n", 
				rc, chip->is_otg_present);

		if(chip->is_otg_present) {
			chip->otg_status = 0;
			smb135x_usbid_handler(chip, 0x08);
		}
		goto out;
	}
	else if(!val){
		pr_smb(PR_OTG,"Invalid stat_irq, ignore handle\n");
		goto out;
	}

	stat_irq_handlers.val = (u8)val;
	pr_smb(PR_OTG, "triggered with val=0x%02x \n", 
				stat_irq_handlers.val);

	for (i = 1; i < ARRAY_SIZE(stat_irq_handlers.irq_info); i++) {
		triggered = stat_irq_handlers.val
		       & (IRQ_LATCHED_MASK << (i * BITS_PER_IRQ));
		rt_stat = stat_irq_handlers.val
			& (IRQ_STATUS_MASK << (i * BITS_PER_IRQ));
		prev_rt_stat = stat_irq_handlers.prev_val
			& (IRQ_STATUS_MASK << (i * BITS_PER_IRQ));
		changed = prev_rt_stat ^ rt_stat;

		if ((triggered || changed)
			&& stat_irq_handlers.irq_info[i].smb_irq != NULL) {
			rc = stat_irq_handlers.irq_info[i].smb_irq(chip,
							stat_irq_handlers.val);
			if (rc) {
				stat_irq_handlers.irq_info[i].error_num++;
				pr_err("Couldn't handle '%s' irq, rc = %d\n",
					stat_irq_handlers.irq_info[i].name, rc);
			}
		}
	}
	stat_irq_handlers.prev_val = stat_irq_handlers.val;
...
static int smbchg_hw_init(struct smbchg_chip *chip)
{
    ...
    rc = vote(chip->hvdcp_disable_votable, HVDCP_PMIC_VOTER,
				chip->hvdcp_not_supported, 0);
	if (rc < 0) {
		dev_err(chip->dev, "Couldn't %s HVDCP rc=%d\n",
				chip->hvdcp_not_supported ? "disable" : "enable",
				rc);
		return rc;
	}
#else
...
#if defined(CONFIG_BBRY)
	OF_PROP_READ(chip, chip->disable_parallel_uv,
				"disable-parallel-uv", rc, 1);
#endif
...
#if defined(CONFIG_BBRY)
	chip->input_suspend = of_property_read_bool(node,
...
#if defined(CONFIG_BBRY) && !defined(CONFIG_KRYPTON)
static int smbchg_create_uevent_device(struct smbchg_chip *chip)
{
	chip->uevent_class = class_create(THIS_MODULE, "pmi_chg");
	if (IS_ERR(chip->uevent_class))
		return PTR_ERR(chip->uevent_class);

	chip->uevent_dev = device_create(chip->uevent_class, NULL, MKDEV(0, 0),
		NULL, "pmi_chg");
	if (IS_ERR(chip->uevent_dev)) {
		class_destroy(chip->uevent_class);
		return PTR_ERR(chip->uevent_dev);
	}

	dev_set_drvdata(chip->uevent_dev, chip);
...
#if defined(CONFIG_BBRY) && !defined(CONFIG_KRYPTON)
static int smbchg_setup_otg_irq(struct smbchg_chip *chip)
{
	int rc=-1, stat_gpio=-1;
	struct pinctrl_state *set_state=NULL;
	struct device_node *node = chip->dev->of_node;

	stat_gpio = of_get_named_gpio(node, "stat-gpio", 0);
	if(!gpio_is_valid(stat_gpio))
		stat_gpio = 0;

	if(stat_gpio <= 0) {
		dev_err(chip->dev, "Invalid stat_gpio %d\n", stat_gpio);
		goto out;
	}

	chip->stat_pinctrl = devm_pinctrl_get(chip->dev);
	if (IS_ERR_OR_NULL(chip->stat_pinctrl)) {
			dev_err(chip->dev, "Error encountered while getting otg pinctrl");
			goto out;
	}

	set_state = pinctrl_lookup_state(chip->stat_pinctrl,
				"smb_int_default");
	if (IS_ERR_OR_NULL(set_state)) {
		pr_err("cannot get smb_int_default active state\n");
		goto failed_pinctl;
	}

	rc = pinctrl_select_state(chip->stat_pinctrl, set_state);
	if(rc) {
		pr_err("cannot set smb_int_default active state\n");
		goto failed_pinctl;
	}

	rc = gpio_request(stat_gpio, "stat-gpio");
	if(rc) {
		dev_err(chip->dev, "failed to request stat_gpio\n");
		goto failed_pinctl;
	}

	rc = gpio_direction_input(stat_gpio);
	if(rc) {
		dev_err(chip->dev, "failed to dir stat_gpio\n");
		goto failed_gpio;
	}

	chip->stat_irq = gpio_to_irq(stat_gpio);
	if(chip->stat_irq < 0) {
		dev_err(chip->dev, "failed to map stat-irq\n");
		goto failed_gpio;
	}

	rc = request_threaded_irq(chip->stat_irq, NULL, smb135x_statirq_handler, 
				IRQF_TRIGGER_FALLING | IRQF_ONESHOT,
				"stat_irq",
				chip);
	if(rc < 0) {
		dev_err(chip->dev, "failed to request stat_irq %d\n",
				rc);
		goto failed_irq;
	}

	rc = enable_irq_wake(chip->stat_irq);
	if(rc < 0) {
		dev_err(chip->dev, "failed to enable stat_irq %d\n",
				rc);
		goto failed_irq;
	}
	return 0;
...
failed_pinctl:
	devm_pinctrl_put(chip->stat_pinctrl);
	chip->stat_pinctrl = NULL;
out:
	chip->stat_irq = -1;
	return rc;
}
...
static int smbchg_probe(struct spmi_device *spmi)
{
	...
	struct qpnp_vadc_chip *vchg_vadc_dev = NULL;
...
#if defined(CONFIG_BBRY)
	chip->vfloat_votable = create_votable(&spmi->dev,
			"SMBCHG: vfloat",
			VOTE_MIN, NUM_VFLOAT_VOTER, 4400,
			set_vfloat_vote_cb);
	if (IS_ERR(chip->vfloat_votable))
		return PTR_ERR(chip->vfloat_votable);
#endif
...
#if defined(CONFIG_BBRY)
	chip->hvdcp_disable_votable = create_votable(&spmi->dev,
			"SMBCHG: hvdcp_disable",
			VOTE_SET_ANY, NUM_HVDCP_VOTERS, 0,
			smbchg_hvdcp_disable_cb);
	if (IS_ERR(chip->hvdcp_disable_votable))
		return PTR_ERR(chip->hvdcp_disable_votable);
#else
...
#if defined(CONFIG_BBRY) && !defined(CONFIG_KRYPTON)
	init_completion(&chip->usb_completed);
	INIT_DELAYED_WORK(&chip->vbus_release_work, 
					smb135x_vbus_release_work);
	smbchg_create_uevent_device(chip);
#endif
...
#if defined(CONFIG_BBRY) && !defined(CONFIG_KRYPTON)
	rc = smbchg_parallel_prop_set(chip,
					POWER_SUPPLY_PROP_OEM_OTG,1);
	if(!rc) {
		chip->smb135x_otg_vreg = true;
		pr_smb(PR_OTG, "SMB135x OTG init done\n");
	}
	else
		dev_err(&spmi->dev,
			"SMB135x OTG not available, rc = %d\n", rc);
#endif
	...
	if (chip->usb_present != usb_present) {
		...
		set_property_on_fg(chip, POWER_SUPPLY_PROP_STATUS,
							get_prop_batt_status(chip));
		pr_err("[Warning]charger insert/removed too quickly: %d\n",
				chip->usb_present);
	}
	mutex_unlock(&chip->usb_status_lock);
#endif
...
#if defined(CONFIG_BBRY) && !defined(CONFIG_KRYPTON)
	if(chip->smb135x_otg_vreg) {
		rc = smbchg_setup_otg_irq(chip);
		if(rc) {
			dev_err(chip->dev, "OTG irq setup failed , rc:%d \n", rc);
			smbchg_parallel_prop_set(chip,
					POWER_SUPPLY_PROP_OEM_OTG, 0);
			chip->smb135x_otg_vreg = false;
		}
		else {
			pr_smb(PR_OTG, "SMB135x OTG Available\n");
		}
	}
#endif
...
#if !defined(CONFIG_KRYPTON)
	pr_emerg("[SD]Disabling OTG Boost...\n");
	rc = smbchg_parallel_prop_set(chip,POWER_SUPPLY_PROP_VREG_CTRL,0);
	if(rc)
		pr_emerg("[SD]Couldn't disable SMB135x OTG, rc=%d.\n", rc);
#endif
