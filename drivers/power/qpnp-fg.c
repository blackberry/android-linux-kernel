...
{
	return clamp(DIV_ROUND_CLOSEST(msoc * (FULL_CAPACITY)*10, FULL_SOC_RAW), EMPTY_CAPACITY*10, FULL_CAPACITY*10);
}
#endif /*CONFIG_BBRY*/
...
static int get_prop_capacity(struct fg_chip *chip)
{
	...
	if (chip->charge_full) {
		if (fg_debug_mask & FG_STATUS)
			pr_info("charge_full=1, returned 100 directly \n");
		return FULL_CAPACITY;
	}
						...
						enum power_supply_property psp)
{
	...
	case POWER_SUPPLY_PROP_BATTERY_INFO:
	case POWER_SUPPLY_PROP_BATTERY_INFO_ID:
#if defined(CONFIG_BBRY) || defined(CONFIG_BBRY_MFG)
        /* mini software also need this */
	case POWER_SUPPLY_PROP_UPDATE_NOW:
...
#if defined(CONFIG_BBRY)
	if (!profiles_same)
		pr_err("profiles differ, reset FG\n");
#else
