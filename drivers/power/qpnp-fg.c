...
/*TCTNB.YQJ Modified Begin, enable fg_reset_on_lockup,defect-1808061, 2016/05/10 */
...
/*TCTNB.YQJ Modified Begin for battery-capacity value not right at first few times, defect-1843037, 2016/03/25 */
static int fg_profile_loaded = 0;
module_param_named(
	fg_profile_loaded, fg_profile_loaded, int, S_IRUSR | S_IWUSR
);
/*TCTNB.YQJ Modified END*/
struct fg_irq {
	int			irq;
...
/* [FEATURE]-MOD-END TCTNB.WJ,1/8/2016 */
...
/* get capacity at 1/1000 accuracy */
static int capacity_from_raw( int msoc)
{
	return clamp(DIV_ROUND_CLOSEST(msoc * (FULL_CAPACITY+3)*10, FULL_SOC_RAW), EMPTY_CAPACITY*10, FULL_CAPACITY*10);
}
#endif /*CONFIG_BBRY*/
...
#endif
#else /*CONFIG_BBRY*/
	return DIV_ROUND_CLOSEST(capacity_from_raw(msoc), 10);
#endif /*CONFIG_BBRY*/
...
#ifdef CONFIG_BBRY
{
	#define MAX_TTF_SEC           (12*3600)
	#define MAX_TTE_SEC           (240*3600)
	#define MIN_CURRENT_UA        (2000)

	int msoc;
	int capacity_1000;
	int cc_uah;
	int current_ua = get_sram_prop_now(chip, FG_DATA_CURRENT);
	if (current_ua > MIN_CURRENT_UA) {
		/* discharging */
		if ( chip->avg_current <= 0 ) chip->avg_current = current_ua;
		else chip->avg_current = (chip->avg_current+current_ua)/2;
	} else if (current_ua < -MIN_CURRENT_UA) {
		/* charging */
		if ( chip->avg_current >=0 ) chip->avg_current = current_ua;
		else chip->avg_current = (chip->avg_current+current_ua)/2;
	} else {
	...
	msoc = get_monotonic_soc_raw(chip);
	capacity_1000 = capacity_from_raw(msoc);
	if (chip->charge_full || chip->status == POWER_SUPPLY_STATUS_FULL) {
		/* fully charged */
		chip->time_to_full = 0;
		chip->time_to_empty = -1;
	} else if (chip->status == POWER_SUPPLY_STATUS_CHARGING) {
		/* charging or try to charging */

		if ( chip->avg_current > 0 ) {
			/* charger can not provide enought power, discharging battery */
			chip->time_to_full = -1;
			cc_uah = chip->learning_data.learned_cc_uah * capacity_1000 / (FULL_CAPACITY*10);
			chip->time_to_empty = (int64_t)cc_uah*36/(chip->avg_current/100);
			if (chip->time_to_empty > MAX_TTE_SEC)
				chip->time_to_empty = MAX_TTE_SEC;
		} else if (chip->avg_current == 0) {
			/* charger only provide enough power to system no charging current */
//			pr_info("charging status=%d current=%d\n", chip->status, chip->avg_current);
			chip->time_to_full = MAX_TTF_SEC;
			chip->time_to_empty = -1;
		} else {
			/* really on charge */
			cc_uah = chip->learning_data.learned_cc_uah*(FULL_CAPACITY*10 - capacity_1000) / (FULL_CAPACITY*10);
			chip->time_to_full = -(int64_t)cc_uah*36/(chip->avg_current/100);
			if (chip->time_to_full > MAX_TTF_SEC)
				chip->time_to_full = MAX_TTF_SEC;
			chip->time_to_empty = -1;
		}
	} else {
		...
		if (chip->avg_current > 0) {
			...
			/* discharging but no current device sleep? */
//			pr_info("discharging status=%d current=%d\n", chip->status, chip->avg_current);
			chip->time_to_empty = MAX_TTE_SEC;
		}
	}
}
...
static void fg_cap_learning_work(struct work_struct *work)
{
	...
	/* MODIFIED-BEGIN by jin.wang, 2016-05-31,BUG-2182600*/
	if (chip->wa_flag & USE_CC_SOC_REG)
		goto fail;
		/* MODIFIED-END by jin.wang,BUG-2182600*/
...
#ifdef CONFIG_BBRY
		if ( chip->health == POWER_SUPPLY_HEALTH_GOOD &&
		     get_monotonic_soc_raw(chip) < settings[FG_MEM_RESUME_SOC].value ) {
			pr_err("Trying to lower SOC, but SOC not reaching to FULL yet...\n");
			goto done;
		}
#endif /*CONFIG_BBRY*/
...
#define PROFILE_LOAD_TIMEOUT_MS		7000  // TCTNB.YQJ MODIFIED for increase the profile load wait time,defect-2144664,2016/05/14
...
no_profile:
	...
	if (chip->power_supply_registered)
		power_supply_changed(&chip->bms_psy);
	fg_relax(&chip->profile_wakeup_source);
/*TCTNB.YQJ Modified Begin for when no battery profile found still set fg_profile_loaded=1, defect-2118221, 2016/03/25 */
	fg_profile_loaded =1;
/*TCTNB.YQJ Modified END*/
	return rc;
reschedule:
