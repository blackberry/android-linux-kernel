...
/* download_mode need set 0 in mp branch */
	...
	*/
	...
	if (cmd != NULL) {
		if (!strncmp(cmd, "bootloader", 10)) {
			...
			__raw_writel(0x77665508, restart_reason);
			qpnp_pon_set_restart_reason(
				PON_RESTART_REASON_DM_VERITY_CORRUPT);
#endif
...
//Add-Begin by TCTNB.lijiang:2016/03/01,Defect-1692658
			...
			if (!ret) {
				...
				if (oem_code >= FIRST_OEM_HARD_RESET)
					qpnp_pon_system_pwr_off(
						PON_POWER_OFF_HARD_RESET);
				qpnp_pon_set_restart_reason(
					OEM_CODE_TO_PON_RESTART_REASON(
					oem_code));
			}
#endif
//Add-End by TCTNB.lijiang:2016/03/01,Defect-1692658
...
#ifdef CONFIG_BBRY
			qpnp_pon_set_restart_reason(
				PON_RESTART_REASON_KERNEL_RESTART_UNKNOWN_ARG);
#endif
		}
	}
#ifdef CONFIG_BBRY
...
/* [PLATFORM]-ADD-BEGIN by TCTNB.WJ, FR-1053888, 2015/12/30, add log for autotest */
