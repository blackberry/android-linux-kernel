	...
	*/
	...
	if (qpnp_pon_check_hard_reset_stored()) {
		...
		if ( ((cmd != NULL && cmd[0] != '\0') && (
			!strcmp(cmd, "recovery") ||
			!strcmp(cmd, "bootloader") ||
			!strcmp(cmd, "rtc") || strstr(cmd , "system_server"))))
...
#ifdef CONFIG_BBRY
	/* Set a specific restart reason for reset combo button command
	*/
		} else if (is_reset_button_combo) {
			__raw_writel(0x62627273, restart_reason);
#endif
	...
	/* BBRY defined the OEM code range of 0x80 - 0xff */
	/* for hard reset.                                */
	if ((oem_code >= 0x80))
		qpnp_pon_system_pwr_off(PON_POWER_OFF_HARD_RESET);
#endif
