...
#else /*CONFIG_BBRY*/
{
			static unsigned long lastpr_time, count=0;
			unsigned long pr_time;
			get_current_time(&pr_time);
			if ( pr_time != lastpr_time ) {
				pr_info("preventing CPU%d from coming online %ld times\n", cpu, count);
				count = 0;
				lastpr_time = pr_time;
			} else {
		...
		const char *buf, size_t count)
{
	...
	if (!strcmp(pbuf, "enable")) {
		bcl_update_online_mask();
		bcl_mode_set(BCL_DEVICE_ENABLED);
		pr_info("bcl enabled\n");
	} else if (!strcmp(pbuf, "disable")) {
