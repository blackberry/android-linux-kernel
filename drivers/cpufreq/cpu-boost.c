...
#ifdef CONFIG_BBRY
#ifdef CONFIG_MASKED_IP_BOOST
	unsigned int input_boost_mask;
...
#ifdef CONFIG_BBRY
#ifdef CONFIG_MASKED_IP_BOOST
static struct work_struct key_boost_work;
static struct work_struct touch_boost_work;
static struct work_struct scroll_boost_work;
#else
...
#ifdef CONFIG_BBRY
#ifdef CONFIG_MASKED_IP_BOOST
static struct delayed_work key_boost_rem;
static struct delayed_work touch_boost_rem;
static struct delayed_work scroll_boost_rem;
#else
...
static int set_input_boost_mask(const char *buf, const struct kernel_param *kp)
{
	int i, ntokens = 0;
	unsigned int val, cpu;
	const char *cp = buf;
	...
	if (!ntokens) {
		if (sscanf(buf, "%u\n", &val) != 1)
			return -EINVAL;
		for_each_possible_cpu(i)
			per_cpu(sync_info, i).input_boost_mask = val;
		goto check_enable;
	}
	...
	for (i = 0; i < ntokens; i += 2) {
		...
		if (cpu > num_possible_cpus())
			return -EINVAL;
		if(val > (TOUCH|KEY|SCROLL))
			return -EINVAL;

		per_cpu(sync_info, cpu).input_boost_mask = val;
		cp = strchr(cp, ' ');
		cp++;
	}
...
check_enable:
	for_each_possible_cpu(i) {
		if (per_cpu(sync_info, i).input_boost_mask) {
			enabled = true;
			break;
		}
	}
...
static int get_input_boost_mask(char *buf, const struct kernel_param *kp)
{
	int cnt = 0, cpu;
	struct cpu_sync *s;
	...
	for_each_possible_cpu(cpu) {
		s = &per_cpu(sync_info, cpu);
		cnt += snprintf(buf + cnt, PAGE_SIZE - cnt,
				"%d:%u ", cpu, s->input_boost_mask);
	}
	cnt += snprintf(buf + cnt, PAGE_SIZE - cnt, "\n");
	return cnt;
}

static const struct kernel_param_ops param_ops_input_boost_mask = {
	.set = set_input_boost_mask,
	.get = get_input_boost_mask,
};

module_param_cb(input_boost_mask, &param_ops_input_boost_mask, NULL, 0644);

static int set_percpu_evtype(int type,int code){
	int i=0;
	struct work_struct * work = NULL;

	for_each_possible_cpu(i) {
		struct cpu_sync * i_sync_info = &per_cpu(sync_info, i);
		if ((type == EV_MSC) || (type == EV_KEY)) {/*CKB reports events as MISC type; so we need to support this */
			if (i_sync_info->input_boost_mask & KEY) {
				i_sync_info->event_type |= KEY;
				work = &key_boost_work;
			}
		}
		else if( (type == EV_ABS) && (code == ABS_MT_TRACKING_ID)) {
			if (i_sync_info->input_boost_mask & TOUCH){
				i_sync_info->event_type |= TOUCH;
				work = &touch_boost_work;
			}
		}
		else if ((type == EV_ABS) && ((code == ABS_MT_POSITION_Y) || (code == ABS_MT_POSITION_X))){
			if (i_sync_info->input_boost_mask & SCROLL) {
				i_sync_info->event_type |= SCROLL;
				work = &scroll_boost_work;
			}
		}
	}
	if (work) {
		...
		queue_work(cpu_boost_wq,work);
	}
	return -1;
}


static int cpu_relaxed=0;
static inline int multi_ip_boost_rem(struct cpu_sync *i_sync_info,struct work_struct *work, int type)
{

	if ((i_sync_info->input_boost_mask & type) == type) {
		i_sync_info->event_type &= ~type;
		if (i_sync_info->event_type == 0){
...
#ifdef CONFIG_BBRY
#ifdef CONFIG_MASKED_IP_BOOST
static void boost_rem(struct work_struct *work, int type)
{
	unsigned int i,ret;
	...
	for_each_possible_cpu(i) {
		...
		multi_ip_boost_rem(i_sync_info,work,type);
	}
...
{
	boost_rem(work,TOUCH);
}

static void  do_scroll_boost_rem(struct work_struct *work)
{
	boost_rem(work,SCROLL);
}

static void  do_key_boost_rem(struct work_struct *work)
{
	boost_rem(work,KEY);
}
...
static void do_touch_boost(struct work_struct *work)
{
	...
	do_input_boost(work);
	queue_delayed_work(cpu_boost_wq, &touch_boost_rem,msecs_to_jiffies(input_boost_ms));
}

static void do_scroll_boost(struct work_struct *work)
{
	cancel_delayed_work_sync(&scroll_boost_rem);
	do_input_boost(work);
	queue_delayed_work(cpu_boost_wq, &scroll_boost_rem,msecs_to_jiffies(input_boost_ms));
}
static void do_key_boost(struct work_struct *work)
{
	cancel_delayed_work_sync(&key_boost_rem);
	do_input_boost(work);
	queue_delayed_work(cpu_boost_wq, &key_boost_rem,msecs_to_jiffies(input_boost_ms));
}

static void init_multi_ip_wq(void)
{
	INIT_WORK(&touch_boost_work, do_touch_boost);
	INIT_WORK(&scroll_boost_work, do_scroll_boost);
	INIT_WORK(&key_boost_work, do_key_boost);

	INIT_DELAYED_WORK(&touch_boost_rem, do_touch_boost_rem);
	INIT_DELAYED_WORK(&key_boost_rem, do_key_boost_rem);
	INIT_DELAYED_WORK(&scroll_boost_rem, do_scroll_boost_rem);
}
#else
...
#ifdef CONFIG_BBRY
#ifdef CONFIG_MASKED_IP_BOOST
		if ((i_sync_info->event_type & i_sync_info->input_boost_mask)==i_sync_info->event_type) {
...
#ifdef CONFIG_BBRY
#ifdef CONFIG_MASKED_IP_BOOST
	if(set_percpu_evtype(type,code)==0)
		return;
#else
