...
#ifdef CONFIG_BBRY_MASKED_IP_BOOST
...
static inline void do_type_boost(struct cpu_sync *i_sync_info)
{
	switch(i_sync_info->event_type){
		case SCROLL:
...
static inline void queue_multi_ip_work(struct cpu_sync *i_sync_info)
{
	switch(i_sync_info->event_type){
		case TOUCH:
			queue_delayed_work(cpu_boost_wq, &touch_boost_rem,
					msecs_to_jiffies(input_boost_ms));
			break;
		case KEY:
			queue_delayed_work(cpu_boost_wq, &key_boost_rem,
					msecs_to_jiffies(input_boost_ms));
			break;
		case SCROLL:
			queue_delayed_work(cpu_boost_wq, &scroll_boost_rem,
					msecs_to_jiffies(input_boost_ms));
			break;
		default:
		break;
...
static inline void set_percpu_evtype(int type,int code){
	int i=0;
...
static inline void init_multi_ip_wq(void)
{
	INIT_DELAYED_WORK(&touch_boost_rem, do_input_boost_rem);
	INIT_DELAYED_WORK(&key_boost_rem, do_input_boost_rem);
	INIT_DELAYED_WORK(&scroll_boost_rem, do_input_boost_rem);
}

static inline void multi_ip_boost_rem(struct cpu_sync *i_sync_info)
{
	if( ((i_sync_info->input_boost_mask & KEY) && (i_sync_info->event_type == KEY))  ||
			((i_sync_info->input_boost_mask & TOUCH) && (i_sync_info->event_type == TOUCH )) ||
			((i_sync_info->input_boost_mask & SCROLL) &&  (i_sync_info->event_type == SCROLL ))){
		i_sync_info->event_type = 0;
...
#ifdef CONFIG_BBRY_MASKED_IP_BOOST
		multi_ip_boost_rem(i_sync_info);
#else
		i_sync_info->input_boost_min = 0;
#endif
	}
...
#ifndef CONFIG_BBRY_MASKED_IP_BOOST
...
#ifdef CONFIG_BBRY_MASKED_IP_BOOST
	queue_multi_ip_work(i_sync_info);
#else
