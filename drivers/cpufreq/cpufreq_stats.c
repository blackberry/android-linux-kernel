...
#ifdef CONFIG_BBRY
static int cpufreq_cpu_down_callback(struct notifier_block *nfb,
...
static struct notifier_block cpu_down_notifier __refdata = {
	.notifier_call = cpufreq_cpu_down_callback,
	.priority = 1,
};
#endif
...
#ifdef CONFIG_BBRY
	ret = register_hotcpu_notifier(&cpu_down_notifier);
	if (ret) {
		unregister_hotcpu_notifier(&cpu_down_notifier);
		return ret;
	}
#endif
