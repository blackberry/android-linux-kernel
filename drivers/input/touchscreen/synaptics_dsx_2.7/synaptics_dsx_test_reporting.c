...
show_prototype(do_tddi_amp_electrode_open)
show_prototype(do_tddi_noise)
show_prototype(do_tddi_ee_short)
...
static struct attribute *attrs[] = {
	...
	attrify(do_tddi_amp_electrode_open),
	attrify(do_tddi_noise),
	attrify(do_tddi_ee_short),
	attrify(tddi_noise),
		...
		struct device_attribute *attr, char *buf)
{
	...
	case F54_FULL_RAW_CAP_TDDI:
		...
		report_data_u16 = (short *)f54->report_data;
		cnt = snprintf(buf, PAGE_SIZE - count, "tx = %d\nrx = %d\n\n",
				tx_num, rx_num);
		buf += cnt;
		count += cnt;
		...
		for(ii = 0; ii < tx_num; ii++) {
			for(jj = 0; jj < (rx_num - 1); jj++) {
				cnt = snprintf(buf, PAGE_SIZE - count, "%-5d ",
						*report_data_u16);
				if(*report_data_u16 <= 1700 || *report_data_u16 >= 2300){
		...
		struct device_attribute *attr, char *buf)
{
	...
	sprintf(report_type, "%d", F54_RAW_16BIT_IMAGE);
...
static ssize_t test_sysfs_do_tddi_ee_short_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int i, j;
	int tx_num = f54->tx_assigned;
	...
	sprintf(cmd, "%d", 1);
	retval = test_sysfs_tddi_ee_short_store(dev, attr, cmd, sizeof(cmd));
	if (retval < 0)
		return snprintf(buf, PAGE_SIZE, "short test failed\n");
...
static ssize_t test_sysfs_do_tddi_noise_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int i, j;
	int tx_num = f54->tx_assigned;
	...
	sprintf(cmd, "%d", 1);
	retval = test_sysfs_tddi_noise_store(dev, attr, cmd, sizeof(cmd));
	if (retval < 0)
		return snprintf(buf, PAGE_SIZE, "noise test failed\n");
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	for (i = 0; i < tx_num; i++) {
		for (j = 0; j < rx_num; j++) {
			...
			k += 2;
			dev_err(f54->rmi4_data->pdev->dev.parent,"%s: p_rt92_image_1(tx%-2drx%-2d)=%-4d, %d %d\n", 
				__func__, i, j, p_rt92_image_1[i * rx_num + j],
				p_report_data_8[k], p_report_data_8[k+1]);
		}
	}
	...
	printk("%s report_size: %d %d\n", __func__, tx_num * rx_num * 2, f54->report_size);
	...
	for (i = 0; i < tx_num; i++) {
		for (j = 0; j < rx_num; j++) {
			printk("%s report_data(tx%drx%d)=%d  %d %d\n", __func__, i, j, *report_data_u16, p_report_data_8[k], p_report_data_8[k+1]);
			report_data_u16++;
			p_rt92_image_2[i * rx_num + j] =
				(signed short)(p_report_data_8[k] & 0xff ) | (signed short)(p_report_data_8[k + 1] << 8);
			k += 2;
			dev_err(f54->rmi4_data->pdev->dev.parent,"%s: p_rt92_image_2(tx%-2drx%-2d)=%d, %d %d\n", 
				__func__, i, j, p_rt92_image_2[i * rx_num + j],
				p_report_data_8[k], p_report_data_8[k+1]);
...
static ssize_t test_sysfs_do_tddi_amp_electrode_open_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int i, j;
	int tx_num = f54->tx_assigned;
	...
	retval = test_sysfs_tddi_amp_electrode_open_store(dev, attr, cmd, sizeof(cmd));
	if (retval < 0)
		return snprintf(buf, PAGE_SIZE, "open test failed\n");
