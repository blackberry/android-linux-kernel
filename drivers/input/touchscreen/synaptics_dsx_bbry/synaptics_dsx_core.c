				...
				unsigned int code, int value)
{
	...
	if (code == SW_LID) {
		...
		event.event_id = HOLSTER_STATE;
		event.event_data[0] = value;
	...
	if (code == SW_SMART_FLIP) {
		...
		event.event_id = SMART_FLIP_STATE;
		event.event_data[0] = value;
		synaptics_record_events(rmi4_data, &event);
		return;
	}
		...
		struct synaptics_rmi4_data *rmi4_data, bool set)
{
	...
	dev_info(rmi4_data->pdev->dev.parent,
		"rc=%u, remaining %lld msec, resol= %lld us, ktime=%lldms\n", rc,
			ktime_to_ms(hrtimer_get_remaining(
				&rmi4_data->mtouch_counter.inadv_tapdet_hrtimer)),
			ktime_to_us(rmi4_data->mtouch_counter.inadv_tapdet_hrtimer.base->resolution),
			ktime_to_ms(MS_TO_KTIME(timeout_ms)));
}
				...
				struct synaptics_rmi4_data *rmi4_data)
{
	...
	rc = hrtimer_start(&rmi4_data->mtouch_counter.inadv_tapdet_hrtimer,
			MS_TO_KTIME(timeout_ms), HRTIMER_MODE_REL);
	dev_info(rmi4_data->pdev->dev.parent,
		"%s: return val rc=%u, remaining %lld msec\n", __func__, rc,
	...
	struct device_attribute *attr, char *buf)
{
	...
	size += snprintf(buf+strlen(buf), PAGE_SIZE - strlen(buf),
			"\nHolster state: 0: none, 1: in holster\n");

	size += snprintf(buf+strlen(buf), PAGE_SIZE - strlen(buf),
			"\nSmart flip state: 0: none, 1: in smart flip case\n");
	...
	size += snprintf(buf+strlen(buf), PAGE_SIZE - strlen(buf),
			"holster - EVENT_DATA format: value\n");
	size += snprintf(buf+strlen(buf), PAGE_SIZE - strlen(buf),
			"sflip - EVENT_DATA format: value\n");
...
#ifdef CONFIG_TOUCHSCREEN_SYNAPTICS_DSX_DDT
	INIT_DELAYED_WORK(&rmi4_data->ddt_work, synaptics_dsx_ddt_send_handler);
#endif
	dev_dbg(&pdev->dev,
		"%s:EXIT\n",
		__func__);
...
int synaptics_rmi4_runtime_resume(struct device *dev)
{
	...
	if (rmi4_data->wakeup_source == WAKEUP_BY_DOUBLE_TAP) {
		double_tap_wakeup = 1;
	}
