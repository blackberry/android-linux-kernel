...
#ifdef CONFIG_BBRY
#define	FT_INIT_MASK			0x00
#define	FT_FWUPG_COMPLETE_MASK		0x01
#define	FT_GESTURE_ID_SET_MASK		0x02
#define	FT_GLOVE_ID_SET_MASK		0x04
#define	FT_INIT_COMPLETE_MASK		(FT_FWUPG_COMPLETE_MASK | \
					 FT_GESTURE_ID_SET_MASK	| \
					 FT_GLOVE_ID_SET_MASK)
#endif
...
#ifdef CONFIG_BBRY
static unsigned long i2c_rw_error_cnt;
static unsigned long backlight_delay_time_msecs;
static unsigned long dtwakeup_time_ms;
static unsigned long dtwakeup_time_ms_store;
#endif
...
/*[FEATURE]-Modified-END by TCTSH.xingchen.wang for task 1238223, 2015/12/25*/
#ifdef CONFIG_BBRY
	struct ft5x06_i2c_stats stats;
	struct ft5x06_i2c_mtouch_counter mtouch_counter;
	unsigned long dtwakeup_complete_time_ms;
	bool touch_state;
	struct work_struct backlight_delay_work;
...
#ifdef CONFIG_BBRY
static void backlight_change_handler(struct ft5x06_ts_data *data);
static void backlight_delay_work(struct work_struct *work);
static ssize_t mtouch_counter_show(struct device *dev,
				   struct device_attribute *attr, char *buf);
static ssize_t stats_show(struct device *dev,
			  struct device_attribute *attr, char *buf);
static void mtouch_counter_init(struct ft5x06_ts_data *data);
static void ft5x06_i2c_mtouch_counter_reset(struct ft5x06_i2c_mtouch_counter
					    *mtouch_counter);
static void stats_handler(unsigned long arg);
static void stats_restart(struct ft5x06_ts_data *data);
static inline u32 get_timestamp(void);
static void stats_timeout_work(struct work_struct *work);
static void stats_init(struct ft5x06_ts_data *data);
static void stats_start(struct ft5x06_ts_data *data);
static void stats_stop(struct ft5x06_ts_data *data);
static inline ssize_t store_error(struct device *dev,
				  struct device_attribute *attr,
				  const char *buf, size_t count);
DEVICE_ATTR(mtouch_counter, 0444,
			mtouch_counter_show, store_error);
DEVICE_ATTR(stats, 0444, stats_show, store_error);
#endif
...
/*[FEATURE]-Modified-END by TCTSH.xingchen.wang for task 1238223, 2015/12/25*/
/*[FEATURE]-Modified-BEGIN by TCTSH.xingchen.wang for defect 1305291, 2016/02/18, add macro to control features*/
#ifdef CONFIG_BBRY
...
#endif
#if defined(VR_GLASS)
static struct workqueue_struct *ft5x06_wq_vr;
#endif
/*[FEATURE]-Modified-END by TCTSH.xingchen.wang for defect 1305291, 2016/02/18*/
...
/*[FEATURE]-Modified-BEGIN by TCTSH.xingchen.wang for task 1356189, 2016/01/18, set func-register after TP reset*/
static void tp_function_set_confirm (struct ft5x06_ts_data *data);
...
#ifdef CONFIG_BBRY
static struct device *tp_stats_dev;
#endif
...
/*[PLATFORM]-Add-BEGIN by TCTSH.xuefei.wang, Task-2459640, 2016/07/01, add TP TYPE */
/*TP_TYPE*/
const char *ft_tp_name;
static ssize_t tp_type_show(struct class *class,struct class_attribute *attr,char *buf)
{
       // struct ft5x06_ts_data *data = NULL;

       // u8 reg;
        int ret;
        printk("xuefei.wang%s\n",__func__);
      //  data = dev_get_drvdata(dev);
        ret = snprintf(buf, 50, "%s\n", ft_tp_name);
        return ret;
}

static CLASS_ATTR(tp_type,0444,tp_type_show,NULL);
/*[PLATFORM]-Add-END by TCTSH.xuefei.wang, Task-2459640, 2016/07/01, add TP TYPE */
/*
					    ...
					    *mtouch_counter)
{
	...
	mtouch_counter->finger_cnt = 0;
	mtouch_counter->backlight_delay_300ms = 0;
	mtouch_counter->backlight_delay_500ms = 0;
	mtouch_counter->backlight_delay_750ms = 0;
	mtouch_counter->backlight_delay_1000ms = 0;
	mtouch_counter->backlight_delay_2000ms = 0;
	mtouch_counter->dtwakeup_delay_200ms = 0;
	mtouch_counter->dtwakeup_delay_400ms = 0;
	mtouch_counter->dtwakeup_delay_1sec = 0;
	mtouch_counter->dtwakeup_delay_2sec = 0;
	mtouch_counter->successful_double_tap = 0;
}

static void mtouch_counter_init(struct ft5x06_ts_data *data)
{
	ft5x06_i2c_mtouch_counter_reset(&data->mtouch_counter);
	mutex_init(&data->mtouch_counter.mutex);
}

static void stats_handler(unsigned long arg)
{
	struct ft5x06_ts_data *data = (struct ft5x06_ts_data *)arg;

	queue_work(ft5x06_wq, &data->stats.timeout_work);
}

static void stats_restart(struct ft5x06_ts_data *data)
{
	mutex_lock(&data->stats.mutex);

	if (data->stats.active) {
		del_timer_sync(&data->stats.timer);
		data->stats.timer.expires = jiffies + msecs_to_jiffies(STATS_PERIOD);
		add_timer(&data->stats.timer);
	}

	mutex_unlock(&data->stats.mutex);
}

static inline u32 get_timestamp()
{
...
static void stats_timeout_work(struct work_struct *work)
{
	struct ft5x06_i2c_stats *stats = container_of(work,
							struct ft5x06_i2c_stats, timeout_work);
	struct ft5x06_ts_data *data = container_of(stats,
							struct ft5x06_ts_data, stats);
	int total, total_served, touch, report_touch;
	int finger_release, touch_release, entry;
	...
	if (stats->next_stats_entry >= STATS_SIZE)
		stats->next_stats_entry = 0;
	if (stats->num_stats_entries < STATS_SIZE)
		stats->num_stats_entries++;

	dev_info(&data->client->dev, "INT=%d INT_S=%d T=%d REPORT_T=%d FR=%d TR=%d\n",
		total, total_served, touch, report_touch, finger_release, touch_release);
end:
...
static void stats_init(struct ft5x06_ts_data *data)
{
	data->stats.total_int_cnt = 0;
	data->stats.total_int_served_cnt = 0;
	data->stats.touch_int_cnt = 0;
	data->stats.touch_frelease_cnt = 0;
	data->stats.touch_trelease_cnt = 0;
	data->stats.active = false;
	data->stats.update = false;
	data->stats.num_stats_entries = 0;
	data->stats.next_stats_entry = 0;

	mutex_init(&data->stats.mutex);
	init_timer(&data->stats.timer);
	data->stats.timer.data = (unsigned long)data;
	data->stats.timer.function = stats_handler;
	INIT_WORK(&data->stats.timeout_work, stats_timeout_work);
}
		...
		struct device_attribute *attr, char *buf)
{
	...
	ssize_t size = 0;
	struct ft5x06_i2c_mtouch_counter *mtouch_counter  = &(data->mtouch_counter);
	mtouch_counter->i2c_rw_error = i2c_rw_error_cnt;

	if (data->gesture_id == 0x01)
		mtouch_counter->double_tap_enabled = 1;
	else
		mtouch_counter->double_tap_enabled = 0;

	size += snprintf(buf + size, PAGE_SIZE - size,
			"Mtouch counter for %s\n", data->input_dev->name);
	size += snprintf(buf + size, PAGE_SIZE - size,
			"Double tab enabled = %d\n", mtouch_counter->double_tap_enabled);
	size += snprintf(buf + size, PAGE_SIZE - size,
			"Swipe wake = %d\n", mtouch_counter->swipe_wakeup);
	size += snprintf(buf + size, PAGE_SIZE - size,
			"Double tap wakeups delay 200ms to 400ms = %d\n",
			mtouch_counter->dtwakeup_delay_200ms);
	size += snprintf(buf + size, PAGE_SIZE - size,
			"Double tap wakeups delay 400ms to 1000ms= %d\n",
			mtouch_counter->dtwakeup_delay_400ms);
	size += snprintf(buf + size, PAGE_SIZE - size,
			"Double tap wakeups delay 1000ms to 2000ms = %d\n",
			mtouch_counter->dtwakeup_delay_1sec);
	size += snprintf(buf + size, PAGE_SIZE - size,
			"Double tap wakeups delay greater than 2000ms = %d\n",
			mtouch_counter->dtwakeup_delay_2sec);
	size += snprintf(buf + size, PAGE_SIZE - size,
			"Backlight delay 300ms to 500ms = %d\n",
			mtouch_counter->backlight_delay_300ms);
	size += snprintf(buf + size, PAGE_SIZE - size,
			"Backlight delay 500ms to 750ms = %d\n",
			mtouch_counter->backlight_delay_500ms);
	size += snprintf(buf + size, PAGE_SIZE - size,
			"Backlight delay 750ms to 1000ms = %d\n",
			mtouch_counter->backlight_delay_750ms);
	size += snprintf(buf + size, PAGE_SIZE - size,
			"Backlight delay 1000ms to 2000ms = %d\n",
			mtouch_counter->backlight_delay_1000ms);
	size += snprintf(buf + size, PAGE_SIZE - size,
			"Backlight delay greater than 2000ms = %d\n",
			mtouch_counter->backlight_delay_2000ms);
	size += snprintf(buf + size, PAGE_SIZE - size,
			"double tap interrupt = %d\n", mtouch_counter->double_tap);
	size += snprintf(buf + size, PAGE_SIZE - size,
			"Successful double tap wakeups = %d\n",
	...
	struct device_attribute *attr, char *buf)
{
	...
	struct ft5x06_ts_data *data = dev_get_drvdata(dev);
	struct ft5x06_i2c_stats *stats = &data->stats;
	...
	n = snprintf(&buf[len], size, "INT,INT_SERVED,TOUCH,FINGER_RELEASE,TOUCH_RELEASE\n");
	size -= n;
	len  += n;

	for (i = 0; i < num; i++) {
		entry = (entry > 0) ? (entry-1) : (STATS_SIZE-1);
...
static void backlight_change_handler(struct ft5x06_ts_data *data)
{
	if (!backlight_delay_time_msecs)
		return;
	mutex_lock(&data->mtouch_counter.mutex);
	if (backlight_delay_time_msecs < 300) {
		//nothing
	}
	else if (backlight_delay_time_msecs < 500) {
		data->mtouch_counter.backlight_delay_300ms++;
	}
	else if (backlight_delay_time_msecs < 750) {
		data->mtouch_counter.backlight_delay_500ms++;
	}
	else if (backlight_delay_time_msecs < 1000) {
		data->mtouch_counter.backlight_delay_750ms++;
	}
	else if (backlight_delay_time_msecs < 2000) {
		data->mtouch_counter.backlight_delay_1000ms++;
	}
	else {
		data->mtouch_counter.backlight_delay_2000ms++;
	}
	mutex_unlock(&data->mtouch_counter.mutex);
	backlight_delay_time_msecs = 0;
}

static void backlight_delay_work(struct work_struct *work)
{
	struct ft5x06_ts_data *data = container_of(work, struct ft5x06_ts_data, backlight_delay_work);

	if (!data)
		return;
	if (backlight_delay_time_msecs && backlight_level)
		backlight_change_handler(data);
}
#endif
...
#ifdef CONFIG_BBRY /* No need to error out in this case */
	if ((data->init_complete & FT_GESTURE_ID_SET_MASK) == 0) {
		data->init_complete |= FT_GESTURE_ID_SET_MASK;
		queue_work(ft5x06_wq, &data->power_state_work);
	}
	dev_info(&data->client->dev, "gesture_id = %d, init_complete = 0x%08x \n",
		data->gesture_id, data->init_complete);
#else
	printk("gesture_id = %d \n", data->gesture_id);
#endif
...
/*[FEATURE]-Modified-BEGIN by TCTSH.xingchen.wang for defect 1305291, 2016/02/18,set func-register after TP FW_update*/
	tp_function_set_confirm(data);
...
/*[FEATURE]-Modified-BEGIN by TCTSH.xingchen.wang for defect 1305291, 2016/02/18,modify TP debug log*/
				...
				const char *buf, size_t size)
{
	...
	ret = kstrtoul(buf, 10, &val);
	if(ret){
		printk("[TP-debug]kstrtoul failed, ret = %zd\n", ret);
		return size;
		}
	data->touch_log_switch = val;
	printk("[TP-debug]%s, TP debug log, show function set:\n", __func__);

#if defined(LEATHER_COVER)
	ret = ft5x0x_read_reg(data->client, 0xC1, &value);
	printk("[TP-debug]cover register:%d, flag set:%d\n", value, data->cover_on);
#endif

#if defined(VR_GLASS)
		ret = ft5x0x_read_reg(data->client, 0xc1, &value);
		printk("[TP-debug]vr register:%d, flag set:%d\n", value, data->vr_on);
#endif

#if defined(CONFIG_TOUCHPANEL_PROXIMITY_SENSOR)
		ret = ft5x0x_read_reg(data->client, 0xC1, &value);
		printk("[TP-debug]proximity register:%d, flag set:%d\n", value, vps_ft5436->vps_enabled);
#endif

#if defined(FOCALTECH_TP_GLOVE)
			ret = ft5x0x_read_reg(data->client, 0xC0, &value);
			printk("[TP-debug]glove register:%d, flag set:%d\n", value, data->glove_id);
#endif
...
/*[Feature]-Add-END by xingchen.wang, task 1305291, 2016/02/18 */
				...
				const char *buf, size_t size)
{
	...
	ft5x06_i2c_read(data->client, &txbuf[0], 1, &val_read, 1);
	printk("set cover = %d\n", val_read);
...
/*[PLATFORM]-Add-BEGIN by TCTSH.xuefei.wang, Task-2459640, 2016/07/01, add TP TYPE */
         rc=class_create_file(tp_device_class,&class_attr_tp_type);
         if ( rc < 0)
		pr_err("Failed to create device file(%s)!\n", class_attr_tp_type.attr.name);
/*[PLATFORM]-Add-END by TCTSH.xuefei.wang, Task-2459640, 2016/07/01, add TP TYPE */
...
*/
/*[FEATURE]-Modified-BEGIN by TCTSH.xingchen.wang for task 1356189, 2016/01/18, read register value for glove-func*/
...
#ifdef CONFIG_BBRY /* No need to error out in this case */
	if ((data->init_complete & FT_GLOVE_ID_SET_MASK) == 0) {
		data->init_complete |= FT_GLOVE_ID_SET_MASK;
		queue_work(ft5x06_wq, &data->power_state_work);
	}
	dev_info(&data->client->dev, "glove_id = %d, init_complete = 0x%08x \n",
		data->glove_id, data->init_complete);
#endif
	return size;
}
...
/*[FEATURE]-Modified-END by TCTSH.xingchen.wang for task 1238223, 2015/12/25*/
	...
	txbuf[1] = 0x00;//disable tp glove when power on the phone
	ft5x06_i2c_write(data->client, txbuf, sizeof(txbuf));
	ft5x06_i2c_read(data->client, &txbuf[0], 1, &val_read, 1);
	printk("[wxc]~~~~~ %s set 0xC0 to %d\n", __func__, val_read);
...
static void tp_prox_sensor_enable(struct i2c_client *client, int enable)
{
    ...
    if(client == NULL)
		return;
	printk("======[wxc]===== %s, enable %d\n", __func__, enable);
	if (gpio_is_valid(g_ft5x06_ts_data->pdata->reset_gpio)) {
/*[PLATFORM]-Add-BEGIN by TCTSH.xuefei.wang, Defect-2226578, 2016/06/02, The gpio_set_value_cansleep function does not work .*/ 
		//gpio_set_value_cansleep(g_ft5x06_ts_data->pdata->reset_gpio, 0);
                gpio_direction_output(g_ft5x06_ts_data->pdata->reset_gpio,0);
		printk("reset tp ~~~ \n");
		msleep(g_ft5x06_ts_data->pdata->hard_rst_dly);
		//gpio_set_value_cansleep(g_ft5x06_ts_data->pdata->reset_gpio, 1);
               gpio_direction_output(g_ft5x06_ts_data->pdata->reset_gpio,1);
/*[PLATFORM]-Add-END by TCTSH.xuefei.wang, Defect-2226578, 2016/06/02, The gpio_set_value_cansleep function does not work .*/ 
	}

	msleep(g_ft5x06_ts_data->pdata->soft_rst_dly);
/*[FEATURE]-Modified-BEGIN by TCTSH.xingchen.wang for task 1356189, 2016/01/18, set func-register after TP reset*/
	tp_function_set_confirm(g_ft5x06_ts_data);
/*[FEATURE]-Modified-END by TCTSH.xingchen.wang for task 1356189, 2016/01/18*/
	if (enable){
		state = 0x01;
	}else{
...
/*[FEATURE]-Modified-END by TCTSH.xingchen.wang for task 1238223, 2015/12/25*/
{
	...
	printk("FT proxi_fts 0xB0 state value is 0x%02X\n", reg_value);
	if(enable == 1)
	{
		if(!(reg_value&0x01))
		{
		...
		ft5x0x_read_reg(g_ft5x06_ts_data->client, 0xB0, &reg_value);
		printk("[wxc] proxi_fts 0xB0 state value is0x%02X, enable %d\n", reg_value, enable);
...
/*[FEATURE]-Modified-BEGIN by TCTSH.xingchen.wang for task 1356189, 2016/01/18, modify virtual P-sensor func*/
	}else if (enable == 0)
	{
		tp_prox_sensor_enable(g_ft5x06_ts_data->client, 0);
...
ssize_t ft_virtual_proximity_enable_show(struct device *pDevice, struct device_attribute *pAttr, char *pBuf)
{
	...
	ft5x0x_read_reg(g_ft5x06_ts_data->client, 0xB0, &reg_value);
	return sprintf(pBuf, "vps_enabled %d, 0xB0 state value is 0x%02X\n", vps_ft5436->vps_enabled, reg_value);
}
ssize_t ft_virtual_proximity_enable_store(struct device *pDevice, struct device_attribute *pAttr, const char *pBuf, size_t nSize)
{
	...
	if (pBuf != NULL)
	{
		...
		printk("======[wxc]===== %s, gesture_id %d\n", __func__, g_ft5x06_ts_data->gesture_id);
 ...
 void ft5x06_enable_vr(void)
{
	...
	queue_work(ft5x06_wq_vr, &data->work_vr);
}
EXPORT_SYMBOL(ft5x06_enable_vr);
void ft5x06_disable_vr(void)
{
	struct ft5x06_ts_data *data;
	//printk(KERN_ERR"[TPD]%s, %d\n", __FUNCTION__, __LINE__);
	if(!ft_g_client)
...
EXPORT_SYMBOL(ft5x06_disable_vr);
void ft5x06_change_vr_switch(struct work_struct *work)
{
	u8 vr_flag = 0;
	struct ft5x06_ts_data *data;
...
static int ft_tp_interrupt(struct ft5x06_ts_data *data)
{
	...
	if( (data->gesture_id > 0) && (0x01 == data->gesture_set) )
	{
		...
		"%s: possible wakeup gesture detected, gesure_id=%d, suspend=%d\n",
		__func__, data->gesture_id, data->suspended);
#endif
		...
		if(GESTURE_DB == reg_value)
		{
			if ( 0x01 == data->gesture_id) {
				...
				input_sync(data->input_dev);
#ifdef CONFIG_BBRY
				dev_info(&data->client->dev, "%s:gesture KEY_UNLOCK\n", __func__);
				data->mtouch_counter.double_tap++;
#else
			...
			if ( 0x02 == data->gesture_id) {
				...
				input_sync(data->input_dev);
#ifdef CONFIG_BBRY
				dev_info(&data->client->dev, "%s:gesture KEY_POWER\n", __func__);
#else
				printk("[Fu]gesture KEY_POWER\n");
#endif
			}
#ifdef CONFIG_BBRY
			dtwakeup_time_ms = get_timestamp();
			dtwakeup_time_ms_store = dtwakeup_time_ms;
#endif
		} else {
...
#ifdef CONFIG_BBRY
	data->stats.total_int_cnt++;
	data->stats.total_int_served_cnt++;
#endif
#if defined(FOCALTECH_TP_GESTURE)
	if( (data->gesture_id > 0) && (0x01 == data->gesture_set) ) {
#ifdef CONFIG_BBRY
		rc = ft_tp_interrupt(data);
		if (rc >= 0)
			data->mtouch_counter.successful_double_tap++;
#else
...
//[BUGFIX]-Add-Begin by TCTSH xingchen.wang, defect 1546333 , tp still report points during TP-suspend period, 2016/02/19
	if (backlight_level == 0) {
		pr_err("%s: backlight_level = %d, no pointers reported\n", __func__, backlight_level);
#ifdef CONFG_BBRY
		dev_err("%s: gesture_id = %d, gesture_set=%d\n",
			__func__, data->gesture_id, data->gesture_set);
#endif
		return IRQ_HANDLED;
	}
//[BUGFIX]-Add-End by TCTSH xingchen.wang, defect 1546333 ,  2016/02/19
...
#ifdef CONFIG_BBRY
	data->stats.touch_int_cnt++;
	if (num_touches < data->finger_cnt) {
		data->stats.touch_frelease_cnt += data->finger_cnt - num_touches;
		if (!num_touches && data->finger_cnt)
			data->stats.touch_trelease_cnt++;
	}
	data->finger_cnt = num_touches;
	data->mtouch_counter.finger_cnt = data->finger_cnt;
#endif
	if (update_input) {
...
#ifdef CONFIG_BBRY
	if (level && dtwakeup_time_ms) {
		backlight_delay_time_msecs = get_timestamp();
		backlight_delay_time_msecs -= dtwakeup_time_ms;
		queue_work(backlight_workqueue, &g_ft5x06_ts_data->backlight_delay_work);
		dtwakeup_time_ms = 0;
	}
#endif
}
...
static int ft_tp_resume(struct ft5x06_ts_data *data)
{
	...
	if ( data->gesture_id > 0)
	{
		...
		//msleep(100);
        msleep(20);//TCTSH ZhaiQingbo optimize time from 100 to 20ms for defect 1707329, 2013.3.15
		return 0 ;
	}
...
#if defined(CONFIG_FB)
#ifdef CONFIG_BBRY
static void ft5x06_power_state_work(struct work_struct *work)
{
	struct ft5x06_ts_data *ts =
		container_of(work, struct ft5x06_ts_data, power_state_work);
	unsigned char	fb_blank = 0;
	unsigned char	fb_event = 0;
	unsigned long total_dtwakeup_time_msecs;

	fb_blank = ts->fb_blank;
	fb_event = ts->fb_event;

	dev_info(&ts->client->dev, "%s: init_complete=0x%08x, gesture_id=%d, fb_blank=%d, fb_event=%d\n",
		__func__, ts->init_complete, ts->gesture_id, fb_blank, fb_event);

	if ((ts->init_complete == FT_INIT_COMPLETE_MASK) && (fb_event == FB_EVENT_BLANK)) {
		if (fb_blank == FB_BLANK_UNBLANK)
			ft5x06_ts_resume(&ts->client->dev);
		else if (fb_blank == FB_BLANK_POWERDOWN)
			ft5x06_ts_suspend(&ts->client->dev);
	}
	if (dtwakeup_time_ms_store) {
		total_dtwakeup_time_msecs = ts->dtwakeup_complete_time_ms - dtwakeup_time_ms_store;
		if (total_dtwakeup_time_msecs < 200) {
			//nothing
		}
		else if (total_dtwakeup_time_msecs < 400) {
			ts->mtouch_counter.dtwakeup_delay_200ms++;
		}
		else if (total_dtwakeup_time_msecs < 1000) {
			ts->mtouch_counter.dtwakeup_delay_400ms++;
		}
		else if (total_dtwakeup_time_msecs < 2000) {
			ts->mtouch_counter.dtwakeup_delay_1sec++;
		} else {
			ts->mtouch_counter.dtwakeup_delay_2sec++;
		}
		dtwakeup_time_ms_store = 0;
	}
	return;
}
				 ...
				 unsigned long event, void *data)
{
	...
	int fb_blank;
	struct ft5x06_ts_data *ft5x06_data = NULL;

	if ((self == NULL) || (evdata == NULL) || (evdata->data  == NULL))
		return 0;
	...
	dev_info(&ft5x06_data->client->dev, "%s: event=%lu, fb_blank=%d\n",
		__func__, event, fb_blank);
	if (!(FB_BLANK_NORMAL == fb_blank)) {
		flush_workqueue(ft5x06_wq);
		ft5x06_data->fb_blank = fb_blank;
		ft5x06_data->fb_event = event;
		ft5x06_data->dtwakeup_complete_time_ms = get_timestamp();
		queue_work(ft5x06_wq, &ft5x06_data->power_state_work);
	}
        return 0;
}
#else //CONFIG_BBRY
...
#endif
//[BUGFIX]-Add-Begin by TCTSH WXF, task 2545632 ,2016/07/18
        if(!booting_into_recovery) {
		if (data->fw_vendor_id == 0x57) {
			rc = request_firmware(&fw, data->fw_name, dev);
		} else if (data->fw_vendor_id == 0x3B) {
			strcpy(data->fw_name, "ft_fw_Biel.bin");
			rc = request_firmware(&fw, data->fw_name, dev);//furong modify, 2015.07.01, for ChaoSheng ft5436, pr1030423
		} else {
			printk("[Fu]unknown fw_vendor_id error");
#ifdef CONFIG_BBRY
...
//	rc = request_firmware(&fw, data->fw_name, dev);
//[BUGFIX]-Add-Begin by TCTSH xingchen.wang, task 2545632 ,2016/07/18
//[BUGFIX]-Add-Begin by TCTSH xingchen.wang, 2015/12/16
	...
	if (force)
		fw_upgrade = true;
//[BUGFIX]-Add-Begin by TCTSH xingchen.wang, task 1305291 ,2016/01/07
	else if (fw_file_maj == 18 && data->fw_ver[0] != 18){
		fw_upgrade = true;
		printk("\n[wxc]roll back firmware to v12 for temp\n");
	}
//[BUGFIX]-Add-End by TCTSH xingchen.wang, 2016/01/07
	...
	if (!fw_upgrade) {
#ifdef CONFIG_BBRY
		dev_info(dev, "No fw upgrade required. Exiting fw upgrade...\n");
		rc = 0;
#else
		...
		const char *buf, size_t count)
{
	...
	struct ft5x06_ts_data *data = g_ft5x06_ts_data;
...
/*[FEATURE]-Modified-BEGIN by TCTSH.xingchen.wang for task 1356189, 2016/01/18, modify rawdata func*/
//wxc add for resolved chaos jumping point after rawdata test
	disable_irq(data->client->irq);
			if (gpio_is_valid(data->pdata->reset_gpio)) {
/*[PLATFORM]-Add-BEGIN by TCTSH.xuefei.wang, Defect-2226578, 2016/06/02, The gpio_set_value_cansleep function does not work .*/ 
		//gpio_set_value_cansleep(data->pdata->reset_gpio, 0);
...
/*[FEATURE]-Modified-BEGIN by TCTSH.xingchen.wang for task 1356189, 2016/01/18, set func-register after TP reset*/
	tp_function_set_confirm(data);
/*[FEATURE]-Modified-END by TCTSH.xingchen.wang for task 1356189, 2016/01/18*/
	printk("=====wxc==== add for resolved chaos jumping point\n");
...
/*[PLATFORM]-Add-BEGIN by TCTSH.xuefei.wang, Task-2459640, 2016/07/01, add TP TYPE */
        rc = of_property_read_string(np, "focaltech,name", &ft_tp_name);
        if (rc && (rc != -EINVAL)) {
		dev_err(dev, "Unable to read name\n");
		return rc;
	}
...
static void ft_init_update_work(struct work_struct *work)
{
	...
	ret = ft5x06_fw_upgrade(dev, true);// xingchen.wang add for forced upgrading of tp firmware in MINI software
#else
	ret = ft5x06_fw_upgrade(dev, false);
#endif
	if (ret < 0) {
		ts->mtouch_counter.upgrade_failure++;
	}
#else

#ifdef MINI_MODE_TO_CTP
	ft5x06_fw_upgrade(dev, true);// xingchen.wang add for forced upgrading of tp firmware in MINI software
#else
	ft5x06_fw_upgrade(dev, false);
#endif
#endif
...
#ifdef CONFIG_BBRY /* No need to error out in this case */
	ts->init_complete |= FT_FWUPG_COMPLETE_MASK;
	queue_work(ft5x06_wq, &ts->power_state_work);
#endif
}
...
extern struct ft5x06_rawdata_test_result ft5436_rawdata_test_result;
extern int rawdata_fail_num;
extern int rawdata_keyarea_fail_num;
...
/*[FEATURE]-Modified-BEGIN by TCTSH.xingchen.wang for task 1356189, 2016/01/18, modify rawdata interface*/
	if (rawdata_keyarea_fail_num == 0){
		s += sprintf(s,"PASS\n");
	} else {
		s += sprintf(s,"NG\n");
		for (i = 0; i < rawdata_keyarea_fail_num; i ++){
			if (ft5436_rawdata_test_result.key_index[i][1] < 25){
				s += sprintf(s,"X%dY%d(%u)\n", ft5436_rawdata_test_result.key_index[i][0],
							ft5436_rawdata_test_result.key_index[i][1],
							ft5436_rawdata_test_result.key_index[i][2]);
			}
		}
	}
	s += sprintf(s,"Spec Min: %u  Max: %u\n", ft5436_rawdata_test_result.min_key_limited_value, ft5436_rawdata_test_result.max_key_limited_value);
/*[FEATURE]-Modified-END by TCTSH.xingchen.wang for task 1356189, 2016/01/18*/
	s += sprintf(s,"**************************\n");
...
//glove function
	txbuf[0] = FOCALTECH_TP_GLOVE_SET;
	txbuf[1] = data->glove_id;//set tp glove
	ft5x06_i2c_write(data->client, txbuf, sizeof(txbuf));
	ft5x06_i2c_read(data->client, &txbuf[0], 1, &val_read, 1);
	printk("[wxc]~~~~~ %s set 0xC0 to %d\n", __func__, val_read);
}
...
//			goto exit_power_off;
...
#ifdef CONFIG_BBRY
	data->finger_cnt = 0;
	mtouch_counter_init(data);
	stats_init(data);
	backlight_workqueue = create_singlethread_workqueue("backlight_workqueue");
	INIT_WORK(&data->backlight_delay_work, backlight_delay_work);
	...
	data->fb_blank = 0;
	data->fb_event = 0;
	data->init_complete = FT_INIT_MASK;
	INIT_WORK(&data->power_state_work,
			ft5x06_power_state_work);
#endif
#elif defined(CONFIG_HAS_EARLYSUSPEND)
...
#endif
#ifdef CONFIG_BBRY
	/*Needs to be after the tp_device_class is created*/
	if (tp_device_class) {
		tp_stats_dev = device_create(tp_device_class, NULL, 0, tp_stats_dev, "tp_stats");
		if (IS_ERR(tp_stats_dev)) {
			pr_err("Failed to create device(tp_stats)!\n");
			goto no_mtouch_counter;
		}
		err = device_create_file(tp_stats_dev, &dev_attr_mtouch_counter);
		if (err < 0) {
			pr_err("Failed to create device file(%s)!\n", dev_attr_mtouch_counter.attr.name);
			device_remove_file(tp_stats_dev, &dev_attr_mtouch_counter);
		}
		err = device_create_file(tp_stats_dev, &dev_attr_stats);
		if (err < 0) {
			pr_err("Failed to create device file(%s)!\n", dev_attr_stats.attr.name);
			device_remove_file(tp_stats_dev, &dev_attr_stats);
		}
		dev_set_drvdata(tp_stats_dev, data);
	}
no_mtouch_counter:
#endif
//[PLATFORM] Add by wangxingchen 12/22/2014 PR.874996 Idol 3 5.5 TP Glove Function Development.
...
/*[FEATURE]-Modified-BEGIN by TCTSH.xingchen.wang for task 1208391, 2016/01/06, new rawdata interface*/
	{
		...
		static struct kobject *ft5x06_CTP_properties_kobj;
		int rc = 0;
...
//create CPT sys node
	        ft5x06_CTP_properties_kobj =
	                        kobject_create_and_add("CTP", NULL);

	        if (ft5x06_CTP_properties_kobj)
	                rc = sysfs_create_group(ft5x06_CTP_properties_kobj,
	                                &ft5x06_rawdata_properties_attr_group);

	        if (!ft5x06_CTP_properties_kobj || rc)
	                pr_err("%s: failed to create rawdata\n", __func__);
	}
/*[FEATURE]-Modified-END by TCTSH.xingchen.wang for task 1208391, 2016/01/06*/
#endif
...
/*[FEATURE]-Modified-BEGIN by TCTSH.xingchen.wang for defect 1305291, 2016/02/18,set func-register in probe*/
	tp_function_set_confirm(data);
/*[FEATURE]-Modified-BEGIN by TCTSH.xingchen.wang for defect 1305291, 2016/02/18*/
...
/*[FEATURE]-Modified-BEGIN by TCTSH.xingchen.wang for task 1238223, 2015/12/25, add vr function*/
#if defined(VR_GLASS)
	ft5x06_wq_vr = create_singlethread_workqueue("ft5x06_wq_vr");
	if (!ft5x06_wq_vr)
	{
		printk("Creat ft5x06_wq_vr workqueue failed. \n");
		return -ENOMEM;
	}
#endif
/*[FEATURE]-Modified-END by TCTSH.xingchen.wang for task 1238223, 2015/12/25*/
