...
/*[FEATURE]-Modified-BEGIN by TCTSH.xingchen.wang for task 1208391, 2016/01/06, new rawdata interface*/
struct ft5x06_rawdata_test_result ft5436_rawdata_test_result;
int rawdata_fail_num = 0;
int rawdata_keyarea_fail_num = 0;
/*[FEATURE]-Modified-END by TCTSH.xingchen.wang for task 1208391, 2016/01/06*/
...
boolean FT5X46_TestItem(void)
{
	...
	printk("[focal] %s result = %d  \n", __func__, bTestReuslt);
	printk("[focal] rawdata_fail_num = %d\n", rawdata_fail_num);
	printk("[focal] rawdata_keyarea_fail_num = %d\n", rawdata_keyarea_fail_num);
	focal_msleep(1000);
	//bTestReuslt = bTestReuslt & TestItem_WeakShortTest();
...
boolean TestItem_RawDataTest_FT5X46(void)
{
	...
	if(g_TestParam_FT5X46.RawDataTest_HighFreq== 1)
	{
		...
		ft5436_rawdata_test_result.min_value = rawdata[0][0];
...
/*[FEATURE]-Modified-BEGIN by TCTSH.xingchen.wang for task 1356189, 2016/01/18, modify rawdata interface*/
				if( j < (iRxNum - 1 )){
					ft5436_rawdata_test_result.min_limited_value = min_value;
					ft5436_rawdata_test_result.max_limited_value = max_value;

					ft5436_rawdata_test_result.max_value = rawdata[i][j] > ft5436_rawdata_test_result.max_value ? rawdata[i][j] : ft5436_rawdata_test_result.max_value;
					ft5436_rawdata_test_result.min_value = rawdata[i][j] < ft5436_rawdata_test_result.min_value ? rawdata[i][j] : ft5436_rawdata_test_result.min_value;
					if(rawdata[i][j] < min_value || rawdata[i][j] > max_value){
						bTestReuslt  = false;
						FTS_DBG("rawdata test failure. min_value=%d max_value=%d rawdata[%d][%d]=%d \n", \
							min_value, max_value, i+1, j+1, rawdata[i][j]);

						ft5436_rawdata_test_result.index[rawdata_fail_num][0] = i+1;
						ft5436_rawdata_test_result.index[rawdata_fail_num][1] = j+1;
						ft5436_rawdata_test_result.index[rawdata_fail_num][2] = rawdata[i][j];

						printk("[wxc]AA zone, x:%d, y:%d, value:%d, NO.%d\n", ft5436_rawdata_test_result.index[rawdata_fail_num][0],
														ft5436_rawdata_test_result.index[rawdata_fail_num][1],
														ft5436_rawdata_test_result.index[rawdata_fail_num][2],
														rawdata_fail_num);
						rawdata_fail_num++;
					}
				} else if (j == (iRxNum - 1)) {
					ft5436_rawdata_test_result.min_key_limited_value = min_value;
					ft5436_rawdata_test_result.max_key_limited_value = max_value;

					ft5436_rawdata_test_result.max_key_value = rawdata[i][j] > ft5436_rawdata_test_result.max_value ? rawdata[i][j] : ft5436_rawdata_test_result.max_value;
					ft5436_rawdata_test_result.min_key_value = rawdata[i][j] < ft5436_rawdata_test_result.min_value ? rawdata[i][j] : ft5436_rawdata_test_result.min_value;
					if(rawdata[i][j] < min_value || rawdata[i][j] > max_value){
						bTestReuslt  = false;
						FTS_DBG("rawdata test failure. min_value=%d max_value=%d rawdata[%d][%d]=%d \n", \
							min_value, max_value, i+1, j+1, rawdata[i][j]);

						ft5436_rawdata_test_result.key_index[rawdata_keyarea_fail_num][0] = i+1;
						ft5436_rawdata_test_result.key_index[rawdata_keyarea_fail_num][1] = j+1;
						ft5436_rawdata_test_result.key_index[rawdata_keyarea_fail_num][2] = rawdata[i][j];

						printk("[wxc]Key zone, x:%d, y:%d, value:%d, NO.%d\n", ft5436_rawdata_test_result.key_index[rawdata_keyarea_fail_num][0],
														ft5436_rawdata_test_result.key_index[rawdata_keyarea_fail_num][1],
														ft5436_rawdata_test_result.key_index[rawdata_keyarea_fail_num][2],
														rawdata_keyarea_fail_num);
						rawdata_keyarea_fail_num++;
					}
				}
