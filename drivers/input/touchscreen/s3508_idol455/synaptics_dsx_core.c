...
//[BUGFIX]-Add-BEGIN by TCTSH xingchen.wang for task: 1052817, optimize "Freeze situation" for S3508. 2015/12/14
#define FB_READY_RESET
#define FB_READY_WAIT_MS 100
#define FB_READY_TIMEOUT_S 30
//[BUGFIX]-Add-END by TCTSH xingchen.wang for task: 1052817. 2015/12/14
#define RPT_TYPE (1 << 0)
...
/*[Feature]-Add-BEGIN by xingchen.wang, task 526254, 2015/11/05, add read FW version node*/
...
/*[Feature]-Add-BEGIN by xingchen.wang, task 526254, 2015/11/09, add tp_gesture_id sys node*/
static DEVICE_ATTR(tp_gesture_id, 0644, synaptics_rmi4_wake_gesture_show, synaptics_rmi4_wake_gesture_store);
/*[Feature]-Add-END by xingchen.wang,  2015/11/09*/
...
/*[Feature]-Add-END by xingchen.wang,  2015/11/05*/
static ssize_t synaptics_rmi4_f01_reset_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	int retval;
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	if (rmi4_data->suspend && rmi4_data->enable_wakeup_gesture) {
		...
		if (retval < 0)
			return 0;
/*[Feature]-Add-BEGIN by xingchen.wang, task 526254, 2015/11/09, change tp_gesture event*/
...
static int synaptics_rmi4_set_gpio(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (bdata->reset_gpio >= 0) {
//[BUGFIX]-Add-BEGIN by TCTSH xingchen.wang for task: 1052817  , check TP reset pin in during TP probe period. 2015/12/14
		printk("[wxc]reset_gpio:%d, bdata->reset_on_state:%d\n", bdata->reset_gpio, bdata->reset_on_state);
		printk("[wxc]reset_gpio:%d, !bdata->reset_on_state:%d\n", bdata->reset_gpio, !bdata->reset_on_state);
//[BUGFIX]-Add-END by TCTSH xingchen.wang for task: 1052817. 2015/12/14
		...
		bool rebuild)
{
	int retval;
	struct synaptics_rmi4_exp_fhandler *exp_fhandler;
	printk("[wxc] synaptics_rmi4_reset_device+\n");
	if (rebuild) {
...
static int synaptics_rmi4_resume(struct device *dev)
{
	...
	printk("[wxc] %s+\n", __func__);
	printk("[wxc] rmi4_data->suspend:%d\n", rmi4_data->suspend);
