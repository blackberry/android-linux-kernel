	...
	while (remaining_msgs) {
		...
		if (retry == SYN_I2C_RETRY_TIMES) {
			dev_err(rmi4_data->pdev->dev.parent,
					...
					__func__);
//[BUGFIX]-Add-BEGIN by TCTSH xingchen.wang for task: 1052817  , use TP reset pin 954 when there is "freeze" situation. 2015/12/14
			printk("[wxc]reset_gpio 954 +\n");
			gpio_set_value(954, 0);
			msleep(20);
