...
//[BUGFIX]-Add-BEGIN by TCTSH.xingchen.wang for task:982245 , update firmware for S3508. 2015/11/26
//#define FW_IMAGE_NAME "Synaptics_fw_update.img"
#define FW_S3508_IMAGE_NAME "Synaptics_S3508_fw_update.img"
#define FW_S3708_IMAGE_NAME "Synaptics_S3708_fw_update.img"
//[BUGFIX]-Add-End by TCTSH.xingchen.wang. 2015/11/26
//[BUGFIX]-Add-BEGIN by xingchen.wang for task:526254 , update firmware for S3708. 2015/11/04
...
#endif
#endif
#define NAME_S3508 "s3508"
#define NAME_S3708 "S3708"
//[BUGFIX]-Add-End by xingchen.wang. 2015/11/04
//[BUGFIX]-Add-BEGIN by TCTSH xingchen.wang for task:982245 , update firmware for S3508. 2015/11/28
#define S3508_ORIGINAL_FW_ID 1708542
#define S3508_BAD_FW_ID 2046117
//[BUGFIX]-Add-BEGIN by TCTSH xingchen.wang for task:982245 , update firmware for S3508. 2015/12/01
#define S3508_BAD_FW_ID2 2110785
//[BUGFIX]-Add-End by TCTSH xingchen.wang. 2015/12/01
//[BUGFIX]-Add-End by TCTSH xingchen.wang. 2015/11/28
...
static int fwu_get_device_config_id(void)
{
	...
	struct synaptics_rmi4_data *rmi4_data = fwu->rmi4_data;
//[BUGFIX]-Add-BEGIN by TCTSH xingchen.wang for task:982245 , update firmware for S3508. 2015/11/28
	printk("[stars]%s fwu->bl_version:%d\n", __func__, fwu->bl_version);
//[BUGFIX]-Add-End by TCTSH xingchen.wang. 2015/11/28
	...
	retval = synaptics_rmi4_reg_read(rmi4_data,
				...
				config_id_size);
//[BUGFIX]-Add-BEGIN by TCTSH xingchen.wang for task:982245 , update firmware for S3508. 2015/11/28
	printk("[stars]%s fwu->config_id:%s\n", __func__, fwu->config_id);
//[BUGFIX]-Add-End by TCTSH xingchen.wang. 2015/11/28
...
//TCTSH xingchen.wang add begin, when need enforce update for s3508 of firmware 1708542. 2015/11/25
//[BUGFIX]-Add-BEGIN by TCTSH xingchen.wang for task:982245 , update firmware for S3508. 2015/12/01
	if ( image_fw_id == (unsigned int)(S3508_ORIGINAL_FW_ID)
		 && (device_fw_id == (unsigned int)(S3508_BAD_FW_ID)
		 ||  device_fw_id == (unsigned int)(S3508_BAD_FW_ID2))){
		printk("[stars]%s Because image_fw_id = %d and device_fw_id = %d, start enforce roll back update\n", __func__, image_fw_id, device_fw_id);
//[BUGFIX]-Add-End by TCTSH xingchen.wang. 2015/12/01
		flash_area = UI_FIRMWARE;
		goto exit;
	}
...
#endif
//TCTSH xingchen.wang add end. 2015/11/25
...
static int fwu_erase_configuration(void)
{
	...
	struct synaptics_rmi4_data *rmi4_data = fwu->rmi4_data;
//[BUGFIX]-Add-BEGIN by TCTSH xingchen.wang for task:982245 , update firmware for S3508. 2015/11/28
	printk("[stars]%s fwu->config_area:%d\n", __func__, fwu->config_area);
//[BUGFIX]-Add-End by TCTSH xingchen.wang. 2015/11/28
...
//[BUGFIX]-Add-BEGIN by TCTSH.xingchen.wang for task:982245 , update firmware for S3508. 2015/11/26
static int synaptics_check_IC_name(struct synaptics_rmi4_data *rmi4_data)
{
	int retval = 0;
	unsigned char id_addr3508 = 0x0035;
	unsigned char id_addr3708 = 0x0042;
//check whether product is S3508.
	retval = synaptics_rmi4_reg_read(rmi4_data, id_addr3508, rmi4_data->IC_name, MAX_IC_NAME_LEN);
	rmi4_data->IC_name[5] = '\0';
//[BUGFIX]-Add-BEGIN by TCTSH xingchen.wang for task:982245 , update firmware for S3508. 2015/11/28
	printk("[stars]%s : TP IC is %s\n", __func__, rmi4_data->IC_name);
//[BUGFIX]-Add-End by TCTSH.xingchen.wang. 2015/11/28
	if (strcmp(rmi4_data->IC_name, NAME_S3508) == 0){
		return 1;//s3508
	}
//check whether product is S3708.
	retval = synaptics_rmi4_reg_read(rmi4_data, id_addr3708, rmi4_data->IC_name, MAX_IC_NAME_LEN);
	rmi4_data->IC_name[5] = '\0';
//[BUGFIX]-Add-BEGIN by TCTSH xingchen.wang for task:982245 , update firmware for S3508. 2015/11/28
	printk("[stars]%s : TP IC is %s\n", __func__, rmi4_data->IC_name);
//[BUGFIX]-Add-End by TCTSH.xingchen.wang. 2015/11/28
	if (strcmp(rmi4_data->IC_name, NAME_S3708) == 0){
		return 2;//s3708
	}
		printk("[stars]%s : TP IC is unrecognized.\n", __func__);
		return 0;//others, including firmware damaged situation
}
//[BUGFIX]-Add-End by TCTSH.xingchen.wang. 2015/11/26
static int fwu_start_reflash(void)
{
	...
	struct synaptics_rmi4_data *rmi4_data = fwu->rmi4_data;
//[BUGFIX]-Add-BEGIN by TCTSH.xingchen.wang for task:982245 , update firmware for S3508. 2015/11/26
	retval = synaptics_check_IC_name(rmi4_data);
//[BUGFIX]-Add-BEGIN by TCTSH xingchen.wang for task:982245 , update firmware for S3508. 2015/11/28
//	if (retval != 1 && retval != 2) {
//		printk("[stars]%s: Quit reflash\n", __func__);
//		return -ENODEV;
//	}
//[BUGFIX]-Add-End by TCTSH.xingchen.wang. 2015/11/28
//[BUGFIX]-Add-End by TCTSH.xingchen.wang. 2015/11/26
...
//[BUGFIX]-Add-BEGIN by TCTSH.xingchen.wang for task:982245 , update firmware for S3508. 2015/11/26
	if (fwu->image == NULL) {
//[BUGFIX]-Add-BEGIN by TCTSH xingchen.wang for task:982245 , update firmware for S3508. 2015/11/28
		if (retval == 1 || retval == 0){	//for temp. used for s3508 damaged-firmware update.
//[BUGFIX]-Add-End by TCTSH.xingchen.wang. 2015/11/28
			retval = secure_memcpy(fwu->image_name, MAX_IMAGE_NAME_LEN,
					FW_S3508_IMAGE_NAME, sizeof(FW_S3508_IMAGE_NAME),
					sizeof(FW_S3508_IMAGE_NAME));
		} else if (retval == 2){
			retval = secure_memcpy(fwu->image_name, MAX_IMAGE_NAME_LEN,
			FW_S3708_IMAGE_NAME, sizeof(FW_S3708_IMAGE_NAME),
			sizeof(FW_S3708_IMAGE_NAME));
		}
		if (retval < 0) {
			dev_err(rmi4_data->pdev->dev.parent,
		...
		printk("[stars]%s : idol4_fw_name : %s\n", __func__, fwu->image_name);
//[BUGFIX]-Add-End by TCTSH.xingchen.wang. 2015/11/26
		dev_dbg(rmi4_data->pdev->dev.parent,
...
//wxc
	printk("[stars]%s : input = %d\n", __func__, input);

	if (fwu->in_ub_mode) {
		printk("[stars]%s : fwu->in_ub_mode\n", __func__);
		dev_err(rmi4_data->pdev->dev.parent,
				"%s: In microbootloader mode\n",
				__func__);
		retval = -EINVAL;
	...
	if (!fwu->ext_data_source){
		printk("[stars]%s : !fwu->ext_data_source\n", __func__);
		return -EINVAL;
	}else
		fwu->image = fwu->ext_data_source;

	if (input & LOCKDOWN) {
		printk("[stars]%s : input & LOCKDOWN : %d\n", __func__, input & LOCKDOWN);
