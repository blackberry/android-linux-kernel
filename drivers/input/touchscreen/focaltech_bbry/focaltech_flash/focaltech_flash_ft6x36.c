...
*******************************************************************************/
#define FOCALTECH_FLASH_FT6X36_INFO  "File Version of  focaltech_flash_ft6x36.c:  V1.0.0 2016-03-21"

#define FTS_REG_FT6X36_VENDOR_ID 		0xA8
...
*******************************************************************************/
static int fts_ic_program_ft6x36_upgrade( u8 *pbt_buf, u32 dw_lenth);
static int fts_ft6x36_get_i_file(u8 **file_buff, int *file_len);
...
/************************************************************************
*   Name: fts_ic_program_ft6x36_upgrade
* Brief:  fw upgrade
...
static int fts_ic_program_ft6x36_upgrade( u8 *pbt_buf, u32 dw_lenth)
{
	...
	fts_i2c_write( auc_i2c_write_buf, 5);
...
static int fts_ft6x36_get_i_file(u8 **file_buff, int *file_len)
{
	...
	{
		i_ret = fts_read_reg(FTS_REG_FT6X36_VENDOR_ID, &uc_vender_id);
		if(i_ret < 0)
		{
...
int fts_ft6x36_upgrade_with_i_file(void)
{
	u8 *pbt_buf = NULL;
	int i_ret=0;
	int fw_len = 0;
	...
	/*get i file*/
	i_ret = fts_ft6x36_get_i_file(&pbt_buf, &fw_len);
	if(i_ret < 0)
	{
		return -1;
	}
	...
	i_ret = fts_ic_program_ft6x36_upgrade( pbt_buf, fw_len);
	
	return i_ret;
}
int fts_ft6x36_get_i_file_version(void)
{
	u8 *pbt_buf = NULL;
	int i_ret=0;
	int fw_len = 0;
	...
	if (fw_len > 2)
	{
              return pbt_buf[0x10A];	//addr of fw ver
	}	
	
	return 0x00;						/*default value */
}
int fts_ft6x36_upgrade_with_bin_file(u8 *file_buff, int file_len)
{
	u8 *pbt_buf = NULL;
	int i_ret=0;
	int fw_len = 0;
