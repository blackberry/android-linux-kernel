...
static int  fts_ic_program_ft8707_write_pram( u8* pbt_buf, u32 dw_lenth)
{
	...
	for (i = 0; i < 20; i++)
	{
		...
		/* MODIFIED-BEGIN by Haojun Chen, 2016-11-04,BUG-3337832*/
		mdelay(fts_upgrade_info_curr.delay_aa);
		fts_write_reg( 0xfc, FTS_UPGRADE_55);
		mdelay(200);
		/* MODIFIED-END by Haojun Chen,BUG-3337832*/
		...
		*/
		...
		mdelay(1); // MODIFIED by Haojun Chen, 2016-11-04,BUG-3337832
		...
		{
			mdelay(50); // MODIFIED by Haojun Chen, 2016-11-04,BUG-3337832
			break;
		}
		else
		{
	...
	*/
	...
	DBG("Read flash and compare");
		*/
	mdelay(100); // MODIFIED by Haojun Chen, 2016-11-04,BUG-3337832
	//-----------------------------------------------------------------------------------------------------
	...
	mdelay(20); // MODIFIED by Haojun Chen, 2016-11-04,BUG-3337832
...
static int fts_ic_program_ft8707_upgrade( u8 *pbt_buf, u32 dw_lenth)
{
	...
	u32 uFlash_OperationFlag = 0x1000;
	u32 uCheckOff = 0x20;
	u32 uStartAddr = 0x00;
	u8 chip_id_1 = 0;
	...
	fts_read_reg(0xf5, &chip_id_1);
	FTS_COMMON_DBG("chip_id_1 = %x", chip_id_1);
	if (chip_id_1 != 0x01)
		goto step7;
    ...
    */
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) {
		...
		//	continue;
		//}
		mdelay(10); // MODIFIED by Haojun Chen, 2016-11-04,BUG-3337832
		auc_i2c_write_buf[0] = FTS_UPGRADE_55;
    ...
    */
	...
	for (j = 0; j < packet_number; j++) {
		...
		for(i = 0;i < 30;i++)
		{
			...
			if ((uCheckOff +uFlash_OperationFlag) == (((reg_val[0]) << 8) | reg_val[1]))
			{
				break;
			}
			mdelay(1); // MODIFIED by Haojun Chen, 2016-11-04,BUG-3337832
	...
	mdelay(dw_lenth/256); // MODIFIED by Haojun Chen, 2016-11-04,BUG-3337832
...
step7:
	...
	fts_i2c_write( auc_i2c_write_buf, 1);
	mdelay(200); // MODIFIED by Haojun Chen, 2016-11-04,BUG-3337832
...
int fts_ft8707_get_i_file_version(void)
{
	...
	if (fw_len > 2)
	{
		return pbt_buf[0x210E];	//addr of fw ver
	}
