...
*******************************************************************************/
#define FOCALTECH_FLASH_FTE716_INFO  "File Version of  focaltech_flash_ftE716.c:  V1.0.0 2016-05-18"

#define FTS_REG_FTE716_VENDOR_ID 		0xA8
#define LEN_FLASH_ECC_MAX 			0xFFFE

/*******************************************************************************
* Static function prototypes
*******************************************************************************/
static int  fts_ic_program_ftE716_write_pram( u8* pbt_buf, u32 dw_lenth);
static int fts_ic_program_ftE716_upgrade( u8 *pbt_buf, u32 dw_lenth);
static int fts_ftE716_get_i_file(u8 **file_buff, int *file_len);
...
u8 reg_val[4] = {0};
	...
	for (i = 0; i < 20; i++) 
	{
		...
		*/	
		...
		if ((reg_val[0] == 0xE7 && reg_val[1] == 0x16)
			||(reg_val[0] == 0xE7 && reg_val[1] == 0x36)) 
		{
			/*
			i_ret = 0x00;
	...
	{
		//如果软件复位不成功，采用硬件复位
		if(0 != fts_upgrade_hardware_reset(200, 0xE7, 0x16))
			return -EIO;
	}
...
/************************************************************************
*   Name: fts_ic_program_ftE716_upgrade
* Brief:  fw upgrade
...
static int fts_ic_program_ftE716_upgrade( u8 *pbt_buf, u32 dw_lenth)
{
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) {
		...
		else if ((reg_val[0] == 0xE7 && reg_val[1] == 0xA7)) {
				FTS_COMMON_DBG("[FTS] Step 3: READ OK CTPM ID,ID1 = 0x%x,ID2 = 0x%x", 
					reg_val[0], reg_val[1]);
		...
		else if ((reg_val[0] == 0xE7 && reg_val[1] == 0xC6)) {
				FTS_COMMON_DBG("[FTS] Step 3: READ OK CTPM ID,ID1 = 0x%x,ID2 = 0x%x", 
					reg_val[0], reg_val[1]);
...
static int fts_ftE716_get_i_file(u8 **file_buff, int *file_len)
{
	...
	{
		i_ret = fts_read_reg(FTS_REG_FTE716_VENDOR_ID, &uc_vender_id);
		if(i_ret < 0)
		{
...
int fts_ftE716_upgrade_with_i_file(void)
{
	u8 *pbt_buf = NULL;
	int i_ret=0;
	int fw_len = 0;
	...
	/*get i file*/
	i_ret = fts_ftE716_get_i_file(&pbt_buf, &fw_len);
	if(i_ret < 0)
	{
		return -1;
	}
	if(NULL == pbt_buf)
	{
	 ...
	 /* call the write pram function */
    	i_ret = fts_ic_program_ftE716_write_pram( BUFFER_PRAM_BOOT_FILE, g_len_pram_boot_file);
	...
	i_ret = fts_ic_program_ftE716_upgrade( pbt_buf, fw_len);
...
int fts_ftE716_get_i_file_version(void)
{
	u8 *pbt_buf = NULL;
	int i_ret=0;
	int fw_len = 0;
...
int fts_ftE716_upgrade_with_bin_file(u8 *file_buff, int file_len)
{
	int i_ret=0;
	...
	/*call upgrade function*/
	i_ret = fts_ic_program_ftE716_upgrade( file_buff, file_len);
