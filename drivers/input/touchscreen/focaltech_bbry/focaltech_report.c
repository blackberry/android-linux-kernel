...
static void fts_nav_key_report_work(struct work_struct *work)
{
	if (nav_key_need_report) {
		printk("%s report nav key down x = %d, y = %d key_num = %d \n", 
...
static int fts_input_dev_report_A(struct ts_event *data)
 {
	...
	#if REPORT_TOUCH_DEBUG_EN
		printk("\n [fts] touch 1 current-touchs=%d, last-touchs=%d, touch_point=%d, up_point=%d \n ", 
...
#if defined(CONFIG_BBRY) || defined(BBRY_MINISW)
	/* MODIFIED-BEGIN by Haojun Chen, 2016-12-13,BUG-3717125*/
	//if (!btn_mask_on && !btn_mask_hold && (FTS_KEY_ALL_Y_POS == data->au16_y[i]) )
	//{ msleep(50); // delay if location is in Nav area (CKB mask might occur)
	//}
...
#if defined(CONFIG_BBRY) || defined(BBRY_MINISW)
/* MODIFIED-END by Haojun Chen,BUG-3717125*/
	/*is enable?*/
	if(1 != FTS_KEY_EN)
		return -1;
#endif
...
static int fts_read_touch_data(struct ts_event *data)
{
	...
	for (i = 0; i < FTS_MAX_TOUCH_POINTS; i++)
	{
		...
		#if REPORT_TOUCH_DEBUG_EN
			printk("\n [fts] touch data  2 (id= %d ,x=(0x%02x),y= (0x%02x)),point_num=%d, event=%d \n ",
				data->au8_finger_id[i],
				data->au16_x[i],
				data->au16_y[i],
				data->touch_point,
