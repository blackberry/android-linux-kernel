...
static int fts_gesture_read_data(void)
{
    ...
    /* FW */
	 ...
	 for(i = 0;i < pointnum;i++)
	 {
		...
		coordinate_y[i] = (((s16) buf[2 + (4 * i+2)]) & 0x0F) << 8 | (((s16) buf[3 + (4 * i+2)]) & 0xFF);
	 }
	 return -1;
   }
   else
	...
	/*if (0x24 == buf[0])
	{
	    gestrue_id = 0x24;
		fts_gesture_report(fts_input_dev,gestrue_id);
		FTS_COMMON_DBG( "%d check_gesture gestrue_id.",  gestrue_id);
	    return -1;
	}
	...
	fts_gesture_report(fts_input_dev,gestrue_id);
	FTS_COMMON_DBG( "%d read gestrue_id.",  gestrue_id);
