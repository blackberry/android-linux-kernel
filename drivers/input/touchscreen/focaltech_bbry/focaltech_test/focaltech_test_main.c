...
int set_param_data(char * TestParamData)
{
	...
	else if(IC_FT8707>>4 == g_ScreenSetParam.iSelectedIC>>4)
        {
                OnInit_FT8707_TestItem(g_testparamstring);
                OnInit_FT8707_BasicThreshold(g_testparamstring);
                OnInit_MCap_DetailThreshold(g_testparamstring);
                SetTestItem_FT8707();
        }
...
boolean start_test_tp(void) 
{
	...
	switch(g_ScreenSetParam.iSelectedIC>>4)
		...
		//	#endif
			break;
		case IC_FT8707>>4:
                //      #if FTS_AUTO_RESET_EN^M
                        fts_auto_reset_suspend();
                        fts_auto_reset_record_time();
                //      #endif

                        bTestResult = FT8707_StartTest();
...
int get_test_data(char *pTestData)
{
	...
	switch(g_ScreenSetParam.iSelectedIC>>4)
		...
		case IC_FT8716>>4:
			...
			break;	
		case IC_FT8707>>4:
                        iLen = FT8707_get_test_data(pTestData);
                        break;
