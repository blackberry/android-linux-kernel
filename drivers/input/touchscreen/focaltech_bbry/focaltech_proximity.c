...
{
	return snprintf(buf, PAGE_SIZE, "%d", fts_proximity_flag);
}
...
int  fts_touch_proximity_enter_mode( int mode )
{
	...
	{
		FTS_COMMON_DBG("[fts][Touch] proximity mode :  %d ", mode);
	}
...
int  fts_touch_proximity_get_status( int* status )
{
	...
	{
		*status=buf_value[0];
		FTS_COMMON_DBG("[fts][Touch] proximity status :  %d ", *status);
	}
...
int fts_touch_proximity_init(void)
{
	...
	FTS_COMMON_DBG("[focal] %s ",  FTS_TOUCH_PROXIMITY_INFO);	//show version
	fts_proximity_flag = false;
	
	err = sysfs_create_group(&fts_i2c_client->dev.kobj, &fts_touch_proximity_group);
	if (0 != err) 
	{
		FTS_COMMON_DBG( "[focal] %s(void) - ERROR: fts_touch_proximity_init(void) failed.",  __func__);
		sysfs_remove_group(&fts_i2c_client->dev.kobj, &fts_touch_proximity_group);
		return -EIO;
	} 
	else 
	{
		FTS_COMMON_DBG("[focal] %s(void) - fts_touch_proximity_init(void) succeeded.", __func__);
	}
