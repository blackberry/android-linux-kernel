...
#define CHKBITS_8           8
/*[PLATFORM]-Add-BEGIN by TCTSH.xuefei.wang, Defect-2269495, 2016/06/06, add TP double click and p-sensor function */
int gesture_enabled = 1;    /* module switch */
/*[PLATFORM]-Add-END by TCTSH.xuefei.wang, Defect-2269495, 2016/06/06, add TP double click and p-sensor function */
       ...
       if (likely(need_chk)) { /* calc checksum */
            ...
            if (unlikely(!val)) { /* check failed */
                ...
                if (err_flag1) {
                    ...
                    err_flag1 = 1;
                  /* MODIFIED-BEGIN by xuefei.wang, 2016-06-06,BUG-2269495*/
                  //  return 0;
                }
            }
...
#endif
/*[PLATFORM]-Add-END by TCTSH.xuefei.wang, Defect-2269495, 2016/06/06, add TP double click and p-sensor function */
/* MODIFIED-END by xuefei.wang,BUG-2269495*/
...
/*[PLATFORM]-Add-END by TCTSH.xuefei.wang, Defect-2269495, 2016/06/06, add TP double click and p-sensor function */
	...
	input_sync(dev);
        GTP_ERROR("Gesture OK.");
/*[PLATFORM]-Add-BEGIN by TCTSH.xuefei.wang, Defect-2269495, 2016/06/06, add TP double click and p-sensor function */
//clear_reg:
/*[PLATFORM]-Add-END by TCTSH.xuefei.wang, Defect-2269495, 2016/06/06, add TP double click and p-sensor function */
	doze_buf[0] = 0; // clear ges flag
	gt1x_i2c_write(GTP_REG_WAKEUP_GESTURE, doze_buf, 1);
        GTP_ERROR("clear the ges flag.");
        /* MODIFIED-END by xuefei.wang,BUG-2269495*/
	return ret;
...
void gt1x_gesture_debug(int on)
{
    if (on) {
        ...
        gesture_enabled = 0;
        memset(gestures_flag, 0xFF, sizeof(gestures_flag)); // MODIFIED by xuefei.wang, 2016-06-06,BUG-2269495
