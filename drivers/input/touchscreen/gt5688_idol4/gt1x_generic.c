...
s32 gt1x_touch_event_handler(u8 * data, struct input_dev * dev, struct input_dev * pen_dev)
{
    ...
    for (i = 0; i < 1 + 8 * touch_num + 2; i++) {
        ...
        GTP_INFO("touch_num1=%x\n ",touch_num);
        GTP_INFO("touch data1=%x\n ",touch_data[i]);
        GTP_INFO("check_sum1=%x\n ",check_sum);
    }
    if (check_sum) { /* checksum error*/
        ...
        for (i = 0, check_sum = 0; i < 3 + 8 * touch_num; i++) {
            check_sum += touch_data[i];
            GTP_INFO("touch_num2=%x\n ",touch_num);
            GTP_INFO("touch data2=%d\n ",touch_data[i]);
            GTP_INFO("check_sum2=%d\n ",check_sum);
        }
        if (check_sum) {
...
static void gt1x_ps_report(int state)
{
	...
	input_report_abs(vps_gt5688->proximity_dev, ABS_DISTANCE, !!state);
	input_sync(vps_gt5688->proximity_dev);
#endif /* End PLATFROM_MTK */

    GTP_INFO("Report proximity state: %s", state == PS_FARAWAY? "FARAWAY":"NEAR");
}
/*[PLATFORM]-Add-BEGIN by TCTSH.xuefei.wang, Defect-2268967, 2016/06/06, add TP double click and p-sensor function */
static int gt1x_ps_enable(struct sensors_classdev *sensors_cdev,unsigned int enable)
{
	u8 state;
	s32 ret = -1;
        u8 buf[3]={0};
	GTP_ERROR("Proximity function to be %s.", enable ? "on" : "off");
	state = enable ? 1 : 0;
