...
/*[PLATFORM]-Add-BEGIN by TCTSH.xuefei.wang, Task-2638051, 2016/07/30, optimize the p-sensor function */
u8 screen_flag = 0;
/*[PLATFORM]-Add-END by TCTSH.xuefei.wang, Task-2638051, 2016/07/30, optimize the p-sensor function */
/*******************GLOBAL VARIABLE*********************/
...
u8 gt1x_vr_config[GTP_CONFIG_MAX_LENGTH] = { 0 };
u32 gt1x_vr_cfg_length = GTP_CONFIG_MAX_LENGTH;
static int gt1x_ps_enable(struct sensors_classdev *sensors_cdev,unsigned int enable);
...
#if defined(VR_GLASS)
 void gt5688_enable_vr(void)
{
       GTP_INFO("enter enable vr switch");
       if(!gt1x_i2c_client)
             return;
       if(gt5688_init_ok==0)
             return;
        vr_on=1;
        GTP_INFO("enble vr,vr_on=%x.",vr_on);
        queue_work(gt5688_wq_vr,&gt5688_work_vr);        
}
EXPORT_SYMBOL(gt5688_enable_vr);

void gt5688_disable_vr(void)
{     GTP_INFO("enter disable vr switch");
      if(!gt1x_i2c_client)
             return;
       if(gt5688_init_ok==0)
             return;
        vr_on=0;
        GTP_INFO("disble vr,vr_on=%x.",vr_on);
        queue_work(gt5688_wq_vr,&gt5688_work_vr);
}
EXPORT_SYMBOL(gt5688_disable_vr);

void gt5688_change_vr_switch(struct work_struct *work)
{      
	u8 ret;
         if(!gt1x_i2c_client)
            return;
          if(gt1x_halt){
             GTP_INFO("Suspend,gt1x_halt=%d\n",gt1x_halt);
             return;
           }
          GTP_INFO("enter change vr switch.\n");
          if(vr_on==1){
             ret = gt1x_send_cfg(gt1x_vr_config, gt1x_vr_cfg_length);
#if 0
             if(ret<0){
              GTP_INFO("GT5688 vr error failed!");
...
struct virtualpsensor *vps_gt5688;

#define VPS_NAME "virtual-proximity"

struct sensors_classdev virtual_sensors_proximity_cdev3 = {
		.name = VPS_NAME,
		.vendor = "NULL",
		.version = 1,
...
#ifdef CONFIG_TOUCHPANEL_PROXIMITY_SENSOR
int virtual_psensor_input_register3(struct i2c_client *pClient)
{
	s32 nRetVal = 0;

	pr_err("*** %s() ***\n", __func__);

	vps_gt5688->proximity_dev= input_allocate_device();
	if (vps_gt5688->proximity_dev == NULL)
	{
		pr_err("*** input device allocation failed ***\n");
		return -ENOMEM;
	}

	vps_gt5688->proximity_dev->name = "proximity";
	vps_gt5688->proximity_dev->id.bustype = BUS_I2C;

	/* set the supported event type for input device */
	set_bit(EV_ABS, vps_gt5688->proximity_dev->evbit);
	set_bit(ABS_DISTANCE, vps_gt5688->proximity_dev->absbit);
	input_set_abs_params(vps_gt5688->proximity_dev, ABS_DISTANCE, 0, 1, 0, 0);

	nRetVal = input_register_device(vps_gt5688->proximity_dev);
	if (nRetVal < 0)
	{
	...
	vps_gt5688->vps_cdev = virtual_sensors_proximity_cdev3;
	vps_gt5688->vps_cdev.sensors_enable = gt1x_ps_enable;
	vps_gt5688->vps_cdev.sensors_poll_delay = NULL;

	nRetVal = sensors_classdev_register(&pClient->dev, &vps_gt5688->vps_cdev);
	if (nRetVal) {
...
s32 gt1x_init_panel(void)
{
	...
	u8 cfg_lens[] = {
        ...
        const u8 cfg_vr_grp0[] = GTP_CFG_VR_GROUP0;
	const u8 cfg_vr_grp1[] = GTP_CFG_VR_GROUP1;
	const u8 cfg_vr_grp2[] = GTP_CFG_VR_GROUP2;
	const u8 cfg_vr_grp3[] = GTP_CFG_VR_GROUP3;
	const u8 cfg_vr_grp4[] = GTP_CFG_VR_GROUP4;
	const u8 cfg_vr_grp5[] = GTP_CFG_VR_GROUP5;
	const u8 *cfgvrs[] = {
		cfg_vr_grp0, cfg_vr_grp1, cfg_vr_grp2,
		cfg_vr_grp3, cfg_vr_grp4, cfg_vr_grp5
	};
	u8 cfg_vr_lens[] = {
		CFG_GROUP_LEN(cfg_vr_grp0),
		CFG_GROUP_LEN(cfg_vr_grp1),
		CFG_GROUP_LEN(cfg_vr_grp2),
		CFG_GROUP_LEN(cfg_vr_grp3),
		CFG_GROUP_LEN(cfg_vr_grp4),
		CFG_GROUP_LEN(cfg_vr_grp5)
	};
	...
	cfg_len = cfg_lens[sensor_id];
        cfg_vr_len = cfg_vr_lens[sensor_id];
	...
	if (cfg_len < GTP_CONFIG_MIN_LENGTH || cfg_len > GTP_CONFIG_MAX_LENGTH) {
        ...
        if (cfg_vr_len < GTP_CONFIG_MIN_LENGTH || cfg_vr_len > GTP_CONFIG_MAX_LENGTH) {
		GTP_ERROR("Config group%d is INVALID! You need to check you header file CFG_GROUP section!", sensor_id + 1);
		return -1;
	}
	memset(gt1x_config, 0, sizeof(gt1x_config));
	memcpy(gt1x_config, cfgs[sensor_id], cfg_len);
        memset(gt1x_vr_config, 0, sizeof(gt1x_vr_config));
	memcpy(gt1x_vr_config, cfgvrs[sensor_id], cfg_vr_len);
...
/*[PLATFORM]-Add-BEGIN by TCTSH.xuefei.wang, PR-1992707, 2016/04/25, add the sencond TP source driver */
	//gt1x_config[0] &= 0x7F;
/*[PLATFORM]-Add-END by TCTNB.xuefei.wang, PR-1992707, 2016/04/25, add the sencond TP source driver */
#if GTP_CUSTOM_CFG
	...
	if (ret < 0) {
        ...
        cfg_vr_len = GTP_CONFIG_MAX_LENGTH;
	ret = gt1x_i2c_read(GTP_REG_CONFIG_DATA, gt1x_vr_config, cfg_vr_len);
	if (ret < 0) {
		return ret;
	}
	...
	if ((gt1x_abs_x_max == 0) && (gt1x_abs_y_max == 0)) {
		...
		gt1x_config[TRIGGER_LOC] = (gt1x_config[TRIGGER_LOC] & 0xFC) | gt1x_int_type;
       	...
       	if ((gt1x_abs_x_max == 0) && (gt1x_abs_y_max == 0)) {
		gt1x_abs_x_max = (gt1x_vr_config[RESOLUTION_LOC + 1] << 8) + gt1x_vr_config[RESOLUTION_LOC];
		gt1x_abs_y_max = (gt1x_vr_config[RESOLUTION_LOC + 3] << 8) + gt1x_vr_config[RESOLUTION_LOC + 2];
		gt1x_int_type = (gt1x_vr_config[TRIGGER_LOC]) & 0x03;
		gt1x_wakeup_level = !(gt1x_vr_config[MODULE_SWITCH3_LOC] & 0x20);
	} else {
		gt1x_vr_config[RESOLUTION_LOC] = (u8) gt1x_abs_x_max;
		gt1x_vr_config[RESOLUTION_LOC + 1] = (u8) (gt1x_abs_x_max >> 8);
		gt1x_vr_config[RESOLUTION_LOC + 2] = (u8) gt1x_abs_y_max;

		gt1x_vr_config[RESOLUTION_LOC + 3] = (u8) (gt1x_abs_y_max >> 8);
		set_reg_bit(gt1x_vr_config[MODULE_SWITCH3_LOC], 5, !gt1x_wakeup_level);
		gt1x_vr_config[TRIGGER_LOC] = (gt1x_vr_config[TRIGGER_LOC] & 0xFC) | gt1x_int_type;
	}
...
s32 gt1x_touch_event_handler(u8 * data, struct input_dev * dev, struct input_dev * pen_dev)
{
    ...
    if (check_sum) { /* checksum error*/
        GTP_ERROR("xuefei.wang Checksum error[%x]",check_sum);
...
#define GTP_REG_PROXIMITY_ENABLE    0x8040 // MODIFIED by xuefei.wang, 2016-06-06,BUG-2268967
#define PS_FARAWAY                  1
#define PS_NEAR                     0
...
static void gt1x_ps_report(int state)
{
    ...
    input_report_abs(vps_gt5688->proximity_dev, ABS_DISTANCE, state);//!!state
    input_sync(vps_gt5688->proximity_dev);

    GTP_INFO("xuefei.wang Report proximity state: %s", state == PS_FARAWAY? "FARAWAY":"NEAR");
}
static int gt1x_ps_enable(struct sensors_classdev *sensors_cdev,unsigned int enable)
{
	u8 state;
    ...
    if (gt1x_chip_type == CHIP_TYPE_GT1X){
        GTP_ERROR("szy_enter CHIP_TYPE_GT1X. \n");
	//ret = gt1x_i2c_write(GTP_REG_PROXIMITY_ENABLE, &state, 1);
             if(enable){     //modify by timing 20160620
              buf[0]=0x12;
              buf[1]=0x00;
              buf[1]=0xee;
       } else{
      ...
      GTP_ERROR("PROXIMITY CMD = 0x%2x,0x%2x,0x%2x",buf[0],buf[1],buf[2]);
      ret = gt1x_i2c_write(GTP_REG_PROXIMITY_ENABLE, buf, 3);
    }else if (gt1x_chip_type == CHIP_TYPE_GT2X){
        //GTP_ERROR("szy_enter CHIP_TYPE_GT2X. \n");
	...
	if (!ret && enable) {
		vps_gt5688->vps_enabled = 1;
	} else {
		vps_gt5688->vps_enabled = 0;
	}
        rs = gt1x_i2c_read(GTP_READ_COOR_ADDR,point_buffer,sizeof(point_buffer));
        status = (point_buffer[0] & 0x60) ? 0 : 1;
        vps_gt5688->state = status;
        gt1x_ps_report(vps_gt5688->state);
...
int gt1x_prox_event_handler(u8 * data)
{
	u8 ps = 0;
        ps=(data[0]& 0x60)?0:1;
	GTP_DEBUG("xuefei.wang %s : vps_enabled=%d; ps=%d; state=%d\n", __func__, vps_gt5688->vps_enabled, (data[0] & 0x60) ? 0 : 1, vps_gt5688->state);
	if (vps_gt5688->vps_enabled) {
		gt1x_ps_report(ps);
		vps_gt5688->state = ps;
...
static s32 gt1x_ps_operate(void *self, u32 command, void *buff_in, s32 size_in, void *buff_out, s32 size_out, s32 * actualout)
{
	...
	case SENSOR_ENABLE:
		if ((buff_in == NULL) || (size_in < sizeof(int))) {
			...
			value = *(int *)buff_in;
			err = gt1x_ps_enable(&vps_gt5688->vps_cdev,value);
		}
...
/*[PLATFORM]-Add-BEGIN by TCTSH.xuefei.wang, Task-2344941, 2016/06/14, add vr function */
#ifndef PLATFORM_MTK
static ssize_t gt1x_ps_enable_show(struct device *pDevice, struct device_attribute *pAttr, char *pBuf) {
	return scnprintf(pBuf, PAGE_SIZE, "%d", vps_gt5688->vps_enabled);
}

static ssize_t gt1x_ps_enable_store(struct device *pDevice, struct device_attribute *pAttr, const char *pBuf, size_t nSize) 
    {
	unsigned int input;
	if(sscanf(pBuf, "%u", &input) != 1) {
		return -EINVAL;
	}
	if(input == 1) {
		gt1x_ps_enable(&vps_gt5688->vps_cdev,1);
		gt1x_ps_report(PS_FARAWAY);
	} else if(input == 0) {
		gt1x_ps_report(PS_FARAWAY);
		gt1x_ps_enable(&vps_gt5688->vps_cdev,0);
	} else {
		return -EINVAL;
	}
	return nSize;
...
static ssize_t gt1x_ps_state_show(struct kobject *kobj, struct kobj_attribute *attr,
		char *buf) {
	return scnprintf(buf, PAGE_SIZE, "%d", vps_gt5688->state);
}
...
static ssize_t gt1x_ps_state_store(struct kobject *kobj, struct kobj_attribute *attr,
    ...
    if (!vps_gt5688->enabled) {
        return -EINVAL;
    }

	if(input == 1) {
		vps_gt5688->state = PS_FARAWAY;
	} else if(input == 0) {
		vps_gt5688->state = PS_NEAR;
	} else {
		return -EINVAL;
	}
...
static DEVICE_ATTR(enable, 0664, gt1x_ps_enable_show, gt1x_ps_enable_store);
...
#endif
/*[PLATFORM]-Add-BEGIN by TCTSH.xuefei.wang, Task-2344941, 2016/06/14, add vr function */
static void gt1x_ps_deinit(struct i2c_client *client)
{
	if(vps_gt5688) {

			device_remove_file(&client->dev, &dev_attr_enable);
		     }
		input_free_device(vps_gt5688->proximity_dev);
                kfree(vps_gt5688);
 }
/*[PLATFORM]-Add-END by TCTSH.xuefei.wang, Task-2344941, 2016/06/14, add vr function */
/**
...
#ifdef CONFIG_TOUCHPANEL_PROXIMITY_SENSOR
	if (vps_gt5688 && vps_gt5688->vps_enabled) {
...
#ifdef CONFIG_TOUCHPANEL_PROXIMITY_SENSOR
	if (vps_gt5688 && vps_gt5688->vps_enabled) {
               GTP_INFO("xuefei.wang gt1x_resume vps enabled!");
               if (!gesture_enabled)
		{
			gt1x_halt = 0;
			gt1x_irq_enable();
...
#ifdef CONFIG_TOUCHPANEL_PROXIMITY_SENSOR
	vps_gt5688 = kzalloc(sizeof(struct virtualpsensor), GFP_KERNEL);
	if (!vps_gt5688) {
		//dev_err("Not enough memory\n");
		return -ENOMEM;
	}
#endif
/*[PLATFORM]-Add-END by TCTSH.xuefei.wang, Task-2344941, 2016/06/14, add vr function */
...
void gt1x_deinit(struct i2c_client *client)
{
