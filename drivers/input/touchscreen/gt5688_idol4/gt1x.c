...
/*[PLATFORM]-Add-BEGIN by TCTSH.xuefei.wang, Task-2344941, 2016/06/14, add vr and p-sensor function */
int gt5688_init_ok =0;
#if defined(VR_GLASS)
struct workqueue_struct *gt5688_wq_vr;
struct work_struct gt5688_work_vr;
#endif
/*[PLATFORM]-Add-END by TCTSH.xuefei.wang, Task-2344941, 2016/06/14, add vr and p-sensor function */
...
/*[PLATFORM]-Add-BEGIN by TCTSH.xuefei.wang, Task-2340576, 2016/06/13, add FW_VERSION NODE */
static struct class * tp_device_class;
				...
				struct device_attribute *attr, char *buf)
{
       ...
       u8 reg_data;
       int err;
       err= gt1x_i2c_read(GTP_REG_CONFIG_DATA,&reg_data, 1);
       ret = snprintf(buf, 100, "fw_version is 0x%x\n", reg_data);
       return ret;
}


static DEVICE_ATTR(fw_version, 0444, tp_fw_version_show, NULL);
/*[PLATFORM]-Add-END by TCTSH.xuefei.wang, Task-2340576, 2016/06/13, add FW_VERSION NODE */
/**
...
static int gt1x_parse_dt(struct device *dev)
{
	struct device_node *np;
        /* MODIFIED-BEGIN by xuefei.wang, 2016-06-28,BUG-2459640*/
        int ret = 0;
/*[PLATFORM]-Add-BEGIN by TCTSH.xuefei.wang, Task-2459640, 2016/07/01, add TP TYPE */
        int rc=0;
/*[PLATFORM]-Add-END by TCTSH.xuefei.wang, Task-2459640, 2016/07/01, add TP TYPE */
        ...
        np = dev->of_node;
/*[PLATFORM]-Add-BEGIN by TCTSH.xuefei.wang, Task-2459640, 2016/07/01, add TP TYPE */
         rc = of_property_read_string(np, "goodix,name", &gt_tp_name);
         if(rc <0)
             GTP_ERROR("get goodix,name failed");
/*[PLATFORM]-Add-END by TCTSH.xuefei.wang, Task-2459640, 2016/07/01, add TP TYPE */
/* MODIFIED-END by xuefei.wang,BUG-2459640*/
...
/*[PLATFORM]-Add-BEGIN by TCTSH.xuefei.wang, Task-2344941, 2016/06/14, add vr and p-sensor function */
        #if defined(VR_GLASS)
        INIT_WORK(&gt5688_work_vr, gt5688_change_vr_switch);
        #endif
/*[PLATFORM]-Add-END by TCTSH.xuefei.wang, Task-2344941, 2016/06/14, add vr and p-sensor function */
...
/*[PLATFORM]-Add-BEGIN by TCTSH.xuefei.wang, Task-2344941, 2016/06/14, add vr and p-sensor function */
#ifdef CONFIG_TOUCHPANEL_PROXIMITY_SENSOR
     virtual_psensor_input_register3(client) ;
#endif
/*[PLATFORM]-Add-END by TCTSH.xuefei.wang, Task-2344941, 2016/06/14, add vr and p-sensor function */
/*[PLATFORM]-Add-BEGIN by TCTSH.xuefei.wang, Task-2344941, 2016/06/14, add vr and p-sensor function */
	gt5688_init_ok=1;
...
#ifdef CONFIG_BBRY
static struct gt1x_ts_data g_gt1x_ts_info = {0};
static struct gt1x_ts_data* g_gt1x_ts_data = &g_gt1x_ts_info;
static void gt1x_power_state_work(struct work_struct *work)
{
	struct gt1x_ts_data *ts =
		container_of(work, struct gt1x_ts_data, power_state_work);
	...
	GTP_INFO("%s: fb_blank=%d, fb_event=%d\n",
		__func__, fb_blank, fb_event);

	if (fb_event == FB_EVENT_BLANK) {
					...
					unsigned long event, void *data)
{
	...
	int fb_blank;
	struct gt1x_ts_data *gt1x_data = g_gt1x_ts_data;

	if ((noti == NULL) || (evdata == NULL) || (evdata->data  == NULL))
		return 0;
	...
	GTP_INFO("%s: event=%lu, fb_blank=%d\n",
		__func__, event, fb_blank);
	if (!(FB_BLANK_NORMAL == fb_blank)) {
		flush_workqueue(gt1x_wq);
		gt1x_data->fb_blank = fb_blank;
		gt1x_data->fb_event = event;
		queue_work(gt1x_wq, &gt1x_data->power_state_work);
	}
        return 0;
}
#else //CONFIG_BBRY
...
{
#ifdef CONFIG_BBRY
	INIT_WORK(&g_gt1x_ts_data->power_state_work, gt1x_power_state_work);
#endif
...
/*[PLATFORM]-Add-BEGIN by TCTSH.xuefei.wang, Task-2344941, 2016/06/14, add vr and p-sensor function */
#if defined(VR_GLASS)
	gt5688_wq_vr = create_singlethread_workqueue("gt5688_wq_vr");
	if (!gt5688_wq_vr)
	{
		printk("Creat gt1x_wq_vr workqueue failed. \n");
		return -ENOMEM;
	}
#endif
/*[PLATFORM]-Add-END by TCTSH.xuefei.wang, Task-2344941, 2016/06/14, add vr and p-sensor function */
