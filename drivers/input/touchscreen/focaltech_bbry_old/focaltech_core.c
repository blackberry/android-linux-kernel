...
#define	FT_INIT_COMPLETE_MASK		0x00  /* Need to update */
#endif
...
{    
	...
	dev_dbg(&data->client->dev,"%s:%d: touch_point=%d", __func__, __LINE__, event->touch_point);
	...
	dev_dbg(&data->client->dev,"%s: touch_point=%d", __func__, event->touch_point);
	...
	else
	{
		...
		for (i = 0; i < event->touch_point; i++)
		{
			...
			dev_dbg(&data->client->dev,"%s: i=%d(%d), touch_event=%s, x=%d, y=%d", 
			__func__, i, event->touch_point,
			(event->au8_touch_event[i] == FTS_TOUCH_DOWN || event->au8_touch_event[i] == FTS_TOUCH_CONTACT) ? "down" : "up",
...
static void fts_touch_irq_work(struct work_struct *work)
{
	...
	dev_dbg(&fts_wq_data->client->dev,
			"fs_read_touchdata returned ret = %d", ret);
	if (ret == 0)
...
#if 0 /* JEGA: Display seems using it. Workaround for now */
				goto err_irq_gpio_dir;
#endif
			}
...
static int fts_power_on(struct fts_ts_data *data, bool on)
{
	...
	if (!on) {
		dev_err(&data->client->dev, "Power is shared with the display. It is always on...");
		return rc;
	}
	...
	if (gpio_is_valid(data->pdata->pwr_gpio)) {
		gpio_set_value_cansleep(data->pdata->pwr_gpio, 1);
	}
...
power_off:
	if (gpio_is_valid(data->pdata->pwr_gpio)) {
		gpio_set_value_cansleep(data->pdata->pwr_gpio, 0);
	}
...
static int fts_power_init(struct fts_ts_data *data, bool on)
{
	...
	dev_err(&data->client->dev, "fts_power_init %s \n", (on!=0) ? "true" : "false");
	if (!on)
	{
...
#if defined(CONFIG_FB)
#ifdef CONFIG_BBRY_FB
static void fts_power_state_work(struct work_struct *work)
{
	struct fts_ts_data *ts =
		container_of((struct delayed_work *)work, struct fts_ts_data, power_state_work);
	...
	dev_info(&ts->client->dev, "%s: init_complete=0x%08x, fb_blank=%d, fb_event=%d\n",
		__func__, ts->init_complete, fb_blank, fb_event);

	if (((ts->init_complete & FT_INIT_COMPLETE_MASK) == FT_INIT_COMPLETE_MASK) && 
	    (fb_event == FB_EVENT_BLANK)) {
		if (fb_blank == FB_BLANK_UNBLANK)
			fts_ts_resume(&ts->client->dev);
		else if (fb_blank == FB_BLANK_POWERDOWN)
			fts_ts_suspend(&ts->client->dev);
	}
} /* fts_power_state_work */
				 ...
				 unsigned long event, void *data)
{
	...
	dev_info(&fts_data->client->dev, "%s: event=%lu, fb_blank=%d\n",
		__func__, event, fb_blank);
	if (!(FB_BLANK_NORMAL == fb_blank)) {
		flush_workqueue(fts_data->ts_workqueue);
		fts_data->fb_blank = fb_blank;
		fts_data->fb_event = event;
		queue_delayed_work(fts_data->ts_workqueue, &fts_data->power_state_work, 2000);
	}
        return 0;
}
#else
...
#if 0
static int get_panel_dt_vreg_data(struct device *dev, struct fts_ts_platform_data *pdata)
{
	int i = 0, rc = 0;
	u32 tmp = 0;
...
static int fts_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	*/
	...
	INIT_DELAYED_WORK(&data->power_state_work,
			fts_power_state_work);
#endif
