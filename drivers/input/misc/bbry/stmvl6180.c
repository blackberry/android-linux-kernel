...
static int proxi_oneshot(struct i2c_client *client)
{
	...
	/* make sure prox is not in use */
	retval = read_byte(client,
		RESULT_RANGE_STATUS, &buf[0]);
	...
	/* make sure vl6180 is not in interleaved mode */
	retval = read_byte(client,
		INTERLEAVED_MODE_ENABLE, &buf[1]);

	if (retval < 0) {
		dev_err(&client->dev,
			"Failed to read INTERLEAVED_MODE_ENABLE (%d)\n",
			retval);
		return -EIO;
	}

	/* prox is in use, get the latest sample from history buffer */
	if (0 == (buf[0] & RESULT_RANGE_DEVICE_READY) ||
			(INTERLEAVED_MODE_ENABLED == buf[1])) {
		dev_dbg(&client->dev,
			"vl6180_oneshot: prox not ready\n");

		/* Try reading history buffer for a valid sample */
		while (count < 10) {
			retval = read_byte(client,
				RESULT_HISTORY_BUFFER_0, &buf[0]);

			if (retval < 0) {
				dev_err(&client->dev,
					"Failed to read RESULT_HISTORY_BUFFER_0 (%d)\n",
					retval);
				return -EIO;
			}
			...
			if (buf[0] != 0) {
				dev_err(&client->dev,
					"vl6180_oneshot: history range=%d\n",
					buf[0]);
		...
		/* There should be a sample by 100 ms */
		return -EAGAIN;
	}

	/* prox is not in use, start a single measurement */
	dev_dbg(&client->dev,
		"vl6180_oneshot: prox is ready\n");

	retval = write_byte(client,
		RANGE_START,
		(SYSRANGE_MODE_SELECT_SINGLE | SYSRANGE_STARTSTOP_START));
	...
	/* wait for prox to return a sample */
	while (count < 1000) {
		retval = read_byte(client,
			RESULT_RANGE_STATUS, &buf[0]);
