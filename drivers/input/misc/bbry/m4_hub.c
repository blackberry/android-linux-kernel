...
static int m4_hub_resume(struct device *dev)
{
	...
	// Make sure interrupt line is not active already
	if( !wake_lock_active(&m4_config->hub_wake_lock) && gpio_get_value(m4_config->hub_int) != 0){
		dev_err(&client->dev, "m4_hub_resume: wakelock not active but GPIO is active");
		// We already have released our ISR, we can safely execute the isr function from here
		m4_hub_wake_irq(0, m4_config);
	}
	return 0;
}
