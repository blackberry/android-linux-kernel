...
static int l3gd20_gyr_config_regulator(struct l3gd20_gyr_status *stat, bool on);
...
static struct l3gd20_gyr_platform_data default_l3gd20_gyr_pdata = {
	...
	.power_on = l3gd20_gyr_config_regulator,
	.power_off = l3gd20_gyr_config_regulator,
};
...
struct sensor_regulator l3gd20_gyr_vreg[] = {
...
{
	int rc = 0, i;
	int num_reg = sizeof(l3gd20_gyr_vreg) / sizeof(struct sensor_regulator);

	if (on) {
		for (i = 0; i < num_reg; i++) {
			l3gd20_gyr_vreg[i].vreg =
				regulator_get(&stat->client->dev,
				l3gd20_gyr_vreg[i].name);
			if (IS_ERR(l3gd20_gyr_vreg[i].vreg)) {
				rc = PTR_ERR(l3gd20_gyr_vreg[i].vreg);
				pr_err("%s:regulator get failed rc=%d\n",
								__func__,rc);
				l3gd20_gyr_vreg[i].vreg = NULL;
				goto error_vdd;
			}

			if (regulator_count_voltages(
				l3gd20_gyr_vreg[i].vreg) > 0) {
				rc = regulator_set_voltage(
					l3gd20_gyr_vreg[i].vreg,
					l3gd20_gyr_vreg[i].min_uV,
					l3gd20_gyr_vreg[i].max_uV);
				if (rc) {
					pr_err("%s: set voltage failed rc=%d\n",
					__func__, rc);
					regulator_put(l3gd20_gyr_vreg[i].vreg);
					l3gd20_gyr_vreg[i].vreg = NULL;
					goto error_vdd;
				}
			}

			rc = regulator_enable(l3gd20_gyr_vreg[i].vreg);
			if (rc) {
...
error_vdd:
	while (--i >= 0) {
		if (!IS_ERR_OR_NULL(l3gd20_gyr_vreg[i].vreg)) {
			if (regulator_count_voltages(
			l3gd20_gyr_vreg[i].vreg) > 0) {
				regulator_set_voltage(l3gd20_gyr_vreg[i].vreg,
						0, l3gd20_gyr_vreg[i].max_uV);
			}
			regulator_disable(l3gd20_gyr_vreg[i].vreg);
			regulator_put(l3gd20_gyr_vreg[i].vreg);
			l3gd20_gyr_vreg[i].vreg = NULL;
		}
	}
...
*/
	return i2c_transfer(stat->client->adapter, msgs, 2);
/***************** HUB HACK ENDS ******************/
}
...
static void l3gd20_gyr_device_power_off(struct l3gd20_gyr_status *stat)
{
	...
	if (stat->pdata->power_off) {
		...
		stat->pdata->power_off(stat, false);
...
static int l3gd20_gyr_device_power_on(struct l3gd20_gyr_status *stat)
{
	...
	if (stat->pdata->power_on) {
		err = stat->pdata->power_on(stat, true);
		if (err < 0)
			return err;
	}
...
static void l3gd20_timer_start(struct hrtimer * timer, ktime_t tim, const enum
		hrtimer_mode mode, struct device * dev)
{
	timer->function = &poll_function_read;
	BUG_ON(NULL == timer->function);
...
static int l3gd20_gyr_enable(struct l3gd20_gyr_status *stat)
{
	...
	if (!atomic_cmpxchg(&stat->enabled, 0, 1)) {
		...
		if (stat->polling_enabled) {
			l3gd20_timer_start(&(stat->hr_timer), stat->ktime, HRTIMER_MODE_REL, &stat->client->dev);
		}
					...
					const struct i2c_device_id *devid)
{
	...
	dev_info(&client->dev, "l3gd20 hrtimer_init 1\n");
...
static int l3gd20_gyr_resume(struct device *dev)
{
	...
	if (atomic_read(&stat->enabled)) {
		...
		if (stat->polling_enabled) {
			...
			l3gd20_timer_start(&(stat->hr_timer), stat->ktime, HRTIMER_MODE_REL, &stat->client->dev);
#endif
			dev_info(&stat->client->dev, "l3gd20 hrtimer_init 2\n");
		}
#ifdef SLEEP
		err = l3gd20_gyr_register_update(stat, buf, CTRL_REG1,
