...
struct hall_drvdata {
	...
	enum hall_state_e prev_hall_state;
...
static void hall_gpio_report_event(struct hall_drvdata *ddata)
{
	...
	switch(ddata->hall_state)
	{
		case STATE_SLIDER_OPEN:
			if(SLIDER_TRANSITION_EVENT(open_state, mid_state, closed_state)) {
				...
				if(pdata->sf_debounce > 0) {
					if (true != queue_delayed_work(ddata->work_queue,
						&ddata->sf_work, msecs_to_jiffies(pdata->sf_debounce))) {
							pr_info("%s: queue_delayed_work failed.\n", __func__);
					}
				}
			...
			} else if(HOLSTER_CLOSE_EVENT(open_state, mid_state, closed_state)) {
				/* The slider is open and we got a holster close event
				   This may happen in the following scenario:
				   1. Device was closed and in the holster.
				   2. User pulled out only the slider while the base remained in
				      the holster (we enter STATE_SLIDER_OPEN at this point).
				   3. User pushed the slider back into the base. The base never
				      left the holster (we now get the HOLSTER_CLOSE_EVENT).*/
				if(pdata->sf_debounce > 0) {
					if (true != queue_delayed_work(ddata->work_queue,
						&ddata->sf_work, msecs_to_jiffies(pdata->sf_debounce))) {
							pr_info("%s: queue_delayed_work failed.\n", __func__);
		...
		case STATE_HOLSTER_CLOSED:
			if(SMART_FLIP_CLOSE_EVENT(open_state, mid_state, closed_state)) {
				/* The holster is closed and we got a smart flip event.
				   This may be caused by the following:
				   1. The device is being removed from a holster and this is just an
				      intermediate state (the flip case is not really being used).
				      Note the delayed work function will only send a FLIP_CLOSED
				      event if gpio's remain in this state for the debounce period.
				   2. The device is actually in a closed flip case but ended up in 
				      STATE_HOLSTER_CLOSED (perhaps the open sensor was toggled by
				      an external magnet). We would see a SMART_FLIP_CLOSE_EVENT
				      once the open sensor toggled back high. */
				if(pdata->sf_debounce > 0) {
					if (true != queue_delayed_work(ddata->work_queue,
						&ddata->sf_work, msecs_to_jiffies(pdata->sf_debounce))) {
							pr_info("%s: queue_delayed_work failed.\n", __func__);
			...
			} else if(HOLSTER_OPEN_EVENT(open_state, mid_state, closed_state) && ddata->holster_state == HOLSTER_CLOSED) {
				...
				STATE_CHANGE(STATE_SLIDER_CLOSED);
				ddata->holster_state = HOLSTER_OPEN;
			} else if(SLIDER_OPEN_EVENT(open_state, mid_state, closed_state)) {
				/* The slider was pulled open while the base of the device stayed
				   in the holster. This may be caused by the following:
				   1. Device was closed and in the holster.
				   2. User pulled out only the slider while the base remained in
				      the holster (we get the SLIDER_OPEN_EVENT at this point).  */
				if(pdata->sf_debounce > 0) {
					if (true != queue_delayed_work(ddata->work_queue,
						&ddata->sf_work, msecs_to_jiffies(pdata->sf_debounce))) {
							pr_info("%s: queue_delayed_work failed.\n", __func__);
	...
	pr_info("%s: open %d mid %d close %d prev_state %d state %d\n",
		__func__, open_state, mid_state, closed_state,
		ddata->prev_hall_state, ddata->hall_state);
	ddata->prev_hall_state = ddata->hall_state;
}
...
static void hall_sf_work_func(struct work_struct *work)
{
	...
	switch(ddata->hall_state)
	{
		case STATE_SMART_FLIP_CLOSED:
			...
			break;
		case STATE_SLIDER_OPEN:
			if(HOLSTER_CLOSE_EVENT(open_state, mid_state, closed_state)) {
				hall_set_cur_state(input, SW_KEYPAD_SLIDE, SLIDER_CLOSED);
				hall_set_cur_state(input, SW_LID, HOLSTER_CLOSED);
				ddata->holster_state = HOLSTER_CLOSED;
				STATE_CHANGE(STATE_HOLSTER_CLOSED);
		...
		case STATE_SLIDER_TRANSITION:
			...
			if(ddata->flip_state == FLIP_CLOSED) {
				pr_info("%s: device still in transition, setting flip_state to OPEN\n", __func__);
				ddata->flip_state = FLIP_OPEN;
				hall_set_cur_state(input, SW_SMART_FLIP, FLIP_OPEN);
			}
			break;
...
static void hall_gpio_timer(unsigned long _data)
{
	...
	if (true != queue_work(ddata->work_queue, &bdata->work)) {
		pr_info("%s: gpio:%s, queue_work failed!\n",
...
static irqreturn_t hall_gpio_isr(int irq, void *dev_id)
{
	...
	if (bdata->timer_debounce)
		...
		queue_work(ddata->work_queue, &bdata->work);
...
static int hall_probe(struct platform_device *pdev)
{
	...
	// Create workqueue which only allows one work item to be processed at a time.
	ddata->work_queue = alloc_ordered_workqueue("hall_work_queue", 0);
	if (!ddata->work_queue) {
