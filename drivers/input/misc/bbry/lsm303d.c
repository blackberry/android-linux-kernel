...
static int lsm303d_parse_dt(struct device *dev, struct lsm303d_status *stat)
{
	...
	if (np != NULL) {
		rc = of_property_read_u32(np, "stm,rotation_matrix",&tmp);
		if ( rc < 0 ) {
			dev_err(dev,"failed to read rotation matrix\n");
			return -1;
		}
		memcpy(stat->pdata_acc->rot_matrix, r[tmp], 3*3*sizeof(short));
		memcpy(stat->pdata_mag->rot_matrix, r[tmp], 3*3*sizeof(short));
	}
	return 0;
}
#endif
...
#ifndef CONFIG_BBRY
	lsm303d_parse_dt(&client->dev, stat);
#endif
