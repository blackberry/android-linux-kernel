...
{
	/*Modified by yejunwen,5233887, 2017/08/30 */
	//return sprintf ( buf, "hall_cover_state:%d hall_vr_state:%d \n",hall->hall_cover_state,hall->hall_vr_state);
	return sprintf ( buf, "hall_cover_state:%d \n",hall->hall_cover_state);
}
...
//	queue_work(hall->hall_wq, &hall->hall_vr_work);
	...
	if(rc < 0)
	{
        ...
        enable_irq_wake(gpio_to_irq(hall->irq_gpio_cover)); // MODIFIED by hongwei.tian, 2018-05-10,BUG-6313524
/*Modified by yejunwen,5233887, 2017/08/30 */
/*
	if (hall->irq_gpio_vr > 0)
	{
