...
#else
static int __devinit export_bsi_probe(struct platform_device *pdev)
#endif
{
	void *bsi_data;
	unsigned bsi_size;
...
#else
	bsi_data = smem_get_entry(SMEM_ID_VENDOR1, &bsi_size);
#endif
	if (bsi_data == NULL) {
...
#else
static int __devexit export_bsi_remove(struct platform_device *pdev)
#endif
{
...
#else
	.remove = __devexit_p(export_bsi_remove),
#endif
	.driver = {
		.owner = THIS_MODULE,
		.name = "export_bsi",
