...
wl_cfgp2p_listen_expired(unsigned long data)
{
	...
	wl_cfg80211_event(wl_to_p2p_bss_ndev(cfg, P2PAPI_BSSCFG_DEVICE), &msg,
		NULL);
#else
	if (wl_to_p2p_bss_ndev(cfg, P2PAPI_BSSCFG_DEVICE)) {
		wl_cfg80211_event(wl_to_p2p_bss_ndev(cfg, P2PAPI_BSSCFG_DEVICE), &msg,
			NULL);
	} else {
		CFGP2P_ERR(("Failed to get p2p netdev, most likely not initialized.\n"));
		wlan_ddt_event_send(WLAN_DRV_EVENT_P2P_NETDEV_NULL);
	}
#endif /* CONFIF_BBRY */
#endif /* WL_ENABLE_P2P_IF */
}
