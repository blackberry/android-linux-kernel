...
pcie_init_bus_idle_watchdog(dhd_bus_t *bus)
{
	...
	bus->bus_idle_in_host_mem_access = FALSE;
	bus->bus_idle_current_req = PCIE_BUS_IDLE_NONE;
...
pcie_execute_bus_idle_suspend(dhd_pub_t *dhdp, bool suspend)
{
	...
	DHD_ERROR(("%s : process  req %s \n", __FUNCTION__, suspend ? "SUSPEND":"RESUME"));

	/* Start PCIE clock if we are resuming, exit early if clock start fails */
	if (!suspend && bus->bus_idle_is_suspended) {
		DHD_IDLE(("%s 0.25 pcie_bus_on_start\n", PICE_IDLE_SUSPEND_TAG));
		while (retry--) {
			ret = dhdpcie_bus_clock_start(bus);
			if (!ret) {
				break;
			} else {
				DHD_ERROR(("%s: host clock start failed (%d) retry\n",
					__FUNCTION__, ret));
				OSL_SLEEP(10);
			}
		}
		if (ret && !retry) {
			DHD_ERROR(("%s: host pcie clock start failed: %d\n",
				__FUNCTION__, ret));
			goto done;
		}
		DHD_IDLE(("%s 0.5 pcie_bus_on_done\n", PICE_IDLE_SUSPEND_TAG));
	}

	/* Suspend/resume wifi chipset */
	ret = dhdpcie_bus_suspend(bus, suspend);
	if (ret) {
		DHD_ERROR(("Failed to %s PCIe bus\n", suspend ? "suspend":"resume"));
		bus->bus_idle_is_suspended = suspend ? FALSE : TRUE;
		goto done;
	} else {
		DHD_IDLE(("%s %s %s_bus_done\n", PICE_IDLE_SUSPEND_TAG,
			suspend ? "0.5" : "1", suspend ? "suspend":"resume"));
		/* Stop PCIE clock if we are suspending */
		if (suspend) {
			DHD_IDLE(("%s 0.25 pcie_bus_off_start\n", PICE_IDLE_SUSPEND_TAG));
			ret = dhdpcie_bus_clock_stop(bus);
			DHD_IDLE(("%s 0 pcie_bus_off_done\n", PICE_IDLE_SUSPEND_TAG));
			if (ret) {
				DHD_ERROR(("%s: host pcie clock stop failed: %d\n", __FUNCTION__, ret));
			}
		}
...
pcie_bus_idle_resume(dhd_pub_t *dhdp, uint32 src)
{
	...
	DHD_IDLE(("enter : (src::%d), host mem %d busstate %d suspended %d idle suspend %d req %d caller %pS\n",
		src,
		bus->bus_idle_in_host_mem_access,
		dhdp->busstate,
		bus->suspended,
	...
	if (bus->bus_idle_in_host_mem_access) {
		ret = BCME_OK;
		DHD_INFO(("%s: Within recursive resume call, drop this (src::%d)\n", __FUNCTION__, src));
		goto done;
	}
