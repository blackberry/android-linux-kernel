...
#ifdef CONFIG_BBRY
static void pcie_bus_idle_watchdog_handler(ulong data);
static int pcie_init_bus_idle_watchdog(dhd_bus_t *bus);
static int pcie_remove_bus_idle_watchdog(dhd_bus_t *bus);
#endif /* CONFIG_BBRY */
...
dhdpcie_dongle_attach(dhd_bus_t *bus)
{
	...
	bus->suspended = FALSE;
#ifdef CONFIG_BBRY
	pcie_init_bus_idle_watchdog(bus);
#endif /* CONFIG_BBRY */
	DHD_TRACE(("%s: EXIT: SUCCESS\n",
		__FUNCTION__));
	return 0;
...
#ifdef CONFIG_BBRY
		pcie_remove_bus_idle_watchdog(bus);
#endif /* CONFIG_BBRY */
...
int dhd_bus_rxctl(struct dhd_bus *bus, uchar *msg, uint msglen)
{
	...
	if (timeleft == 0) {
#if defined (CONFIG_BBRY) && defined (CONFIG_ARCH_MSM8992)
		unsigned long rem_nsec = do_div(nsec, 1000000000);
		DHD_ERROR(("%s: resumed on timeout, from %5lu.%06lu cpu %d\n",
			__FUNCTION__, (unsigned long)nsec, rem_nsec / 1000, smp_processor_id()));
#else
...
#ifdef CONFIG_BBRY
	/* kill watchdog to avoid recursive call during suspend/resume */
	pcie_remove_bus_idle_watchdog(bus);
#endif /* CONFIG_BBRY */
...
#ifdef CONFIG_BBRY
		DHD_IDLE(("%s 0.5 suspend_started\n", PICE_IDLE_SUSPEND_TAG));
#endif
...
#else
				DHD_ERROR(("Suspend failed because of wakelock flag 0x%x\n", active));
				DHD_IDLE(("%s 1 suspend_failed-wakelock\n", PICE_IDLE_SUSPEND_TAG));
#endif
...
#ifdef CONFIG_BBRY
				DHD_IDLE(("%s 0.5 suspend_in_progress\n", PICE_IDLE_SUSPEND_TAG));
#endif
				dhdpcie_bus_intr_disable(bus);
...
#ifdef CONFIG_BBRY
			DHD_IDLE(("%s 1 suspend_failed-timeout\n", PICE_IDLE_SUSPEND_TAG));
#endif
			bus->dev->current_state = PCI_D3hot;
			pci_set_master(bus->dev);
			rc = pci_set_power_state(bus->dev, PCI_D0);
...
#ifdef CONFIG_BBRY
			DHD_IDLE(("%s 1 suspend_failed-linkdown\n", PICE_IDLE_SUSPEND_TAG));
#endif
			bus->suspended = FALSE;
			bus->dhd->busstate = DHD_BUS_DOWN;
			rc = -ETIMEDOUT;
...
#ifdef CONFIG_BBRY
			DHD_IDLE(("%s 0.25 resume_failed-linkdown\n", PICE_IDLE_SUSPEND_TAG));
#endif
			rc = -ETIMEDOUT;
			bus->dhd->busstate = DHD_BUS_DOWN;
			dhdpcie_bus_report_pcie_linkdown(bus);
...
#ifdef CONFIG_BBRY
			DHD_IDLE(("%s 0.5 resume_in_progress\n", PICE_IDLE_SUSPEND_TAG));
#endif
			bus->dhd->busstate = DHD_BUS_DATA;
...
{
#ifdef CONFIG_BBRY
	pcie_kick_bus_idle_watchdog(bus, 0, PCIE_BUS_IDLE_WD_MAILBOX_INTR);
#endif /* CONFIG_BBRY */
...
#ifdef CONFIG_BBRY
	pcie_kick_bus_idle_watchdog(bus, 0, PCIE_BUS_IDLE_WD_RX);
#endif /* CONFIG_BBRY */
...
#ifdef CONFIG_BBRY
	pcie_kick_bus_idle_watchdog(bus, 0, PCIE_BUS_IDLE_WD_FLOWRING_DEL);
#endif /* CONFIG_BBRY */
...
#ifdef CONFIG_BBRY
/* Allocate about 2KB memory for PCIE_IDLE log */
#define PCIE_IDLE_HISTORY_DEPTH	20
#define PCIE_IDLE_HISTORY_WIDTH	100
static char history_buf[PCIE_IDLE_HISTORY_DEPTH][PCIE_IDLE_HISTORY_WIDTH];
int
pcie_idle_record_history(dhd_bus_t *bus, const char *fmt, ...)
{
	static uint row = 0;
	uint column = 0;
	va_list args;
	...
	column = sprintf(history_buf[row],
		"%d %4d %5lu.%06lu cur R %d %d %d%d%d%d%d ",
		smp_processor_id(),
		current->pid,
		(unsigned long)nsec,
		rem_nsec / 1000,
		bus->bus_idle_current_req,
		bus->dhd->busstate,
		bus->bus_idle_is_suspended,
		bus->bus_idle_suspend_scheduled,
		bus->bus_idle_resume_scheduled,
		bus->bus_idle_suspend_now,
		bus->bus_idle_resume_now);

	va_start(args, fmt);
	vsnprintf(&history_buf[row][column], PCIE_IDLE_HISTORY_WIDTH - column, fmt, args);
	va_end(args);

	if (++row >= PCIE_IDLE_HISTORY_DEPTH) row = 0;
	return 0;
}

/* This func is to dump previously _DEPTH lines to kmsg before the calling point. */
void
pcie_idle_replay_history(void)
{
	int row = 0;
	DHD_CRITICAL(("%s:\n", __FUNCTION__));
	do {
		DHD_CRITICAL(("%s\n", history_buf[row]));
	} while (++row < PCIE_IDLE_HISTORY_DEPTH);
}

static void
pcie_bus_idle_watchdog_handler(ulong data)
{
	...
	if (dhd->dongle_reset) {
		DHD_ERROR(("%s: dongle is in reset\n", __FUNCTION__));
		return;
	}

	if (dhd->busstate == DHD_BUS_DATA && !bus->bus_idle_is_suspended &&
		bus->bus_idle_current_req == PCIE_BUS_IDLE_NONE) {
		DHD_IDLE(("%s: schedule deferred bus suspend\n", __FUNCTION__));
		ret = pcie_schedule_bus_idle_suspend(dhd->info, TRUE, TRUE);
	} else if (dhd->busstate != DHD_BUS_SUSPEND) {
		DHD_ERROR(("%s busstate is not DHD_BUS_DATA nor DHD_BUS_SUSPEND, "
			"reschedule bus suspend\n", __FUNCTION__));
		pcie_kick_bus_idle_watchdog(bus, 0, PCIE_BUS_IDLE_WD_RESCH);
	}
}

static int
pcie_init_bus_idle_watchdog(dhd_bus_t *bus)
{
	if (!bus) {
	...
	bus->pcie_idle_timer.data = (ulong)bus;
	bus->pcie_idle_timer.function = pcie_bus_idle_watchdog_handler;
	bus->pcie_idle_watchdog_ms = DEFAULT_IDLE_WATCHDOG_MS;
	bus->bus_idle_is_suspended = FALSE;
	bus->bus_idle_allowed = FALSE;
	bus->bus_idle_blocked = FALSE;
	bus->bus_idle_allowed_restore = TRUE;
	bus->bus_idle_processing = FALSE;
	bus->bus_idle_current_req = PCIE_BUS_IDLE_NONE;
	bus->bus_idle_suspend_scheduled = FALSE;
	bus->bus_idle_resume_scheduled = FALSE;
	bus->bus_idle_suspend_now = FALSE;
	bus->bus_idle_resume_now = FALSE;
	bus->bus_idle_is_roaming = FALSE;
	bus->bus_idle_timer_valid = FALSE;
#ifdef CONFIG_BBRY_MFG
	/* Disable bus idle suspend feature completely in MFI */
	bus->bus_idle_feature_enabled = FALSE;
#else
	if (dhd_get_interrupt_source() == WLAN_INTR_PME_L) {
		DHD_ERROR(("%s: feature disabled because PME_L is used as interrupt\n",
				__FUNCTION__));
		bus->bus_idle_feature_enabled = FALSE;
	} else {
		bus->bus_idle_feature_enabled = TRUE;
	}
#endif
	DHD_ERROR(("%s: bus idle suspend feature is %s\n", __FUNCTION__,
		bus->bus_idle_feature_enabled ? "enabled" : "disabled"));

	/* Initialize and start idle watchdog timer */
	init_timer(&bus->pcie_idle_timer);
	bus->bus_idle_timer_valid = TRUE;

	return BCME_OK;
}

static int
pcie_remove_bus_idle_watchdog(dhd_bus_t *bus)
{
	if (!bus) {
		DHD_ERROR(("%s: dhd_bus_t is NULL\n", __FUNCTION__));
		return BCME_ERROR;
	...
	if (bus->bus_idle_timer_valid) {
		del_timer_sync(&bus->pcie_idle_timer);
		bus->bus_idle_timer_valid = FALSE;
	}
...
int
pcie_kick_bus_idle_watchdog(dhd_bus_t *bus, uint custom_time, uint32 src)
{
	uint watchdog_extend_ms;
	...
	if (!bus->bus_idle_feature_enabled || !bus->bus_idle_allowed) {
		DHD_TRACE(("%s bus idle kick is disallowed!\n", __FUNCTION__));
		return BCME_OK;
	}

	watchdog_extend_ms = custom_time ? custom_time : bus->pcie_idle_watchdog_ms;

	/* Only kick watchdog if the bus is resumed */
	if (!bus->bus_idle_is_suspended){
		mod_timer(&bus->pcie_idle_timer,
					jiffies + msecs_to_jiffies(watchdog_extend_ms));
		bus->bus_idle_timer_valid = TRUE;
	} else{
		DHD_TRACE(("%s: Not kicking suspended bus (src::%d)\n",
					__FUNCTION__, src));
	}
...
int
pcie_execute_bus_idle_suspend(dhd_pub_t *dhdp, bool suspend)
{
	dhd_bus_t *bus = NULL;
	int ret = 0;
	...
	if (!bus->bus_idle_feature_enabled ||
		(suspend && !bus->bus_idle_allowed) ||
		(!suspend && !bus->bus_idle_allowed && !bus->bus_idle_is_suspended)) {
		DHD_ERROR(("%s bus idle suspend disallowed!\n", __FUNCTION__));
		return BCME_OK;
	}

	if (bus->bus_idle_processing) {
		pcie_idle_record_history(bus, "%s on-going suspend/resume, no-op (%d)",
			__FUNCTION__, suspend);
		DHD_ERROR(("%s on-going suspend/resume, no-op (%d)\n", __FUNCTION__, suspend));
		return BCME_OK;
	}

	if (suspend == bus->bus_idle_is_suspended) {
		DHD_TRACE(("%s Already %s, no-op\n", __FUNCTION__,
						suspend ? "suspended":"resumed"));
		return BCME_OK;
	}

	/* kill watchdog to prevent possible recursive call during suspend/resume */
	pcie_remove_bus_idle_watchdog(bus);

	bus->bus_idle_processing = TRUE;
	bus->bus_idle_current_req = suspend ? PCIE_BUS_IDLE_SUSPENDING : PCIE_BUS_IDLE_RESUMING;

	pcie_idle_record_history(bus, "%s : process  req %s", __FUNCTION__, suspend ? "SUSPEND":"RESUME");
	DHD_IDLE(("%s : process  req %s \n", __FUNCTION__, suspend ? "SUSPEND":"RESUME"));

	/* Suspend/resume wifi chipset */
	ret = dhdpcie_bus_suspend(bus, suspend);
	if (ret) {
		pcie_idle_record_history(bus, "Failed to %s PCIe bus", suspend ? "suspend":"resume");
		DHD_IDLE(("Failed to %s PCIe bus\n", suspend ? "suspend":"resume"));
		bus->bus_idle_is_suspended = suspend ? FALSE : TRUE;
		goto done;
	} else {
		DHD_IDLE(("%s %s %s_bus_done\n", PICE_IDLE_SUSPEND_TAG,
			suspend ? "0.5" : "1", suspend ? "suspend":"resume"));
		bus->bus_idle_is_suspended = suspend ? TRUE : FALSE;
	}

	/* refresh watchdog if we have resumed */
	if (!suspend && !bus->bus_idle_is_suspended) {
		pcie_kick_bus_idle_watchdog(bus, 0, PCIE_BUS_IDLE_WD_REFRESH);
		/* allow xmit from network layer now the
		 * scheduled bus idle operation has completed */
		dhd_bus_start_queue(bus);
	}
...
int
pcie_bus_idle_resume(dhd_pub_t *dhdp, uint32 src)
{
	struct dhd_info *dhd_info = dhdp->info;
	dhd_bus_t *bus = dhdp->bus;
	int ret = BCME_OK;

	DHD_IDLE(("enter : (src::%d), busstate %d suspended %d idle suspend %d req %d\n",
		src,
		dhdp->busstate,
		bus->suspended,
		bus->bus_idle_is_suspended,
	...
	if (!bus->bus_idle_feature_enabled) {
		/* Lots of spam if print below is enabled */
		ret = BCME_DISABLED;
		DHD_INFO(("%s: bus idle suspend feature is disabled (src::%d)\n", __FUNCTION__, src));
		goto done;
	}

	if ((dhdp->busstate == DHD_BUS_SUSPEND && bus->bus_idle_is_suspended) ||
		(bus->bus_idle_current_req == PCIE_BUS_IDLE_SUSPENDING)) {
		if (!in_atomic() && !in_interrupt()) {
			if (bus->bus_idle_current_req == PCIE_BUS_IDLE_SUSPENDING) {
				/* TODO - clean up repeated code */
				pcie_idle_record_history(bus, "%s 0 bus_snoozed_schedule_resume while suspending (src::%d)",
					PICE_IDLE_SUSPEND_TAG, src);
				DHD_ERROR(("%s 0 bus_snoozed_schedule_resume while suspending (src::%d)\n",
							PICE_IDLE_SUSPEND_TAG, src));
				ret = pcie_schedule_bus_idle_suspend(dhd_info, FALSE, TRUE);
				if (!ret) {
					/* Return dongle down to reflect present bus state, as the actual
					 * bus resume is to be carried out later (as scheduled work), asynchronously */
					ret = BCME_NOTREADY;
					goto done;
				} else {
					pcie_idle_record_history(bus, "%s: Failed to schedule resume (rc::%d)",
						__FUNCTION__, ret);
					DHD_ERROR(("%s: Failed to schedule resume (rc::%d)\n", __FUNCTION__, ret));
				}
			} else {
				/* Check for scheduled resumes that hasn't yet started execution */
				if (!bus->bus_idle_resume_scheduled) {
					pcie_idle_record_history(bus, "%s 0 bus_snoozed_resume_now (src::%d)",
						PICE_IDLE_SUSPEND_TAG, src);
					DHD_ERROR(("%s 0 bus_snoozed_resume_now (src::%d)\n",
							PICE_IDLE_SUSPEND_TAG, src));
					bus->bus_idle_resume_now = TRUE;
					ret = pcie_execute_bus_idle_suspend(dhdp, FALSE);
					if (ret) {
						pcie_idle_record_history(bus, "%s: Failed to initiate resume (rc::%d)",
							__FUNCTION__, ret);
						DHD_ERROR(("%s: Failed to initiate resume (rc::%d)\n", __FUNCTION__, ret));
					}
					bus->bus_idle_resume_now = FALSE;
				} else {
					pcie_idle_record_history(bus, "%s: Resume is pending, won't resume now (src::%d)",
						__FUNCTION__, src);
					DHD_ERROR(("%s: Resume is pending, won't resume now (src::%d)\n",
							__FUNCTION__, src));
					ret = BCME_NOTREADY;
					goto done;
				}
			...
			pcie_idle_record_history(bus, "%s 0 bus_snoozed_schedule_resume (src::%d)",
				PICE_IDLE_SUSPEND_TAG, src);
			DHD_ERROR(("%s 0 bus_snoozed_schedule_resume (src::%d)\n",
							PICE_IDLE_SUSPEND_TAG, src));
			ret = pcie_schedule_bus_idle_suspend(dhd_info, FALSE, TRUE);
			if (!ret) {
				/* Return dongle down to reflect present bus state, as the actual
		...
		if (bus->bus_idle_current_req == PCIE_BUS_IDLE_RESUMING) {
			pcie_idle_record_history(bus, "%s: Bus is resuming, stall (src::%d)", __FUNCTION__, src);
			DHD_ERROR(("%s: Bus is resuming, stall (src::%d)\n", __FUNCTION__, src));
			ret = BCME_NOTREADY;
			goto done;
		}
		else if ((dhdp->busstate == DHD_BUS_DATA || !bus->bus_idle_is_suspended) &&
			(bus->bus_idle_suspend_scheduled == true)) {
			/* This case is to cover the duration from a Suspend is scheduled to starting execuated.
			 * Sometimes the (Suspend) work could be scheduled to a very late time, e.g.,
			 *  - it is observed as one second later during device booting up
			 *  - it is not rare to see 10ms+ delay in normal operation
			 * If during that time an ioctl coming in, we use the caller's retry mechanism to do
			 * the backoff, E.g., in dhd_wl_ioctrl(), there are backoff retries as long as
			 * 	IOCTL_BUS_BUSY_WAIT_MS * IOCTL_BUS_BUSY_RETRY = 150ms.
			 * Will see if this will covery all the occurrences.
			 */
			pcie_idle_record_history(bus, "%s: To back off Resume, when Suspend is just scheduled (src::%d)",
				__FUNCTION__, src);
			DHD_ERROR(("%s: To back off Resume, when Suspend is just scheduled (src::%d)\n",
				__FUNCTION__, src));
			ret = BCME_NOTREADY;
			goto done;
		} else {
...
int
pcie_allow_bus_idle_suspend(dhd_pub_t *dhdp, bool allow)
{
	dhd_bus_t *bus = NULL;
	int ret = BCME_OK;
	...
	if (bus->bus_idle_blocked) {
		DHD_ERROR(("%s: Bus idle suspend is blocked\n", __FUNCTION__));
		bus->bus_idle_allowed_restore = allow;
		goto done;
	}

	if (bus->bus_idle_allowed == allow) {
		DHD_ERROR(("%s: Already %s\n", __FUNCTION__, allow ? "allowed" : "disallowed"));
		goto done;
	}

	bus->bus_idle_allowed = allow;

	/* check is here instead of at the beginning because we still
	 * want the flags to be manipulated in case feature is toggled
	 * during runtime */
	if (!bus->bus_idle_feature_enabled) {
		DHD_ERROR(("%s: Bus idle suspend feature is disabled\n", __FUNCTION__));
		goto done;
	}
	...
	if (allow) {
		pcie_kick_bus_idle_watchdog(bus, 0, PCIE_BUS_IDLE_WD_ALLOW_API);
	} else {
		/* Retry the idle resume call here to make sure we exit out
		 * of this function with an active bus */
		do {
			ret = pcie_bus_idle_resume(dhdp, PCIE_BUS_IDLE_RESUME_ALLOW_API);
			if (ret == BCME_NOTREADY) {
				if (!in_atomic() && !in_interrupt()){
					DHD_IDLE(("    bus is not ready, retry::%d\n", retry));
					OSL_SLEEP(IOCTL_BUS_BUSY_WAIT_MS);
				} else {
					DHD_ERROR(("    bus is not ready, drop resume call\n"));
					ret = BCME_NOTREADY;
					goto done;
				}
			} else {
...
int
pcie_block_bus_idle_suspend(dhd_pub_t *dhdp, bool enable)
{
	dhd_bus_t *bus = NULL;
	...
	if (enable == bus->bus_idle_blocked) {
		DHD_TRACE(("%s: Already blocked/unblocked, skip\n", __FUNCTION__));
		goto done;
	}

	if (enable) {
		bus->bus_idle_allowed_restore = bus->bus_idle_allowed;
		pcie_allow_bus_idle_suspend(dhdp, FALSE);
		bus->bus_idle_blocked = TRUE;
	} else {
		bus->bus_idle_blocked = FALSE;
		pcie_allow_bus_idle_suspend(dhdp, bus->bus_idle_allowed_restore);
	}
...
int
pcie_enable_bus_idle_suspend_feature(dhd_pub_t *dhdp, bool enable)
{
	dhd_bus_t *bus = NULL;
	int ret = 0;
	...
	if (!enable) {
		/* Ensure bus is resumed before disabling feature */
		ret = pcie_bus_idle_resume(dhdp, PCIE_BUS_IDLE_RESUME_ALLOW_API);
		if (ret == BCME_NOTREADY)
			return BCME_ERROR;
	}

	bus->bus_idle_feature_enabled = enable;
