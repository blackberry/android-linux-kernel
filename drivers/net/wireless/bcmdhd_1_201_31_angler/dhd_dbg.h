...
#ifdef CONFIG_BBRY
/*
 * Use this one to avoid ratelimit restriction.
 * Currently it's for PCIE_IDLE suspend/resume operations only.
 */
#define DHD_CRITICAL(args)	do {if (true) printf args;} while (0)
#endif /* CONFIG_BBRY */
...
#else
#define DHD_EVENT(args)		do {if ((dhd_msg_level & DHD_EVENT_VAL) && USE_NET_RATELIMIT) \
								printf args;} while (0)
#endif //CONFIG_BBRY
...
#ifdef CONFIG_BBRY
#ifdef BUS_IDLE_DEBUG
#define DHD_IDLE(args)      do {if (USE_NET_RATELIMIT) printf args;} while (0)
#else
#define DHD_IDLE(args)
#endif /* BUS_IDLE_DEBUG */
#endif /* CONFIG_BBRY */
