...
#ifdef CONFIG_BBRY
	/* A scan without wildcard or SSID's should be performed passively */
	if (n_ssids == 0) {
		params->scan_type |= WL_SCANFLAGS_PASSIVE;
		params->passive_time = 105 /* ms */;

		WL_SCAN(("Perform passive scan since no SSID's specified\n"));
		WL_SCAN(("scan_type=%#x\n", params->scan_type));
		WL_SCAN(("passive_time=%d\n", params->passive_time));
...
#ifdef CONFIG_BBRY
	if (err == BCME_NOTUP) {
		WL_ERR(("Scan err = (%d), device not up", err));
		err = -EBUSY;
	}
#endif
...
#ifdef CONFIG_BBRY
	WL_DBG(("Connecting to " MACDBG " with channel (%s) ssid %s\n",
	...
	struct regulatory_request *request)
{
	struct bcm_cfg80211 *cfg = (struct bcm_cfg80211 *)wiphy_priv(wiphy);
#ifndef CONFIG_BBRY
// JI:755829 - To be in line with kernel/include/net/cfg80211.h. Newer driver code will adopt this change.
	int ret = 0;
#endif
...
#ifdef CONFIG_BBRY
	wdev->wiphy->regulatory_flags |= WIPHY_FLAG_DISABLE_BEACON_HINTS;
#endif
#else
	wdev->wiphy->flags |= WIPHY_FLAG_CUSTOM_REGULATORY;
#ifdef CONFIG_BBRY
	wdev->wiphy->flags |= WIPHY_FLAG_DISABLE_BEACON_HINTS;
#endif
#endif

#ifdef CONFIG_BBRY
	wdev->wiphy->country_ie_pref |= NL80211_COUNTRY_IE_IGNORE_CORE;
#endif
...
out:
	...
	if(cfg->bss_list && cfg->bss_list->count == 0)
		WL_ERR(("No scan results!!! Scanned AP count is 0\n"));
#endif /* CONFIG_BBRY */
...
#ifdef CONFIG_BBRY
void wl_give_regulatory_hint(const char *alpha2)
{
    if (alpha2 == NULL) {
        return;
    }
