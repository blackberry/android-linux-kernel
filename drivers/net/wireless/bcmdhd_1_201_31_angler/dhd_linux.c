...
#endif
#ifdef CONFIG_BBRY
static void pcie_bus_idle_defer_handler(void *handle, void *event_info, u8 event);
#endif /* CONFIG_BBRY */
...
#ifdef CONFIG_BBRY
#if 0 /* TODO - come back to clean this up after feature is matured */
	/**
	 * drop packet if we are suspending, let all the other cases through
	 * to queue the packet in the flowring. This relaxation relies on the
	 * delay in queuing and execution to offset the time used to resume
	 * the pcie bus
	 *
	 * Impact of a stricter model is large amount of TX packet drop during
	 * bus resume
	 **/
	if (dhdp->bus &&
		(dhdp->bus->bus_idle_current_req == PCIE_BUS_IDLE_SUSPENDING ||
		(dhdp->busstate == DHD_BUS_SUSPEND))) {
		DHD_ERROR(("%s : pcie bus is suspending/ed, drop outgoing packet\n", __FUNCTION__));
		PKTFREE(dhdp->osh, pktbuf, TRUE);
		return -EBUSY;
	}
#endif
...
#ifdef PCIE_FULL_DONGLE
#ifdef CONFIG_BBRY
	/* Restore non bus idle suspend code path if feature is disabled/disallowed */
	if (dhdp->busstate == DHD_BUS_SUSPEND &&
		(!dhdp->bus->bus_idle_feature_enabled || !dhdp->bus->bus_idle_allowed)) {
#else
	if (dhdp->busstate == DHD_BUS_SUSPEND) {
#endif
...
#ifdef CONFIG_BBRY
	ret = pcie_bus_idle_resume(dhdp, PCIE_BUS_IDLE_RESUME_TX);
	if (ret == BCME_NOTREADY) {
		if (dhdp->bus->bus_idle_is_suspended &&
			(dhdp->bus->bus_idle_feature_enabled && dhdp->bus->bus_idle_allowed)) {
			DHD_ERROR(("%s: bus not ready, no xmit until bus resumes\n", __FUNCTION__));
...
#ifndef CONFIG_BBRY
	/*
	 * when the pcie bus causes the hang (timeout) this function will crash
	 * the kernel at this time.  Disabling until we can fix the crash
	 */
...
#ifdef CONFIG_BBRY
	if (ioc->cmd == WLC_GET_FIRMWARE_PATH) {
		if (data_buf != NULL) {
			strlcpy(data_buf, dhd->fw_path, buflen);
			DHD_INFO(("get firmware %s\n", (char *)data_buf));
			bcmerror = BCME_OK;
		} else {
			DHD_ERROR(("data_buf to get firmware path is null\n"));
			bcmerror = BCME_BADADDR;
		}
		goto done;
	}
	if (ioc->cmd == WLC_SET_FIRMWARE_PATH) {
		if (data_buf != NULL) {
			strlcpy(firmware_path, data_buf, sizeof(firmware_path));
			DHD_INFO(("set firmware %s\n", firmware_path));
			bcmerror = BCME_OK;
		} else {
			DHD_ERROR(("data_buf to set firmware path is null\n"));
			bcmerror = BCME_BADADDR;
		}
		goto done;
	}
...
dhd_ioctl_entry(struct net_device *net, struct ifreq *ifr, int cmd)
{
	...
	do {
		...
		ret = pcie_bus_idle_resume(&dhd->pub, PCIE_BUS_IDLE_RESUME_IOCTL_ENTRY);
		if (ret == BCME_NOTREADY) {
			if (!in_atomic() && !in_interrupt()){
				if (retry > 1) {
					...
					busy_cnt++;
					pcie_idle_record_history(dhd->pub.bus,
				...
				DHD_ERROR(("    bus is suspending, drop ioctl\n"));
...
dhd_stop(struct net_device *net)
{
	...
	dhd->pub.up = 0;
#ifdef CONFIG_BBRY
	pcie_idle_record_history(dhd->pub.bus, "%s, dhd->pub.up %d", __FUNCTION__, dhd->pub.up);
#endif /* CONFIG_BBRY */
...
bool dhd_update_fw_nv_path(dhd_info_t *dhdinfo)
{
	...
	int property_val_len = 0;

	/* Locate wlan_cfg node from DT */
	node = of_find_node_by_name(NULL, "wlan_cfg");
	if (node != NULL) {
		/* Retrieve requested property from DT node */
		property_val = of_get_property(node, "config", &property_val_len);
		if (property_val) {
			DHD_ERROR(("%s: Config property: %s\n", __func__, property_val));
		} else {
			DHD_ERROR(("%s: Failed to find property: config\n", __func__));
			property_val = NULL; /* just to be sure */
		}
	} else {
		DHD_ERROR(("%s: Failed to find DT node: wlan_cfg\n", __func__));
		property_val = NULL; /* just to be sure */
	}
#endif /* CONFIG_BBRY */
...
#ifdef CONFIG_BBRY
		if (property_val && property_val_len > 0)
			nv = property_val;
#endif /* CONFIG_BBRY */
...
#ifdef CONFIG_BBRY
		DHD_ERROR(("%s: Configured INI file: %s\n", __func__, dhdinfo->nv_path ? dhdinfo->nv_path : "NULL"));
#endif /* CONFIG_BBRY */
	}
...
#ifdef CONFIG_BBRY
	ret = dhd_iovar(dhd, 0, "stbc_rx", (char *)&stbc_rx, sizeof(stbc_rx), NULL, 0, TRUE);
	if (ret < 0)
		DHD_ERROR(("%s stbc_rx set failed %d\n", __FUNCTION__, ret));

	ret = dhd_iovar(dhd, 0, "spect", (char *)&spect, sizeof(spect), NULL, 0, TRUE);
	if (ret < 0)
		DHD_ERROR(("%s spect set failed %d\n", __FUNCTION__, ret));
#endif /* CONFIG_BBRY */
...
{
#ifdef CONFIG_BBRY
	DHD_IDLE(("dhd_ioctl_timeout_msec: %d -> %d by %pS\n",
		dhd_ioctl_timeout_msec, timeout_msec, __builtin_return_address(0)));
#endif /* CONFIG_BBRY */
...
dhd_net_bus_devreset(struct net_device *dev, uint8 flag)
{
	...
	if (flag == TRUE) {
#ifdef CONFIG_BBRY
		/* disallow bus idle before we powerdown */
		pcie_allow_bus_idle_suspend(&dhd->pub, FALSE);
#endif /* CONFIG_BBRY */
	...
	if (!flag) {
		if (!(dhd->pub.op_mode & DHD_FLAG_MFG_MODE)) {
			/* allow bus idle when operating in non mfg mode */
			pcie_allow_bus_idle_suspend(&dhd->pub, TRUE);
		} else {
			DHD_ERROR(("%s: Running MFG firmware, disable bus idle "
				"suspend\n", __FUNCTION__));
			pcie_enable_bus_idle_suspend_feature(&dhd->pub, FALSE);
		}
	}
#endif /* CONFIG_BBRY */
	return ret;
...
#ifdef WL_CFG80211
#ifdef CONFIG_BBRY
        /* Update kernel regulatory domain infrastructure with country code */
		wl_give_regulatory_hint(cspec->country_abbrev);
#endif /* CONFIG_BBRY */
...
#else
#define WAKELOCK_WIFI_MASK	(1<<0)
#define WAKELOCK_RXWAKE_MASK	(1<<1)
#define WAKELOCK_CTRLWAKE_MASK	(1<<2)
#define WAKELOCK_WDWAKE_MASK	(1<<3)
	if (dhd) {
		int flag = 0;
		if (wake_lock_active(&dhd->wl_wifi))
			flag |= WAKELOCK_WIFI_MASK;
		if (wake_lock_active(&dhd->wl_rxwake))
			flag |= WAKELOCK_RXWAKE_MASK;
		if (wake_lock_active(&dhd->wl_ctrlwake))
			flag |= WAKELOCK_CTRLWAKE_MASK;
		if (wake_lock_active(&dhd->wl_wdwake))
			flag |= WAKELOCK_WDWAKE_MASK;
#if 0
		DHD_ERROR(("%s: wl_wifi::%d wl_wdwake::%d wl_rxwake::%d wl_ctrlwake::%d\n",
				__FUNCTION__,
...
#ifdef CONFIG_BBRY
static void
pcie_bus_idle_defer_handler(void *handle, void *event_info, u8 event)
{
	dhd_info_t *dhd = handle;
	dhd_pub_t *pub = &dhd->pub;
	bool suspend = (event == DHD_WQ_WORK_BUS_SUSPEND) ? TRUE : FALSE;

	if (pub->bus->bus_idle_current_req != PCIE_BUS_IDLE_RESUMING) {
		if (!suspend && pub->bus->bus_idle_current_req == PCIE_BUS_IDLE_SUSPENDING) {
			/* Reschedule resume if bus is suspending */
			pcie_idle_record_history(pub->bus, "%s 0 bus_snoozed_deferred_resume while suspending",
				PICE_IDLE_SUSPEND_TAG);
			DHD_ERROR(("%s 0 bus_snoozed_deferred_resume while suspending\n", PICE_IDLE_SUSPEND_TAG));
			pub->bus->bus_idle_resume_scheduled = FALSE;
			pcie_schedule_bus_idle_suspend(dhd, FALSE, FALSE);
			goto done;
		} else {
			/* We don't check for scheduled resumes here becuase
			 * we are in a middle of a scheduled execution */
			if (suspend)
				pub->bus->bus_idle_suspend_now = TRUE;
			else
				pub->bus->bus_idle_resume_now = TRUE;

			pcie_execute_bus_idle_suspend(pub, suspend);

			if (suspend)
				pub->bus->bus_idle_suspend_now = FALSE;
			else
				pub->bus->bus_idle_resume_now = FALSE;
		}
	}

	if (suspend)
		dhd->pub.bus->bus_idle_suspend_scheduled = FALSE;
	else
		dhd->pub.bus->bus_idle_resume_scheduled = FALSE;

	/* allow xmit from network layer now the
	 * scheduled bus idle operation has completed */
	dhd_bus_start_queue(pub->bus);
...
int
pcie_schedule_bus_idle_suspend(dhd_info_t *dhd_info, bool suspend, bool high_priority)
{
	dhd_pub_t *pub = &dhd_info->pub;
	int ret = BCME_OK;

	pcie_idle_record_history(pub->bus, "%s: schedule bus %s", __FUNCTION__, suspend ? "SUSPEND":"RESUME");
	DHD_IDLE(("%s: schedule bus %s\n", __FUNCTION__, suspend ? "SUSPEND":"RESUME"));
	if ((!suspend && pub->bus->bus_idle_resume_scheduled) ||
		(suspend && pub->bus->bus_idle_suspend_scheduled) ||
		(!suspend && pub->bus->bus_idle_resume_now)) {
		pcie_idle_record_history(pub->bus, "%s: A scheduled request already exists, skip",
			__FUNCTION__);
		DHD_ERROR(("%s: A scheduled request already exists, skip\n", __FUNCTION__));
		goto done;
	}

	ret = dhd_deferred_schedule_work(dhd_info->dhd_deferred_wq,
				(void *)NULL,
				suspend ? DHD_WQ_WORK_BUS_SUSPEND : DHD_WQ_WORK_BUS_RESUME,
				pcie_bus_idle_defer_handler,
				high_priority ? DHD_WORK_PRIORITY_HIGH : DHD_WORK_PRIORITY_LOW);
	if (ret != DHD_WQ_STS_OK) {
		pcie_idle_record_history(pub->bus, "%s: Failed to schedule work (rc::%d)",
			 __FUNCTION__, ret);
		DHD_ERROR(("%s: Failed to schedule work (rc::%d)\n", __FUNCTION__, ret));
		goto done;
	} else {
		if (suspend)
			pub->bus->bus_idle_suspend_scheduled = TRUE;
		else
			pub->bus->bus_idle_resume_scheduled = TRUE;

		/* prevent xmit from network layer because
		 * a bus idle operation has been scheduled */
...
int
net_os_set_bus_idle(struct net_device *dev, bool allow)
{
	int ret = BCME_ERROR;
	dhd_info_t *dhd = DHD_DEV_INFO(dev);

	if (dhd)
		ret = pcie_allow_bus_idle_suspend(&dhd->pub, allow);

	return ret;
}

int
net_os_block_bus_idle(struct net_device *dev, bool block)
{
	int ret = BCME_ERROR;
	dhd_info_t *dhd = DHD_DEV_INFO(dev);

	if (dhd)
		ret = pcie_block_bus_idle_suspend(&dhd->pub, block);

	return ret;
}

int
dhd_get_interrupt_source(void)
{
	struct device_node *np;
	const char *intr_src = NULL;
	int intr_src_str_len = 0;

	np = of_find_compatible_node(NULL, NULL, "bcm,venice_wlan_cfg");
	if (np) {
		intr_src = of_get_property(np, "intr_src", &intr_src_str_len);
		if (intr_src) {
			DHD_TRACE(("%s: Wlan interrupt source: %s\n", __FUNCTION__, intr_src));
			if (strncmp(intr_src, "pme_l", sizeof("pme_l")) == 0)
				return WLAN_INTR_PME_L;
		} else {
			DHD_ERROR(("%s: Unable to find wlan interrupt source: intr_src\n",
				__FUNCTION__));
		}
	} else {
		DHD_ERROR(("%s: Unable to find wlan device tree node\n",
			__FUNCTION__));
	}
	return WLAN_INTR_GPIO0;
}
#endif /* CONFIG_BBRY */
