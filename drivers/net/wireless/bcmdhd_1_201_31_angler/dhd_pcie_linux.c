...
static int dhdpcie_set_suspend_resume(struct pci_dev *pdev, bool state)
{
	...
	if (bus) {
		if (!bus->bus_idle_feature_enabled || !bus->bus_idle_allowed) {
			if (bus->bus_idle_current_req == PCIE_BUS_IDLE_NONE) {
				...
				DHD_ERROR(("%s pcie bus is being suspended/resumed, return error\n",
					__FUNCTION__));
				ret = -EBUSY;
			}
		} else {
			if (state &&
				bus->dhd->busstate == DHD_BUS_SUSPEND && bus->bus_idle_is_suspended){
				DHD_ERROR(("%s already suspended, bypass PM suspend call\n",
					__FUNCTION__));
			} else if (!state &&
						bus->dhd->busstate == DHD_BUS_DATA && !bus->bus_idle_is_suspended) {
				DHD_ERROR(("%s already resumed, bypass PM resume call\n",
					__FUNCTION__));
			} else if (!state &&
						bus->bus_idle_current_req == PCIE_BUS_IDLE_SUSPENDING) {
				DHD_ERROR(("%s cannot resume suspending bus, return error\n",
					__FUNCTION__));
				/* TODO - may want to schedule a resume? */
				ret = -EBUSY;
			} else if (state &&
						bus->bus_idle_current_req == PCIE_BUS_IDLE_RESUMING) {
				DHD_ERROR(("%s cannot suspend resuming bus, return error\n",
					__FUNCTION__));
				ret = -EBUSY;
			} else {
				/* requested state != current state, issue idle call */
				DHD_ERROR(("%s mismatch state, call idle API\n", __FUNCTION__));
				/* Check for scheduled operations that hasn't yet started execution,
				 * which would escape all the test conditions above */
				if (!bus->bus_idle_resume_scheduled) {
					bus->bus_idle_resume_now = TRUE;
					ret = pcie_execute_bus_idle_suspend(bus->dhd, state);
...
#else
	DHD_ERROR(("%s %s wake_irq %d\n",
		__FUNCTION__, dhdpcie_info->pciname, dhdpcie_info->wake_irq));
#endif /* CONFIG_BBRY */
...
#ifdef CONFIG_BBRY
			if (noti->data == NULL) {
				DHD_ERROR(("%s: noti->data null pointer!\n", __FUNCTION__));
			} else {
				struct msm_pcie_notify_data *data = (struct msm_pcie_notify_data *) noti->data;
				switch (data->type) {
					case MSM_PCIE_NOTIFY_DATA_LINKDOWN:
						wlan_ddt_event_send(WLAN_DRV_EVENT_PCIE_LINKDOWN);
						break;
					case MSM_PCIE_NOTIFY_DATA_AER:
						/* This is for AER but we don't send ddt from here */
						DHD_ERROR(("%s: MSM_PCIE_NOTIFY_DATA_AER received.\n", __FUNCTION__));
						break;
					default:
						DHD_ERROR(("%s: Invalid data type %d.\n", __FUNCTION__, data->type));
				}
			}
#endif /* CONFIG_BBRY */
		}
...
#ifdef CONFIG_BBRY
void dhdpcie_wakeup_cb(struct msm_pcie_notify *noti)
{
	struct pci_dev *pdev = (struct pci_dev *)noti->user;
	dhdpcie_info_t *pch = NULL;
	dhd_bus_t *bus;
		...
		if ((bus = pch->bus) && (dhd = bus->dhd)) {
			if (bus->bus_idle_feature_enabled && bus->bus_idle_allowed) {
				if (noti->data == NULL) {
					...
					struct msm_pcie_notify_data *data = (struct msm_pcie_notify_data *) noti->data;
					switch(data->type) {
						case MSM_PCIE_NOTIFY_DATA_WAKEUP:
							pcie_bus_idle_resume(dhd, PCIE_BUS_IDLE_RESUME_WAKE_CB);
							break;
						default:
							DHD_ERROR(("%s: invalid data type %d.\n", __FUNCTION__, data->type));
					}
...
void dhdpcie_event_handler_cb(struct msm_pcie_notify *noti)
{
	switch (noti->event) {
		case MSM_PCIE_EVENT_LINKDOWN:
			dhdpcie_linkdown_cb(noti);
			break;
		case MSM_PCIE_EVENT_WAKEUP:
			dhdpcie_wakeup_cb(noti);
			break;
		default:
			DHD_ERROR(("%s: Unexpected PCIE events\n",
				__FUNCTION__));
			break;
	}
}
...
#ifdef CONFIG_BBRY
		bus->pcie_event.events = MSM_PCIE_EVENT_LINKDOWN | MSM_PCIE_EVENT_WAKEUP;
#else
...
#ifdef CONFIG_BBRY
		bus->pcie_event.callback = dhdpcie_event_handler_cb;
#else
