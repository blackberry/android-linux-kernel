...
#ifdef CONFIG_BBRY
#define DEBUG_P2P_DISCONNECT 0

static struct net_device* get_ndev_from_attr(const void  *data,
	struct bcm_cfg80211 *cfg)
{
	...
	type = nla_type(data);
	if (type == BBRY_ATTRIBUTE_CMD_INTERFACE) {
		use_p2p_intf = nla_get_u32(data);
	} else {
		WL_ERR(("Unable to get BBRY_ATTRIBUTE_CMD_INTERFACE\n"));
		goto Error;
	}
	WL_ERR(("%s p2p interface\n", use_p2p_intf ? "Using" : "NOT using"));

	/* Retrieve net device from P2P's connection interface */
	if (use_p2p_intf)
		return wl_to_p2p_bss_ndev(cfg, P2PAPI_BSSCFG_CONNECTION);

Error:
	return bcmcfg_to_prmry_ndev(cfg);
}

#if DEBUG_P2P_DISCONNECT
...
static int wl_cfgvendor_bbry_get_fw_ver(struct wiphy *wiphy,
	struct wireless_dev *wdev, const void  *data, int len)
{
	...
	err = wldev_iovar_getbuf(bcmcfg_to_prmry_ndev(cfg), "ver", NULL, 0,
		cfg->ioctl_buf, WLC_IOCTL_MAXLEN, NULL);
	if (unlikely(err)) {
		WL_ERR(("wldev_iovar_getbuf error (%d)\n", err));
		return err;
	}
	data_len = strlen(cfg->ioctl_buf);
	cfg->ioctl_buf[data_len] = '\0';
...
static int wl_cfgvendor_bbry_get_p2p_macmode(struct wiphy *wiphy,
	struct wireless_dev *wdev, const void *data, int len)
{
	struct bcm_cfg80211 *cfg = NULL;
	int ret = -1;
	...
	ndev = get_ndev_from_attr(data, cfg);
	if (!ndev) {
		WL_ERR(("ndev is NULL!\n"));
		goto Error;
	}

	ret = wldev_ioctl(ndev, WLC_GET_MACMODE, &macmode, sizeof(macmode), false);
	if (unlikely(ret)) {
		WL_ERR(("wldev_ioctl WLC_GET_MACMODE error (%d)\n", ret));
		return ret;
	}

	ret = wl_cfgvendor_send_cmd_reply(wiphy, ndev, &macmode, sizeof(macmode));
	if (unlikely(ret))
		WL_ERR(("get_macmode command reply failed ret:%d\n", ret));

Error:
	return ret;
}

static int wl_cfgvendor_bbry_set_p2p_macmode(struct wiphy *wiphy,
	struct wireless_dev *wdev, const void *data, int len)
{
	struct bcm_cfg80211 *cfg = NULL;
	int ret = -1;
	...
	nla_for_each_attr(iter, data, len, tmp) {
		type = nla_type(iter);
		switch (type) {
		case BBRY_ATTRIBUTE_CMD_INTERFACE:
			ndev = get_ndev_from_attr(data, cfg);
			break;
		case BBRY_ATTRIBUTE_MACMODE:
			macmode = nla_get_u32(iter);
			break;
		default:
			WL_ERR(("%s: No such attribute %d\n", __func__, type));
			break;
	...
	ret = wldev_ioctl(ndev, WLC_SET_MACMODE, &macmode, sizeof(macmode), true);
	if (unlikely(ret))
		WL_ERR(("wldev_ioctl WLC_SET_MACMODE error (%d)\n", ret));

Error:
	return ret;
}

static int wl_cfgvendor_bbry_set_p2p_maclist(struct wiphy *wiphy,
	struct wireless_dev *wdev, const void *data, int len)
{
	struct bcm_cfg80211 *cfg = NULL;
	int ret = -1;
	...
	const struct nlattr *iter;
	struct net_device *ndev;
	int macmode = WLC_MACMODE_DENY;

	uint32 mem_needed = 0, flush = 0;
	int maclist_idx = 0;
	maclist_t *blacklist = NULL;
	...
	/* Get existing list first and append new bssid during the
	 * parsing of NL attributes below (order matters) */
	bzero(buf, WLC_IOCTL_MAXLEN);

	ret = wldev_ioctl(ndev, WLC_GET_MACLIST, buf, WLC_IOCTL_MAXLEN, false);
	if (unlikely(ret)) {
		WL_ERR(("wldev_ioctl WLC_GET_MACLIST error (%d)\n", ret));
		goto Error;
	}

	blacklist = maclist = (struct maclist *) buf;

	if (maclist->count >= 1) {
		maclist_idx = (maclist->count - 1);
		mem_needed = sizeof(maclist_t) +
					sizeof(struct ether_addr) * maclist_idx;
		WL_ERR(("Existing p2p maclist count is :: %d (%d bytes)\n",
				maclist->count, mem_needed));
	} else {
		WL_ERR(("Existing p2p maclist is empty\n"));
	}
	...
	nla_for_each_attr(iter, data, len, tmp) {
		...
		switch (type) {
		case BBRY_ATTRIBUTE_CMD_INTERFACE:
			/* consume quietly, parsed in get_ndev_from_attr() */
			break;
		case BBRY_ATTRIBUTE_MACLIST_FLUSH:
			flush = nla_get_u32(iter);
			if (flush)
				macmode = WLC_MACMODE_DISABLED;
			break;
		case BBRY_ATTRIBUTE_MACLIST_BSSID:
			if (!flush) {
				maclist->count++;
				maclist_idx = maclist->count - 1;
				mem_needed = sizeof(maclist_t) +
						sizeof(struct ether_addr) * maclist_idx;
				memcpy(&(maclist->ea[maclist_idx]),
						nla_data(iter), ETHER_ADDR_LEN);
				WL_ERR(("New p2p maclist count is :: %d (%d bytes)\n",
						maclist->count, mem_needed));
			}
			break;
		default:
	...
	if (!flush && blacklist) {
		ret = wldev_ioctl(ndev, WLC_SET_MACLIST, blacklist, mem_needed, true);
		if (unlikely(ret)) {
			WL_ERR(("wldev_ioctl WLC_SET_MACLIST error (%d)\n", ret));
			goto Error;
		}
	}

	/* set/clear macmode based on flush attribute */
	ret = wldev_ioctl(ndev, WLC_SET_MACMODE, &macmode, sizeof(macmode), true);
	if (unlikely(ret)) {
		WL_ERR(("wldev_ioctl WLC_SET_MACMODE error (%d)\n", ret));
	}
...
static int wl_cfgvendor_bbry_set_apsd(struct wiphy *wiphy,
	struct wireless_dev *wdev, const void *data, int len)
{
	struct bcm_cfg80211 *cfg = NULL;
	int ret = -1;
	...
	nla_for_each_attr(iter, data, len, tmp) {
		...
		switch (type) {
		case BBRY_ATTRIBUTE_CMD_INTERFACE:
			ndev = get_ndev_from_attr(iter, cfg);
			break;
		case BBRY_ATTRIBUTE_APSD_ENABLE:
			enable = nla_get_u32(iter);
			break;
		default:
			WL_ERR(("%s: No such attribute %d\n", __func__, type));
			break;
	...
	val = 1;
	ret = wldev_ioctl(ndev, WLC_DOWN, &val, sizeof(val), true);
	if (unlikely(ret)) {
		WL_ERR(("WLC_DOWN error %d\n", ret));
		goto Error;
	}
	WL_ERR(("wldev_iovar_setint wme_apsd (%d)\n", enable));
	ret = wldev_iovar_setint(ndev, "wme_apsd", enable);
	if (unlikely(ret)) {
		WL_ERR(("wldev_iovar_setint wme_apsd error (%d)\n", ret));
		goto Error;
	}
	ret = wldev_ioctl(ndev, WLC_UP, &val, sizeof(val), true);
	if (unlikely(ret)) {
...
static const struct wiphy_vendor_command wl_vendor_cmds [] = {
	...
	},
	{
		{
			.vendor_id = OUI_GOOGLE,
			.subcmd = BBRY_SUBCMD_GET_FW_VERSION
		},
		.flags = WIPHY_VENDOR_CMD_NEED_WDEV | WIPHY_VENDOR_CMD_NEED_NETDEV,
		.doit = wl_cfgvendor_bbry_get_fw_ver
	},
	{
		{
			.vendor_id = OUI_GOOGLE,
			.subcmd = BBRY_SUBCMD_GET_MACMODE
		},
		.flags = WIPHY_VENDOR_CMD_NEED_WDEV | WIPHY_VENDOR_CMD_NEED_NETDEV,
		.doit = wl_cfgvendor_bbry_get_p2p_macmode
	},
	{
		{
			.vendor_id = OUI_GOOGLE,
			.subcmd = BBRY_SUBCMD_SET_MACMODE
		},
		.flags = WIPHY_VENDOR_CMD_NEED_WDEV | WIPHY_VENDOR_CMD_NEED_NETDEV,
		.doit = wl_cfgvendor_bbry_set_p2p_macmode
	},
	{
		{
			.vendor_id = OUI_GOOGLE,
			.subcmd = BBRY_SUBCMD_SET_MACLIST
		},
		.flags = WIPHY_VENDOR_CMD_NEED_WDEV | WIPHY_VENDOR_CMD_NEED_NETDEV,
		.doit = wl_cfgvendor_bbry_set_p2p_maclist
	},
	{
		{
			.vendor_id = OUI_GOOGLE,
			.subcmd = BBRY_SUBCMD_SET_APSD
		},
		.flags = WIPHY_VENDOR_CMD_NEED_WDEV | WIPHY_VENDOR_CMD_NEED_NETDEV,
		.doit = wl_cfgvendor_bbry_set_apsd
	},
#endif /* CONFIG_BBRY */
};
