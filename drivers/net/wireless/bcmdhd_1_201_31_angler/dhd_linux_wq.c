...
struct dhd_deferred_wq {
	 ...
	 */
#ifdef CONFIG_BBRY
#define DHD_PRIO_WORK_FIFO_SIZE	(10 * sizeof(struct dhd_deferred_event_t))
#else
...
#else
	status = schedule_work((struct work_struct *)deferred_wq);
	return status ? DHD_WQ_STS_OK : DHD_WQ_STS_SCHED_FAILED;
#endif
}
