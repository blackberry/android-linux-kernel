...
#ifdef CONFIG_BBRY
				pcie_block_bus_idle_suspend(dhd, TRUE);
#endif
#ifdef PKT_FILTER_SUPPORT
...
#ifdef CONFIG_BBRY
				pcie_block_bus_idle_suspend(dhd, FALSE);
#endif
			}
	}
...
static int dhd_suspend_resume_helper(struct dhd_info *dhd, int val, int force)
{
		...
		dhd_support_sta_mode(dhdp))
	{
		...
		/* in case dhd_set_suspend is not called
		 * we still need to block idle suspend */
		pcie_block_bus_idle_suspend(dhdp, val);
	}
#endif /* CONFIG_BBRY */
...
pcie_schedule_bus_idle_suspend(dhd_info_t *dhd_info, bool suspend, bool high_priority)
{
	...
	if ((!suspend && pub->bus->bus_idle_resume_scheduled) ||
		...
		DHD_ERROR(("%s: On-going bus request, skip\n", __FUNCTION__));
		goto done;
	}
