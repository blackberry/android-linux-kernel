...
int
net_os_set_bus_idle_feature_enabled(struct net_device *dev, bool enable)
{
	int ret = BCME_ERROR;
	dhd_info_t *dhd = DHD_DEV_INFO(dev);

	if (dhd)
		ret = pcie_enable_bus_idle_suspend_feature(&dhd->pub, enable);

	return ret;
}

bool
net_os_get_bus_idle_feature_enabled(struct net_device *dev)
{
	bool ret = FALSE;
	dhd_info_t *dhd = DHD_DEV_INFO(dev);

	if (dhd)
		ret = pcie_is_bus_idle_suspend_feature_enabled(&dhd->pub);
