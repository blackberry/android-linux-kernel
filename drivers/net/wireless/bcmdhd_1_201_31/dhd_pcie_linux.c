...
int dhdpcie_get_resource(dhdpcie_info_t *dhdpcie_info)
{
	...
	do {
		...
		dhdpcie_info->tcm_size = (bar1_size < DONGLE_TCM_MAP_SIZE) ? bar1_size : DONGLE_TCM_MAP_SIZE;
	...
	if (pdev) {
		...
		if (pch) {
			...
			if (bus) {
				...
				if (dhdp && bus->bus_idle_feature_enabled && bus->bus_idle_allowed) {
					pcie_bus_idle_resume(dhdp, PCIE_BUS_IDLE_RESUME_WAKE_CB);
				}
			}
		}
	}
...
static void dhd_pcie_create_flow_worker(struct work_struct *worker)
{
	...
	while (skb) {
		...
		if (BCME_OK != dhd_flowid_update(bus->dhd, ifidx, bus->dhd->flow_prio_map[(PKTPRIO(skb))], skb)) {
...
int dhdpcie_init(struct pci_dev *pdev)
{
	...
	do {
		...
		/* Bus initialization */
		bus = dhdpcie_bus_attach(osh, dhdpcie_info->regs, dhdpcie_info->tcm, dhdpcie_info->tcm_size);
		if (!bus) {
