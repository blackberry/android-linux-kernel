...
#ifdef CONFIG_BBRY
#define CMD_SET_BUSIDLE      		"SETBUSIDLESUSPEND"
#define CMD_BLOCK_BUSIDLE    		"BLOCKBUSIDLESUSPEND"
#define CMD_SET_BUSIDLEFEATURE		"SETBUSIDLEFEATURE"
#define CMD_GET_BUSIDLEFEATURE		"GETBUSIDLEFEATURE"
#endif /* CONFIG_BBRY */
...
#ifdef CONFIG_BBRY
static int wl_android_set_busidle(struct net_device *dev, char *command, int total_len)
{
	int ret = 0;
	int allow_flag;

	allow_flag = *(command + strlen(CMD_SET_BUSIDLE) + 1) - '0';
	if (allow_flag != 0)
		allow_flag = 1;

	if (!(ret = net_os_set_bus_idle(dev, allow_flag ? TRUE:FALSE)))
		DHD_INFO(("%s: Bus Idle allowed %d\n", __FUNCTION__, allow_flag));
	else
		DHD_ERROR(("%s: failed %d\n", __FUNCTION__, ret));
...
static int wl_android_block_busidle(struct net_device *dev, char *command, int total_len)
{
	int ret = 0;
	int block_flag;

	block_flag = *(command + strlen(CMD_BLOCK_BUSIDLE) + 1) - '0';
	if (block_flag != 0)
		block_flag = 1;

	if (!(ret = net_os_block_bus_idle(dev, block_flag ? TRUE:FALSE)))
		DHD_ERROR(("%s: Bus Idle blocked %d\n", __FUNCTION__, block_flag));
	else
		DHD_ERROR(("%s: failed %d\n", __FUNCTION__, ret));
...
static int wl_android_set_busidlefeature(struct net_device *dev, char *command, int total_len)
{
	int ret = 0;
	int enable_flag;

	enable_flag = *(command + strlen(CMD_SET_BUSIDLEFEATURE) + 1) - '0';
	if (enable_flag != 0)
		enable_flag = 1;

	ret = net_os_set_bus_idle_feature_enabled(dev, enable_flag ? TRUE : FALSE);
	if (!ret)
		DHD_ERROR(("%s: Bus Idle Feature enabled %d\n", __FUNCTION__, enable_flag));
	else
		DHD_ERROR(("%s: failed %d\n", __FUNCTION__, ret));
...
static int wl_android_get_busidlefeature(struct net_device *dev, char *command, int total_len)
{
	bool enabled;

	enabled = net_os_get_bus_idle_feature_enabled(dev);
	/* Note: changes were also made to wpa_driver_nl80211_driver_command
	 * in order for this result string to be passed through the supplicant.
	 * Otherwise the result was always overridden to be `OK`.
	 */
	return snprintf(command, total_len, enabled ? "1" : "0");
}
#endif /* CONFIG_BBRY */
...
#ifdef CONFIG_BBRY
	else if (strnicmp(command, CMD_SET_BUSIDLE, strlen(CMD_SET_BUSIDLE)) == 0) {
		bytes_written = wl_android_set_busidle(net, command, priv_cmd.total_len);
	}
	else if (strnicmp(command, CMD_BLOCK_BUSIDLE, strlen(CMD_BLOCK_BUSIDLE)) == 0) {
		bytes_written = wl_android_block_busidle(net, command, priv_cmd.total_len);
	} else if (strnicmp(command, CMD_SET_BUSIDLEFEATURE, strlen(CMD_SET_BUSIDLEFEATURE)) == 0) {
		bytes_written = wl_android_set_busidlefeature(net, command, priv_cmd.total_len);
	} else if (strnicmp(command, CMD_GET_BUSIDLEFEATURE, strlen(CMD_GET_BUSIDLEFEATURE)) == 0) {
		bytes_written = wl_android_get_busidlefeature(net, command, priv_cmd.total_len);
	}
#endif /* CONFIG_BBRY */
	else {
