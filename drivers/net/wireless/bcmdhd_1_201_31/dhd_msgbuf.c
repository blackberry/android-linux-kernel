...
#ifdef CONFIG_BBRY
	if (!p) {
		DHD_ERROR(("Unable to alloc memory for packet\n"));
		goto alloc_err_return;
	}
#endif
...
dhd_prot_process_msgbuf_rxcpl(dhd_pub_t *dhd)
{
	...
	while (TRUE) {
		...
		if (dhd->hang_was_sent) {
			DHD_ERROR(("%s: found hang, aborting\n", __func__));
			break;
		}
...
dhd_process_msgtype(dhd_pub_t *dhd, msgbuf_ring_t *ring, uint8* buf, uint16 len)
{
	...
	while (pktlen > 0) {
...
#ifdef CONFIG_BBRY
		/*
		 * catch the case where the bus goes down while waiting for it complete
		 */
		if (dhd->hang_was_sent) {
			DHD_ERROR(("FW hang detected - %s\n", __func__));
			ret = BCME_ERROR;
			goto done;
		}
#endif /* CONFIG_BBRY */
		...
		if (msgtype >= DHD_PROT_FUNCS) {
			DHD_ERROR(("%s: msgtype = %d, aborting processing\n", __func__, msgtype));
			ret = BCME_ERROR;
			goto done;
		}
#endif /* CONFIG_BBRY */
...
#if defined(BCMPCIE)
#ifdef CONFIG_BBRY
		pcie_kick_bus_idle_watchdog(dhd->bus, 0, PCIE_BUS_IDLE_WD_TX_BUF_OUT);
#endif /* CONFIG_BBRY */
...
#ifdef CONFIG_BBRY
/* TODO: May not need this anymore now that we stop queue at dhd_start_xmit(),
 * too risky to change, leaving it here for now */
if (!dhd->bus->bus_idle_in_host_mem_access) {
	if (dhd->bus->bus_idle_current_req != PCIE_BUS_IDLE_NONE ||
		dhd->bus->bus_idle_is_suspended) {
		DHD_ERROR(("  pcie bus is not ready, re-queue packet\n"));
...
#ifdef CONFIG_BBRY
	pcie_kick_bus_idle_watchdog(dhd->bus, 0, PCIE_BUS_IDLE_WD_TX_BUF_IN);
#endif /* CONFIG_BBRY */
...
#ifdef CONFIG_BBRY
	if (dhd->bus->bus_idle_feature_enabled && dhd->bus->bus_idle_allowed) {
		if (dhd->bus &&
			!dhd->bus->bus_idle_in_host_mem_access &&
			dhd->bus->bus_idle_current_req != PCIE_BUS_IDLE_NONE) {
			DHD_ERROR(("%s : pcie is in the middle of suspending/resuming\n", __FUNCTION__));
			ret = BCME_BUSY;
			goto done;
		}
	}
...
#ifdef CONFIG_BBRY
	DHD_INFO(("%s Send Flow create Req msglen flow ID %d for peer %s prio %d ifindex %d\n",
...
#ifdef CONFIG_BBRY
	/* Re-enable bus idle suspend if this
	 * flow ring creation is a result of roaming */
	if (dhd->bus->bus_idle_is_roaming) {
		pcie_allow_bus_idle_suspend(dhd, TRUE);
		dhd->bus->bus_idle_is_roaming = FALSE;
	}
#endif
}
