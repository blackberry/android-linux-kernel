		...
		while (size) {
			if (size >= sizeof(uint64) && little_endian){
				...
				if (unlikely(is_64bit_unaligned)) {
					DHD_ERROR(("%s: write unaligned %lx\n",
...
#else
	volatile uint8 data = *(volatile uint8 *)(bus->tcm + dhd_bus_cmn_check_offset(bus, offset));
#endif
	return data;
}
...
#else
	volatile uint16 data = *(volatile uint16 *)(bus->tcm + dhd_bus_cmn_check_offset(bus, offset));
#endif
	return data;
}
...
#else
	volatile uint32 data = *(volatile uint32 *)(bus->tcm + dhd_bus_cmn_check_offset(bus, offset));
#endif
	return data;
}
...
#else
	volatile uint64 data = *(volatile uint64 *)(bus->tcm + dhd_bus_cmn_check_offset(bus, offset));
#endif
	return data;
}
...
dhdpcie_bus_suspend(struct  dhd_bus *bus, bool state)
{
	...
	if (state) {
		...
		if (bus->wait_for_d3_ack == 1) {
			...
			bus->suspended = FALSE;
			bus->dhd->busstate = DHD_BUS_DATA;
			/* force to enable host memory access */
			dhdpcie_ctrl_hostmem_access(bus, TRUE);
			rc = -ETIMEDOUT;
...
int
dhdpcie_ctrl_hostmem_access(struct dhd_bus *bus, bool set)
{
	dhd_pub_t *dhd;
	uint8 iovbuf[32];
	int val = set ? 0 : 1;
	dhd = bus->dhd;
	if (!dhd) {
		DHD_ERROR(("%s: dhd_pub_t is NULL\n", __FUNCTION__));
		return BCME_ERROR;
	}
	DHD_INFO(("%s: hostmem access %s\n",
		__FUNCTION__, set ? "enabled" : "disabled"));
	memset(iovbuf, 0, sizeof(iovbuf));
	bcm_mkiovar("bus:sim_host_enter_d3", (char *)&val, 4,
		(char *)iovbuf, sizeof(iovbuf));
#ifdef CONFIG_BBRY
	bus->bus_idle_in_host_mem_access = TRUE;
#endif /* CONFIG_BBRY */
	if (dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf,
		sizeof(iovbuf), TRUE, 0) < 0) {
		DHD_ERROR(("%s: failed to set sim_host_enter_d3\n",
			__FUNCTION__));
#ifdef CONFIG_BBRY
...
pcie_execute_bus_idle_suspend(dhd_pub_t *dhdp, bool suspend)
{
	...
	if (suspend) {
		bus->bus_idle_is_suspended = TRUE;
	}
...
pcie_enable_bus_idle_suspend_feature(dhd_pub_t *dhdp, bool enable)
{
	...
	/* JI:1340544 Drop call if already enabled/disabled */
	if (enable == bus->bus_idle_feature_enabled) {
		DHD_TRACE(("%s: Already enabled/disabled, skip\n", __FUNCTION__));
		goto done;
	}
...
bool
pcie_is_bus_idle_suspend_feature_enabled(dhd_pub_t *dhdp)
{
	if (!dhdp) {
		DHD_ERROR(("%s: dhdp is null!\n", __FUNCTION__));
		return FALSE;
	...
	return dhdp->bus->bus_idle_feature_enabled;
}
#endif /* CONFIG_BBRY */
