...
wcnss_dt_read_mac_addr (void) {
	...
	if (!node) {
		...
		if (!macp) {
			...
			rc = -1;
		} else if (mac_len != WLAN_MAC_ADDR_SIZE) {
			...
			memcpy(penv->wlan_nv_macAddr, macp, WLAN_MAC_ADDR_SIZE);
			pr_info("%s: " MAC_ADDRESS_STR "\n", __func__,
