...
struct uid_stat {
	...
	uid_t uid;
	atomic_unchecked_t tcp_rcv;
	atomic_unchecked_t tcp_snd;
};
...
static struct uid_stat *create_stat(uid_t uid) {
	...
	atomic_set_unchecked(&new_uid->tcp_rcv, INT_MIN);
	atomic_set_unchecked(&new_uid->tcp_snd, INT_MIN);
...
int uid_stat_tcp_snd(uid_t uid, int size) {
	...
	if (!entry)
		return -1;
	atomic_add_unchecked(size, &entry->tcp_snd);
	return 0;
}
...
int uid_stat_tcp_rcv(uid_t uid, int size) {
	...
	if (!entry)
		return -1;
	atomic_add_unchecked(size, &entry->tcp_rcv);
	return 0;
}
