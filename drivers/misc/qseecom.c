...
#ifdef CONFIG_BBRY
		/* JI:981359
		 * QC does not expect any TA to be loaded before the init of the
		 * qseecom driver. But it is the case for the BB TA which is first
		 * loaded by the bootloader. Add an exception here to bypass this
		 * check.
		 */
		static const char bbta_name[] = "blackber";
		if (0 == memcmp((char *)data->client.app_name, bbta_name,
				sizeof(bbta_name))) {
			pr_info("app_id %d (%s) is not found (IGNORED)\n",
					data->client.app_id,
					(char *)data->client.app_name
					);
		} else {
...
#ifdef CONFIG_BBRY
				struct qseecom_check_app_ireq bb_app = {QSEOS_APP_LOOKUP_COMMAND, "blackber"};
				if (__qseecom_check_app_exists(bb_app) > 0) {
					pr_info("secapp region already set by LK\n");
					qseecom.commonlib_loaded = true;
				} else {
#endif
