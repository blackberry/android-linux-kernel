...
#define VFSSPI_WAKE_TIME   (5 * HZ)//[FEATURE]-Add-by TCTNB.XQJ,2016/01/20, task 1019320,wake system from sleep

static LIST_HEAD(device_list);
static DEFINE_MUTEX(device_list_mutex);
static struct class *vfsspi_device_class;
static int gpio_irq;
static int ocp_irq;/*TCT-NB Tianhongwei add for bad chip ocp*/
...
/*[FEATURE]-Add-BEGIN by TCTNB.XQJ,2016/01/20, task 1019320,wake system from sleep*/
...
/*[FEATURE]-End-by TCTNB.XQJ*/
/*TCT-NB Tianhongwei add for bad chip ocp defect.1988334 2016.4.26*/
...
static void vfsspi_wake_unlock_timer_handler(unsigned long ptr)
{
    ...
    pr_debug("%s: enter\n", __func__);
    vfsspi_enableIrq(data);//[FEATURE]-Add-by TCTNB.XQJ,2016/01/20, bug 1969935,fingerpirnt no irq in sometimes
...
static irqreturn_t vfsspi_irq(int irq, void *context)
{
	...
	if (gpio_get_value(vfsspi_device->drdy_pin) == DRDY_ACTIVE_STATUS) {
		schedule_work(&vfsspi_device->irq_worker);//[FEATURE]-Add-by TCTNB.XQJ,2016/01/20, task 1019320,wake system from sleep
	}

	return IRQ_HANDLED;
}
/*TCT-NB Tianhongwei add for bad chip ocp defect.1988334 2016.4.26*/
static irqreturn_t vfsspi_ocpirq(int irq, void *context)
{


	struct vfsspi_device_data *vfsspi_device = context;
	mdelay(2);
	printk(KERN_ERR"vfsspi_ocpirq \n");
	if(gpio_get_value(vfsspi_device->ocp_pin)==0)
	{
		pr_err("vfsspi ocp irq low \n");
		gpio_set_value(vfsspi_device->vdd18_en,0);
	}
//	else
//	{
//		pr_err("vfsspi ocp high \n");
...
static int vfsspi_sendDrdyEventFd(struct vfsspi_device_data *vfsSpiDev)
{
	...
	pr_info("vfsspi_sendDrdyEventFd\n");
    if (vfsSpiDev->user_pid != 0) {
        ...
        if (efd_file == NULL) {
            pr_info("No such efd_file\n");
            return -ENODEV;
        }
        
        efd_ctx = eventfd_ctx_fileget(efd_file);
        if (efd_ctx == NULL) {
            pr_info("eventfd_ctx_fileget is failed\n");
            return -ENODEV;
        }
...
static int vfsspi_enableIrq(struct vfsspi_device_data *vfsspi_device)
{
	...
	vfsspi_device->is_drdy_irq_enabled = DRDY_IRQ_ENABLE;
        ...
        if (enable_irq_wake(gpio_irq)) {//[FEATURE]-Add-by TCTNB.XQJ,2016/01/20, task 1019320,wake system from sleep
...
static int vfsspi_disableIrq(struct vfsspi_device_data *vfsspi_device)
{
	...
	if (disable_irq_wake(gpio_irq)) {//[FEATURE]-Add-by TCTNB.XQJ,2016/01/20, task 1019320,wake system from sleep
			       ...
			       unsigned long arg)
{
	...
	if (drdy_enable_flag == 0){
			...
			vfsspi_wake_lock_delayed_unlock(vfsspi_device);//[FEATURE]-Add-by TCTNB.XQJ,2016/01/20, task 1019320,wake system from sleep
...
/*[BUGFIX]-MOd-by TCTNB.XQJ,2016/03/25, bug 1762168,wake system from sleep*/
		gpio_set_value(vfsspi_device->vdd18_en,1);
		gpio_set_value(vfsspi_device->vdd33_en,1);
/*[BUGFIX]-MOd-by TCTNB.XQJ*/
        mdelay(5);/*[BUGFIX]-MOd-by TCTNB.XQJ,2016/05/25, bug 2202615,avoid potential risk when finger sensor power on and off  is switched high frequency*/
...
static void vfsspi_init_gpio(struct vfsspi_device_data* _vfsspi_device_data){
	struct gpio all_gpio [] = {
		...
		{_vfsspi_device_data->ocp_pin , GPIOF_DIR_IN  , "ocp_irq_gpio"},/*TCT-NB Tianhongwei add for bad chip ocp*/
...
/*TCT-NB Tianhongwei add for bad chip ocp defect.1988334 2016.4.26*/
static int vfsspi_pinctrl_init(struct device *dev, struct vfsspi_device_data *data)
{
	int retval = 0;

	data->pinctrl = devm_pinctrl_get(dev);
	if (IS_ERR_OR_NULL(data->pinctrl)) {
		retval = PTR_ERR(data->pinctrl);
		goto err_state;
	}
	data->gpio_state_default =
		pinctrl_lookup_state(data->pinctrl,
				"gpio_state_default");
	if (IS_ERR_OR_NULL(data->gpio_state_default)) {
		retval = PTR_ERR(data->gpio_state_default);
		goto err_state;
	}

	retval = pinctrl_select_state(data->pinctrl, data->gpio_state_default);

	return retval;

err_state:
	data->gpio_state_default = NULL;
	return retval;
}
/*TCT-NB Tianhongwei end defect.1988334 2016.4.26*/
...
/*TCT-NB Tianhongwei add for bad chip ocp defect.1988334 2016.4.26*/
	gpio = of_get_named_gpio(np, "vfsspi,vfsspi-ocp-pin", 0);
	if (gpio < 0) {
		gpio = errorno;
		goto dt_exit;
	} else {
...
static int vfsspi_probe(struct platform_device *spi)
{
	...
	vfsspi_pinctrl_init(&spi->dev, vfsspi_device);/*TCT-NB Tianhongwei add for bad chip ocp defect.1988334 2016.4.26*/
...
/*TCT-NB Tianhongwei add for bad chip ocp defect.1988334 2016.4.26*/
	status = gpio_direction_input(vfsspi_device->ocp_pin);
	if (status < 0) {
		pr_err("gpio_direction_input ocp_pin failed\n");
		status = -EBUSY;
		goto vfsspi_probe_gpio_init_failed;
	}

	ocp_irq = gpio_to_irq(vfsspi_device->ocp_pin);
	...
	if (request_irq(ocp_irq, vfsspi_ocpirq, IRQF_TRIGGER_FALLING | IRQF_TRIGGER_LOW,
			"vfsspi-ocpirq", vfsspi_device) < 0) {
		pr_err("request_irq failed\n");
		status = -EBUSY;
		goto vfsspi_probe_irq_failed;
	}
...
/*[FEATURE]-MOd-by TCTNB.XQJ,2016/01/20, task 1019320,wake system from sleep*/
    /* init wake lock */
...
/*[FEATURE]-End-by TCTNB.XQJ*/
	...
	pr_err("vfs Debug : %s vfsspi_device->is_opened = %d******\n",__func__,vfsspi_device->is_opened);
	pr_err("vfsspi_probe successful");
