...
/* MODIFIED-BEGIN by hongwei.tian, 2016-04-26,BUG-1999268*/
/*TCT_NB Tianhongwei add for CR 968643*/
static void register_status_debug(struct spmi_pmic_arb_dev *pmic_arb)
{
	...
	for (apid = 0; apid < pmic_arb->max_periph_intrs; ++apid)
		dev_err(pmic_arb->dev, "IRQ Status[0x%llx]: 0x%x, ACC enabled[0x%llx]: 0x%x\n",
				(unsigned long long)pmic_arb->ver->irq_status(apid), readl_relaxed(pmic_arb->intr + pmic_arb->ver->irq_status(apid)),
	...
	for (i=0; i<= 6; i++) {
		...
		dev_err(pmic_arb->dev, "Acc Status[0x%llx]: 0x%x\n",
			(unsigned long long)pmic_arb->ver->owner_acc_status(i, j), readl_relaxed(pmic_arb->intr + pmic_arb->ver->owner_acc_status(i, j)));
		}
	}
}
/*TCT_NB Tianhongwei add for CR 968643  end*/
/* MODIFIED-END by hongwei.tian,BUG-1999268*/
static irqreturn_t
periph_interrupt(struct spmi_pmic_arb_dev *pmic_arb, u8 apid, bool show)
{
	...
	if (!(status & SPMI_PIC_ACC_ENABLE_BIT)) {
		 ...
		 */
		/* MODIFIED-BEGIN by hongwei.tian, 2016-04-26,BUG-1999268*/
		pr_err("interrupts from this peripheral are disabled status=%x\n", status);
		register_status_debug(pmic_arb); /*TCT_NB Tianhongwei add for CR 968643*/
		/* MODIFIED-END by hongwei.tian,BUG-1999268*/
		return IRQ_HANDLED;
	}
...
__pmic_arb_periph_irq(int irq, void *dev_id, bool show)
{
	...
	u32 ret = IRQ_NONE;
	u32 status;
	static u32 cnt = 0;/*TCT_NB Tianhongwei add for CR 968643*/
...
static int spmi_pmic_arb_probe(struct platform_device *pdev)
{
	...
	pmic_arb_clear_spmi(pmic_arb);/*TCT_NB Tianhongwei add for CR 968643*/
