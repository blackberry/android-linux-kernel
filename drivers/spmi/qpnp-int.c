...
struct q_perip_data {
	...
	uint8_t use_count;
	spinlock_t lock; /*TCT_NB Tianhongwei add for CR 968643*/
};
...
static void qpnpint_irq_mask(struct irq_data *d)
{
	...
	int rc;
	uint8_t prev_int_en;/*TCT_NB Tianhongwei add for CR 968643*/
	...
	spin_lock(&per_d->lock);/*TCT_NB Tianhongwei add for CR 968643*/
	prev_int_en = per_d->int_en;/*TCT_NB Tianhongwei add for CR 968643*/
	...
	spin_unlock(&per_d->lock);/*TCT_NB Tianhongwei add for CR 968643*/
					...
					unsigned long hwirq)
{
	...
	if (!per_d) {
		...
		spin_lock_init(&per_d->lock);/*TCT_NB Tianhongwei add for CR 968643*/
