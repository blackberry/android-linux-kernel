...
/*[FEATURE]-ADD-BEGIN TCTNB.Yubin, 05/26/2018, take hotplug can set by app*/
#ifdef CONFIG_MERCURY
static struct mmc_host *hotplug_host;
static ssize_t sdcard_hotplug_show(struct device *dev, struct device_attribute *attr, char *buf)
{
        return 0;
}
static ssize_t sdcard_hotplug_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{

        mmc_gpio_free_cd(hotplug_host);

        gpio_direction_output(133, 0);
        mdelay(100);
        gpio_direction_output(133, 1);
        mdelay(100);
        gpio_direction_input(133);
        mdelay(20);
        mmc_gpio_request_cd(hotplug_host,133,0);
        mdelay(20);
        mmc_gpiod_request_cd_irq(hotplug_host);

        pr_info("mmc gpio 133 hotplug finish\n");

        return count;
}


static DEVICE_ATTR(hotplug_enable, 0644, sdcard_hotplug_show, sdcard_hotplug_store);
static struct class * sdcard_device_class;
static struct device * sdcard_hotplug_dev;
static int sdcard_hotplug_flag = 0;
static void sdcard_class_device_register(void)
{
        int rc = 0;
        if (sdcard_hotplug_flag == 0)
        {
                sdcard_hotplug_flag = 1;
        ...
        sdcard_hotplug_dev = device_create(sdcard_device_class, NULL, 0, NULL, "sdcard_hotplug");
        if (IS_ERR(sdcard_hotplug_dev))
                pr_err("Failed to create device(sdcard_hotplug)!\n");

        device_create_file(sdcard_hotplug_dev, &dev_attr_hotplug_enable);
        if ( rc < 0)
                pr_err("Failed to create device file(%s)!\n", dev_attr_hotplug_enable.attr.name);
        }
}
...
/* MODIFIED-BEGIN by bin.yu, 2018-05-26,BUG-6291161*/
/*[FEATURE]-ADD-BEGIN TCTNB.Yubin, 05/26/2018, take hotplug can set by app*/
#ifdef CONFIG_MERCURY
        if (pdev->dev.of_node) {
                if (of_alias_get_id(pdev->dev.of_node, "sdhc") == 2)
                {
                        hotplug_host = msm_host->mmc;
                        sdcard_class_device_register();
                }
        }
#endif
/*[FEATURE]-ADD-END TCTNB.Yubin*/
/* MODIFIED-END by bin.yu,BUG-6291161*/
