...
struct sdhci_msm_host {
	...
	u32 caps_0;
	#ifdef CONFIG_BBRY
	u8 *tlmm_sdc2_hdrv_pull_ctl; /*register to change sd drive strength*/
	#endif
};
...
#ifdef CONFIG_BBRY
		if (host->mmc)
			atomic_long_inc(&host->mmc->ctrs.pwr_irq_timeout);
	}
#endif
					...
					unsigned int uhs)
{
	...
	#ifdef CONFIG_BBRY
	/*set the SD controller drive strength based on the uhs mode*/
	if (host->mmc->card && mmc_card_sd(host->mmc->card)) {
		u32 strength;

		if (msm_host->tlmm_sdc2_hdrv_pull_ctl != NULL) {

			/*power off case*/
			if (host->mmc->ios.power_mode == MMC_POWER_OFF)
				strength = 0xa00; /*2mA,2mA,2mA*/
			else {
				switch (uhs) {
				...
				case MMC_TIMING_UHS_SDR25:
					strength = 0x1fd2; /*16mA,6mA,6mA*/
					break;
				case MMC_TIMING_UHS_SDR50:
				case MMC_TIMING_UHS_DDR50:
					strength = 0x1fdb; /*16mA,8mA,8mA*/
					break;
				case MMC_TIMING_UHS_SDR104:
					strength = 0x1fe4; /*16mA,10mA,10mA*/
					break;
				default:
					pr_debug("ignoring %s: %s-clock:%u uhs mode:%u"
						"ctrl_2:0x%x\n", mmc_hostname(host->mmc), __func__,
						host->clock, uhs, ctrl_2);
					strength = 0x1fd2; /*16mA,6mA,6mA*/
					break;
				}
			}

			writel_relaxed(strength, msm_host->tlmm_sdc2_hdrv_pull_ctl);
		}
	}
	#endif
...
static int sdhci_msm_probe(struct platform_device *pdev)
{
	...
	#ifdef CONFIG_BBRY
	msm_host->tlmm_sdc2_hdrv_pull_ctl = ioremap(0xFD512048, 16);
	#endif
...
static int sdhci_msm_remove(struct platform_device *pdev)
{
	...
	#ifdef CONFIG_BBRY
	if (msm_host->tlmm_sdc2_hdrv_pull_ctl != NULL)
		iounmap(msm_host->tlmm_sdc2_hdrv_pull_ctl);
	#endif
...
#endif
#ifdef CONFIG_BBRY
void sdhci_msm_set_sdc2_hdrv_pull_ctl(struct sdhci_host *host) {
	struct sdhci_pltfm_host *pltfm_host = sdhci_priv(host);
	struct sdhci_msm_host *msm_host = pltfm_host->priv;
	if (msm_host->tlmm_sdc2_hdrv_pull_ctl != NULL) {
		writel_relaxed(0xa00, msm_host->tlmm_sdc2_hdrv_pull_ctl);
	}
	return;
}
EXPORT_SYMBOL(sdhci_msm_set_sdc2_hdrv_pull_ctl);
#endif
