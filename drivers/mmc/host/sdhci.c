 ...
 *
 *  Copyright (C) 2015-2016 BlackBerry Limited
...
#ifdef CONFIG_BBRY
#define MAX_LOG_INDEX (1 << 2) /* must be 2^n */
struct sdhci_history {
	u32 opcode;
	u32 arg;
	u32 length;
	u32 start_time;
...
static struct sdhci_history emmc_log[MAX_LOG_INDEX];
static struct sdhci_history sdcard_log[MAX_LOG_INDEX];
static u32 emseq  = MAX_LOG_INDEX - 1;
static u32 sdseq  = MAX_LOG_INDEX - 1;
static u32 emmc_cmd_cnt;
static u32 sdcard_cmd_cnt;

static inline u32 get_timestamp(void);
static inline int is_sdcard(struct sdhci_host *host);
static void sdhci_log_history(struct sdhci_host *host, u32 opcode,
	u32 arg, u32 length);
static int sdhci_dump_history(struct sdhci_host *host);
...
static void sdhci_log_history(struct sdhci_host *host, u32 opcode,
	u32 arg, u32 length)
{
	struct sdhci_history *hp;

	if (is_sdcard(host)) {
		/* point to the next oldest slot */
		sdseq = (sdseq + 1) & (MAX_LOG_INDEX - 1);
		hp = &sdcard_log[sdseq];
		sdcard_cmd_cnt++;
	} else {
		emseq = (emseq + 1) & (MAX_LOG_INDEX - 1);
		hp = &emmc_log[emseq];
		emmc_cmd_cnt++;
	}
	hp->opcode     = opcode;
...
static u32  count_print;
struct check_dup {
	u32  last_printed_cmd_cnt;
	u32  last_opcode;
	u32  last_arg;
	u32  last_length;
};
static struct check_dup mmc_port[MAX_PORTS];
...
static int sdhci_dump_history(struct sdhci_host *host)
{
	...
	sdcard = is_sdcard(host);
	if (sdcard) {
		seq = sdseq;

		index = (seq + MAX_LOG_INDEX - 1 ) & (MAX_LOG_INDEX - 1);
		if ((sdcard_log[index].opcode == mmc_port[SD_PORT].last_opcode) &&
		    (sdcard_log[index].arg    == mmc_port[SD_PORT].last_arg) &&
		    (sdcard_log[index].length == mmc_port[SD_PORT].last_length) &&
		    (abs(sdcard_cmd_cnt  - mmc_port[SD_PORT].last_printed_cmd_cnt) < 8)) {
			return 1;
		} else if ((count_print > MAX_PRINTS) &&
			   (abs(sdcard_cmd_cnt  - mmc_port[SD_PORT].last_printed_cmd_cnt) < 8)) {
			return 1;
		}

		pr_info("SDcard: total %u transactions, recent cmd:\n",
			sdcard_cmd_cnt);
		mmc_port[SD_PORT].last_opcode          = sdcard_log[index].opcode;
		mmc_port[SD_PORT].last_arg             = sdcard_log[index].arg;
		mmc_port[SD_PORT].last_length          = sdcard_log[index].length;
		mmc_port[SD_PORT].last_printed_cmd_cnt = sdcard_cmd_cnt;
	} else {
		seq = emseq;

		index = (seq + MAX_LOG_INDEX - 1 ) & (MAX_LOG_INDEX - 1);
		if ((emmc_log[index].opcode == mmc_port[eMMC_PORT].last_opcode) &&
		    (emmc_log[index].arg    == mmc_port[eMMC_PORT].last_arg) &&
		    (emmc_log[index].length == mmc_port[eMMC_PORT].last_length) &&
		    (abs(emmc_cmd_cnt  - mmc_port[eMMC_PORT].last_printed_cmd_cnt) < 8)) {
			return 1;
		} else if ((count_print > MAX_PRINTS) &&
			   (abs(emmc_cmd_cnt  - mmc_port[eMMC_PORT].last_printed_cmd_cnt) < 8)) {
			return 1;
		...
		pr_info("emmc: total %u transactions, recent cmd:\n",
			emmc_cmd_cnt);
		mmc_port[eMMC_PORT].last_opcode          = emmc_log[index].opcode;
		mmc_port[eMMC_PORT].last_arg             = emmc_log[index].arg;
		mmc_port[eMMC_PORT].last_length          = emmc_log[index].length;
		mmc_port[eMMC_PORT].last_printed_cmd_cnt = emmc_cmd_cnt;
	}

	++count_print;
	if (count_print > (MAX_PRINTS + 3))
		count_print = 0;

	for (i = 0; i < MAX_LOG_INDEX; i++) {
		seq = (seq + 1) & (MAX_LOG_INDEX - 1);
		if (sdcard)
			hp = &sdcard_log[seq];
		else
			hp = &emmc_log[seq];

		pr_info("cmd %d arg 0x%x len %d %ums at %u\n",
			hp->opcode,
			hp->arg,
			hp->length,
			hp->end_time ?
				(hp->end_time -	hp->start_time) :
				(get_timestamp() - hp->start_time),
			hp->start_time);
	}
	return 0;
}

struct first_error_debug {
	u32 cmd;
	u32 len;
	u32 cmd_err;
	u32 data_err;
...
#define FIRST_ERR_CLEAR		1000

static u8 do_once_sd = true, do_once_mmc = true;
static struct first_error_debug s_dev[MAX_DEV];
static u32 last_sd_error, last_mmc_error;
/* keep printing first recorded error along with any error debug printing to counter dmesg overflow */
void mmc_debug_first_error(struct mmc_command cmd, struct mmc_data data, char *dname)
{
	u8 eMMC_SD_ERR = SD_ERR;

	if (!dname)
		return;
	/* check if error is for SD or eMMC */
	if (!strncmp(dname, "mmcblk0", sizeof("mmcblk0")))
		eMMC_SD_ERR = MMC_ERR;
	/* if error happens after FIRST_ERR_CLEAR successful transactions rerecord the first error */
	if ((abs(emmc_cmd_cnt - last_mmc_error) > FIRST_ERR_CLEAR) && (eMMC_SD_ERR == MMC_ERR) && (do_once_mmc == false))
		do_once_mmc = true;

	if ((abs(sdcard_cmd_cnt - last_sd_error) > FIRST_ERR_CLEAR) && (eMMC_SD_ERR == SD_ERR) && (do_once_sd == false))
		do_once_sd = true;
	/* keep updating last error variable to determine number of error free transactions in sequence */
	if (eMMC_SD_ERR)
		last_mmc_error = emmc_cmd_cnt;
	else
		last_sd_error = sdcard_cmd_cnt;
	/* record error parameters once for FIRST_ERR_CLEAR successful transactions */
	if (((eMMC_SD_ERR == MMC_ERR) && (do_once_mmc == true)) || ((eMMC_SD_ERR == SD_ERR) && (do_once_sd == true))) {
		s_dev[eMMC_SD_ERR].cmd = cmd.opcode;
		s_dev[eMMC_SD_ERR].len = (data.blocks * data.blksz);
		s_dev[eMMC_SD_ERR].cmd_err = cmd.error;
		s_dev[eMMC_SD_ERR].data_err = data.error;

		if (eMMC_SD_ERR == MMC_ERR) {
			do_once_mmc = false;
			s_dev[eMMC_SD_ERR].last_cmd_cnt = (emmc_cmd_cnt-1);
		} else {
			do_once_sd = false;
			s_dev[eMMC_SD_ERR].last_cmd_cnt = (sdcard_cmd_cnt-1);
		}
	}
	/* keep printing for every error */
	pr_info("dev->%s seq: %d cmd: %d len: %d cmd_err: %d data_err: %d\n",
		((eMMC_SD_ERR == MMC_ERR) ? "MMC" : "SD"),
		s_dev[eMMC_SD_ERR].last_cmd_cnt,
		s_dev[eMMC_SD_ERR].cmd,
		s_dev[eMMC_SD_ERR].len,
		s_dev[eMMC_SD_ERR].cmd_err,
		s_dev[eMMC_SD_ERR].data_err);
...
{
#ifdef CONFIG_BBRY
	if (sdhci_dump_history(host)) {
		return;
	}

	sdhci_dump_state(host);

	if (host->runtime_suspended || sdhci_check_state(host) || host->mmc->clk_gated) {
		pr_info(DRIVER_NAME "NO REGISTER DUMP IN BAD STATE (%s)\n",
	...
	pr_info(DRIVER_NAME ": ===========================================\n");
#else
	pr_info(DRIVER_NAME ": Sys addr: 0x%08x | Cmd:  0x%08x\n",
		sdhci_readl(host, SDHCI_DMA_ADDRESS),
		sdhci_readw(host, SDHCI_COMMAND));
	pr_info(DRIVER_NAME ": Argument: 0x%08x | Trn mode: 0x%08x\n",
		sdhci_readl(host, SDHCI_ARGUMENT),
	...
	pr_info(DRIVER_NAME ": Present:  0x%08x | Host ctl: 0x%08x\n",
		sdhci_readl(host, SDHCI_PRESENT_STATE),
		sdhci_readb(host, SDHCI_HOST_CONTROL));
	pr_info(DRIVER_NAME ": Power:    0x%08x | Clock:  0x%08x\n",
		sdhci_readb(host, SDHCI_POWER_CONTROL),
		sdhci_readw(host, SDHCI_CLOCK_CONTROL));
	pr_info(DRIVER_NAME ": Timeout:  0x%08x | Int stat: 0x%08x\n",
		sdhci_readb(host, SDHCI_TIMEOUT_CONTROL),
	...
	pr_info(DRIVER_NAME ": Int enab: 0x%08x | Sig enab: 0x%08x\n",
		sdhci_readl(host, SDHCI_INT_ENABLE),
		sdhci_readl(host, SDHCI_SIGNAL_ENABLE));
	pr_info(DRIVER_NAME ": AC12 err: 0x%08x | Host ctl2: 0x%08x\n",
		host->auto_cmd_err_sts,
		sdhci_readw(host, SDHCI_HOST_CONTROL2));
#endif
}
...
#ifdef CONFIG_BBRY
	if (cmd->data)
		length = cmd->data->blksz * cmd->data->blocks;
	else
		length = 0;

	sdhci_log_history(host, cmd->opcode, cmd->arg, length);
#endif
}
...
static void sdhci_finish_command(struct sdhci_host *host)
{
	...
	if (host->cmd == host->mrq->sbc) {
		...
		if (is_sdcard(host))
			sdcard_log[sdseq].end_time = get_timestamp();
		else
			emmc_log[emseq].end_time = get_timestamp();
#endif
		host->cmd = NULL;
	}
}
...
#ifdef CONFIG_BBRY
extern void sdhci_msm_set_sdc2_hdrv_pull_ctl(struct sdhci_host *host);
#endif
...
#ifdef CONFIG_BBRY
	if ((ios->power_mode == MMC_POWER_OFF) && (host->mmc->card && mmc_card_sd(host->mmc->card)))
		sdhci_msm_set_sdc2_hdrv_pull_ctl(host);
#endif
...
#ifdef CONFIG_BBRY
			if (host->mmc)
				atomic_long_inc(&host->mmc->ctrs.data_request_timeout);
#endif
			sdhci_finish_data(host);
		} else {
			if (host->cmd)
...
#ifdef CONFIG_BBRY
			if (host->mmc)
				atomic_long_inc(&host->mmc->ctrs.cmd_request_timeout);
#endif
