...
static ssize_t  mmchealth_show(struct device *dev,
				  struct device_attribute *attr, char *buf)
{
	struct mmc_blk_data *md = mmc_blk_get(dev_to_disk(dev));
	struct mmc_card *card;
	...
	ret = get_emmc_health(card->host, buf);
	mmc_release_host(card->host);
	mmc_rpm_release(card->host, &card->dev);
	mmc_blk_put(md);
	return ret;
...
static ssize_t  mmchealth_store(struct device *dev,
				 struct device_attribute *attr,
				 const char *buf, size_t count)
{
...
void add_mmchealth_to_sysfs(struct mmc_blk_data *md)
{
	mmc_health.show = mmchealth_show;
	mmc_health.store = mmchealth_store;
