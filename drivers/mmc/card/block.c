...
void mmc_debug_dump(struct mmc_host *host)
{
	if (host->parent)
		sdhci_dumpregs(dev_get_drvdata(host->parent));
}
static	struct wake_lock assd_wakelock;
#endif
	...
	unsigned int cmd, unsigned long arg)
{
	...
	if (cmd == MMC_IOC_CMD)
		...
		case SDMMC_ASSD_GET_CARD_INFO:
			...
			ret = sdmmc_assd_ioctl(card->host, cmd, arg);
			mmc_release_host(card->host);
			mmc_rpm_release(card->host, &card->dev);
			mmc_blk_put(md);
			break;
		case SDMMC_ASSD_CARD_POWER:
			ret = copy_from_user((u8 *)&assd_pm, (u8 __user *)arg, sizeof(assd_pm));
			if (ret)
				break;
			switch (assd_pm) {
			case ASSD_POWER_HOLD:
				wake_lock(&assd_wakelock);
				break;
			case ASSD_POWER_RELEASE:
				wake_unlock(&assd_wakelock);
				break;
			case ASSD_POWER_RESET:
				md = mmc_blk_get(bdev->bd_disk);
				if (!md)
					return -EINVAL;
				card = md->queue.card;
			     ...
			     struct mmc_async_req *areq)
{
	...
	if (brq->sbc.error || brq->cmd.error || brq->stop.error ||
	    brq->data.error) {
#ifdef CONFIG_BBRY
		mmc_debug_first_error(brq->cmd, brq->data, req->rq_disk->disk_name);
		mmc_debug_dump(card->host);
#endif
...
struct device_attribute mmc_health;
static ssize_t mmc_health_show(struct device *dev,
				  struct device_attribute *attr, char *buf)
{
	struct mmc_blk_data *md = mmc_blk_get(dev_to_disk(dev));
	struct mmc_card *card;
	...
	ret = get_emmc_health(card->host, buf, PAGE_SIZE);
	mmc_release_host(card->host);
	mmc_rpm_release(card->host, &card->dev);
	mmc_blk_put(md);
...
static ssize_t  mmc_health_store(struct device *dev,
				 struct device_attribute *attr,
				 const char *buf, size_t count)
{
	dev_warn(dev, "%s Attempted to modify read-only attribute %s\n",
			__func__, attr->attr.name);
	return -EPERM;
}

struct device_attribute mmc_simple_health;
static ssize_t mmc_simple_health_show(struct device *dev,
				  struct device_attribute *attr, char *buf)
{
	struct mmc_blk_data *md = mmc_blk_get(dev_to_disk(dev));
	struct mmc_card *card;
	...
	ret = get_emmc_simple_health(card->host, buf, PAGE_SIZE);
	mmc_release_host(card->host);
	mmc_rpm_release(card->host, &card->dev);
	mmc_blk_put(md);
...
void register_mmchealth_to_sysfs(struct mmc_blk_data *md)
{
#ifdef CONFIG_BBRY_DEBUG
	mmc_health.show = mmc_health_show;
	mmc_health.store = mmc_health_store;
	sysfs_attr_init(&mmc_health.attr);
	mmc_health.attr.name = "mmchealth";
	mmc_health.attr.mode = S_IRUGO;
	device_create_file(disk_to_dev(md->disk), &mmc_health);
#endif

	mmc_simple_health.show = mmc_simple_health_show;
	mmc_simple_health.store = mmc_health_store;
	sysfs_attr_init(&mmc_simple_health.attr);
	mmc_simple_health.attr.name = "mmc_simple_health";
	mmc_simple_health.attr.mode = (S_IRUSR | S_IRGRP);
	device_create_file(disk_to_dev(md->disk), &mmc_simple_health);
...
#ifdef CONFIG_BBRY
	if (mmc_card_mmc(card))
		register_mmchealth_to_sysfs(md);
#endif /* CONFIG_BBRY */
...
static int __init mmc_blk_init(void)
{
	...
	if (res)
		goto out2;
#ifdef CONFIG_BBRY
	wake_lock_init(&assd_wakelock, WAKE_LOCK_SUSPEND, "assd");
#endif
	return 0;
 out2:
...
{
#ifdef CONFIG_BBRY
	wake_lock_destroy(&assd_wakelock);
#endif
