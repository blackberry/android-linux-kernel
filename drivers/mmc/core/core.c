...
#ifdef CONFIG_BBRY
			if (host->card && host->card->part_curr < ARRAY_SIZE(host->part_ctrs)) {
				struct ctrs_part *part = &host->part_ctrs[host->card->part_curr];
				if (mrq->data->flags == MMC_DATA_READ)
					atomic_long_add(mrq->data->bytes_xfered, &part->read);
				else if (mrq->data->flags == MMC_DATA_WRITE)
					atomic_long_add(mrq->data->bytes_xfered, &part->write);
			}
#endif
...
{
#ifdef CONFIG_BBRY
	/* check the arguments as this function can be called from outside */
	if (host && mrq) {
...
#ifdef CONFIG_BBRY
	if (card->part_curr < ARRAY_SIZE(card->host->part_ctrs)) {
		part = &card->host->part_ctrs[card->part_curr];
		switch (arg) {
		case MMC_DISCARD_ARG:
			atomic_long_add(nr, &part->discard);
			break;
		case MMC_TRIM_ARG:
			atomic_long_add(nr, &part->trim);
			break;
		case MMC_ERASE_ARG:
			atomic_long_add(nr, &part->erase);
			break;
		};
	}
#endif
