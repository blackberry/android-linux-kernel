...
#ifdef CONFIG_BBRY_DEBUG
static bool force_sd_mode;
static short sdmode = -1;
static bool prev_force;
#endif
...
#ifdef CONFIG_BBRY_DEBUG
		if (force_sd_mode) {
			if (sdmode >= MMC_TIMING_LEGACY &&
				sdmode < MMC_TIMING_MMC_HS200 &&
				sdmode != MMC_TIMING_MMC_HS) {
				/*clear all host capbilities, then set only the desired one*/
				host->caps &= ~(MMC_CAP_SD_HIGHSPEED |
								...
								MMC_CAP_UHS_SDR104 |
								MMC_CAP_UHS_DDR50);
				/*assume 1.8V signaling as it applies to all except HS and DS*/
				rocr |= SD_ROCR_S18A;

				if (sdmode == MMC_TIMING_UHS_SDR104) {
					host->caps |= MMC_CAP_UHS_SDR104;
				} else if (sdmode == MMC_TIMING_UHS_SDR50) {
					host->caps |= MMC_CAP_UHS_SDR50;
				} else if (sdmode == MMC_TIMING_UHS_DDR50) {
					host->caps |= MMC_CAP_UHS_DDR50;
				} else if (sdmode == MMC_TIMING_UHS_SDR25) {
					host->caps |= MMC_CAP_UHS_SDR25;
				} else if (sdmode == MMC_TIMING_UHS_SDR12) {
					host->caps |= MMC_CAP_UHS_SDR12;
				} else if (sdmode == MMC_TIMING_SD_HS) {
					rocr &= ~SD_ROCR_S18A;
					host->caps |= MMC_CAP_SD_HIGHSPEED;
				} else if (sdmode == MMC_TIMING_LEGACY) {
					rocr &= ~SD_ROCR_S18A;
				}
			}
			...
			if (prev_force) {
				/*reset the host capabilities
				 since force_sd_mode just got toggled*/
				host->caps |= (MMC_CAP_SD_HIGHSPEED |
							   MMC_CAP_UHS_SDR12 |
							   MMC_CAP_UHS_SDR25 |
							   MMC_CAP_UHS_SDR50 |
...
#ifdef CONFIG_BBRY_DEBUG
static int __init sd_drv_init(void)
{
	force_sd_mode = false;
	prev_force = false;
	return 0;
...
static void __exit sd_drv_exit(void)
{
	return;
}

module_init(sd_drv_init);
module_exit(sd_drv_exit);

/* root can write, others read */
module_param(force_sd_mode, bool, S_IRUGO|S_IWUSR);
module_param(sdmode, short, S_IRUGO|S_IWUSR);

MODULE_PARM_DESC(force_mode, "Enable/Disable forcing a particular sd mode");
MODULE_PARM_DESC(sdmode, "The mode to set. 0=DS, 2=HS, 3=SDR12, 4=SDR25, 5=SDR50, 6=SDR104, 7=DDR50");

MODULE_AUTHOR("BlackBerry Limited");
MODULE_DESCRIPTION("SD driver");
MODULE_LICENSE("GPL");
#endif
