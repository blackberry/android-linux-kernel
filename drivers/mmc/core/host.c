...
#ifdef CONFIG_BBRY
static ssize_t mmc_transaction_counters_show(struct device *dev,
                                 struct device_attribute *attr, char *buf)
{
	...
	return mmc_get_trans_ctrs(host, buf, PAGE_SIZE);
}
#endif
...
#ifdef CONFIG_BBRY
	host->transaction_counters.show = mmc_transaction_counters_show;
	host->transaction_counters.store = NULL;
	sysfs_attr_init(&host->transaction_counters.attr);
	host->transaction_counters.attr.name = "transaction_counters";
	host->transaction_counters.attr.mode = (S_IRUSR | S_IRGRP);
	if (device_create_file(&host->class_dev, &host->transaction_counters))
		pr_err("%s: Failed to create transaction counter sysfs entry\n",
				mmc_hostname(host));
...
#ifdef CONFIG_BBRY
	device_remove_file(&host->class_dev, &host->transaction_counters);
#endif
