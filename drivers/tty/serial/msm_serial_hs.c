...
#ifdef CONFIG_BBRY
static int hs_serial_debug_level = WARN_LEV;

/* The major device number.  */
#define MAJOR_NUM 0xFF

/* IOCTL: Set the debug status of the stack */
#define IOCTL_SET_DEBUG_STATUS _IOW('T', 0xFF, int)


#define MSM_HS_DBG(fmt, args...) do { \
	if (hs_serial_debug_level >= DBG_LEV) { \
		printk(KERN_DEBUG "(%s:%d) " fmt, __func__, __LINE__, ##args); \
	} \
} while (0)

#define MSM_HS_INFO(fmt, args...) do { \
	if (hs_serial_debug_level >= INFO_LEV) {\
		printk(KERN_INFO "(%s:%d) " fmt, __func__, __LINE__, ##args); \
	} \
} while (0)

#define MSM_HS_NOTICE(fmt, args...) do { \
	if (hs_serial_debug_level >= NOTICE_LEV) { \
		printk(KERN_NOTICE "(%s:%d) " fmt, __func__, __LINE__, ##args); \
	} \
} while (0)

#define MSM_HS_WARN(fmt, args...) do { \
	if (hs_serial_debug_level >= WARN_LEV) { \
		printk(KERN_WARNING "(%s:%d) " fmt, __func__, __LINE__, ##args); \
	} \
} while (0)

#define MSM_HS_ERR(fmt, args...) do { \
	if (hs_serial_debug_level >= ERR_LEV) { \
...
#ifdef CONFIG_BBRY
static int shutdown_status = 0;    /* 0 = idle, 1 = active, 2 = complete */
static int startup_status = 0;     /* 0 = idle, 1 = active, 2 = complete */
#endif
/*
...
#ifdef CONFIG_BBRY
	case IOCTL_SET_DEBUG_STATUS: {
		 hs_serial_debug_level = (int)arg;
		 MSM_HS_WARN("Debug Level set to %d\n", hs_serial_debug_level);
		 break;
	}
#endif
	default: {
...
#ifdef CONFIG_BBRY
	/* Only print out first 6 bytes, this will tell us packet
	 * type and opt-code
	 */
	hex_dump_to_buffer(string, size < 6 ? size : 6, 16, 1,
...
#ifdef CONFIG_BBRY
	MSM_HS_WARN("%s(): TX Timer timedout", __func__);
#endif
	if (msm_uport->pm_state != MSM_HS_PM_ACTIVE) {
...
#ifdef CONFIG_BBRY
	if(!tty || shutdown_status == 1 || startup_status == 1){
		MSM_HS_WARN("%s: tty = %p, shutdown_status = %d, startup_status = %d",
			__func__, tty, shutdown_status, startup_status);
		if (!tty && !wait_event_timeout(msm_uport->rx.wait, tty, 500)) {
			MSM_HS_WARN("%s(): tty still NULL. Ignoring job.", __func__);
			msm_hs_resource_unvote(msm_uport);
			return;
		}
	}else{
		shutdown_status = startup_status = 0;
	}
#endif
	spin_lock_irqsave(&uport->lock, flags);
...
static void msm_hs_sps_rx_callback(struct sps_event_notify *notify)
{
	...
	if (msm_uport->rx.flush == FLUSH_NONE) {
		...
		if(!uport->state->port.tty)
			MSM_HS_WARN("%s(): Scheduling rx_tlet while tty=NULL", __func__);
#endif
...
#ifdef CONFIG_BBRY
	int rc = atomic_read(&msm_uport->clk_count);
	MSM_HS_NOTICE("Request Clock Off: Resource Count %d\n", rc);
#endif
...
#ifdef CONFIG_BBRY
	int rc = atomic_read(&msm_uport->clk_count);
	MSM_HS_NOTICE("Request Clock On: Resource Count %d\n", rc);
#endif
...
#ifdef CONFIG_BBRY
	startup_status = 1;
	MSM_HS_WARN("msm_hs_startup started, tty = %p", tty);
#endif
...
#ifdef CONFIG_BBRY
	if(!tty)
		MSM_HS_WARN("%s(): Flushing rx_tlet while tty=NULL", __func__);
#endif
...
#else
	startup_status = 2;
	MSM_HS_WARN("msm_hs_startup complete");
#endif
	return 0;
...
#ifdef CONFIG_BBRY
	MSM_HS_DBG("%s(): Disabling core irq\n", __func__);
	disable_irq(msm_uport->uport.irq);
	MSM_HS_NOTICE("return suspend\n");
#else
...
static int msm_hs_pm_resume(struct device *dev)
{
	...
	if (!atomic_read(&msm_uport->client_req_state))
		toggle_wakeup_interrupt(msm_uport);
#ifdef CONFIG_BBRY
	MSM_HS_DBG("%s(): Enabling core irq\n", __func__);
	enable_irq(msm_uport->uport.irq);
#endif
...
#ifdef CONFIG_BBRY
	shutdown_status = 1;
	MSM_HS_WARN("msm_hs_shutdown started");
#endif
	if (is_use_low_power_wakeup(msm_uport))
...
#ifdef CONFIG_BBRY
	shutdown_status = 2;
	MSM_HS_WARN("msm_hs_shutdown complete");
#endif
}
