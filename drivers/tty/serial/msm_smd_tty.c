...
struct smd_tty_info {
	...
	u64    cnt;
	int    cnt_ref;
	struct wakeup_source pending_ws_copy;
	struct wakeup_source ra_wakeup_source_copy;
#endif
};
...
static void smd_tty_read(unsigned long param)
{
	...
	for (;;) {
		...
		if (avail <= 0) {
			if (avail != 0) {
				SMD_TTY_ERR(
					"%s - Unexpected read avail value '%d' on %s",
					__func__, -avail, info->ch_name);
			}
#else
		if (avail == 0) {
#endif /* CONFIG_BBRY */
...
#ifdef CONFIG_BBRY_DEBUG
	memcpy(&info->pending_ws_copy, &info->pending_ws, sizeof(info->pending_ws));
	memcpy(&info->ra_wakeup_source_copy, &info->ra_wakeup_source, sizeof(info->ra_wakeup_source));
#endif
