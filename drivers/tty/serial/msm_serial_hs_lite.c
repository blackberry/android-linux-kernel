...
#ifdef CONFIG_BBRY
/* Delay of 30 seconds */
#define WAKELOCK_DELAY_30_SEC (30 * HZ)
#endif
...
static const unsigned int regmap[][UARTDM_LAST] = {
	...
	[UARTDM_VERSION_14] = {
		...
		[UARTDM_RXFS] = 0x50,
#ifdef CONFIG_BBRY
		[UARTDM_GENERICS] = UARTDM_GENERICS_ADDR,
#endif
	},
};
...
#ifdef CONFIG_BBRY
	/* IMR register will be written when the console write is done
	 * This function is called with the spinlock being held
	 */
	if (!msm_hsl_port->console_wip)
#endif
...
#ifdef CONFIG_BBRY
static irqreturn_t msm_hsl_rx_irq(int irq, void *dev_id)
{
	...
	if (pdata && pdata->use_pm)
		wake_lock_timeout(&msm_hsl_port->port_open_wake_lock,
				WAKELOCK_DELAY_30_SEC);
	return IRQ_HANDLED;
}
#endif /* CONFIG_BBRY */
...
#ifdef CONFIG_BBRY
	if (pdata && pdata->config_gpio) {
		snprintf(msm_hsl_port->gpio_name, sizeof(msm_hsl_port->gpio_name),
				"msm_serial_hsl_gpio_%d", pdata->uart_rx_gpio);
		ret = request_irq(gpio_to_irq(pdata->uart_rx_gpio), msm_hsl_rx_irq,
					IRQF_TRIGGER_FALLING, msm_hsl_port->gpio_name, port);
		if (unlikely(ret)) {
			pr_err("failed to request_irq %d %d ret %d %s\n",
					gpio_to_irq(pdata->uart_rx_gpio),
					pdata->uart_rx_gpio,
					ret,
					msm_hsl_port->gpio_name);
			goto release_wakelock;
		}
	}
#endif /* CONFIG_BBRY */
...
#ifdef CONFIG_BBRY
	if (pdata && pdata->config_gpio) {
		free_irq(gpio_to_irq(pdata->uart_rx_gpio), port);
	}
#endif /* CONFIG_BBRY */
				  ...
				  unsigned int count)
{
	...
	msm_hsl_write(port, 0, regmap[vid][UARTDM_IMR]);
#ifdef CONFIG_BBRY
	/* We release the spinlock otherwise preemption is disabled
	 * for too long and this create problems for some latency
	 * sensitive software (e.g. bluetooth audio).
	 *
	 * We will prevent the tty layer from changing the IMR
	 * register while we are in console_write.  Other functions
	 * could be called and could break this interface but as this driver
	 * is only used for debug images, we will live with this
	 * for the time being.
	 */
	msm_hsl_port->console_wip = 1;
	if (locked)
...
#ifdef CONFIG_BBRY
	/* The bootloader allocate most of the fifo to TX, split it evenly here */
	if (vid == UARTDM_VERSION_14) {
		u8 fifo_size = UARTDM_GENERICS_FIFO_WIDTH(
				msm_hsl_read(port, regmap[vid][UARTDM_GENERICS]));
		pr_info("console fifo size is %d\n", port->fifosize);
		/* The code seem to expect the fifosize to be of a certain size
		 * and yet, it does not configure it.
		 */
		WARN_ON(port->fifosize != fifo_size/2);
		msm_hsl_write(port, fifo_size/2, regmap[vid][UARTDM_BADR]);
	}
#endif
...
static int msm_serial_hsl_suspend(struct device *dev)
{
	...
	if (port) {
		...
		{
			if (pdata && pdata->config_gpio)
				enable_irq_wake(gpio_to_irq(pdata->uart_rx_gpio));
		}
#else
...
static int msm_serial_hsl_resume(struct device *dev)
{
	...
	if (port) {
		...
		{
			if (pdata && pdata->config_gpio)
				disable_irq_wake(gpio_to_irq(pdata->uart_rx_gpio));
		}
#else
