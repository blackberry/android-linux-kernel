		...
		struct mdss_mdp_mixer *mixer, struct mdss_panel_info *pdata)
{
	...
	if (mixer->num == MDSS_MDP_INTF_LAYERMIXER0 ||
			mixer->num == MDSS_MDP_INTF_LAYERMIXER1) {
#else
	// apply QC suggested patch until it's part of future delivery
	if ((mixer->pingpong_base) && (mixer->type != MDSS_MDP_MIXER_TYPE_WRITEBACK) &&
		(mixer->num == MDSS_MDP_INTF_LAYERMIXER0 ||
			mixer->num == MDSS_MDP_INTF_LAYERMIXER1) {
#endif
