...
/* [PLATFORM]-Mod-BEGIN by TCTNB.WPL, FR-988955, 2015/12/10, configure mpp4 pin for pmi8994 amoled power supply*/
#if defined(CONFIG_TCT_8996_IDOL4S_CN)
static int pmi8994_mpp4 = -1;
static struct qpnp_pin_cfg pmi8994_mpp4_endis = {
...
/* [FEATURE]-Mod-BEGIN by TCTNB.WPL, defect-1752747, porting from task-1061400, 2016/03/08, enable panel HBM mode*/
#ifdef CONFIG_FEATURE_PANEL_HBM // MODIFIED by Alvin Li, 2016-09-12,BUG-2852919
...
#endif
/* [FEATURE]-Mod-END by TCTNB.WPL, 2016/03/08*/
...
/* [PLATFORM]-Mod-BEGIN by TCTNB.WPL, FR-988955, 2015/12/10, configure mpp4 pin for pmi8994 amoled power supply*/
#if defined(CONFIG_TCT_8996_IDOL4S_CN)
			if (gpio_is_valid(pmi8994_mpp4)) {
				pmi8994_mpp4_endis.master_en = QPNP_PIN_MASTER_DISABLE;
				qpnp_pin_config(pmi8994_mpp4, &pmi8994_mpp4_endis);
			}
#endif
/* [PLATFORM]-Mod-END by TCTNB.WPL, 2015/12/10*/
...
#if defined(CONFIG_TCT_8996_IDOL4S_CN)
			if (gpio_is_valid(pmi8994_mpp4)) {
				pmi8994_mpp4_endis.master_en = QPNP_PIN_MASTER_ENABLE;
				qpnp_pin_config(pmi8994_mpp4, &pmi8994_mpp4_endis);
			}
#endif
/* [PLATFORM]-Mod-END by TCTNB.WPL, 2015/12/10*/
...
/* [PLATFORM]-Mod-BEGIN by TCTNB.WPL, task-2822877, porting from 2467049, 2016/08/31, from qualcomm case 02531526 advice */
...
/* [PLATFORM]-Mod-END by TCTNB.WPL, 2016/08/31 */
...
/* [BUGFIX]-Mod-BEGIN by TCTNB.WPL, 2016/08/19, PR-2751666, panel ESD */
//#ifdef ESD_ERR_FLAG_DETECT
#if 1
...
#endif
/* [BUGFIX]-Mod-END by TCTNB.WPL, 2016/08/19 */
...
/* [PLATFORM]-Mod-BEGIN by TCTNB.WPL, 2016/03/23, defect-1660471, power on performance, change log level*/
		pr_warn("%s: can't find panel node %s\n", __func__, panel_cfg);
...
static int mdss_dsi_ctrl_probe(struct platform_device *pdev)
{
	...
	if (!dsi_pan_node) {
/* [PLATFORM]-Mod-BEGIN by TCTNB.WPL, 2016/03/23, defect-1660471, power on performance, change log level*/
		pr_warn("%s: panel configuration failed\n", __func__);
/* [PLATFORM]-Mod-END by TCTNB.WPL*/
		return -EINVAL;
	}
...
#ifdef CONFIG_BBRY
static char acl_cmd[] = {0x55, 0x00};

static struct dsi_cmd_desc acl_cmd_desc[] = {
	{{DTYPE_DCS_WRITE1, 1, 0, 0, 0, sizeof(acl_cmd)},acl_cmd},
	{{DTYPE_DCS_WRITE1, 1, 0, 0, 1, sizeof(acl_cmd)},acl_cmd},
};
#endif
...
#ifdef CONFIG_BBRY
	acl_cmd[1] = cmd ? 0x00 : 0x02;     //When we enable HBM, disable ACL.

	memset(&cmdreq, 0, sizeof(cmdreq));
	cmdreq.cmds = acl_cmd_desc;
	cmdreq.cmds_cnt = 2;
	cmdreq.flags = CMD_REQ_COMMIT | CMD_CLK_CTRL | CMD_REQ_HS_MODE;
	cmdreq.rlen = 0;
	cmdreq.cb = NULL;
	...
	struct device_node *pan_node, struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	if (!data || len <= 0)
/* [PLATFORM]-Mod-BEGIN by TCTNB.WPL, 2016/03/23, defect-1660471, power on performance, change log level*/
		pr_warn("%s:%d Unable to read qcom,display-id, data=%pK,len=%d\n",
/* [PLATFORM]-Mod-END by TCTNB.WPL*/
...
/* [PLATFORM]-Mod-BEGIN by TCTNB.WPL, FR-988955, 2015/12/10, configure mpp4 pin for pmi8994 amoled power supply*/
#if defined (CONFIG_TCT_8996_IDOL4S_CN)
		pmi8994_mpp4 = of_get_named_gpio(
				ctrl_pdev->dev.of_node, "qcom,pmi8994-mpp4-gpio", 0);
		if (!gpio_is_valid(pmi8994_mpp4))
				pr_info("%s:%d, MPP4 gpio not specified\n",
								__func__, __LINE__);
#endif
/* [PLATFORM]-Mod-END by TCTNB.WPL, 2015/12/10*/
