...
#ifdef CONFIG_BBRY
static void mdss_panel_err_recovery(struct work_struct *work)
{
	char *envp[2] = {"PANEL_ALIVE=0", NULL};
	struct mdss_panel_data *pdata =
		container_of(work, typeof(*pdata), panel_recovery_work);
	struct msm_fb_data_type *mfd = platform_get_drvdata(pdata->fb_pdev);

	pdata->panel_info.panel_dead = true;
	pdata->panel_info.panel_recovery = true;
	kobject_uevent_env(&mfd->fbi->dev->kobj, KOBJ_CHANGE, envp);
	pr_err("%s: display error reported, restarting now\n", __func__);
}

irqreturn_t mdss_panel_err_isr(int irq, void *ptr)
{
	struct mdss_panel_data *pdata = ptr;

	pr_err("%s: display error reported, queue restart\n", __func__);
	schedule_work(&pdata->panel_recovery_work);
...
#ifdef CONFIG_BBRY
	if (gpio_is_valid(ctrl_pdata->err_gpio)) {
		rc = devm_request_irq(&pdev->dev,
			gpio_to_irq(ctrl_pdata->err_gpio),
			mdss_panel_err_isr, IRQF_TRIGGER_RISING,
			"ERROR_GPIO", &ctrl_pdata->panel_data);
		if (rc) {
			pr_err("err_gpio request_irq failed.\n");
			return rc;
		}

		INIT_WORK(&ctrl_pdata->panel_data.panel_recovery_work,
				mdss_panel_err_recovery);
	}
#endif
...
#ifdef CONFIG_BBRY
	ctrl_pdata->err_gpio = of_get_named_gpio(ctrl_pdev->dev.of_node,
			 "oem,platform-error-gpio", 0);
	if (!gpio_is_valid(ctrl_pdata->err_gpio))
		pr_err("%s:%d, error gpio not specified\n", __func__, __LINE__);
#endif
