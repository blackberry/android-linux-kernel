...
#ifdef CONFIG_BBRY
	pr_debug("lcd-backlight brightness %d\n", value);
#endif
...
#ifdef CONFIG_BBRY
	.flags          = LED_LOG_FIRST_BRIGHTNESS
#endif
};
...
static ssize_t set_disable_bl_scaling(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	struct fb_info *fbi = dev_get_drvdata(dev);
	struct msm_fb_data_type *mfd = fbi->par;
	...
	mfd->disable_bl_scaling = (en != 0);

	return count;
}

static ssize_t get_disable_bl_scaling(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct fb_info *fbi = dev_get_drvdata(dev);
	struct msm_fb_data_type *mfd = fbi->par;

	return snprintf(buf, 3, "%u\n", (mfd->disable_bl_scaling)?1:0);
}
...
static DEVICE_ATTR(disable_bl_scaling, S_IRUGO | S_IWUSR, get_disable_bl_scaling, set_disable_bl_scaling);
#ifdef CONFIG_BBRY_DEBUG
...
static struct attribute *mdss_fb_attrs_internal[] = {
	...
	&dev_attr_disable_bl_scaling.attr,
#ifdef CONFIG_BBRY_DEBUG
...
static void mdss_fb_scale_bl(struct msm_fb_data_type *mfd, u32 *bl_lvl)
{
	u32 temp = *bl_lvl;
#ifdef CONFIG_BBRY
	if (mfd->disable_bl_scaling)
		return;
	pr_debug("input = %d, scale = %d  mfd->bl_min_lvl = %d\n", temp, mfd->bl_scale,  mfd->bl_min_lvl);
#else
...
#ifdef CONFIG_BBRY
	pr_info("%pS mode:%d count:%u\n", __builtin_return_address(0),
		blank_mode, info->blank_counter);
#else
	pr_debug("%pS mode:%d\n", __builtin_return_address(0),
		blank_mode);
#endif
...
int mdss_fb_alloc_fb_ion_memory(struct msm_fb_data_type *mfd, size_t fb_size)
{
	...
	pr_debug("alloc 0x%zuB vaddr = %pK (%pa iova) for fb%d\n", fb_size, // MODIFIED by jianhua.yu, 2016-11-12,BUG-3455244
		...
		struct vm_area_struct *vma)
{
	...
	if (page) {
		for_each_sg(table->sgl, sg, table->nents, i) {
			...
			pr_debug("vma=%pK, addr=%x len=%ld\n", // MODIFIED by jianhua.yu, 2016-11-12,BUG-3455244
...
static int mdss_fb_open(struct fb_info *info, int user)
{
	...
	if (!mfd->ref_cnt) {
#ifdef CONFIG_BBRY
		/* Set an initial brightness for the transition from charging screen
		 * to boot animation.
		*/
...
#ifdef CONFIG_BBRY
	pr_debug("Commit frame flags %08x, l_roi (%d %d %d %d), r_roi (%d %d %d %d)\n",
				disp_commit.flags,
				disp_commit.l_roi.x, disp_commit.l_roi.y, disp_commit.l_roi.w, disp_commit.l_roi.h,
				disp_commit.r_roi.x, disp_commit.r_roi.y, disp_commit.r_roi.w, disp_commit.r_roi.h);
#endif
