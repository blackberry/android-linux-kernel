...
/* TODO: remove this once it is added to the defconfig */
#define CONFIG_BBRY 1
#define CONFIG_BBRY_DEBUG 1
...
/*TCTNB,LCY,add begin,change lcd backlight flash current to 30ma when use front camera,defect-3717000,2016/12/13*/
#if defined(CONFIG_BBRY)
#define CURRENT_FOR_FRONT_CAMERA  		30
#define CURRENT_FOR_NORMAL  			20
static u32 store_brightness; 
static int camera_mode = 0;
extern  int qpnp_wled_read_reg_fcamera(u8 *data, u16 addr);
extern  int qpnp_wled_write_reg_fcamera( u8 *data, u16 addr);

enum {
	FRONT_CAMERA_PRESS_BUTTON_WITH_FLASH_ON 	= 0,
	FRONT_CAMERA_PREFLASH,
	FRONT_CAMERA_FLASH,
	FRONT_CAMERA_AFTER_FLASH,
};

#endif
/*TCTNB,LCY,add end,defect-3717000,2016/12/13*/
...
/*TCTNB,LCY,add begin,change lcd backlight flash current to 30ma when use front camera,defect-3717000,2016/12/13*/
#if defined(CONFIG_BBRY)
static void qpnp_wled_config_for_camera(int flag)
{
	int rc, temp,ca_cur;
	u8 reg = 0;
	ca_cur=flag;

	if (ca_cur==CURRENT_FOR_FRONT_CAMERA)
		temp = 12;   //30ma  0xC
	else if (ca_cur==CURRENT_FOR_NORMAL)
		temp = 8;    //20ma  0x8

		rc = qpnp_wled_read_reg_fcamera(&reg,0xd952);
		reg &= 0xF0;
		reg |= temp;
		rc = qpnp_wled_write_reg_fcamera(&reg,0xd952);

		rc = qpnp_wled_read_reg_fcamera(&reg,0xd962);
		reg &= 0xF0;
		reg |= temp;
		rc = qpnp_wled_write_reg_fcamera(&reg,0xd962);

		rc = qpnp_wled_read_reg_fcamera(&reg,0xd972);
		reg &= 0xF0;
		reg |= temp;
		rc = qpnp_wled_write_reg_fcamera(&reg,0xd972);

		rc = qpnp_wled_read_reg_fcamera(&reg,0xd982);
		reg &= 0xF0;
		reg |= temp;
		rc = qpnp_wled_write_reg_fcamera(&reg,0xd982);
...
/*TCTNB,LCY,add begin,change lcd backlight flash current to 30ma when use front camera,defect-3717000,2016/12/13*/
#if defined(CONFIG_BBRY)
int brightness_set_frontcamera_mode(int flag)
{
	struct led_classdev *led_cdev = NULL;
	struct msm_fb_data_type *mfd = NULL;

	led_cdev = &backlight_led;
	mfd = dev_get_drvdata(led_cdev->dev->parent);

	printk("flag = %d\n",flag);
	if (flag==FRONT_CAMERA_AFTER_FLASH) {
		qpnp_wled_config_for_camera(CURRENT_FOR_NORMAL);
		mutex_lock(&mfd->bl_lock);
		mdss_fb_set_backlight(mfd,store_brightness);
		mutex_unlock(&mfd->bl_lock);
		printk("Use store_brightness = %d\n",store_brightness);
		camera_mode = 0;
		}
	if (flag==FRONT_CAMERA_FLASH) {
		qpnp_wled_config_for_camera(CURRENT_FOR_FRONT_CAMERA);
		mutex_lock(&mfd->bl_lock);
		mdss_fb_set_backlight(mfd,4095);
		mutex_unlock(&mfd->bl_lock);
	}
	if (flag==FRONT_CAMERA_PREFLASH) {
		mutex_lock(&mfd->bl_lock);
		mdss_fb_set_backlight(mfd,4095);
		mutex_unlock(&mfd->bl_lock);
	}
	if (flag==FRONT_CAMERA_PRESS_BUTTON_WITH_FLASH_ON) {
		camera_mode = 1;
		printk("Store_brightness = %d\n",store_brightness);
	}
		return 1;
}
EXPORT_SYMBOL(brightness_set_frontcamera_mode);
#endif
/*TCTNB,LCY,add end,defect-3717000,2016/12/13*/
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (cur_i != 0) {
		...
		if (ret) {
			pr_err("Error reading command header.\n");
			return -EINVAL;
		}
	} else {
...
#if defined(CONFIG_TOUCHSCREEN_FOCALTECH_ALL) && defined(CONFIG_BBRY)
...
static char wa_init_code1[2] = {0x00, 0x80};
static char wa_init_code2[7] = {0xC8, 0x04, 0x00, 0x03, 0x9b, 0x03, 0xa3};
static char wa_init_code3[2] = {0x00, 0x90};
static char wa_init_code4[7] = {0xC8, 0x04, 0x00, 0x04, 0x00, 0x04, 0x00};
static char wa_init_code5[2] = {0x00, 0xa0};
static char wa_init_code6[7] = {0xC8, 0x04, 0x00, 0x03, 0x82, 0x03, 0x5e};
static struct dsi_cmd_desc dsi_wa_init_cmd[] = {
	{{DTYPE_GEN_LWRITE, 1, 0, 0, 0, sizeof(wa_init_code1)}, wa_init_code1},
	{{DTYPE_GEN_LWRITE, 1, 0, 0, 0, sizeof(wa_init_code2)}, wa_init_code2},
	{{DTYPE_GEN_LWRITE, 1, 0, 0, 0, sizeof(wa_init_code3)}, wa_init_code3},
	{{DTYPE_GEN_LWRITE, 1, 0, 0, 0, sizeof(wa_init_code4)}, wa_init_code4},
	{{DTYPE_GEN_LWRITE, 1, 0, 0, 0, sizeof(wa_init_code5)}, wa_init_code5},
	{{DTYPE_GEN_LWRITE, 1, 0, 0, 0, sizeof(wa_init_code6)}, wa_init_code6}
};
static int wa_value = 0;
void mdss_dsi_wa_init(struct mdss_dsi_ctrl_pdata *ctrl)
{
	struct dcs_cmd_req cmdreq;
	struct mdss_panel_info *pinfo;
...
static ssize_t wa_set_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	struct fb_info *fbi = dev_get_drvdata(dev);
	struct msm_fb_data_type *mfd = fbi->par;
	...
	mdss_dsi_wa_init(ctrl);

	memset(&cmdreq, 0, sizeof(cmdreq));

	if (sscanf(buf, "%x", &wa_value) != 1)
		return -EINVAL;

	if (wa_value < 0 || wa_value > 0xff)
		return -EINVAL;

	cmds_buf[0] = 0x94;
	cmds_buf[1] = wa_value;

	//printk("wa regiester = %x %x\n", cmds_buf[0], cmds_buf[1]);
	cmds.dchdr.dtype = 0x15;
	cmds.dchdr.vc = 0;
	cmds.dchdr.ack = 0;
	cmds.dchdr.wait = 0;
...
static ssize_t wa_set_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	ssize_t ret = 0;
...
static char wa_resume_code[2] = {0x94, 0x00};
static struct dsi_cmd_desc dsi_wa_resume_cmd = {
	{DTYPE_GEN_LWRITE, 1, 0, 0, 0, sizeof(wa_resume_code)}, wa_resume_code};

void mdss_dsi_wa_resume(struct mdss_dsi_ctrl_pdata *ctrl)
{
	struct dcs_cmd_req cmdreq;
	struct mdss_panel_info *pinfo;
	...
	wa_resume_code[1] = wa_value;
	memset(&cmdreq, 0, sizeof(cmdreq));
	cmdreq.cmds = &dsi_wa_resume_cmd;
	cmdreq.cmds_cnt = 1;
	cmdreq.flags = CMD_REQ_COMMIT | CMD_REQ_LP_MODE;
	cmdreq.rlen = 0;
	cmdreq.cb = NULL;
...
#endif
/* MODIFIED-BEGIN by Haojun Chen, 2017-08-02,BUG-5103149*/
...
#endif
/* MODIFIED-END by Haojun Chen,BUG-5103149*/
...
#if defined(CONFIG_TOUCHSCREEN_FOCALTECH_ALL) && defined(CONFIG_BBRY)
static DEVICE_ATTR(aie_level, S_IRUGO|S_IRGRP|S_IWUSR|S_IWGRP, NULL, aie_level_set);
static DEVICE_ATTR(wa_level, S_IRUGO|S_IRGRP|S_IWUSR|S_IWGRP, wa_set_show, wa_set_store);
#endif

/* MODIFIED-BEGIN by Haojun Chen, 2017-08-02,BUG-5103149*/
#ifdef CONFIG_KRYPTON
static DEVICE_ATTR(aie_level, S_IRUGO|S_IRGRP|S_IWUSR|S_IWGRP, NULL, sre_enable);
#endif
/* MODIFIED-END by Haojun Chen,BUG-5103149*/
...
#endif
#ifdef CONFIG_TOUCHSCREEN_FOCALTECH_ALL
	&dev_attr_aie_level.attr,
	&dev_attr_wa_level.attr,
#endif
/* MODIFIED-BEGIN by Haojun Chen, 2017-08-02,BUG-5103149*/
#ifdef CONFIG_KRYPTON
	&dev_attr_aie_level.attr,
