 ...
 * and the V coordinate is 0.4680 +/- 0.015.  The values read from the panel
...
#define LOGAN_V_MIN 0x1239 /* 4665 = (4680 - 15) */
#define LOGAN_V_MAX 0x1257 /* 4695 = (4680 + 15) */
...
static int mdss_fb_open(struct fb_info *info, int user)
{
	...
	if (!mfd->ref_cnt) {
#ifdef CONFIG_BBRY
		/*Turn ON the panel to a brightness that will actually show
			 ...
			 unsigned long arg)
{
	...
	case MSMFB_READ_PANEL_RECOVERY_DATA:
	{
		struct panel_recovery_data rec_data;
		int set, length;
		ret = copy_from_user(&set, argp, sizeof(set));
		if (ret) {
			pr_err("%s: MSMFB_READ_PANEL_RECOVERY_DATA ioctl failed\n", __func__);
			goto exit;
		}
		pr_info("MSMFB_READ_PANEL_RECOVERY_DATA set %d", set);
		memset(&rec_data, 0, sizeof(rec_data));

		if (pdata->panel_info.read_recovery_data) {
			length = pdata->panel_info.read_recovery_data(&pdata->panel_info, set, rec_data.buf, sizeof(rec_data.buf));
		...
		ret = copy_to_user(argp, &rec_data, sizeof(rec_data));
		break;
	}
#endif
	default:
