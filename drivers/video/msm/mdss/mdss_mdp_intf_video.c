	...
	struct mdss_panel_data *pdata, int inum)
{
	...
	pr_debug("stop ctl=%d video Intf #%d base=%pK", ctl->num, ctx->intf_num, // MODIFIED by jianhua.yu, 2016-11-12,BUG-3455244
			ctx->base);
	...
	if (is_pingpong_split(ctl->mfd)) {
		...
		pr_debug("stop ctl=%d video Intf #%d base=%pK", ctl->num, // MODIFIED by jianhua.yu, 2016-11-12,BUG-3455244
...
#ifdef CONFIG_BBRY
		/* Propegate error up to caller so we can shut down in POPs where there
		   is no panel connected */
		if (rc < 0)
			return rc;
#endif
	...
	struct mdss_panel_data *pdata, int inum)
{
	...
	if (inum < mdata->nintf) {
		...
		pr_debug("video Intf #%d base=%pK", ctx->intf_num, ctx->base); // MODIFIED by jianhua.yu, 2016-11-12,BUG-3455244
		ctx->ref_cnt++;
	} else {
