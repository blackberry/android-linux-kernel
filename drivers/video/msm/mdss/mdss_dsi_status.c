...
#ifdef CONFIG_BBRY
	ctrl_pdata = container_of(dev_get_platdata(&pdsi_status->mfd->pdev->dev),
...
#ifdef CONFIG_BBRY
	if (ctrl_pdata->esd_counter > 15){
		/* If we get more than 15 interrupts on the esd GPIO in 300ms,
		 * Ignore them to avoid triggerring a recovery loop
		 */
		disable_irq(gpio_to_irq(ctrl_pdata->err_flag_gpio));
		ctrl_pdata->esd_counter = 0;
		atomic_set(&ctrl_pdata->err_flag_ready, 0);
		pr_err("=== esd flag is abnormal diable esd intr \n");
	} else {
		if (atomic_read(&ctrl_pdata->err_flag_ready)) {
			pr_err("=== err_flag_ready set with esd_counter %d \n", ctrl_pdata->esd_counter);
		}
		ctrl_pdata->esd_counter = 0;
#endif
