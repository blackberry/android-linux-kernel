...
#ifdef CONFIG_BBRY
static int bbry_supported_DTD_modes[] = {
...
/* Allow custom DTD video timings that are "close" to known
   supported modes */
int get_bbry_supported_DTD_mode(
		struct msm_hdmi_mode_timing_info *mode)
{
	int i;
	struct msm_hdmi_mode_timing_info info = {0};
	for (i = 0; i < sizeof(bbry_supported_DTD_modes)/sizeof(bbry_supported_DTD_modes[0]); i++) {

		msm_hdmi_get_timing_info(&info, bbry_supported_DTD_modes[i]);
		/* Accept a custom video mode if the active resolution matches,
		   the framerate is within 1Hz of an accepted mode with the same
		   resolution, the mode is not interlaced, and is supported in
		   its standard form. */
		if ((info.active_h == mode->active_h) &&
		    (info.active_v == mode->active_v) &&
		    ((info.refresh_rate + 1000) > mode->refresh_rate) &&
		    ((info.refresh_rate - 1000) < mode->refresh_rate) &&
		    (mode->interlaced == 0) && info.supported)
		    {
			DEV_INFO("New mode is close enough to mode %d", i);
			return 1;
		}
	}
	return 0;
