...
#ifdef CONFIG_BBRY
static ssize_t hdmi_edid_sysfs_rda_pref(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	ssize_t ret = 0;
	u32 mode = -1;
	...
	buf[0] = 0;
	if (hdmi_get_supported_mode(info, edid_ctrl->init_data.ds_data,
			edid_ctrl->sink_data.preferred_video_format)) {
		mode = edid_ctrl->sink_data.preferred_video_format;
	}
	...
	return ret;
} /* hdmi_edid_sysfs_rda_pref */

static DEVICE_ATTR(edid_pref_mode, S_IRUGO, hdmi_edid_sysfs_rda_pref, NULL);
#endif

static struct attribute *hdmi_edid_fs_attrs[] = {
	&dev_attr_edid_modes.attr,
#ifdef CONFIG_BBRY
	&dev_attr_edid_pref_mode.attr,
#endif
	&dev_attr_pa.attr,
	...
	const u8 *data_buf, u32 *disp_mode)
{
	...
	*disp_mode = HDMI_VFRMT_FORCE_32BIT;
	...
	if (*disp_mode == HDMI_VFRMT_FORCE_32BIT) {
		struct msm_hdmi_mode_timing_info timing = {0};
#ifdef CONFIG_BBRY
		u64 rr_tmp = (u64) pixel_clk * 1000 * 1000;
#else
...
#ifdef CONFIG_BBRY
		timing.supported     = get_bbry_supported_DTD_mode(&timing);
#else
	...
	const u8 *data_buf, u32 num_of_cea_blocks)
{
	...
	if (svd != NULL) {
		...
		for (i = 0; i < len; ++i, ++svd) {
			...
			hdmi_edid_add_sink_video_format(edid_ctrl,
				video_format);
#ifndef CONFIG_BBRY /* SVD doesn't indicate preferred mode */
...
#ifdef CONFIG_BBRY
	/* Edid read failure should result in a fallback to
	 * Safe Mode, not the mode last selected to be used.
	 */
	edid_ctrl->sink_data.disp_mode_list[0] = HDMI_VFRMT_640x480p60_4_3;
#else
