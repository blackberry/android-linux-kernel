...
#ifdef CONFIG_BBRY
	edid_init_data.ds_read_edid_block = &hdmi_ctrl->ds_read_edid_block;
#endif
	...
	int event, void *arg)
{
	...
	case MDSS_EVENT_CHECK_PARAMS:
		 ...
		 */
#ifdef CONFIG_BBRY
		/* If new_vic is a reserved (custom) vic, then it could be a resolution
			change since the reserved/custom formats get reset on each connection */
		if (new_vic != hdmi_ctrl->video_resolution || new_vic >= HDMI_VFRMT_RESERVE1)
#else
