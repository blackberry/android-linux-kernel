...
/* Function for debug-printing a dsi_cmd_desc.
 * Not thread-safe, because buf is static
 * must always return a valid string, so that callers
 * can print without checking return value */
char *print_dsi_cmd_desc(struct dsi_cmd_desc *cmd)
{
	int i, offset = snprintf(print_buf, sizeof(print_buf), "dt %02x: ",
					cmd->dchdr.dtype);
	if (offset <= 0) {
		print_buf[0] = 0;
		return print_buf;
	}
	for (i = 0; (i < cmd->dchdr.dlen)&&(offset < sizeof(print_buf)); i++) {
		int ret = snprintf(print_buf + offset,
			sizeof(print_buf) - offset, "%02x ", cmd->payload[i]);
		if (ret > 0)
			offset += ret;
		else
			break;

		if (offset >= sizeof(print_buf)) {
			sprintf(print_buf + sizeof(print_buf) - sizeof("..."),
									"...");
			break;
		}
	}
...
char *print_dsi_buf(char *data, int length)
{
	int i, offset = 0;
	print_buf[0] = 0;
	for (i = 0; (i < length)&&(offset < sizeof(print_buf)); i++) {
		int ret = snprintf(print_buf + offset,
				sizeof(print_buf) - offset, "%02x ", data[i]);
		if (ret > 0)
			offset += ret;
		else
			break;
...
#ifdef CONFIG_BBRY
void mdss_dsi_read_extra_status(struct mdss_dsi_ctrl_pdata *ctrl)
{
       struct dcs_cmd_req cmdreq;
       char rbuf_temp[2];
       int ret;
       ...
       cmdreq.cmds[0].payload[0] = 0xED;
       ret = mdss_dsi_cmdlist_put(ctrl, &cmdreq);
       if(ret>0) {
               pr_err("%s: 0xED = %02x\n", __func__, rbuf_temp[0]);
       } else {
               pr_err("%s: 0xED read failed\n", __func__);
       }

       cmdreq.cmds[0].payload[0] = 0x05;
       cmdreq.rlen = 2;
       ret = mdss_dsi_cmdlist_put(ctrl, &cmdreq);
       if(ret>0) {
               pr_err("%s: 0x05 = %02x %02x\n", __func__, rbuf_temp[0], rbuf_temp[1]);
       } else {
               pr_err("%s: 0x05 read failed\n", __func__);
       }

       cmdreq.cmds[0].payload[0] = 0x0F;
       cmdreq.rlen = 1;
       ret = mdss_dsi_cmdlist_put(ctrl, &cmdreq);
       if(ret>0) {
               pr_err("%s: 0x0F = %02x\n", __func__, rbuf_temp[0]);
       } else {
               pr_err("%s: 0x0F read failed\n", __func__);
       }

       cmdreq.cmds[0].payload[0] = 0x0E;
       ret = mdss_dsi_cmdlist_put(ctrl, &cmdreq);
       if(ret>0) {
               pr_err("%s: 0x0E = %02x\n", __func__, rbuf_temp[0]);
       } else {
               pr_err("%s: 0x0E read failed\n", __func__);
       }

       cmdreq.cmds[0].payload[0] = 0x0A;
       ret = mdss_dsi_cmdlist_put(ctrl, &cmdreq);
       if(ret>0) {
               pr_err("%s: 0x0A = %02x\n", __func__, rbuf_temp[0]);
       } else {
               pr_err("%s: 0x0A read failed\n", __func__);
       }
}
#endif
...
/* [PLATFORM]-Mod-BEGIN by TCTNB.WPL, task-2822877, porting from 2467049, 2016/08/31, from qualcomm case 02531526 advice */
			data = data | BIT(16);/* keep this bit set */
			MIPI_OUTP((ctrl_pdata->ctrl_base + 0x1b8), data);
			ctrl_pdata->burst_mode_enabled = 0; /*disable burst mode */
			...
			struct dsi_cmd_desc *cmds, int cnt, int use_dma_tpg)
{
	...
	while (cnt--) {
		...
		if (!len) {
#ifdef CONFIG_BBRY
			pr_err("%s: failed to add cmd: %s\n", __func__,
				print_dsi_cmd_desc(cm));
#else
...
#ifdef CONFIG_BBRY
				pr_err("%s: failed to call cmd_dma_tx for cmd: %s\n",
					__func__, print_dsi_cmd_desc(cm));
#else
		...
		struct dsi_cmd_desc *cmds, int cnt, int use_dma_tpg)
{
	...
	int i;
	for (i = 0; i < cnt; i++)
		pr_debug("%s: %s\n", __func__, print_dsi_cmd_desc(cmds+i));
#endif
	/*
...
#ifdef CONFIG_BBRY
	pr_debug("%s: %s\n", __func__, print_dsi_cmd_desc(cmds));
#endif
...
skip_max_pkt_size:
...
#ifdef CONFIG_BBRY
	pr_debug("%s: read %s\n", __func__,
			print_dsi_buf(rp->data, MAX(rp->len, 4)));
#endif
	switch (cmd) {
	case DTYPE_ACK_ERR_RESP:
#ifdef CONFIG_BBRY
		pr_warning("%s: rx DTYPE_ACK_ERR_RESP: %s\n", __func__,
				print_dsi_cmd_desc(cmds));
#else
	...
	default:
#ifdef CONFIG_BBRY
		pr_warning("%s:Invalid response %02x\n trying to read: %s\n",
				__func__, cmd, print_dsi_cmd_desc(cmds));
#else
