...
#ifdef CONFIG_BBRY
	struct work_struct gls_on_work;
#endif
...
#ifdef CONFIG_BBRY
	if (ctx->ctl &&
		ctx->ctl->panel_data->panel_info.panel_power_state &&
		(!ctx->ctl->panel_data->panel_info.panel_glass_on))
		schedule_work(&ctx->gls_on_work);
#endif
...
#ifdef CONFIG_BBRY
static void glass_on_work(struct work_struct *work)
{
	struct mdss_mdp_cmd_ctx *ctx =
		container_of(work, typeof(*ctx), gls_on_work);
	...
	if (ctx->ctl) {
		ctrl = container_of(ctx->ctl->panel_data,
							struct mdss_dsi_ctrl_pdata, panel_data);
		/* After we have finally sent out some frames it is
		 *  safe to send the disp_on cmd to avoid snowscreen.
		 */
		ctrl->glass_on(ctx->ctl->panel_data);
	} else
		pr_err("%s: invalid mdss_dsi_ctrl_pdata\n", __func__);
}
#endif
...
#else
		if (mdss_mdp_cmd_panel_on(ctl, sctl) < 0)
			return -EIO;
#endif
...
#ifdef CONFIG_BBRY
	INIT_WORK(&ctx->gls_on_work, glass_on_work);
#endif
