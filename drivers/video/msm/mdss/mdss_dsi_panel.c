...
#ifdef CONFIG_BBRY
extern void backlight_on_notif(bool backlight_on) __attribute__((weak));
#endif
/* [PLATFORM]-Mod-BEGIN by TCTNB.WPL, FR-2226930, 2016/05/31, WQHD/FHD LCD compatible for argon */
#ifdef CONFIG_TCT_8996_IDOL4S_CN
...
#endif
/* [PLATFORM]-Mod-END by TCTNB.WPL, 2016/05/31*/
...
/* [BUGFIX]-Mod-BEGIN by TCTNB.WPL, porting PR-1048316 to task-1629668, 2016/02/18, change brightness dimming mode when panel on*/
...
/* [PLATFORM]-Mod-BEGIN by TCTNB.WPL, porting from FR-526579 to FR-988955, 2015/12/10, modify for send backlight cmd more stably*/
...
/* [PLATFORM]-Mod-END by TCTNB.WPL, 2015/12/10*/
...
/* [BUGFIX]-Mod-BEGIN by TCTNB.WPL, porting from task-988883 to FR-988955, 2015/12/10, change send cmd mode to hs mode*/
#if defined(CONFIG_TCT_8996_IDOL4S_CN)
	cmdreq.flags |= CMD_REQ_HS_MODE;
#endif
/* [BUGFIX]-Mod-END by TCTNB.WPL, 2015/12/10*/
	cmdreq.rlen = 0;
	cmdreq.cb = NULL;
...
/* [PLATFORM]-Mod-BEGIN by TCTNB.WPL, FR-2758558, 2016/09/08, split_dsi WQHD timing for argon */
	if ((mdss_dsi_is_left_ctrl(ctrl_pdata) &&
		mdss_dsi_is_hw_config_split(ctrl_pdata->shared_data)) ||
			pinfo->is_dba_panel) {
/* [PLATFORM]-Mod-END by TCTNB.WPL, 2016/09/08*/
...
/* [PLATFORM]-Mod-BEGIN by TCTNB.WPL, FR-2758558, 2016/09/21, argon add 2ms delay before reset low */
		mdelay(2);
/* [PLATFORM]-Mod-END by TCTNB.WPL, 2016/09/21*/
		...
		struct dsi_panel_cmds *pcmds, char *cmd_key, char *link_key)
{
	...
	if (!data) {
/* [PLATFORM]-Mod-BEGIN by TCTNB.WPL, 2016/03/23, defect-1660471, power on performance, change log level*/
		pr_warn("%s: failed, key=%s\n", __func__, cmd_key);
...
{
#ifdef CONFIG_BBRY
	if (!mdss_dsi_cmp_panel_reg_masked(ctrl_pdata->status_buf,
		ctrl_pdata->status_value, ctrl_pdata->status_value_mask, 0)) {
		pr_err("%s: Read back value from panel is incorrect. expected:%x (mask %x) actual:%x\n",
							__func__, ctrl_pdata->status_value[0], ctrl_pdata->status_value_mask[0], ctrl_pdata->status_buf.data[0]);
...
#ifdef CONFIG_BBRY
		pr_debug("%s: expected:%x (mask %x) actual:%x\n",
							__func__, ctrl_pdata->status_value[0], ctrl_pdata->status_value_mask[0], ctrl_pdata->status_buf.data[0]);
#endif
		return 1;
	}
	...
	struct mdss_dsi_ctrl_pdata *ctrl)
{
...
#ifdef CONFIG_BBRY
	ctrl->status_value_mask = kzalloc(sizeof(u32) * ctrl->status_cmds_rlen,
				GFP_KERNEL);
	if (!ctrl->status_value_mask) {
		pr_err("%s: Error allocating memory for status buffer\n",
			__func__);
		pinfo->esd_check_enabled = false;
		return;
	...
	data = of_find_property(np, "oem,mdss-dsi-panel-status-value-mask", &tmp);
	tmp /= sizeof(u32);
	if (!data || (tmp != ctrl->status_cmds_rlen)) {
		pr_debug("%s: Panel status mask values not found\n", __func__);
		memset(ctrl->status_value_mask, 0, ctrl->status_cmds_rlen);
	} else {
		rc = of_property_read_u32_array(np,
			"oem,mdss-dsi-panel-status-value-mask",
			ctrl->status_value_mask, tmp);
		if (rc) {
			pr_debug("%s: Error reading panel status value mask\n",
					__func__);
			memset(ctrl->status_value_mask, 0, ctrl->status_cmds_rlen);
		}
	}
...
/* [PLATFORM]-Mod-BEGIN by TCTNB.WPL, FR-2226930, 2016/05/31, WQHD/FHD LCD compatible for argon */
	...
	pr_info("wangpl, panel_res_type=<%d>\n", panel_res_type);
#endif
/* [PLATFORM]-Mod-END by TCTNB.WPL, 2016/05/31*/
