...
static void mdss_dsi_panel_bklt_dcs(struct mdss_dsi_ctrl_pdata *ctrl, int level)
{
	...
	if (pinfo->blank_state == MDSS_PANEL_BLANK_LOW_POWER) {
		...
		for (i = 0; i < NUM_ALPM_BRIGHTNESS_BB_VALUES-1; i++) {
			int level_nits = level * 600 / pinfo->brightness_max;
	...
	/* If we are not in ALPM mode, go through the normal oled_panel_set_brightness
	 * Also, if we are in ALPM transitioning to OFF, there will be a call here with level=0.
	 * In this case, call oled_panel_set_brightness as well, so that it knows that we went off
	 * and it writes all the neccesary register values the next time there is a non-zero brightness
	 */
	if (pinfo->blank_state != MDSS_PANEL_BLANK_LOW_POWER || level == 0) {
...
#ifdef CONFIG_BBRY
	pr_notice("%s: ctrl=%pK ndx=%d enable=%d\n", __func__, ctrl, ctrl->ndx,
		enable);
	if (enable) {
