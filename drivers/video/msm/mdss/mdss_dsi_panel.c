...
static int mdss_dsi_panel_on(struct mdss_panel_data *pdata)
{
	...
	pr_debug("%s: ctrl=%pK ndx=%d\n", __func__, ctrl, ctrl->ndx); // MODIFIED by jianhua.yu, 2016-11-12,BUG-3455244
	...
	pr_debug("%s: ctrl=%pK cmd_cnt=%d\n", __func__, ctrl, on_cmds->cmd_cnt); // MODIFIED by jianhua.yu, 2016-11-12,BUG-3455244

	if (on_cmds->cmd_cnt)
#ifndef CONFIG_BBRY
		mdss_dsi_panel_cmds_send(ctrl, on_cmds);
#else
		if (mdss_dsi_panel_cmds_send(ctrl, on_cmds) <= 0)
			return -EIO;
#endif
