...
int mdss_dsi_panel_reset(struct mdss_panel_data *pdata, int enable)
{
	...
	if (enable) {
		...
		if (!pinfo->cont_splash_enabled) {
			if (gpio_is_valid(ctrl_pdata->disp_en_gpio)) {
#ifdef CONFIG_BBRY
				pr_debug("%s: disp_en_gpio(%d) = %d\n", __func__, ctrl_pdata->disp_en_gpio, enable);
#endif
				rc = gpio_direction_output(
...
#ifdef CONFIG_BBRY
				pr_debug("%s: rst_gpio(%d) = %d\n", __func__, ctrl_pdata->rst_gpio, pdata->panel_info.rst_seq[0]);
#endif
...
#ifdef CONFIG_BBRY
				pr_debug("%s: rst_gpio(%d) = %d\n", __func__, ctrl_pdata->rst_gpio, pdata->panel_info.rst_seq[i]);
#endif
			...
			if (gpio_is_valid(ctrl_pdata->bklt_en_gpio)) {
#ifdef CONFIG_BBRY
				pr_debug("%s: bklt_en_gpio(%d) = %d\n", __func__, ctrl_pdata->bklt_en_gpio, enable);
#endif
				rc = gpio_direction_output(
...
extern bool bootloader_display_is_off; // MODIFIED by Haojun Chen, 2017-01-17,BUG-4018467
							...
							u32 bl_level)
{
	...
	/* MODIFIED-BEGIN by Haojun Chen, 2017-01-17,BUG-4018467*/
	if (bootloader_display_is_off)
		return;
		/* MODIFIED-END by Haojun Chen,BUG-4018467*/
...
extern	void mdss_dsi_wa_resume(struct mdss_dsi_ctrl_pdata *ctrl);
#endif
