		...
		const char __user *user_buf, size_t count, loff_t *ppos)
{
	...
	for (i = 0; i < len; i++) {
		p = buf + i * 3;
		p[2] = 0;
		pr_debug("p[%d] = %pK:%s\n", i, p, p); // MODIFIED by jianhua.yu, 2016-11-12,BUG-3455244
		cnt = sscanf(p, "%x", &tmp);
		reg[i] = tmp;
		pr_debug("reg[%d] = %x\n", i, (int)reg[i]);
	}
		...
		struct mdss_mdp_ctl *ctl, struct mdss_data_type *mdata)
{
	...
	char *intf_base = NULL;
        ...
        /*SH-TCT-JHYU fix 2902854: sync CR1001224 09-13-2006*/
			...
			struct mdss_mdp_ctl *ctl)
{
	...
	bool use_mdp_up_misr = false;
        /* SH-TCT-JHYU fix 2902854: sync CR1001224 09-13-2006 */
	if (!mdata || !req || !ctl) {
		pr_err("Invalid input params: mdata = %pK req = %pK ctl = %pK", // MODIFIED by jianhua.yu, 2016-11-12,BUG-3455244
			mdata, req, ctl);
		return -EINVAL;
	}
