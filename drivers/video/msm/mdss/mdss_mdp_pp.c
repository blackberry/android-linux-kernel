...
static int pp_hist_disable(struct pp_hist_col_info *hist_info)
{
	...
	if (hist_info->col_en == false) {
		spin_unlock_irqrestore(&hist_info->hist_lock, flag);
		pr_debug("Histogram already disabled (%pK)\n", hist_info->base); // MODIFIED by jianhua.yu, 2016-11-12,BUG-3455244
		ret = -EINVAL;
		goto exit;
	}
...
int mdss_mdp_hist_intr_req(struct mdss_intr *intr, u32 bits, bool en)
{
	...
	int ret = 0;
	if (!intr) {
		pr_err("NULL addr passed, %pK\n", intr); // MODIFIED by jianhua.yu, 2016-11-12,BUG-3455244
		return -EINVAL;
	}
...
static int pp_ad_invalidate_input(struct msm_fb_data_type *mfd)
{
	...
	ret = mdss_mdp_get_ad(mfd, &ad);
	if (ret || !ad) {
		pr_err("Fail to get ad: ret = %d, ad = 0x%pK\n", ret, ad); // MODIFIED by jianhua.yu, 2016-11-12,BUG-3455244
		return -EINVAL;
	}
