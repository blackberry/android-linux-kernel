...
static int dev_check_access(void)
{
	...
	if (util_uid_eq(uid, BIDE_UID) ||
	    util_uid_eq(uid, ROOT_UID) ||
	    util_uid_eq(uid, SYSTEM_UID)) {
		return 0;
	} else {
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 18, 0)
		logError("Access from unregistered UID %d is denied.", (unsigned int)uid.val);
#else
		      ...
		      unsigned long arg)
{
	...
	case BIDE_IOCTL_REGISTER_INSTALLD:
			...
			auth_add_pid(current_pid, AUTH_PERM_PRIVELEGED_CHILDREN);
			rc = auth_add_pid(current_pid, AUTH_PERM_INSTALLD);
		break;
	...
	case BIDE_IOCTL_WHITELIST_ZYGOTEMGR:
		rc = auth_add_pid(current_pid, AUTH_PERM_ZYGOTEMGR);
		break;
	...
	case BIDE_IOCTL_WHITELIST_PRIVILEGED_CHILD:
		rc = auth_add_pid(current_pid, AUTH_PERM_PRIVELEGED_CHILDREN);
		break;
	...
	case BIDE_IOCTL_WHITELIST_PRIVILEGED_ZYGOTE:
		rc = auth_add_pid(current_pid, AUTH_PERM_PRIVILEGED_ZYGOTE);
		break;
	...
	case BIDE_IOCTL_REGISTER_BUGREPORT_FOR_CAPS:
		rc = auth_add_pid(current_pid, AUTH_PERM_BUGREPORT);
		break;

	case BIDE_IOCTL_REGISTER_ADBD_FOR_CAPS:
		rc = auth_add_pid(current_pid, AUTH_PERM_ADBD);
		break;

	case BIDE_IOCTL_WHITELIST_ZYGOTEMGR_CHILD:
		rc = auth_add_pid(current_pid, AUTH_PERM_ZYGOTEMGR_CHILD);
		rc = auth_add_pid(current_pid, AUTH_PERM_PRIVILEGED_GID);
		break;

	case BIDE_IOCTL_REGISTER_DEBUG_APP:
		rc = auth_add_debug_pid(current_pid);
		break;
