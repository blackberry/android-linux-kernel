				  ...
				  int flags)
{
	...
	if (!rc) {
		...
		logDebug("New process being hashed [%s]", path);
		  ...
		  int flags)
{
	...
	logDebug("Hashing Process: %s(%d, %d).", name, (unsigned int)task->pid, (unsigned int)task->tgid);
	...
	if(hash_mismatch == 1) {
		...
		rc = report_incident(SEVERITY_ERROR,
...
void vma_print_section_tree(void)
{
	...
	logInfo("Tree size: %d", count);
	logInfo("---- End section tree ----");
}
#endif
