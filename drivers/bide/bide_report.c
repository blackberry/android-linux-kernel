...
int report_end(bide_reports_t *rep)
{
	...
	bide_reports_t *dummy_report = NULL;
	...
	/* Report if the report we're trying to sign is not in the buffer
	 * and if there is a problem with the signature */
	invalid_report = report_verify_signature(rep);
	if (invalid_report && report_lookup_buffers(&dummy_report) == -ENOENT) {
		...
		rc = report_incident(SIGNATURE_INCIDENT,
					...
					current);
		if (rc)
			logError("Failed on report_incident() from report_end. rc=%d.", -rc);

		memset(rep->tzbide, 0, BIDE_MAX_REPORT_SIZE);
		memset(rep->sig, 0, ECC256_SIG_SIZE);
	} else if (invalid_report) {
		memcpy(rep->tzbide, &rc, sizeof(int));
	}
...
int report_unbind_current(bide_reports_t **rep)
{
	...
	if (report_verify_signature(buf)) {
		memcpy(&error_code, buf->tzbide, sizeof(int));

		rc = xml_nonce_mismatch_msg(msg, sizeof(msg), error_code);
		if (rc < 0) {
			logError("Failed on xml_property_msg(). rc=%d.", -rc);
			return rc;
		}
		...
		rc = report_incident(SIGNATURE_INCIDENT,
					...
					current);
		if (rc)
			logError("Failed on report_incident() from unbind. rc=%d.", -rc);

		memset(buf->tzbide, 0, BIDE_MAX_REPORT_SIZE);
		memset(buf->sig, 0, ECC256_SIG_SIZE);
...
void report_print_queue(void)
{
	...
	list_for_each(pos, &ctx.queue) {
		...
		logInfo("Size of report %d : %d", count, report->report_sz);
	}
	spin_unlock(&ctx.rwq.lock);

	logInfo("Report queue length = %d", count);
	logInfo("---- End Report queue ----");
}
#endif
