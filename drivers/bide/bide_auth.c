			...
			const char *value)
{
	...
	static const struct property_value_pair_t BANNED[] = {
		{"ro.debuggable",    sizeof("ro.debuggable"),       "1", sizeof("1") },
		{"service.adb.root", sizeof("ro.service.adb.root"), "1", sizeof("1") },
		{"ro.secure",        sizeof("ro.secure"),           "0", sizeof("0") },
		{"ro.kernel.qemu",   sizeof("ro.kernel.qemu"),      "1", sizeof("1") } 
        };
	...
	for (i = 0; i < COUNT_OF(BANNED); i++) {
		...
		/* these comparisons lengths include the null terminator,
                   so it will fail if the name or value string is shorter or longer
                   the banned property name */
		if (strncmp(BANNED[i].name, name, BANNED[i].nameLength))
			continue;

		if (strncmp(BANNED[i].value, value, BANNED[i].valueLength))
			continue;
