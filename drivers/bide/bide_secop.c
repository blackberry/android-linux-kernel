...
static void secop_task_free(struct task_struct *task)
{
	...
	perm &= ~(AUTH_PERM_ZYGOTEMGR | AUTH_PERM_ZYGOTEMGR_KID);
...
static int secop_bprm_set_creds(struct linux_binprm *bprm)
{
	...
	if (!auth_check_permission(current->pid, AUTH_PERM_PRIVILEGED_GID)) {

		/* If Zyogte class is created, add extra credential */
		if (!auth_check_permission(ppid, AUTH_PERM_ZYGOTEMGR))
			rc = auth_add_pid(current->pid,
					AUTH_PERM_ZYGOTEMGR_KID);
			if (rc)
				logError("Failed on auth_add_pid(). rc=%d.", -rc);
...
#endif
{
	...
	if (auth_check_permission(current->pid, AUTH_PERM_ZYGOTEMGR_KID) == 0)
		return 0;
			...
			const kernel_cap_t *permitted)
{
	...
	if (!ctl_snapshot_complete()) {
		...
		if(rc != 0) {
			caps_update_process(current->pid, rc);
		}

		return 0;
	}

	allowed_caps = caps_get_caps_for_process(current->pid);
