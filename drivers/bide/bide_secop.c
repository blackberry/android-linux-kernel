...
static void secop_task_free(struct task_struct *task)
{
	...
	rc = auth_remove_pid(task->pid, &perm);
	if (rc) {
		if (rc != -ENOENT)
...
static int secop_bprm_set_creds(struct linux_binprm *bprm)
{
	...
	if (current->pid == KTHREAD_PID ||
	    current->pid == INIT_PID)
		return 0;

	/* Ignore whitelisted processes */
	if (!auth_check_permission(current->pid, AUTH_PERM_ZYGOTEMGR) ||
	    !auth_check_permission(current->pid, AUTH_PERM_PRIVILEGED_ZYGOTE) ||
	    !auth_check_permission(current->pid, AUTH_PERM_PRIVILEGED_GID))
		return 0;


	/* If Parent can have privileged children, then promote this child */
	if (!auth_check_permission(sys_getppid(), AUTH_PERM_PRIVELEGED_CHILDREN) ||
	...
	if (!ctl_snapshot_complete() || sys_getppid() == INIT_PID) {
		rc = auth_add_pid(current->pid, AUTH_PERM_PRIVILEGED_GID);
		if (rc)
			logError("Failed on auth_add_pid(). rc=%d.", -rc);
	} else {
		rc = report_incident(SEVERITY_CRITICAL,
				     ...
				     current);
		if (rc)
			logDebug("Failed on report_incident(). rc=%d", -rc);
	}
				 ...
				 int flags)
{
	...
	if (perm)
		auth_drop_pid_permission(current->pid, perm);

	if (!auth_check_permission(sys_getppid(), AUTH_PERM_INSTALLD) ||
           (!auth_check_permission(current->pid, AUTH_PERM_INSTALLD))) {
logError("Registering PID:%d, (PPID:%d) for AUTH_PERM_SYSTEM_UID)", current->pid, (int)sys_getppid());
	...
	if (groups_search(new->group_info, ROOT_UID) ||
	    ...
	    new->uid % USER_UID == ROOT_UID ||
	    new->euid % USER_UID == ROOT_UID) {

		/* Check root permission was whitelisted */
		rc = auth_check_permission(current->pid, AUTH_PERM_PRIVILEGED_GID);
		if (!rc)
			return 0;
		...
		rc = report_incident(SEVERITY_CRITICAL,
				     ...
				     current);
		if (rc)
			logError("Failed on report_incident(). rc=%d.", -rc);
	}
	...
	if (new->uid % USER_UID == SYSTEM_UID) {
		...
		rc = auth_check_permission(current->pid, AUTH_PERM_SYSTEM_UID);
		if (!rc)
			return 0;
 ...
 *	process @p to @pgid.
 *
 * @param   p             The set of credentials that will be installed.
 * @param   pgid          The set of credentials that are being replaced.
 *
 * @return  0             Permission allowed.
 *          -EPERM        Permission denied.
 *          -ENOMEM       Out of memory, permission denied.
 */
static int secop_task_setpgid(struct task_struct *p,
			      pid_t pgid)
{
	int rc = 0;

	/* Don't interfere with the system during boot */
	if (!ctl_snapshot_complete())
		return 0;

	if (pgid == ROOT_UID || pgid == SYSTEM_UID) {
		rc = auth_check_permission(pgid, AUTH_PERM_PRIVILEGED_GID);
		if (rc == 0) {
			logDebug("Allowed pid:%ld->%d to run privileged.",
				 sys_getppid(),
				 current->pid);
			return 0;
		}

		/* The proposed group for the process is
		   not allowed */
		rc = report_incident(SEVERITY_CRITICAL,
				     SN_ESCALATED_GID,
				     NULL,
...
#endif
{
	...
	/* File system requires these flags during boot */
	if (flags & MS_NOSUID &&
	    flags & MS_NODEV)
		return 0;

	/* Allow the parent zygote processes to mount */
	if (auth_check_permission(sys_getppid(), AUTH_PERM_ZYGOTEMGR) == 0)
		return 0;
...
#endif
{
	...
	if (mmap_min_addr < CONFIG_DEFAULT_MMAP_MIN_ADDR) {
		logDebug("mmap_min_addr < CONFIG_DEFAULT_MMAP_MIN_ADDR");
			...
			const kernel_cap_t *permitted)
{
	...
	/* Check to see if the capabilites are allowed */
	if (auth_check_capabilities(new->uid, rc)) {
		char msg[EXTRA_XML_MESSAGE_LEN] = { 0 };
		char *p = msg;
...
int __init secop_init(void)
{
	...
	static struct security_hook_list hooks[] = {
		...
		LSM_HOOK_INIT(task_setpgid, 		secop_task_setpgid),
	...
	static struct security_operations sec_ops = {
		...
		.task_setpgid		= secop_task_setpgid,
