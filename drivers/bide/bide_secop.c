...
static const struct gid_whitelist_t GID_WHITELIST[] = {
	{"/system/vendor/bin/loc_launcher", PRIVILEGED_GID_IDX_TO_MASK(1)}
};
...
static void secop_task_created_notify(struct task_struct *task, unsigned long flags)
{
	...
	if (util_get_tgid(task) == IDLE_PID)
		return;
...
static int secop_bprm_set_creds(struct linux_binprm *bprm)
{
	...
	if (auth_has_permission(ppid, AUTH_PERM_PRIVILEGED_CHILDREN)) {
				 ...
				 int flags)
{
	...
	const struct process_info *ppinfo = NULL;
	pid_t proc_ppid = -1;
	...
	if ((ROOT_UID == new_uid) || (ROOT_UID == new_euid)) {
		...
		if (auth_does_not_have_permission(current_pid, AUTH_PERM_PRIVILEGED)) {
			pinfo = proc_get_info(current_pid);
			if (pinfo != NULL) {
				ppinfo = proc_get_parent(pinfo);
				if (ppinfo != NULL) {
					proc_ppid = ppinfo->pid;
					proc_put_info(ppinfo);
				}
				proc_put_info(pinfo);
			}

			/* This hook may be called before zygotes can register */
			if (proc_ppid != -1 && auth_has_permission(proc_ppid, AUTH_PERM_ZYGOTEMGR)) {
				auth_add_pid(current_pid, AUTH_PERM_PRIVILEGED);
			} else {
