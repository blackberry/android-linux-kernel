			 ...
			 struct task_struct *task)
{
	...
	rc = xml_write_tag(&ctx, PROCESS_START_TAG, task->pid) |
	     ...
	     xml_write_tag(&ctx, ECRED_TAG, (unsigned int)cred->euid.val, (unsigned int)cred->egid.val) |
	     xml_write_tag(&ctx, RCRED_TAG, (unsigned int)cred->uid.val, (unsigned int)cred->gid.val);
#else
		     ...
		     unsigned sz)
{
	...
	rc = xml_write_tag(&ctx, REPORT_START_TAG) |
	     ...
	     xml_write_tag(&ctx, FINGERPRINT_TAG, util_get_build_fingerprint());
	if (rc < 0)
		logError("Failed on xml_write_tag(). rc=%d.", -rc);
