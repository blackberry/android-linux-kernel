...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 18, 0)
	if ((cred) && 
	    (uid_eq(cred->euid, GLOBAL_ROOT_UID) || gid_eq(cred->egid, GLOBAL_ROOT_GID)))
		rc = 1;
#else
	if (cred)
