		     ...
		     uint8_t *hash)
{
	int rc = -EINVAL;
	if (sz && data && hash) {
		rc = crypto_once(HASH_ALG_SHA256, data, sz, hash, SHA256_SIZE);
				   ...
				   struct rb_node **parent)
{
	...
	while (n) {
		...
		ret = memcmp(hash, t->hash, SHA256_SIZE);
		...
		void *data)
{
	...
	key_hash = kzalloc(sizeof(uint8_t) * SHA256_SIZE, GFP_ATOMIC);
	if (!key_hash) {
	...
	node = (struct hash_node *) kzalloc(sizeof(struct hash_node), GFP_ATOMIC);
	if (!node) {
	...
	if (!parent)
		...
		ret = memcmp(pnode->hash, key_hash, SHA256_SIZE);
