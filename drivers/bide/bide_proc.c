...
static struct process_info* pinfo_begin_create_or_update(int pid, int ppid,
							 struct process_info_update_ctx *ctx)
{
	...
	if (!ctx->node) {
		...
		ctx->node = pinfo_create(pid, parent_pid);
		if (!ctx->node) {
...
const struct process_info* __proc_get_info(int pid)
{
	...
	if (node == NULL) {
		struct process_info_update_ctx ctx;
		pinfo = pinfo_begin_create_or_update(pid, -1, &ctx);
		if (pinfo)
...
const struct process_info* __proc_create_info(int pid, int ppid)
{
	...
	if (node == NULL) {
		struct process_info_update_ctx ctx;
		pinfo = pinfo_begin_create_or_update(pid, ppid, &ctx);
		if (pinfo)
			pinfo_finish_update(&ctx);
	} else {
		return NULL;
