...
/* UID numbers in the system, from the following files:
 ...
 - Processs.java */
	...
	#define ROOT_GID			KGIDT_INIT(0)
	#define SYSTEM_GID			KGIDT_INIT(1000)
	#define ROOT_UID			KUIDT_INIT(0)
	#define SYSTEM_UID			KUIDT_INIT(1000)
	#define PHONE_UID			KUIDT_INIT(1001)
	#define BLUETOOTH_UID			KUIDT_INIT(1002)
	#define WIFI_UID			KUIDT_INIT(1010)
	#define INSTALLER_UID			KUIDT_INIT(1012)
	#define DHCP_UID			KUIDT_INIT(1014)
	#define GPS_UID				KUIDT_INIT(1021)
	#define CLAT_UID			KUIDT_INIT(1029)
	#define SHELL_UID			KUIDT_INIT(2000)
	#define BIDE_UID			KUIDT_INIT(8000)
	#define NOBODY_UID			KUIDT_INIT(9999)
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 18, 0)
int util_uid_eq(kuid_t uid1, kuid_t uid2);
#else
int util_uid_eq(int uid1, int uid2);
#endif
