...
#define ECC256_PUBL_KEY_SIZE            65      // ECC256 public key size
...
#define BSIS_PRIVATE_KEY_STORE_SIZE     ( sizeof( bsis_priv_key_store_t ) )
...
#define TZ_SECTION_FLAG_CONFIRM_KNOWN_HASH              (1<<1)      /* Compare with precalculated hash passed in via in_buf.mem_hash */
...
#define TZ_CMD_BIDE_VALIDATE_KEYPAIR                    2005

/* This is the only error code we currently use coming from trustzone.
 * This value should be updated if it is in trustzone: tzbb_common.h.
 * It's used to differenciate a nonce mismatch error to any other
 * error. */
#define TZ_BIDE_NONCE_HASH_MISMATCH_PUBLIC              144

// BIDE storage
#define TZ_CMD_BIDE_STORAGE_READ                        1131
#define TZ_CMD_BIDE_STORAGE_UPDATE_SENSOR               1132

/* The number of BIDE Sensors supported in KBIDE/JBIDE */
#define BIDE_STORAGE_NUMBER_SENSORS     64

/* The type of sensor to update */
typedef enum {
    KBIDE_SENSOR = 0,
    JBIDE_SENSOR = 1,
} bide_sensor_type_t;
 ...
 * flags - special details of the memory section (e.g. REMOVABLE)
...
//------------------------------------------------------------------------------
// BIDE storage structures
//------------------------------------------------------------------------------

/**
 * [IN] Send command to retrieve the BIDE database (KBIDE and JBIDE) from TZ
 */
typedef struct {
	uint32_t cmd;                                /* read command id */
} bide_storage_read_in_t;
/**
 * [OUT] Returns the status of the read operation and the database itself
 */
typedef struct {
	uint32_t status;                             /* TZ_CODE_SUCCESS on success */
	uint32_t kbide[BIDE_STORAGE_NUMBER_SENSORS]; /* array of ids per KB sensor */
	uint32_t jbide[BIDE_STORAGE_NUMBER_SENSORS]; /* array of ids per JB sensor */
} bide_storage_read_out_t;

/**
 * [IN] Used to send a BIDE database update to the TZ database
 */
typedef struct {
	uint32_t cmd;            /* update sensor command id */
	uint32_t sensor_type;    /* bide_sensor_type_t val: KBIDE or JBIDE */
	uint32_t sensor_index;   /* must range (0 .. BIDE_STORAGE_NUMBER_SENSORS - 1) */
	uint32_t os_version;     /* build AAA000 format transformed into uint32_t */
} bide_storage_update_sensor_in_t;
/**
 * [OUT] Returns the status of the sensor update operation
 */
typedef struct {
	uint32_t status;                             /* TZ_CODE_SUCCESS on success */
} bide_storage_update_sensor_out_t;

/**
 * [IN] Validate the BIDE public/private key pair
 * nonce_hash - sha512 hash of session nonce and counter (nonce_data_t)
 * bide_public_key - BIDE public key, in octet format
 * bide_priv_key_store - BIDE private key, encrypted
 */
typedef struct {
    uint32_t cmd;
    uint8_t nonce_hash[SHA512_SIZE];
    uint8_t public_key[ECC256_PUBL_KEY_SIZE];
    uint8_t padding[3];
    bide_priv_key_store_t priv_key_store;
} bide_validate_keypair_in_t;
/**
 * [OUT] Validate the BIDE public/private key pair
 * status - result of the validation process (pass/fail)
 */
typedef struct {
    uint32_t status;
} bide_validate_keypair_out_t;
