...
#ifdef CONFIG_BBRY
#define QPNP_MAXIMUM_INTENSITY	100
#define QPNP_DEFAULT_INTENSITY	QPNP_MAXIMUM_INTENSITY
#endif /* CONFIG_BBRY */
...
#ifdef CONFIG_BBRY
 *  @ vmin_mv - min voltage in mv
 *  @ int intensity - vibration intensity percentage
#endif
 */
struct qpnp_hap {
...
static void qpnp_hap_td_enable(struct timed_output_dev *dev, int value)
{
...
#ifdef CONFIG_BBRY
	intensity_mv = (hap->intensity * (hap->vmax_mv - hap->vmin_mv)) / 100;
	intensity_mv += hap->vmin_mv;
	if (intensity_mv < QPNP_HAP_VMAX_MIN_MV)
		intensity_mv = QPNP_HAP_VMAX_MIN_MV;
	else if (intensity_mv > QPNP_HAP_VMAX_MAX_MV)
		intensity_mv = QPNP_HAP_VMAX_MAX_MV;

	/* After setting the intensity, restore to default to be backwards
	 * compatible with older applications that do not support this */
	hap->intensity = QPNP_DEFAULT_INTENSITY;
	...
	reg &= QPNP_HAP_VMAX_MASK;
	temp = intensity_mv / QPNP_HAP_VMAX_MIN_MV;
	reg |= (temp << QPNP_HAP_VMAX_SHIFT);
...
#ifdef CONFIG_BBRY
static void qpnp_hap_set_intensity(struct timed_output_dev *dev, int value)
{
	struct qpnp_hap *hap = container_of(dev, struct qpnp_hap,
					 timed_dev);

	if (value > QPNP_MAXIMUM_INTENSITY)
		hap->intensity = QPNP_MAXIMUM_INTENSITY;
	else if (value < 0)
		hap->intensity = 0;
	else
		hap->intensity = value;
...
static int qpnp_hap_get_intensity(struct timed_output_dev *dev)
{
	struct qpnp_hap *hap = container_of(dev, struct qpnp_hap,
							 timed_dev);

	return hap->intensity;
}

static ssize_t intensity_show(struct device *dev, struct device_attribute *attr,
		char *buf)
{
	struct timed_output_dev *tdev = dev_get_drvdata(dev);
	struct qpnp_hap *hap = container_of(tdev, struct qpnp_hap,
							 timed_dev);
	int intensity;

	mutex_lock(&hap->lock);
	intensity = tdev->get_intensity(tdev);
	mutex_unlock(&hap->lock);

	return sprintf(buf, "%d\n", intensity);
}

static ssize_t intensity_store(
		...
		const char *buf, size_t size)
{
	...
	/* In case intensity is modified while we are programming
	 * the vibrator */
	mutex_lock(&hap->lock);
	tdev->set_intensity(tdev, value);
	mutex_unlock(&hap->lock);

	return size;
}

static DEVICE_ATTR(intensity, S_IRUGO | S_IWUSR | S_IWGRP, intensity_show, intensity_store);
#endif /* CONFIG_BBRY */
...
#ifdef CONFIG_BBRY
	hap->vmin_mv = QPNP_HAP_VMAX_MIN_MV;
	rc = of_property_read_u32(spmi->dev.of_node,
			"bbry,vmin-mv", &temp);
	if (!rc) {
		hap->vmin_mv = temp;
	} else if (rc != -EINVAL) {
		dev_err(&spmi->dev, "Unable to read vmin\n");
		return rc;
	}
#endif /* CONFIG_BBRY */
...
#ifdef CONFIG_BBRY
	hap->timed_dev.set_intensity = qpnp_hap_set_intensity;
	hap->timed_dev.get_intensity = qpnp_hap_get_intensity;
#endif /* CONFIG_BBRY */
...
#ifdef CONFIG_BBRY
	rc = device_create_file(hap->timed_dev.dev, &dev_attr_intensity);
	if (rc < 0) {
		dev_err(&spmi->dev, "Failed to create file %s\n", dev_attr_intensity.attr.name);
		goto timed_output_fail;
	}
#endif /* CONFIG_BBRY */
...
static int qpnp_haptic_remove(struct spmi_device *spmi)
{
	...
	timed_output_dev_unregister(&hap->timed_dev);
#ifdef CONFIG_BBRY
	device_remove_file(hap->timed_dev.dev, &dev_attr_intensity);
#endif /* CONFIG_BBRY */
	mutex_destroy(&hap->lock);
	mutex_destroy(&hap->wf_lock);
