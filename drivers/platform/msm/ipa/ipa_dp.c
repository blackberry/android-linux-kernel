...
{
	int ret;
/* MODIFIED-BEGIN by jianhua.yu, 2016-05-18,BUG-2167045*/
/* TCT-SH jhyu fxi 2167042 rm  05-18-2016 */
/*	if (!atomic_read(&sys->curr_polling_state)) {
		IPAERR("already in intr mode\n");
		goto fail;
	}
...
/* TCT-SH jhyu fxi 2167042 add   05-18-2016 */
	if (!atomic_read(&sys->curr_polling_state) &&
...
/* TCT-SH jhyu fxi 2167042 end 05-18-2016 */
/* MODIFIED-END by jianhua.yu,BUG-2167045*/
	sys->event.options = SPS_O_EOT;
	ret = sps_register_event(sys->ep->ep_hdl, &sys->event);
	if (ret) {
		IPAERR("sps_register_event() failed %d\n", ret);
