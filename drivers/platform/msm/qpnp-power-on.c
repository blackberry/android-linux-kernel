...
#else
#define QPNP_PON_HARD_RESET_MASK		PON_MASK(5, 0)
#endif
...
#ifdef CONFIG_BBRY
#define QPNP_LID_STATUS_DELAY			msecs_to_jiffies(1000)
#endif /* CONFIG_BBRY */
...
static void qpnp_pon_input_event(struct input_handle *handle,
				unsigned int type,
				unsigned int code, int value);
static int qpnp_pon_input_event_connect(struct input_handler *handler,
					struct input_dev *dev,
					const struct input_device_id *id);
static void qpnp_pon_input_event_disconnect(struct input_handle *handle);

static struct input_handler qpnp_pon_input_event_handler = {
	.event		= qpnp_pon_input_event,
	.connect	= qpnp_pon_input_event_connect,
	.disconnect	= qpnp_pon_input_event_disconnect,
	.minor		= 0,
	.name		= "qpnp_pon",
...
#else
	rc = qpnp_pon_masked_write(pon, QPNP_PON_XVDD_RB_SPARE(pon->base),
...
#ifdef CONFIG_BBRY
	if (pon->lid_state && cfg->key_code == KEY_POWER) {
		dev_dbg(&pon->spmi->dev, "Lid is on, ignore power key press\n");
		return rc;
	}
#endif /* CONFIG_BBRY */
...
#else
		/* S1+S2 reset control (timer values, enable and reset type)
		   for KPDPWR will be provided by bootchain. */
		return 0;
#endif /* CONFIG_BBRY */
		break;
	case PON_RESIN:
...
static int qpnp_pon_config_init(struct qpnp_pon *pon)
{
	...
	if (pon->pon_input) {
		...
		input_enable_events_log(pon->pon_input, NULL);
...
#ifdef CONFIG_INPUT_EVENTS_BUFFER
	if (pon->pon_input)
		input_disable_events_log(pon->pon_input);
#endif /*CONFIG_INPUT_EVENTS_BUFFER*/
#endif /*CONFIG_BBRY*/
	if (pon->pon_input)
...
#ifdef CONFIG_BBRY
static void qpnp_pon_input_event(struct input_handle *handle, unsigned int type,
						unsigned int code, int value)
{
	struct qpnp_pon *pon = handle->private;

	if ((type == EV_SW) && (code == SW_LID)) {
		pon->lid_state = value;
		dev_dbg(&pon->spmi->dev, "lid_stat = %d\n", pon->lid_state);
	}
}
					...
					const struct input_device_id *id)
{
	...
	struct qpnp_pon *pon = handler->private;
...
static void lid_work_func(struct work_struct *work)
{
	int rc;
	struct qpnp_pon *pon =
		container_of(work, struct qpnp_pon, lid_work.work);

	/* Create input handler for lid events */
	qpnp_pon_input_event_handler.private = pon;
	rc = input_register_handler(&qpnp_pon_input_event_handler);
	if (rc)
		dev_err(&pon->spmi->dev,
...
static int qpnp_pon_probe(struct spmi_device *spmi)
{
...
#ifndef CONFIG_BBRY  /* BBRY S3 configured in SBL */
	/* program s3 debounce */
...
#ifdef CONFIG_BBRY
	if (pon->lid_state_aware)
		INIT_DELAYED_WORK(&pon->lid_work, lid_work_func);
#endif /* CONFIG_BBRY */
	...
	/* Create input handler for lid events */
	if (pon->lid_state_aware)
		schedule_delayed_work(&pon->lid_work, QPNP_LID_STATUS_DELAY);
