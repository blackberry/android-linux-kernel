...
#else
	pr_info("set hard reason 0x%x at 0x%x\n", reason, QPNP_PON_XVDD_RB_SPARE(pon->base));
...
#ifdef CONFIG_BBRY
	/* PMI set to Dvdd shutdown with PM Dvdd reset or shutdown */
	if (type == PON_POWER_OFF_HARD_RESET)
		type = PON_POWER_OFF_SHUTDOWN;
#endif
...
/* [BUFFIX]-end- by TCTSH.XQJ,*/
...
#ifdef CONFIG_BBRY
	pon->store_hard_reset_reason = 1;
	pr_info("bbry use hard reset reason\n");
#else
	...
	/* Set reset type if it's warm, otherwise use the default bootchain setting */
	type = pwr_off_default_type();
	if (type == PON_POWER_OFF_WARM_RESET )
		qpnp_pon_system_pwr_off(type);
#endif /* CONFIG_BBRY */
