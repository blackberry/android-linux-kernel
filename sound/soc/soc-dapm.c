				...
				const char *pin, int status)
{
	...
	mutex_lock_nested(&dapm->card->dapm_mutex, SND_SOC_DAPM_CLASS_PCM);/*TCT-NB Tianhongwei add CR.1004036*/
	if (!w) {
		dev_err(dapm->dev, "ASoC: DAPM unknown pin %s\n", pin);
		mutex_unlock(&dapm->card->dapm_mutex);/*TCT-NB Tianhongwei add CR.1004036*/
		return -EINVAL;
	}
...
//	mutex_lock_nested(&card->dapm_mutex, SND_SOC_DAPM_CLASS_RUNTIME);/*TCT-NB Tianhongwei  CR.1004036*/
...
//	mutex_unlock(&card->dapm_mutex);/*TCT-NB Tianhongwei CR.1004036*/
