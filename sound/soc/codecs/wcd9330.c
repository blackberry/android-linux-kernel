...
#ifdef CONFIG_BBRY
/* increase the settle time to 21ms according to QC suggestion in case 02296071 */
#define TOMTOM_HPH_PA_SETTLE_COMP_OFF 21000
#else
...
static int tomtom_digital_mute(struct snd_soc_dai *dai, int mute)
{
	...
	mute = (mute) ? 1 : 0;
	/* sleep for 40ms before unmuting the TX path as per HW requirement */
