 ...
 */
//#define DEBUG // MODIFIED by hongwei.tian, 2017-03-06,BUG-4301452
...
#define MBHC_BUTTON_PRESS_THRESHOLD_MIN 1000 // MODIFIED by hongwei.tian, 2017-03-29,BUG-4445450
...
int mb_2p7v_flg = 0; // MODIFIED by hongwei.tian, 2017-06-02,BUG-4883765
				...
				enum snd_jack_types jack_type)
{
	...
	if (!insertion) {
		...
		/* MODIFIED-BEGIN by hongwei.tian, 2017-06-01,BUG-4881899*/
		if(mbhc->is_selfie_stick_insert)
			mbhc->jiffies_atreport = jiffies;
			/* MODIFIED-END by hongwei.tian,BUG-4881899*/
					 ...
					 enum wcd_mbhc_plug_type plug_type)
{
	...
	if (plug_type == MBHC_PLUG_TYPE_HEADPHONE) {
		...
		mbhc->is_selfie_stick_insert = true; // MODIFIED by hongwei.tian, 2017-06-01,BUG-4881899
		...
		if (mbhc->mbhc_cfg->detect_extn_cable && !mbhc->is_selfie_stick_insert) { // MODIFIED by hongwei.tian, 2017-06-01,BUG-4881899
...
static int wcd_check_cross_conn(struct wcd_mbhc *mbhc)
{
	...
	s16 reg1;
	bool hphl_sch_res, hphr_sch_res,mic_sch; // MODIFIED by hongwei.tian, 2017-03-06,BUG-4301452
	...
	pr_debug("%s: swap_res  %x\n", __func__, swap_res); // MODIFIED by hongwei.tian, 2016-12-14,BUG-3672951
	...
	/* MODIFIED-BEGIN by hongwei.tian, 2017-03-06,BUG-4301452*/
	WCD_MBHC_REG_READ(WCD_MBHC_MIC_SCHMT_RESULT, mic_sch);
	pr_debug("%s: hphl_sch_res:%d  hphr_sch_res: %d   mic_sch :%d \n", __func__, hphl_sch_res,hphr_sch_res,mic_sch); // MODIFIED by hongwei.tian, 2016-12-14,BUG-3672951
	/* MODIFIED-END by hongwei.tian,BUG-4301452*/
...
static void wcd_correct_swch_plug(struct work_struct *work)
{
	...
	int cross_conn;
	int try = 0;
	int btn_press_flg = 0; // MODIFIED by hongwei.tian, 2017-04-14,BUG-4566371
	...
	WCD_MBHC_REG_READ(WCD_MBHC_HS_COMP_RESULT, hs_comp_res);
	pr_debug("%s: hs_comp_res: %d  btn_result :%d \n", __func__,hs_comp_res,btn_result); // MODIFIED by hongwei.tian, 2016-12-14,BUG-3672951
	if (!rc) {
	...
	pr_debug("%s: plug_type:%d \n", __func__,plug_type); // MODIFIED by hongwei.tian, 2016-12-14,BUG-3672951
	do {
...
correct_plug_type:
	...
	while (!time_after(jiffies, timeout)) {
		...
		pr_debug("%s: hs_comp_res : %d  is_pa_on: %d \n", __func__,hs_comp_res ,is_pa_on); // MODIFIED by hongwei.tian, 2016-12-14,BUG-3672951
		if ((!hs_comp_res) && (!is_pa_on)) {
			...
			if (ret < 0) {
				...
				btn_press_flg = 0; // MODIFIED by hongwei.tian, 2017-04-14,BUG-4566371
				...
				plug_type = MBHC_PLUG_TYPE_HEADSET;
				/* MODIFIED-BEGIN by hongwei.tian, 2017-04-14,BUG-4566371*/
				if(mbhc->btn_press_intr)
					btn_press_flg ++;

				pr_debug("%s: no_gnd_mic_swap_cnt : %d  \n", __func__,no_gnd_mic_swap_cnt);
				pr_debug("%s: btn_press_intr : %d  \n", __func__,mbhc->btn_press_intr);
				/* MODIFIED-END by hongwei.tian,BUG-4566371*/
				...
				else
				{
					no_gnd_mic_swap_cnt = 0;
					/* MODIFIED-BEGIN by hongwei.tian, 2017-04-14,BUG-4566371*/
					pr_debug("%s: btn_press_flg 11  : %d  \n", __func__,btn_press_flg);
					if(mbhc->btn_press_intr && (btn_press_flg >3 ))
					{
						pr_debug("%s: might slow insertion of headphone\n", __func__);
						wcd_cancel_btn_work(mbhc);
						plug_type = MBHC_PLUG_TYPE_HEADPHONE;
						goto report;
					}
		...
		WCD_MBHC_REG_READ(WCD_MBHC_MIC_SCHMT_RESULT, mic_sch);
		pr_debug("%s: hs_comp_res : %d  hphl_sch: %d  mic_sch: %d \n", __func__,hs_comp_res ,hphl_sch,mic_sch); // MODIFIED by hongwei.tian, 2016-12-14,BUG-3672951
	...
	if (plug_type == MBHC_PLUG_TYPE_HIGH_HPH &&
		(!det_extn_cable_en)&& (!mb_2p7v_flg)) { // MODIFIED by hongwei.tian, 2017-06-02,BUG-4883765
...
/*
	if (mbhc->mbhc_cfg->detect_extn_cable &&
		...
		WCD_MBHC_RSC_UNLOCK(mbhc);
	}*//*TCT-NB Tianhongwei remove for high impedance headset  2016-04-28 defect.1966378 */
	if (mbhc->mbhc_cb->set_cap_mode)
...
static void wcd_mbhc_swch_irq_handler(struct wcd_mbhc *mbhc)
{
	...
	if ((mbhc->current_plug == MBHC_PLUG_TYPE_NONE) &&
		...
		mbhc->is_selfie_stick_insert = false; // MODIFIED by hongwei.tian, 2017-06-01,BUG-4881899
...
static int wcd_mbhc_get_button_mask(struct wcd_mbhc *mbhc)
{
	...
	/* MODIFIED-BEGIN by hongwei.tian, 2017-04-24,BUG-4640994*/
	if(mbhc->mbhc_cfg->key_code[btn] == 0)
		return 0;
		/* MODIFIED-END by hongwei.tian,BUG-4640994*/
...
static irqreturn_t wcd_mbhc_btn_press_handler(int irq, void *data)
{
	...
	if (mbhc->current_plug != MBHC_PLUG_TYPE_HEADSET && !mbhc->is_selfie_stick_insert) { // MODIFIED by hongwei.tian, 2017-06-01,BUG-4881899
