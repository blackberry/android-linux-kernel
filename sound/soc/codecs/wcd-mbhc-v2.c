...
#ifdef CONFIG_BBRY
#define WCD_BUTTON_INIT_MASK 0x00
#endif
#define OCP_ATTEMPT 1
#define HS_DETECT_PLUG_TIME_MS (3 * 1000)
/*[Defect]-Added-BEGIN by TCTSH.haitao.yin, 2994281, 2016/09/24, and headset insert detect deif (hs_comp_res && !(hphl_sch || mic_sch))bounce to avoid slow insertion*/
#define TIME_AFTER_HPH_REPORT_MS (5 * 1000)
/*[Defect]-Added-END   by TCTSH.haitao.yin, 2994281, 2016/09/24, and headset insert detect debounce to avoid slow insertion*/
				...
				const enum wcd_mbhc_cs_mb_en_flag cs_mb_en)
{
	...
	case WCD_MBHC_EN_CS:
		...
		wcd_program_btn_threshold(mbhc, false);
		 /*[983752]-Add-Begin by TCTSH@YK, 2015/12/04, To be compatible with Iphone headset*/
		 /*enable micbias for U recording using iphone headset in CS mode */
		if(micbias2_en)
			mbhc->mbhc_cb->mbhc_micb2_2v7_ctrl(codec,true);
			/*For wait for voltage to 2v15 */
			msleep(50);
		/*[983752]-Add-End by TCTSH@YK, 2015/12/04, To be compatible with Iphone headset*/
		break;
	case WCD_MBHC_EN_MB:
        ...
        /*[983752]-Add-Begin by TCTSH@YK, 2015/12/04, To be compatible with Iphone headset*/
	    /*in here to enable micbias to detect iphone headset,voltage to 2v15,can see 80-NK808-2X@page 1025*/
		mbhc->mbhc_cb->mbhc_micb2_2v7_ctrl(codec,true);
		/*For wait for voltage to 2v15 */
		msleep(50);
        /*[983752]-Add-End by TCTSH@YK, 2015/12/04, To be compatible with Iphone headset*/
...
#ifdef CONFIG_BBRY
		mbhc->headset_init_complete = WCD_BUTTON_INIT_MASK;
#endif
	} else {
		/*
...
static int wcd_check_cross_conn(struct wcd_mbhc *mbhc)
{
	...
	/*[FEATURE]-Add-BEGIN by TCTSH.Cedar, 526254, 2015/10/23, fix slow detection for headphone*/
	#if defined(CONFIG_TCT_8X76_COMMON)
	return false;
	#endif
	/*[FEATURE]-Add-END   by TCTSH.Cedar, 526254, 2015/10/23, fix slow detection for headphone*/
...
static void wcd_correct_swch_plug(struct work_struct *work)
{
	...
	/* Enable HW FSM */
	WCD_MBHC_REG_UPDATE_BITS(WCD_MBHC_FSM_EN, 0);/*TCT-NB Tianhongwei add for headphone detect*/
	WCD_MBHC_REG_UPDATE_BITS(WCD_MBHC_FSM_EN, 1);
	/*
	...
	if (!rc) {
		...
		if (!btn_result && !hs_comp_res)
			...
			mbhc->frist_type=MBHC_PLUG_TYPE_HIGH_HPH;
...
/*[BUGFIX]-Add-BEGIN by TCTNB.bin.su,12/03/2015,Task850685,macro optimization.*/
//Begin for MBHC, more identification precision, by Kun.Guan & Xing.Wang, 2015-08-26
#if 0 //defined(CONFIG_TCT_8X76_COMMON)// haitao.yin fix a error about headset type detection, 20160924
			plug_type = MBHC_PLUG_TYPE_HEADSET;
			wrk_complete = false;
#else
...
enable_supply:
#ifdef CONFIG_BBRY
	mbhc->headset_init_complete |= WCD_MBHC_JACK_BUTTON_MASK;
#endif
	if (mbhc->mbhc_cb->mbhc_micbias_control)
...
static void wcd_mbhc_swch_irq_handler(struct wcd_mbhc *mbhc)
{
	...
	bool micbias1 = false;
	struct snd_soc_codec *codec = mbhc->codec;
	mbhc->time_after_hph_report_xs = 0;
	...
	WCD_MBHC_RSC_UNLOCK(mbhc);
	mbhc->time_after_hph_report_xs= jiffies + msecs_to_jiffies(TIME_AFTER_HPH_REPORT_MS);
	pr_debug("%s: leave\n", __func__);
}
...
/*TCT-NB Tianhongwei not report undefined key*/
...
/*[Defect]-Added-BEGIN by TCTSH.haitao.yin, 2994281, 2016/09/24, and headset insert detect debounce to avoid slow insertion*/
...
static void wcd_btn_lpress_fn(struct work_struct *work)
{
	...
	WCD_MBHC_REG_READ(WCD_MBHC_BTN_RESULT, btn_result);
	if (mbhc->current_plug == MBHC_PLUG_TYPE_HEADSET) {
		if((mbhc->time_after_hph_report_xs > 0) && (time_after(jiffies, mbhc->time_after_hph_report_xs))){
		pr_debug("%s: Reporting long button press event, btn_result: %d\n",
			 __func__, btn_result);
		wcd_mbhc_jack_report(mbhc, &mbhc->button_jack,
				mbhc->buttons_pressed, mbhc->buttons_pressed);
			} else {
		pr_debug("%s: long btn press event discard\n", __func__);
		}
	}
	pr_debug("%s: leave\n", __func__);
	mbhc->mbhc_cb->lock_sleep(mbhc, false);
...
static irqreturn_t wcd_mbhc_btn_press_handler(int irq, void *data)
{
	...
	/*Begin_add TCTSH@YK , Defect:1119400 ,2016/01/05, Not support wired selfie stick capture*/
	if ((mbhc->current_plug != MBHC_PLUG_TYPE_HEADSET)&&(mbhc->frist_type==MBHC_PLUG_TYPE_HIGH_HPH)){
	pr_debug("%s: Plug isn't headset, It is HIGH_HPH TYPE ,it is selfie stick ,maybe\n",
				__func__);
	}
	/*End_add TCTSH@YK ,Defect:1119400,2016/01/05, Not support wired selfie stick capture*/
	else if (mbhc->current_plug != MBHC_PLUG_TYPE_HEADSET) {
...
#ifdef CONFIG_BBRY
	mbhc->buttons_pressed &= mbhc->headset_init_complete;
#endif
	mbhc->mbhc_cb->lock_sleep(mbhc, true);
...
static irqreturn_t wcd_mbhc_release_handler(int irq, void *data)
{
	...
	if (mbhc->buttons_pressed & WCD_MBHC_JACK_BUTTON_MASK) {
		...
		if (ret == 0) {
			...
			pr_debug("%s: long btn release event discard\n", __func__);
			}
		} else {
			if (mbhc->in_swch_irq_handler) {
				pr_debug("%s: Switch irq kicked in, ignore,ret=%d\n",
					__func__,ret);
			} else {
				if((mbhc->time_after_hph_report_xs > 0) && (time_after(jiffies, mbhc->time_after_hph_report_xs))){
				pr_debug("%s: Reporting btn press,ret=%d\n",
					 __func__,ret);
				wcd_mbhc_jack_report(mbhc,
						     &mbhc->button_jack,
						     mbhc->buttons_pressed,
				...
				pr_debug("%s: short btn event discard\n", __func__);
				}
			}
		}
		      ...
		      bool impedance_det_en)
{
	...
	/*Begin_add TCTSH@YK , Defect:1119400 ,2016/01/05, Not support wired selfie stick capture*/
	mbhc->frist_type=MBHC_PLUG_TYPE_INVALID;
	/*End_add TCTSH@YK ,Defect:1119400,2016/01/05, Not support wired selfie stick capture*/
