...
#define AW87319_DRIVER_VERSION  "v1.2.0"
...
static int i2c_write_reg(unsigned char reg_addr, unsigned char reg_data)
{
    ...
    while(cnt < AW_I2C_RETRIES) {
        ret = i2c_smbus_write_byte_data(aw87319->i2c_client, reg_addr, reg_data);
        if(ret < 0) {
...
static ssize_t aw87319_get_reg(struct device* cd,struct device_attribute *attr, char* buf)
{
    ...
    for(i=0;i<0x10;i++)
    {
        ...
        len += snprintf(buf+len, PAGE_SIZE-len, "reg:0x%02x=0x%02x, ", i,reg_val);
    }
...
static ssize_t aw87319_get_mode(struct device* cd,struct device_attribute *attr, char* buf)
{
    ssize_t len = 0;
...
static ssize_t aw87319_set_mode(struct device* cd, struct device_attribute *attr, const char* buf, size_t len)
{
    unsigned int databuf[16];
...
MODULE_LICENSE("GPL v2"); // MODIFIED by hongwei.tian, 2018-03-02,BUG-6049146
