...
/*TCT-NB Tianhongwei add for HAC PA . Task 2340228 2016-06-13*/
struct hac_pinctrl_info {
	struct pinctrl *pinctrl;
	struct pinctrl_state *hac_pa_sus;
	struct pinctrl_state *hac_pa_act;
};
static struct hac_pinctrl_info hac_pinctrl_info;
/*TCT-NB Tianhongwei end 2016-06-13*/
				     ...
				     int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		 ...
		 */
/*TCT-NB Tianhongwei add for HAC PA . Task 2340228 2016-06-13*/
		if (!IS_ERR_OR_NULL(hac_pinctrl_info.pinctrl)) {
			if (pinctrl_select_state(hac_pinctrl_info.pinctrl,
					hac_pinctrl_info.hac_pa_act))
				pr_err("%s: Failed to active hac_pa_act pins\n", __func__);
		}
/*TCT-NB Tianhongwei end 2016-06-13*/
		usleep_range(5000, 5500);
...
/*TCT-NB Tianhongwei add for HAC PA . Task 2340228 2016-06-13*/
		if (!IS_ERR_OR_NULL(hac_pinctrl_info.pinctrl)) {
			if (pinctrl_select_state(hac_pinctrl_info.pinctrl,
					hac_pinctrl_info.hac_pa_sus))
				pr_err("%s: Failed to suspend hac_pa_sus pins\n", __func__);
		}
/*TCT-NB Tianhongwei end 2016-06-13*/
		break;
	};
...
/*TCT-NB Tianhongwei add for HAC PA . Task 2340228 2016-06-13*/
static int ext_hac_pa_pinctrl(struct device *dev)
{
	struct pinctrl *pinctrl;
	...
	hac_pinctrl_info.pinctrl = pinctrl;

	hac_pinctrl_info.hac_pa_act = pinctrl_lookup_state(pinctrl, "hac_pa_act");
	if (IS_ERR_OR_NULL(hac_pinctrl_info.hac_pa_act)) {
		pr_err("%s: Unable to get pinctrl hac_pa_act state handle, err: %ld\n",
				__func__, PTR_ERR(hac_pinctrl_info.hac_pa_act));
		goto pin_cfg_error;
	}

	hac_pinctrl_info.hac_pa_sus = pinctrl_lookup_state(pinctrl, "hac_pa_sus");
	if (IS_ERR_OR_NULL(hac_pinctrl_info.hac_pa_sus)) {
		pr_err("%s: Unable to get pinctrl hac_pa_sus state handle, err: %ld\n",
				__func__, PTR_ERR(hac_pinctrl_info.hac_pa_sus));
		goto pin_cfg_error;
	}
	/*default suspend*/
	if (pinctrl_select_state(hac_pinctrl_info.pinctrl, hac_pinctrl_info.hac_pa_sus))
		pr_err("%s: Failed to suspend hac_pa_sus pins\n", __func__);

	return 0;

pin_cfg_error:
	hac_pinctrl_info.pinctrl = NULL;
	return -EINVAL;
}
/*TCT-NB Tianhongwei end 2016-06-13*/
...
/*TCT-NB Tianhongwei add for HAC PA . Task 2340228 2016-06-13*/
	ext_hac_pa_pinctrl(control->dev);
/*TCT-NB Tianhongwei end 2016-06-13*/
