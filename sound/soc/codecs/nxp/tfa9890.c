...
struct tfa9890_sys_data_s{
    ...
    struct pinctrl_state *nxp9890_rest;
    int reset_pin_state;
...
static int tfa9890_pinctrl_init(struct tfa9890_sys_data_s *sys_data)
{
	...
	if (IS_ERR_OR_NULL(sys_data->pinctrl)) {
		...
		sys_data->nxp9890_rest = pinctrl_lookup_state(sys_data->pinctrl, "nxp9890_rest");
		if (IS_ERR_OR_NULL(sys_data->nxp9890_rest)) {
			ret = PTR_ERR(sys_data->nxp9890_rest);
			b_codec_trace();
			goto err_pinctrl_lookup;
		}
	...
	ret = pinctrl_select_state(sys_data->pinctrl, sys_data->nxp9890_rest);
	if (ret) {
        b_codec_trace();
		return ret;
	}
...
int tfa98xx_codec_put_ftm_spk_top_control(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
    ...
    if (state)
    {
        sec_mi2s_sclk_enable(1);  //Kun.Guan & Xing.Wang, add for FTM test, enable sclk firstly, 2015-09-01
...
//        tfa9890_trans_word(g_tfa9890_sys->client1, 0x04, 0x880b);
    ...
    else
    {
        ...
        sec_mi2s_sclk_enable(0);  //Kun.Guan & Xing.Wang, add for FTM test, disable sclk when reset, 2015-09-01
    }
...
static int tfa9890_i2c_probe(struct i2c_client *i2c, const struct i2c_device_id *id)
{
    ...
    printk(KERN_ERR"i2c addr = 0x%02x\n", i2c->addr);
    printk(KERN_ERR"tfa98xx_snd_controls = %d\n", (int)ARRAY_SIZE(tfa98xx_snd_controls));
...
reg_codec:
	printk(KERN_ERR"========rokyo reg codec \n");
	...
	printk(KERN_ERR"========rokyo reg codec  rc =%d\n",rc);
	return rc;
}
