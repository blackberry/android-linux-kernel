...
#ifdef CONFIG_BBRY
#define HS_REMOVAL_POLL_INTERVAL_MS        50
#define HS_REMOVAL_POLL_MAX_SAMPLES        10
#define HS_REMOVAL_POLL_NUM_SAMPLES         5
#endif
...
#ifdef CONFIG_BBRY  /* QC Patches from case# 02350757 */
/*
				...
				enum snd_jack_types jack_type)
{
	...
	if (!insertion) {
		...
		/* Report insertion */
#ifndef CONFIG_BBRY  /* QC Patches from case# 02350757 */
		mbhc->hph_status |= jack_type;
#endif
		...
		if (mbhc->impedance_detect && impedance_detect_en) {
			...
			if ((mbhc->zl > WCD9XXX_LINEIN_THRESHOLD) &&
				...
				pr_info("%s: Replace with SND_JACK_LINEOUT\n",
				__func__);
			}
		}
			  ...
			  unsigned long event_state)
{
...
#ifdef CONFIG_BBRY
			pr_warn("%s: within invalid range, vcdes=%d, mic_bias=%d\n",
				 __func__, d->_vdces, d->mic_bias);
#else
	...
	for (i = 0, d = dt; i < sz; i++, d++) {
		if ((i > 0) && !d->mic_bias && !d->swap_gnd &&
		    (d->_type != dprev->_type)) {
#ifdef CONFIG_BBRY
			pr_warn("%s: Invalid, inconsistent types(%d,%d)\n",
				__func__, d->_type, dprev->_type);
#else
...
#ifdef CONFIG_BBRY
			pr_warn("%s: Invalid, delta %dmv, %dmv, %dmv and %dmv\n",
...
#ifdef CONFIG_BBRY
				pr_warn("%s: Headset with threshold on MIC detected(%d)\n",
					 __func__, dt->_type);
#else
...
#ifdef CONFIG_BBRY
			pr_warn("%s: Invalid, inconsistent types (%d)\n",
				__func__, dgnd->_type);
#else
			pr_debug("%s: Invalid, inconsistent types\n", __func__);
#endif
...
#ifdef CONFIG_BBRY
		pr_warn("%s: Invalid, inconsistent HPHL..ch=%d, sz=%d\n", __func__, ch, sz);
#else
...
#ifdef CONFIG_BBRY
			/* Fix JI:967837 by reverting below code as it was suggested by QC (QC case#01999325).*/
			...
			if (plug_type == PLUG_TYPE_GND_MIC_SWAP) {
				...
				if (pt_gnd_mic_swap_cnt >=
					...
					wcd9xxx_handle_gnd_mic_swap(mbhc,
							...
							plug_type);
#ifdef CONFIG_BBRY
				pr_warn("%s: unsupported HS detected, continue polling\n",
					 __func__);
#else
...
#ifdef CONFIG_BBRY
		int idx;
		int removal_samples = 0;
		/* make sure the removal is real */
		for (idx = 0;
		    ((removal_samples < HS_REMOVAL_POLL_NUM_SAMPLES) &&
		     (idx < HS_REMOVAL_POLL_MAX_SAMPLES)); idx++) {
			msleep(HS_REMOVAL_POLL_INTERVAL_MS);
			insert = !wcd9xxx_swch_level_remove(mbhc);
			removal_samples = (insert == true) ? 0 : removal_samples + 1;
		}
		pr_debug("%s: idx=%d, insert=%d, removal_samples=%d\n",
				__func__, idx, insert, removal_samples);
		if ((insert == true) || (removal_samples < HS_REMOVAL_POLL_NUM_SAMPLES)) {
			/* Not a real removal. ignore */
			pr_warn("%s: Not a real removal. cur=%d. ignoring it. idx=%d(%d).\n",
			__func__, mbhc->current_plug, idx, removal_samples);
			goto exit;
		}
#endif
		mbhc->lpi_enabled = false;
...
#ifdef CONFIG_BBRY
		pr_info("%s: detect hs removal. current_plug=%d\n",
			__func__, mbhc->current_plug);
#endif
		if (mbhc->current_plug == PLUG_TYPE_HEADPHONE) {
