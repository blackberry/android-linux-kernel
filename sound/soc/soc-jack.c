...
void snd_soc_jack_report(struct snd_soc_jack *jack, int status, int mask)
{
	...
	unsigned int sync = 0;/*TCT-NB Tianhongwei add CR.1004036*/
	int enable;
	...
	list_for_each_entry(pin, &jack->pins, list) {
		...
		/* MODIFIED-BEGIN by hongwei.tian, 2016-05-12,BUG-2145900*/
		/* we need to sync for this case only */
		sync = 1;/*TCT-NB Tianhongwei add CR.1004036*/
	}

	/* Report before the DAPM sync to help users updating micbias status */
	blocking_notifier_call_chain(&jack->notifier, jack->status, jack);

	if (sync)/*TCT-NB Tianhongwei add CR.1004036*/
		snd_soc_dapm_sync(dapm);
		/* MODIFIED-END by hongwei.tian,BUG-2145900*/
