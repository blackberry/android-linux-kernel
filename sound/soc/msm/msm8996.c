...
static int mi2s_rx_bit_format = SNDRV_PCM_FORMAT_S16_LE;/*TCT-NB Tianhongwei add for 1567270*/
...
/*TCT-NB Tianhongwei add for idol4s audio 2015/12/18*/
static struct afe_clk_set sec_mi2s_rx_clk = {
	AFE_API_VERSION_I2S_CONFIG,
...
/*TCT-NB Tianhongwei end 2015/12/18*/
...
static struct wcd_mbhc_config wcd_mbhc_cfg = {
	...
	/*TCT-NB Tianhongwei modfiy for 3keys support*/
...
#ifdef CONFIG_FB_MSM_MDSS_HDMI_PANEL/*TCT-NB Tianhongwei HDMI not used*/
...
/*TCT-NB Tianhongwei add for idol4s audio 2015/12/18*/
static int msm_secmi2s_rx_be_hw_params_fixup(struct snd_soc_pcm_runtime *rtd,
				     struct snd_pcm_hw_params *params)
{
	struct snd_interval *rate = hw_param_interval(params,
					SNDRV_PCM_HW_PARAM_RATE);
	...
	pr_debug("%s: 48 khz 2 channels \n", __func__);
	rate->min = rate->max = SAMPLING_RATE_48KHZ;
	channels->min = channels->max = 2;
	return 0;
}
static int msm_secmi2s_tx_be_hw_params_fixup(struct snd_soc_pcm_runtime *rtd,
				     struct snd_pcm_hw_params *params)
{
	struct snd_interval *rate = hw_param_interval(params,
					SNDRV_PCM_HW_PARAM_RATE);
...
/*TCT-NB Tianhongwei add for 1567270  2016/02/16 */
...
#ifdef CONFIG_SND_TCT_SEC_I2S_HAL_CTL/*TCT-NB Tianhongwei add for smartpa sclk*/
    if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
...
/*TCT-NB Tianhongwei end 2015/12/18*/
#ifdef CONFIG_SND_TCT_SEC_I2S_HAL_CTL/*TCT-NB Tianhongwei add for smartpa sclk*/
/*static struct msm8996_asoc_mach_data *g_tct_msm8996_pdata = NULL;

static struct msm8996_asoc_mach_data *tct_get_msm8996_pdata(void)
{
	return g_tct_msm8996_pdata;
}

static void tct_set_msm8996_pdata(struct msm8996_asoc_mach_data *pdata)
{
	g_tct_msm8996_pdata = pdata;
        pr_err("%s set pdata = 0x%p\n", __func__, (void *)g_tct_msm8996_pdata);
}
*/
static int msm_q6_enable_mi2s_clocks(bool enable)
{
	...
	if(enable) {
		...
		rc = afe_port_start(AFE_PORT_ID_SECONDARY_MI2S_RX, &port_config,
		48000);
		if (IS_ERR_VALUE(rc)) {
		...
		rc = afe_close(AFE_PORT_ID_SECONDARY_MI2S_RX);
		if (IS_ERR_VALUE(rc)) {
...
int sec_mi2s_sclk_enable(int enable)
{
	int ret = 0;
...
EXPORT_SYMBOL_GPL(sec_mi2s_sclk_enable);
#endif
...
/*TCT-NB Tianhongwei end  2016/02/16 */
...
static const struct snd_kcontrol_new msm_snd_controls[] = {
	...
	SOC_ENUM_EXT("MI2S_RX Format", msm_snd_enum[4],/*TCT-NB Tianhongwei add for 1567270  2016/02/16 */
...
#define S(X, Y) ((WCD_MBHC_CAL_PLUG_TYPE_PTR(tasha_wcd_cal)->X) = (Y))
	S(v_hs_max, 1600);/*TCT-NB Tianhongwei modify for more high impedance headset 2016-04-28 defect.1966378 */
#undef S
...
/*TCT-NB Tianhongwei add for ftm loopback*/
	{
...
/*TCT-NB Tianhongwei add for idol4s audio 2015/12/18*/
	{
		...
		.be_hw_params_fixup = msm_secmi2s_rx_be_hw_params_fixup,
	...
	},
	{
		...
		.be_hw_params_fixup = msm_secmi2s_tx_be_hw_params_fixup,
		.ops = &msm8996_sec_mi2s_be_ops,
		.ignore_suspend = 1,
	},
/*TCT-NB Tianhongwei end 2015/12/18*/
