...
struct vss_icommon_cmd_set_ui_property_t {
	...
	/* Unique ID of the parameter. */
	uint16_t param_size;
	/* Size of the parameter in bytes: sizeof(int32_t) */
	uint16_t reserved;
	/* Reserved; set to 0. */
	int32_t value;
};
...
#ifdef CONFIG_SND_SOC_BBRY_VOICE_TOOLS
struct cvp_set_ui_property_cmd {
	struct apr_hdr hdr;
	struct vss_icommon_cmd_set_ui_property_t vss_set_ui_prop;
} __packed;

struct cvp_get_ui_property_cmd {
	struct apr_hdr hdr;
	struct vss_icommon_cmd_get_ui_property_t vss_get_ui_prop;
} __packed;
#endif /* CONFIG_SND_SOC_BBRY_VOICE_TOOLS */
