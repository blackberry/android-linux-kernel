...
#ifdef CONFIG_SND_SOC_BBRY_VOICE_TOOLS
static int voice_send_set_ui_property_cmd(struct voice_data *v,
					uint32_t module_id, uint32_t param_id, int32_t value);
static int voice_send_get_ui_property_cmd(struct voice_data *v,
					uint32_t module_id, uint32_t param_id, int32_t *value);
#endif /* CONFIG_SND_SOC_BBRY_VOICE_TOOLS */
...
static int voice_send_set_ui_property_cmd(struct voice_data *v,
			uint32_t module_id, uint32_t param_id, int32_t value)
{
	struct cvp_set_ui_property_cmd cvp_set_prop_cmd;
	int ret = 0;
	void *apr_cvp;
	u16 cvp_handle;
	...
	cvp_set_prop_cmd.hdr.hdr_field = APR_HDR_FIELD(APR_MSG_TYPE_SEQ_CMD,
							APR_HDR_LEN(APR_HDR_SIZE),
							APR_PKT_VER);
	cvp_set_prop_cmd.hdr.pkt_size = APR_PKT_SIZE(APR_HDR_SIZE,
							sizeof(cvp_set_prop_cmd) -
							APR_HDR_SIZE);
	cvp_set_prop_cmd.hdr.src_port = voice_get_idx_for_session(v->session_id);
	cvp_set_prop_cmd.hdr.dest_port = cvp_handle;
	cvp_set_prop_cmd.hdr.token = 0;
	cvp_set_prop_cmd.hdr.opcode = VSS_ICOMMON_CMD_SET_UI_PROPERTY;

	cvp_set_prop_cmd.vss_set_ui_prop.module_id = module_id;
	cvp_set_prop_cmd.vss_set_ui_prop.param_id = param_id;
	cvp_set_prop_cmd.vss_set_ui_prop.param_size = sizeof(int32_t);
	cvp_set_prop_cmd.vss_set_ui_prop.reserved = 0;
	cvp_set_prop_cmd.vss_set_ui_prop.value = value;
	pr_debug(
	"voice_send_set_ui_property_cmd, module_id=%#x, param_id=%#x, value=%d\n",
		module_id, param_id, value);

	v->cvp_state = CMD_STATUS_FAIL;
	ret = apr_send_pkt(apr_cvp, (uint32_t *) &cvp_set_prop_cmd);
	if (ret < 0) {
		pr_err("Fail: sending cvp set ui property,\n");
		goto fail;
	}
...
static int voice_send_get_ui_property_cmd(struct voice_data *v,
			uint32_t module_id, uint32_t param_id, int32_t *value)
{
	struct cvp_get_ui_property_cmd cvp_get_prop_cmd;
	int ret = 0;
	void *apr_cvp;
	u16 cvp_handle;
	...
	cvp_get_prop_cmd.hdr.hdr_field = APR_HDR_FIELD(APR_MSG_TYPE_SEQ_CMD,
							APR_HDR_LEN(APR_HDR_SIZE),
							APR_PKT_VER);
	cvp_get_prop_cmd.hdr.pkt_size = APR_PKT_SIZE(APR_HDR_SIZE,
							sizeof(cvp_get_prop_cmd) -
							APR_HDR_SIZE);
	cvp_get_prop_cmd.hdr.src_port = voice_get_idx_for_session(v->session_id);
	cvp_get_prop_cmd.hdr.dest_port = cvp_handle;
	cvp_get_prop_cmd.hdr.token = 0;
	cvp_get_prop_cmd.hdr.opcode = VSS_ICOMMON_CMD_GET_UI_PROPERTY;

	cvp_get_prop_cmd.vss_get_ui_prop.module_id = module_id;
	cvp_get_prop_cmd.vss_get_ui_prop.param_id = param_id;
	cvp_get_prop_cmd.vss_get_ui_prop.param_size = sizeof(int32_t);
	pr_debug("voice_send_get_ui_property_cmd, module_id=%#x, param_id=%#x\n",
		module_id, param_id);

	v->cvp_state = CMD_STATUS_FAIL;
	ret = apr_send_pkt(apr_cvp, (uint32_t *) &cvp_get_prop_cmd);
	if (ret < 0) {
		pr_err("Fail: sending cvp get ui property,\n");
		goto fail;
	}
	ret = wait_event_timeout(v->cvp_wait,
				(v->cvp_state == CMD_STATUS_SUCCESS),
...
#ifdef CONFIG_SND_SOC_BBRY_VOICE_TOOLS
int voc_set_ui_parameter(uint32_t session_id, uint32_t module_id,
					uint32_t param_id, int32_t value)
{
	struct voice_data *v = NULL;
	int ret = 0;
	...
	while (voice_itr_get_next_session(&itr, &v)) {
		if (v != NULL) {
			...
			if (v->voc_state == VOC_RUN) {
				ret = voice_send_set_ui_property_cmd(v,
					module_id, param_id, value);
			}
			mutex_unlock(&v->lock);
		} else {
...
int voc_get_ui_parameter(uint32_t session_id, uint32_t module_id,
					uint32_t param_id, int32_t *value)
{
	struct voice_data *v = NULL;
	int ret = 0;
	...
	while (voice_itr_get_next_session(&itr, &v)) {
		if (v != NULL) {
			...
			mutex_lock(&v->lock);
			if (v->voc_state == VOC_RUN) {
				ret = voice_send_get_ui_property_cmd(v,
					module_id, param_id, value);
			}
			mutex_unlock(&v->lock);
		} else {
...
#ifdef CONFIG_SND_SOC_BBRY_VOICE_TOOLS
	} else if (data->opcode == VSS_ICOMMON_RSP_GET_UI_PROPERTY) {
		struct vss_icommon_rsp_get_ui_property_t *rsp;
		uint8_t *pkt = data->payload;
		uint16_t pkt_len = data->payload_size;
		int32_t *payload;

		pr_debug("%s: VSS_ICOMMON_RSP_GET_UI_PROPERTY, payload_size=%d\n",
				 __func__, data->payload_size);

		/* Check payload and process response */
		if ((pkt != NULL) &&
			(pkt_len >=
			sizeof(struct vss_icommon_rsp_get_ui_property_t))) {
			rsp = (struct vss_icommon_rsp_get_ui_property_t *) pkt;
			if (rsp->param_size == sizeof(int32_t)) {
				payload = (int32_t *)
				((int8_t *) data->payload +
				sizeof(struct vss_icommon_rsp_get_ui_property_t));
				v->param_data = *payload;
			} else {
				v->param_data = 0;
			}
		} else {
