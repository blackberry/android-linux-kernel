...
struct afe_ctl {
	...
	struct audio_cal_info_bbry_spkr_prot_cfg
		bbry_spkr_prot_cfg;
	struct rda_spkr_prot_vi_calibration_config_t
...
static int32_t afe_callback(struct apr_client_data *data, void *priv)
{
	...
	if (data->opcode == AFE_PORT_CMDRSP_GET_PARAM_V2) {
...
#ifdef CONFIG_BBRY
		/* Validate payload and token*/
		if ((data->payload_size <
		sizeof(struct afe_bbry_spkr_prot_get_vi_calibration_resp))
		...
		/* Speaker protection calibration response */
		resp =
			(struct afe_bbry_spkr_prot_get_vi_calibration_resp *)
			payload;
		if (!resp->status) {
			/* Store data received from the DSP */
			atomic_set(&this_afe.state, 0);
			this_afe.bbry_spkr_prot_vi_calib.R0 = resp->config.R0;
			this_afe.bbry_spkr_prot_vi_calib.T0 = resp->config.T0;
			this_afe.bbry_spkr_prot_vi_calib.valid =
				resp->config.valid;
			pr_debug("%s: R0=%d, T0=%d, valid=%d\n",
				__func__, resp->config.R0,
				resp->config.T0, resp->config.valid);
#else
...
int afe_bbry_spkr_prot_get_calibration(u16 port_id,
			int32_t *r0, int32_t *t0, int32_t *valid)
{
	int ret = -EINVAL;
	int index = 0;
	...
	*r0 = this_afe.bbry_spkr_prot_vi_calib.R0;
	*t0 = this_afe.bbry_spkr_prot_vi_calib.T0;
	*valid = this_afe.bbry_spkr_prot_vi_calib.valid;

	/* Log debug info */
	pr_debug("%s: R0=%d, T0=%d, valid=%d\n",
		__func__, *r0, *t0, *valid);
			...
			int32_t r0, int32_t t0, int32_t valid)
{
	...
	/* Populate payload */
	command.config.R0 = r0;
	command.config.T0 = t0;
	command.config.valid = valid;
