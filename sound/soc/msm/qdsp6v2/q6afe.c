...
#ifdef CONFIG_SND_SOC_BBRY_SPKRPROT
struct audio_cal_info_bbry_spkr_prot_cfg {
	uint32_t mode;
};
#endif /* CONFIG_SND_SOC_BBRY_SPKRPROT */
...
struct afe_ctl {
	...
	struct audio_cal_info_bbry_spkr_prot_cfg
		bbry_spkr_prot_cfg;
	struct afe_bbry_spkr_prot_get_vi_calibration_resp
		bbry_spkr_prot_vi_calib;
#endif /* CONFIG_SND_SOC_BBRY_SPKRPROT */
};
...
#ifdef CONFIG_SND_SOC_BBRY_SPKRPROT
static int32_t bbry_sp_make_afe_callback(uint32_t *payload,
										uint32_t payload_size)
{
	u32 param_id;
	struct afe_bbry_spkr_prot_get_vi_calibration_resp *resp =
		(struct afe_bbry_spkr_prot_get_vi_calibration_resp *) payload;
	...
	/* Check for speaker protection specific parameter ID */
	param_id = resp->pdata.param_id;
	if (param_id == AFE_PARAM_RDA_SPKR_PROT_VI_CALIBRATION_CONFIG) {
		/* Verify response payload size */
		if (payload_size < sizeof(this_afe.bbry_spkr_prot_vi_calib)) {
			pr_err("%s: Error: received size %d, expected size %zu\n",
				__func__, payload_size,
				sizeof(this_afe.bbry_spkr_prot_vi_calib));
			return -EINVAL;
		}

		/* Copy response data */
		memcpy(&this_afe.bbry_spkr_prot_vi_calib, payload,
			sizeof(this_afe.bbry_spkr_prot_vi_calib));

		if (!this_afe.bbry_spkr_prot_vi_calib.status) {
			atomic_set(&this_afe.state, 0);
		} else {
			pr_debug("%s: calibration response status: %d", __func__,
				  this_afe.bbry_spkr_prot_vi_calib.status);
...
#ifdef CONFIG_SND_SOC_BBRY_SPKRPROT
		if (bbry_sp_make_afe_callback(data->payload, data->payload_size))
			return -EINVAL;
#endif /* CONFIG_SND_SOC_BBRY_SPKRPROT */
...
int afe_get_port_type(u16 port_id)
{
	...
	case AFE_PORT_ID_SENARY_MI2S_TX_VI:
...
#ifdef CONFIG_SND_SOC_BBRY_SPKRPROT
/* Send probe tone ramp down configuration */
static int afe_bbry_spkr_prot_ramp_dn_cfg(int port)
{
	int ret = -EINVAL;
	int index = 0;
	struct afe_bbry_spkr_prot_rx_probetone_ramp_config_command
		command;

	/* Verify that port is RX */
	if (afe_get_port_type(port) != MSM_AFE_PORT_TYPE_RX) {
		pr_debug("%s: port doesn't match 0x%x\n", __func__, port);
		return 0;
	}

	/* Verify that speaker protection is enabled and correct port */
	if (this_afe.bbry_spkr_prot_cfg.mode == MSM_SPKR_PROT_DISABLED ||
		(this_afe.vi_rx_port != port)) {
		pr_debug(
			"%s: BBRY spkr protection disabled port 0x%x %d 0x%x\n",
			__func__, port, ret, this_afe.vi_rx_port);
		return 0;
	}
	...
	command.hdr.src_port = 0;
	command.hdr.dest_port = 0;
	command.hdr.token = index;
	command.hdr.opcode = AFE_PORT_CMD_SET_PARAM_V2;

	/* Populate command fields */
	command.param.port_id = q6audio_get_port_id(port);
	command.param.payload_size =
		sizeof(command) - sizeof(command.hdr) - sizeof(command.param);
	command.pdata.module_id = AFE_MODULE_RDA_SPKR_PROT_RX_PROCESS;
	command.pdata.param_id =
		AFE_PARAM_RDA_SPKR_PROT_RX_PROBETONE_RAMP_CONFIG;
	command.pdata.param_size = sizeof(command.config);

	/* Populate payload: 0.18 dB / sample, -128 dB target */
	command.config.gainStep = 46;
	command.config.gainTarget = -32768;

	/* Send command to DSP and wait for response */
	atomic_set(&this_afe.state, 1);
	ret = apr_send_pkt(this_afe.apr, (uint32_t *) &command);
	if (ret < 0) {
		pr_err("%s: port = 0x%x param = 0x%x failed %d\n",
			__func__, port, command.pdata.param_id, ret);
		goto fail_cmd;
	}
	ret = wait_event_timeout(this_afe.wait[index],
			(atomic_read(&this_afe.state) == 0),
	...
	/* Sleep for 15 ms to allow probe tone ramp down */
	usleep_range(15000, 15010);
	ret = 0;

fail_cmd:
	pr_debug("%s: config.pdata.param_id 0x%x status %d\n",
		__func__, command.pdata.param_id, ret);
return ret;
}
#endif /* CONFIG_SND_SOC_BBRY_SPKRPROT */
	...
	u32 rate) /* This function is no blocking */
{
	...
	pr_debug("%s: port id: 0x%x\n", __func__, port_id);
	/* proxy port to capture vi sense */
	if (port_id == AFE_PORT_ID_SENARY_MI2S_TX_VI)
...
#ifdef CONFIG_SND_SOC_BBRY_SPKRPROT
	if (afe_bbry_spkr_prot_ramp_dn_cfg(port_id) < 0) {
		pr_err("%s: BBRY ramp down configuration failed\n",
			__func__);
	}
#endif /* CONFIG_SND_SOC_BBRY_SPKRPROT */
...
#ifdef CONFIG_SND_SOC_BBRY_SPKRPROT
int afe_bbry_spkr_prot_get_calibration(u16 port_id,
	struct rda_spkr_prot_vi_calibration_config_t *calib_data)
{
	int ret = -EINVAL;
	int index = 0;
	struct afe_bbry_spkr_prot_get_vi_calibration_command
		command;

	/* Verify that port is TX */
	if (afe_get_port_type(port_id) != MSM_AFE_PORT_TYPE_TX) {
		pr_debug("%s: port doesn't match 0x%x\n",
			__func__, port_id);
		return 0;
	...
	if (this_afe.bbry_spkr_prot_cfg.mode == MSM_SPKR_PROT_DISABLED ||
		...
		pr_debug(
			"%s: BBRY spkr protection disabled port 0x%x %d 0x%x\n",
			__func__, port_id, ret, this_afe.vi_tx_port);
		return 0;
	}
	...
	command.hdr.dest_port = 0;
	command.hdr.token = index;
	command.hdr.opcode = AFE_PORT_CMD_GET_PARAM_V2;

	/* Populate command itself */
	command.param.mem_map_handle = 0;
	command.param.module_id = AFE_MODULE_RDA_SPKR_PROT_VI_PROCESS;
	command.param.param_id =
		AFE_PARAM_RDA_SPKR_PROT_VI_CALIBRATION_CONFIG;
	command.param.payload_address_lsw = 0;
	command.param.payload_address_msw = 0;
	command.param.payload_size =
		sizeof(command) - sizeof(command.hdr) - sizeof(command.param);
	command.param.port_id = q6audio_get_port_id(port_id);
	command.pdata.module_id = AFE_MODULE_RDA_SPKR_PROT_VI_PROCESS;
	command.pdata.param_id =
		AFE_PARAM_RDA_SPKR_PROT_VI_CALIBRATION_CONFIG;
	command.pdata.param_size = sizeof(command.config);
	...
	if (ret < 0) {
		pr_err("%s: port = 0x%x param = 0x%x failed %d\n",
			__func__, port_id, command.pdata.param_id, ret);
		goto fail_cmd;
	}
	ret = wait_event_timeout(this_afe.wait[index],
			(atomic_read(&this_afe.state) == 0),
	...
	memcpy(calib_data, &this_afe.bbry_spkr_prot_vi_calib.config,
		sizeof(this_afe.bbry_spkr_prot_vi_calib.config));

	/* Log debug info */
	pr_debug("%s: R0=%d, T0=%d, LE=%d, valid=%d\n", __func__,
		calib_data->R0, calib_data->T0, calib_data->LE, calib_data->valid);
...
int afe_bbry_spkr_prot_put_calibration(u16 port_id,
	const struct rda_spkr_prot_vi_calibration_config_t *calib_data)
{
	int ret = -EINVAL;
	int index = 0;
	struct afe_bbry_spkr_prot_set_vi_calibration_command
		command;
	...
	/* Populate payload */
	memcpy(&command.config, calib_data, sizeof(command.config));
...
static int __init afe_init(void)
{
	...
	this_afe.ex_ftm_cfg.mode = MSM_SPKR_PROT_DISABLED;
#ifdef CONFIG_SND_SOC_BBRY_SPKRPROT
	this_afe.bbry_spkr_prot_cfg.mode = MSM_SPKR_PROT_CALIBRATED;
#endif /* CONFIG_SND_SOC_BBRY_SPKRPROT */
