	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	result = afe_bbry_spkr_prot_get_calibration(
		SLIMBUS_4_TX, &r0, &t0, &valid);

	/* Store results */
	ucontrol->value.integer.value[0] = r0;
	ucontrol->value.integer.value[1] = t0;
	ucontrol->value.integer.value[2] = valid;
...
static int msm_routing_put_bbry_spkr_prot_calib(
	...
	struct snd_ctl_elem_value *ucontrol)
{
	int32_t r0 = ucontrol->value.integer.value[0];
	int32_t t0 = ucontrol->value.integer.value[1];
	int32_t valid = ucontrol->value.integer.value[2];
	int result;
