...
#define RDAVOXSMPARAM_MODULE_ID 0xBB000001
			...
			struct snd_ctl_elem_value *ucontrol)
{
	...
	int32_t value = ucontrol->value.integer.value[2];
	int rc;

	pr_debug("%s: st session_id=%#x, param_id=%#x, value=%d\n", __func__,
		session_id, param_id, value);

	rc = voc_set_ui_parameter(session_id, RDAVOXSMPARAM_MODULE_ID, param_id,
		value);
			...
			struct snd_ctl_elem_value *ucontrol)
{
	...
	pr_debug("%s: st session_id=%#x, param_id=%#x\n", __func__, session_id,
		param_id);

	rc = voc_get_ui_parameter(session_id, RDAVOXSMPARAM_MODULE_ID, param_id,
		&value);

	ucontrol->value.integer.value[2] = value;
...
#if defined(CONFIG_BBRY)
	SOC_SINGLE_MULTI_EXT("RdaVox Parameter", SND_SOC_NOPM, 0, VSID_MAX, 0, 3,
				msm_rda_vox_get_sm_parameter,
				msm_rda_vox_set_sm_parameter),
#endif
