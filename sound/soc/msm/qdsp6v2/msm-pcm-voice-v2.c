...
static int msm_rda_vox_set_sm_parameter(struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_value *ucontrol)
{
	uint32_t session_id = ucontrol->value.integer.value[0];
	uint32_t module_id = ucontrol->value.integer.value[1];
	uint32_t param_id = ucontrol->value.integer.value[2];
	int32_t value = ucontrol->value.integer.value[3];
	int rc;

	pr_debug("%s: st session_id=%#x, module_id=%#x, param_id=%#x, value=%d\n",
		__func__, session_id, module_id, param_id, value);

	rc = voc_set_ui_parameter(session_id, module_id, param_id,
		value);

	return rc;
}

static int msm_rda_vox_get_sm_parameter(struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_value *ucontrol)
{
	uint32_t session_id = ucontrol->value.integer.value[0];
	uint32_t module_id = ucontrol->value.integer.value[1];
	...
	pr_debug("%s: st session_id=%#x, module_id=%#x, param_id=%#x\n",
		__func__, session_id, module_id, param_id);

	rc = voc_get_ui_parameter(session_id, module_id, param_id,
		&value);

	ucontrol->value.integer.value[3] = value;
...
#ifdef CONFIG_SND_SOC_BBRY_VOICE_TOOLS
	SOC_SINGLE_MULTI_EXT("RdaVox Parameter", SND_SOC_NOPM, 0, VSID_MAX, 0, 4,
				msm_rda_vox_get_sm_parameter, msm_rda_vox_set_sm_parameter),
#endif /* CONFIG_SND_SOC_BBRY_VOICE_TOOLS */
};
