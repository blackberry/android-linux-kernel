...
#else
static atomic_unchecked_t msm_rtb_idx;
#endif
...
static int msm_rtb_get_idx(void)
{
	 ...
	 * end of the buffer
	 */
	adjusted_size = atomic_read_unchecked(index) + unused_buffer_size;
	offset = (adjusted_size & (msm_rtb.nentries - 1)) -
		 ((adjusted_size - msm_rtb.step_size) & (msm_rtb.nentries - 1));
	if (offset < 0) {
		uncached_logk_timestamp(i);
		i = atomic_add_return_unchecked(msm_rtb.step_size + unused_buffer_size,
									index);
		i -= msm_rtb.step_size;
	}
...
static int msm_rtb_get_idx(void)
{
	...
	i = atomic_inc_return_unchecked(&msm_rtb_idx);
	i--;
...
#else
	atomic_set_unchecked(&msm_rtb_idx, 0);
	msm_rtb.step_size = 1;
#endif
