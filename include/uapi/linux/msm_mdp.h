...
#ifdef CONFIG_BBRY
/* Start at a higher number to avoid future collisions */
#define MSMFB_PARTIAL_WINDOW	_IOW(MSMFB_IOCTL_MAGIC, 255, struct partial_window_data)
#endif
#ifdef CONFIG_BBRY_MFG
#define MSMFB_READ_PANEL_RECOVERY_DATA	_IOWR(MSMFB_IOCTL_MAGIC, 254, struct panel_recovery_data)
#endif
...
#ifdef CONFIG_BBRY
struct partial_window_data {
	uint32_t sr, er;
};
#endif
#ifdef CONFIG_BBRY_MFG
#define PANEL_RECOVERY_DATA_SIZE 512
struct panel_recovery_data {
	uint8_t buf[PANEL_RECOVERY_DATA_SIZE];
};
#endif
