...
#ifdef CONFIG_BBRY
	enum qpnp_adc_scale_fn_type		adc_sec_scale_fn;
#endif
...
#ifdef CONFIG_BBRY
/**
 * qpnp_adc_scale_tdkntcg_therm_bbry() - Scales the pre-calibrated digital output
 *		of an ADC to the ADC reference and compensates for the
 *		gain and offset. Returns the temperature of the xo therm in mili
		degC.
 * @dev:	Structure device for qpnp vadc
 ...
 * @chan_rslt:	physical result to be stored.
 */
int32_t qpnp_adc_tdkntcg_therm_bbry(struct qpnp_vadc_chip *dev, int32_t adc_code,
			const struct qpnp_adc_properties *adc_prop,
			const struct qpnp_vadc_chan_properties *chan_prop,
			struct qpnp_vadc_result *chan_rslt);
#endif
