...
#else
	PON_POWER_OFF_SHUTDOWN		= 0x05, /* DVDD_SHUTDOWN */
	PON_POWER_OFF_HARD_RESET	= 0x08, /* DVDD_HARD_RESET */
#endif
};
...
enum pon_restart_reason {
	...
	PON_RESTART_REASON_RECOVERY		= 0x01 << 3,
	PON_RESTART_REASON_BOOTLOADER		= 0x02 << 3,
	PON_RESTART_REASON_RTC			= 0x03 << 3,
	PON_RESTART_REASON_OEM_WARM		= 0x20,
	PON_RESTART_REASON_OEM_HARD		= 0x30,
...
({\
	...
	if (_oem <= LAST_OEM_WARM_RESET) \
		_pon = PON_RESTART_REASON_OEM_WARM + (_oem - FIRST_OEM_WARM_RESET); \
	else if ((_oem >= FIRST_OEM_HARD_RESET) && (_oem <= LAST_OEM_HARD_RESET)) \
		_pon = PON_RESTART_REASON_OEM_HARD + (_oem - FIRST_OEM_HARD_RESET); \
	_pon; \
})
