...
#ifdef CONFIG_BBRY
/*
 * The clock stretching timeout was increased to 2 seconds to support
 * the sensor hub, stm32f401C. This version of the chip has bootloader v1.0,
 * which is more limited than v1.1, and needs to clock stretch for 1.8 seconds
 * while erasing flash.
 *
 * This could be removed if the sensor hub is replaced with a newer rev.
 */
#define I2C_MSM_TIMEOUT_MIN_USEC        (2000000)
#else
...
struct i2c_msm_evlog_all {
	u64 sum;
	u32 index;
	unsigned long nivcsw;
	struct i2c_msm_evlog longest;  /* Longest event not an error */
	struct i2c_msm_evlog last_err; /* Last error */
#define EVLOG_NUM_ENTRY 128
	struct i2c_msm_evlog entry[EVLOG_NUM_ENTRY];
};
#endif
...
#ifdef CONFIG_BBRY_I2C_MSM_EVL
	struct i2c_msm_evlog_all   evlog;
	struct i2c_msm_evlog_all   *snapshot;
#endif
};
...
/* function definitions to be used from the i2c-msm-v2-debug file */
#ifndef CONFIG_BBRY_I2C_MSM_EVL
