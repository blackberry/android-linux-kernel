...
#ifdef CONFIG_BBRY
enum msm_pcie_notify_data_type {
	MSM_PCIE_NOTIFY_DATA_INVALID = 0,
	MSM_PCIE_NOTIFY_DATA_WAKEUP,
	MSM_PCIE_NOTIFY_DATA_LINKDOWN,
	MSM_PCIE_NOTIFY_DATA_AER,
};
#endif /* CONFIG_BBRY */
...
#ifdef CONFIG_BBRY
struct msm_pcie_notify_data {
	enum msm_pcie_notify_data_type type;
};
#endif /* CONFIG_BBRY */
...
/*
 * msm_pcie_is_user_suspend - find out if user_suspend flag is set for an RC.
 * @dev:        pci device structure
 *
 * This function checks out the flag status of the RC which current PCI device
 * belongs to.
 *
 * Return: 0/1 flag status on success, negative value on error
 */
