...
struct ctrs_host {
	atomic_long_t pwr_irq_timeout;
	atomic_long_t data_request_timeout;
	atomic_long_t cmd_request_timeout;
};
#endif
...
#ifdef CONFIG_BBRY
	/* We are only going to care about the first four partitions */
	#define CTRS_MAX_PARTS  4
	struct device_attribute transaction_counters;
	struct ctrs_part part_ctrs[CTRS_MAX_PARTS];
	struct ctrs_host ctrs;
#endif
