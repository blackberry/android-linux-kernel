...
struct msm_sensor_info_t {
	...
	char     actuator_name[MAX_SENSOR_NAME];
...
#ifdef CONFIG_BBRY
struct msm_flash_hw_data_t {
	uint32_t flash_count;
	uint32_t videolight_flashes;
	uint32_t capture_timer_flashes;
};

struct msm_flash_limits_t {
	uint32_t max_current;
	uint32_t relative_flux; /* 16.16 fixed point, 0 to 1.0 */
};
#endif /* CONFIG_BBRY */
...
struct msm_flash_cfg_data_t {
	...
	int32_t flash_duration[MAX_LED_TRIGGERS];
#ifdef CONFIG_BBRY
	uint32_t flash_mitigation_level;
	uint32_t torch_mitigation_level;
	struct msm_flash_hw_data_t hw_data;
	enum msm_flash_cfg_type_t state;
	uint32_t use_fixed_ratio;
	struct msm_flash_limits_t flash_limits;
	struct msm_flash_limits_t preflash_limits;
#endif /* CONFIG_BBRY */
	union {
...
struct msm_flash_cfg_data_t32 {
	...
	uint32_t flash_mitigation_level;
	uint32_t torch_mitigation_level;
	struct msm_flash_hw_data_t32 hw_data;
	enum msm_flash_cfg_type_t state;
	uint32_t use_fixed_ratio;
	struct msm_flash_limits_t flash_limits;
	struct msm_flash_limits_t preflash_limits;
